...
| `extras.js` | Shared `TCG.*` module: images/prices/graph panel (image `fallback` via `onerror`), FX, title-fitting, card `condCode`/`langCode`, `legoCondToken`/`funkoCondToken`, `renderItemSpecifics`, and **`TCG.ebayComps({query,container,status,filter?})`** — the shared eBay AU delivered-comps engine (sold-first via Marketplace Insights → asking fallback; delivered totals = item + shipping; AU vs Worldwide; undercut target). Each builder supplies a game-specific query. Loaded via `<script src="/extras.js">`. |
