...
| `keepers.html` | **The Keepers admin** (§20). Five tabs — Queue · Customers · Ledger · Redemptions · Reconcile. Reads `/api/keepers`; every mutating button needs `DIAG_TOKEN`. Mint and revoke are two-tap, and the armed label names the store, the customer, the points and the percent, because a percentage discount code is spendable money. |
| `lib/keepers.mjs` | The engine's wiring: registers the Keepers consumer with the shared Shopify receiver, arms the two jobs, serves `/api/keepers`. Owns the three arming switches (`mode` ∈ off/observe/apply · `project.enabled` · `project.allowLive`) and `validateKeepersConfig`. **The mode and live gates on minting live in the ROUTE** — `lib/keepers-redeem.mjs` takes `store` as a plain parameter and checks neither. |
| `lib/keepers-db.mjs` | The append-only points ledger (`data/keepers.db`, gitignored, WAL). Seven tables; idempotency is partial UNIQUE indexes, not check-then-insert. `TCG_KEEPERS_DB` overrides the path — `bootServer` sets it, so tests never touch the real one. |
| `lib/keepers-ingest.mjs` | What an order is worth. Pure `decideAccrual`; fails closed on an unknown channel, because accruing XP on an eBay order breaks a promise printed on every page of the storefront. |
| `lib/keepers-levels.mjs` · `lib/keepers-badges.mjs` | The rank curve (`min_xp(L) = 62.5(L−1)L`) and the eight badge evaluators. Both read the `bk_keepers_*` metaobjects, which the STOREFRONT reads too — one economy, no redeploy. |
| `lib/keepers-project.mjs` | Ledger → the five customer metafields. A projection, never a source: nothing reads a metafield to compute a new value. Holds a lease so a second instance refuses rather than duels. |
| `lib/keepers-reconcile.mjs` | The cursor-held sweep and `driftReport`. Guards the 60-day `read_orders` window explicitly — below the floor the query returns an EMPTY PAGE, not an error, so an unguarded sweep reports clean forever. |
| `lib/keepers-redeem.mjs` | Redemption: debit before mint, in that order. Mints a percentage code scoped to one customer and **reads the created node back to verify the eligibility echoes that customer** — a code minted store-wide is a money leak `shopifyGraphQL`'s `ok` cannot catch. |
| `lib/keepers-referrals.mjs` · `lib/keepers-checkin.mjs` · `lib/keepers-grants.mjs` | Referral codes (Crockford, no I/L/O/U) with a refund-window hold; the QR show check-in, whose show is inferred from the CLOCK because Liquid cannot read a query string; manual grants, which require a caller-supplied idempotency key. |
| `lib/shopify-hooks.mjs` | The shared Shopify webhook + App Proxy receiver. A separate loopback `node:http` server on its own port serving exactly three paths and bare-404ing everything else — same isolation argument as `lib/ebay-notify.mjs`. |
| `lib/shopify-hooks-verify.mjs` · `lib/keepers-proxy-verify.mjs` | The two signature schemes, deliberately in separate modules: webhooks are base64 over the raw body, the App Proxy is hex over sorted delimiter-less params. Sharing code between them is how one gets used for the other. |
| `data/keepers.config.json` · `data/shopify-hooks.config.json` | **Gitignored, server-owned.** Arming state is per-machine. Seeded from the tracked `*.example.json` on boot. |
...
| `grade-rules.js` | Pure, transparent grade-prediction engine (`window.GradeRules`), loaded like `extras.js` by **`card-grader.html` and `index.html`** (the masthead centering panel). Every tolerance/weight comes from `data/grading.config.json` — data, never hardcoded logic. `centeringBand` is the single ladder walk: it returns the band cleared plus the tighter one missed (and which side failed), and `centeringGrade` is a thin wrapper over it — never walk `cfg.centering` yourself, that is how the old hardcoded 55/60 pills drifted. `centeringWants(hit, le)` is the single **phrasing** of that missed band ("10 wants ≤ 55.0 front"), shared by the dashboard rows, the grader's pill and `grade-report-pdf.js` — the PDF passes `le = '<='` because jsPDF's core fonts are WinAnsi and print a real ≤ as rubbish. `sideFromCorners`/`sideFromEdges` aggregate the v2 granular cells (min + mean; all-null → null). Config-table-driven unit tests in `test/unit/grade-rules.test.mjs`. A documented approximation, never the companies' real math (GR4). |
...
| `/api/keepers` | (middleware, `lib/keepers.mjs`) | **The Keepers engine** (§20). Read: `/state` `/config` `/drift` `/customers[?q=]` `/customers/:gid` `/customers/:gid/projection` `/webhooks[?status=]` `/redemptions[?status=]` `/observations` `/gate-audit` `/self-test`. Gated (`DIAG_TOKEN`): `/grant` `/grant-badge` `/checkin` `/redemptions` `/redemptions/:id/mint` `/redemptions/:id/revoke` `/sweep[?apply=1]` `/pass` `/economy/refresh`. **`/sweep` is dry-run unless `apply=1`.** `/self-test` answers 503 and never 404 when the receiver is down, because `lib/status.mjs` reads a 404 as ok. Route order matters: `/customers/:gid/projection` must stay ABOVE the `/customers/` prefix branch. |
