...
| — the daily digest's bulk buttons | The morning digest's two messages carry **Mark all N picked** (local, one tap) and **Mark all N shipped** (a CompleteSale per order, so it asks first). Both act on `orders.pack_digest_date`, stamped **before** the messages are sent, so a tap acts on the set the message actually listed and never on orders that landed since. An order still unshipped tomorrow is re-stamped, which is also why yesterday's buttons quietly match nothing. Bulk dispatch holds an atomic `meta` claim against a double tap, and a failed eBay write leaves that order unshipped so it returns in the next digest. Callback prefixes `psp:` / `psdq:` / `psdy:` / `psdn:` + a date; they must not collide with the approve/skip `psa:` / `pss:` + id. |
