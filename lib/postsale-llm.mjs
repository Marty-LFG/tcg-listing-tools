...
export function followUpSystemPrompt(cfg = {}, kind = 'dispatch') {
  ...
  if (delivered) {
    ...
    // Repeat business is still the point; it just has to be about NEXT time.
    if (cfg.invite_offers !== false) {
      lines.push(
        '- Finish with one short, no-pressure line inviting them to send an offer NEXT time there is',
...
export function fallbackFollowUp({ order, items = [], postage = {}, cfg = {}, kind = 'dispatch' } = {}) {
  ...
  if (kind === 'delivered') {
    ...
    const trk = postage.tracking
      ...
      : '';
    // Nothing can go in the parcel now, so this invites a FUTURE order rather than an addition to
    // this one. Anything about bundles or combined postage would be an offer we cannot honour.
    const offer = cfg.invite_offers === false ? []
      : ['', "Next time there's something you're after, send an offer through and we'll see what we can do."];
    body = [
      hey,
      '',
