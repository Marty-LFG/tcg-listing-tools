...
export function fallbackDraft({ order, items = [], cfg = {}, shipBy, now = new Date() } = {}) {
  ...
  const cardLine = hook
    ? String(cfg.fallback_card_line ?? DEFAULT_FALLBACK_CARD_LINE).replace(/\{\{\s*card\s*\}\}/g, hook)
    : '';
  const ship = shipByPhrase(shipBy, now, cfg.timezone || 'Australia/Sydney') || 'the next business day';
  const body = String(cfg.fallback_body ?? DEFAULT_FALLBACK_BODY)
  ...
  return {
    ...
    provider: 'template',
    model: 'template',
    subject: String(cfg.fallback_subject || DEFAULT_FALLBACK_SUBJECT).slice(0, 120),
    body,
