...
// The `source` stamped on a card_cache row. Mirrors SOURCE_BY_GAME in lib/collector.mjs — the other
// writer of that column — so the two cannot disagree about where a card came from.
const CACHE_SOURCE_BY_GAME = { pokemon: 'pokemontcg', lorcana: 'lorcast', mtg: 'scryfall', riftbound: 'tcgplayer', swu: 'swudb' };
...
export async function loadSetIndex(base, game, { fresh = false } = {}) {
  ...
  try {
    if (game === 'pokemon') {
      ...
      // Without this branch resolveSet() saw an EMPTY index and returned null for every Magic row,
      // so each one warned 'set "X" not matched' and imported unenriched — with the blame on the set
      // NAME rather than on there being no index at all. Everything downstream was already wired:
      // identityFor gives 'hob-249', lookupPath and imageFrom and mapPrice all handle mtg.
      //
      // Server-side, so it goes through the cache rather than Scryfall directly; a cold cache falls
      // back to the live endpoint the same way every other game's branch does.
      const r = await jfetch(`${base}/api/mtg/sets`);
      if (r.ok) {
        const j = await r.json();
        sets = (j.data || [])
          // Digital-only sets cannot be posted in an envelope, and their names shadow the paper sets
          // they rebalance closely enough that resolveSet's containment rules could seat the wrong one.
          .filter((s) => s && !s.digital)
...
// pokemontcg.io hyphenates the mechanic suffix on the two eras that used it — 'Charizard-EX' (BW4
// and XY), 'Kommo-o-GX' (SM) — where Collectr, TCGplayer and every seller on eBay write 'Charizard
// EX'. The hyphen is punctuation joining a SUFFIX, not part of the Pokémon's name, so it becomes a
// token boundary and the two spellings agree.
//
// ONLY these suffixes, and this is the whole point of the rule rather than a shortcut. Splitting on
// every hyphen is the obvious fix and it is wrong, because the hyphen inside a Pokémon's actual name
// is load-bearing:
//
//   Porygon / Porygon-Z   -> {porygon} ⊆ {porygon, z}      two different Pokémon, not a base and its
//                                                          mechanic card — nothing relates them
//   Kommo-o / Kommo-o-GX  -> {kommo, o} ⊆ {kommo, o, gx}   both names are real (name: query, live)
//   Ho-Oh                 -> {ho, oh}                      two tokens that mean nothing apart
//
// Every one of those is the Mew/Mewtwo false-positive this guard exists to stop, re-entering through
// the punctuation instead of through a substring. With the suffix rule they stay blocked: 'Z' is not
// a mechanic marker, so Porygon-Z remains one token, and Kommo-o-GX splits only at its LAST hyphen.
// V / VMAX / VSTAR / ex are already written with a space in the catalogue and need nothing here.
const MECHANIC_SUFFIX = /-(ex|gx)\b/g;
...
export function identityFor(game, setId, lookupNum) {   // exported for the unit harness
...
export async function enrichRow(base, importRow, opts = {}) {
  ...
  // A NON-ENGLISH ROW IS NOT RESOLVABLE HERE, and letting it try is worse than not trying. Every path
  // below reads an English catalogue — the set index, identityFor, the card fetch, and mapPrice most
  // of all — so a Japanese card sharing a set name and a number with an English print resolves to
  // that print and takes its image and its LIVE MARKET PRICE as its own. It is not that card and it
  // does not trade at that price, and the row would carry the number with no mark on it saying so.
  //
  // Nothing above used to reach this: the importer hardcoded language:'EN', so the question was never
  // asked. Now that langFromProductName reads Collectr's '(JP)' tag, it can be.
  //
  // The JP/CN/KO lane does exist (TCGdex via /api/tcgdex, lib/pokemon-intl.mjs) but this importer has
  // never reached it. Until it does, keeping Collectr's own figure and staying unresolved is the
  // honest answer — and the row still comps correctly, because language now rides on it and
  // compsQueryFor puts the language keyword in the search.
  if (String(row.language || 'EN').toUpperCase() !== 'EN') {
    warnings.push(who + ': ' + row.language + ' card — the card APIs wired here are English-only, so identity, image and live price were left as Collectr had them');
    return { row, warnings };
  }
  ...
  try {
    ...
    if (!card) {
      ...
      // A per-game label, not a two-way ternary. That ternary was written when only pokemon and
      // lorcana could reach this line, so everything else was stamped 'lorcast' — latent until the
      // mtg branch above made it reachable, which is why it is fixed in the same change. Mirrors
      // SOURCE_BY_GAME in lib/collector.mjs, the other writer of this column.
      cachePut(opts.db, row.game, identity, CACHE_SOURCE_BY_GAME[row.game] || null, card);
    }
