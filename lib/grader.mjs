...
async function callAnthropic(images, context, env, system, user) {
  const model = (env.GRADER_MODEL || '').trim() || DEFAULT_ANTHROPIC_MODEL;
  const content = [{ type: 'text', text: user || userText(context) }];
  for (const im of images) {
...
async function callOpenAI(images, context, env, system, user) {
...
// askVision({ images, system, user, env }) -> { ok, provider, model, json, text } | { ok:false, error }
//
// The same provider plumbing analyzeCard uses, with the prompt left to the caller. It exists because a
// SECOND vision task arrived — cross-checking a picked bundle against its manifest (lib/runs-pick.mjs) —
// and the alternative was a second copy of the key handling, the image encoding, the two request shapes
// and the JSON extraction. One of those copies would have learned something the other did not.
//
// Returns ok:false rather than throwing, for the same reason analyzeCard does: a missing key or a dead
// provider must degrade the feature, never take down the request that asked for it (GR7).
export async function askVision({ images, system, user, env }) {
  try {
    const provider = pickProvider(env || {});
    if (!provider) return { ok: false, error: 'no_key', message: 'Set ANTHROPIC_API_KEY or OPENAI_API_KEY in .env (or GRADER_PROVIDER).' };
    const imgs = normalizeImages(images);
    ...
    const res = provider === 'anthropic'
      ? await callAnthropic(imgs, null, env, system, user)
      : await callOpenAI(imgs, null, env, system, user);
    const json = extractJson(res.text);
    if (!json) return { ok: false, error: 'parse', message: 'Model did not return parseable JSON.', raw: (res.text || '').slice(0, 400) };
    return { ok: true, provider, model: res.model, json, text: res.text };
  } catch (e) {
    return { ok: false, error: 'provider', message: String((e && e.message) || e) };
  }
}
