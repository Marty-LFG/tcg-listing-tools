...
function itemFinish(item) {
  ...
  // rarity/variant (Epic / Showcase alt-art / Overnumbered are Foil, everything else Normal).
  if (item.game === 'riftbound') {
    return (/showcase|epic|overnumber/i.test(item.rarity || '') || /alternate|overnumber/i.test(v)) ? 'Foil' : 'Normal';
  }
