...
function itemFinish(item) {
  ...
  // Riftbound: finish isn't stored (only variant/rarity are) — it's a pure function of
  // rarity/variant (Epic + every Showcase treatment — alt-art, Overnumbered, Signature and the
  // SP promos — are Foil, everything else Normal). Mirrors finishOf() in lib/riftbound-data.mjs.
  if (item.game === 'riftbound') {
    return (/showcase|epic|overnumber|signature|ultimate/i.test(item.rarity || '') || /alternate|overnumber|signature|ultimate/i.test(v)) ? 'Foil' : 'Normal';
  }
  // Negations before the holo/foil branches — "Non-holo" contains "holo", "Nonfoil" contains
  // "foil" (same ordering as variantToken/finishClass/ebayFinish). This overwrites the row's own
  // finish on both export paths, so getting it wrong ships a base card to eBay as a foil.
  if (/reverse/i.test(v)) return 'Reverse Holofoil';
  if (/non[-\s]?(holo|foil)/i.test(v)) return 'Normal';
  ...
  return async (req, res) => {
    try {
      ...
      if (p === '/enumerate' && method === 'POST') {
        ...
        let count = 0;
        try {
          // `env` is for the Pokémon enumerator, which reads the shared set cache in-process rather
          // than fetching through `base` — the cache needs the API key the same way the proxy does.
          for await (const out of ENUMERATORS[b.game]({ base, env, setId: b.setId, setName: b.setName, filters: b.filters })) {
      ...
      if (p === '/import/collectr' && method === 'POST') {
        ...
        const stats = { total: 0, resolved: 0, unresolved: 0, graded: 0, needs_price: 0, unsupported_game: 0, intl: 0 };
        ...
        for (const r of parsed.rows) {
          ...
          // Counted, because a non-English row behaves differently in every direction that matters
          // (no catalogue, no live price, a different eBay market) and the page shows counts, not warnings.
          if (String(row.language || 'EN').toUpperCase() !== 'EN') stats.intl++;
          if (row.graded) {
            stats.graded++;
...
const COLLECTR_CONFIG_PATH = configFile('collectr.config.json');
