...
// Blend a per-unit money figure when new units join stock that already has one. Weighted by unit
// count, because the stock row carries ONE cost for all its units — there is no per-unit cost layer
// to run FIFO against, and keeping the original price would make every restock misstate margin.
//
// A null on either side is not zero: an unknown old cost adopts the new figure outright (there is
// nothing to average against, and averaging with 0 would halve a known number using an unknown one),
// and an unknown new cost leaves the old one alone.
function blendUnitCents(oldCents, oldQty, newCents, newQty) {
