...
function itemWithPl(db, row) {
  ...
  return { ...row, spark: vals, val_count: vals.length, placements: getPlacements(db, row.id) };
}
...
function makeRouter({ db, env }) {
  ...
  return async (req, res) => {
    try {
      ...
      // most-used first. Drives the add/edit combobox so a spot set once is easy to reselect.
      if (p === '/locations' && method === 'GET') {
        const rows = db.prepare(`SELECT location, COUNT(*) AS n FROM (
