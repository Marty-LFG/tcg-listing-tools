...
import { suggestCategories, categoryAspects } from './ebay-taxonomy.mjs';
import { publishSealedPool, sealedBandFor, toSealedListing, validateSealedListing, poolUnits } from './sealed-listing.mjs';
import { putSealedImage, sealedImageRows, isFetchableImageUrl } from './sealed-images.mjs';
import { bytesFromUrl } from './ebay-media.mjs';
import { blockIfHeld } from './runs-reserve.mjs';
import { storePhotoOriginal, saveConfig as saveListingConfig } from './listings.mjs';
import { runSealedShopifyPublish } from './sealed-shopify.mjs';
// The Shopify switches, imported rather than re-checked. Four guards in two places would be two
// definitions of "disarmed", and the one that drifts is discovered by publishing something.
import {
  loadConfig as loadShopifyConfig, guardCredentials, guardLiveStore, guardPins, guardPublishEnabled,
} from './shopify.mjs';
import { blendUnitCents } from './purchasing-money.mjs';
import { setQuantity } from './stock-ledger.mjs';
...
// The LOCATION mirror stays this statement's business; the QUANTITY goes through the ledger, so a
// change in how many boxes are on the shelf has a reason attached to it. setQuantity derives the delta
// and returns null when nothing moved — re-saving placements to reorder two spots files no movement,
// which is what keeps the ledger readable.
function recomputeItemStock(db, itemId, move = {}) {
  ...
  db.prepare(`UPDATE sealed_items SET location = ?, updated_at = datetime('now') WHERE id = ?`)
    .run(primary ? primary.location : null, itemId);
  setQuantity(db, {
    kind: 'sealed', itemId, quantity: total,
    reason: move.reason || 'manual', refKind: move.refKind || null, refId: move.refId ?? null,
    location: primary ? primary.location : null, note: move.note || null,
  });
}
...
function setPlacements(db, itemId, rows, move = {}) {
  ...
  recomputeItemStock(db, itemId, move);
}
...
function addStock(db, itemId, location, addQty, move = {}) {
...
/**
 * THE ONLY SUPPORTED WAY TO PUT RECEIVED SEALED STOCK INTO THE TOOL.
 *
 * lib/purchasing.mjs calls this when a purchase order is received. It exists so that receiving does
 * NOT write raw INSERTs into sealed_items: sealed_items.quantity is a cached SUM of sealed_placements
 * and sealed_items.location its primary spot, and any writer that skips setPlacements/addStock
 * corrupts that mirror silently — the item still reads fine, it just stops agreeing with the shelf.
 *
 * Two paths, one door:
 *   itemId given -> MERGE. Each placement is added to that item's spots and the per-unit cost basis
 *                   is blended (a restock at a new price moves the average, it does not overwrite).
 *   itemId null  -> CREATE. Mints a SKU and seeds the placements, exactly as POST /items does.
 *
 * Money is per UNIT and must already be in AUD CENTS. sealed_items.cost_cents has no currency sibling
 * and every existing reader assumes AUD — sealed.html converts in the browser before it POSTs, and
 * literally labels the field "stored in AUD". So this function converts NOTHING (GR3): the caller
 * does the one conversion, at the transaction-date rate, and keeps the untouched native figures in
 * its own tables. cost_cents is the raw price paid; acq_fees_cents is that unit's share of the
 * order's freight/tax/fees, which is what makes cost_cents + acq_fees_cents the true landed cost the
 * summaries already read.
 *
 * Caller supplies the transaction — every write here is expected to sit inside one.
 */
export function receiveSealed(db, opts = {}) {
  const { itemId = null, item = {}, costCents = null, acqFeesCents = null,
    acquiredAt = null, sourceVendor = null, poLineId = null } = opts;
  const rows = sanitizePlacements(opts.placements || []);
  const addQty = rows.reduce((s, r) => s + r.quantity, 0);
  if (!(addQty > 0)) throw new Error('receiveSealed: placements must carry at least one unit');

  if (itemId != null) {
    const cur = db.prepare(`SELECT id, sku, quantity, cost_cents, acq_fees_cents FROM sealed_items WHERE id = ?`).get(itemId);
    if (!cur) throw new Error('receiveSealed: no sealed_items row ' + itemId);
    const oldQty = cur.quantity || 0;
    for (const r of rows) addStock(db, itemId, r.location, r.quantity, { reason: 'receive', refKind: poLineId != null ? 'po_line' : 'item', refId: poLineId });
    db.prepare(`UPDATE sealed_items SET cost_cents = ?, acq_fees_cents = ?,
    ...
    const after = db.prepare(`SELECT quantity FROM sealed_items WHERE id = ?`).get(itemId);
    return { id: itemId, sku: cur.sku, created: false, quantity: after.quantity };
  }

  const game = SEALED_GAMES.includes(item.game) ? item.game : 'other';
  if (!item.name) throw new Error('receiveSealed: name is required to create stock');
  // Same normalisation as POST /items: a non-empty but cross-game type must land on 'other' rather
  // than polluting the facet.
  const pt = typesForGame(game).includes(item.product_type) ? item.product_type : 'other';
  // Drop null descriptor fields BEFORE pick, so the column defaults apply. Several of them are
  // NOT NULL DEFAULT (condition, language, factory_sealed), and writing an explicit NULL into one is
  // a constraint failure rather than "use the default" — a caller that simply has no opinion about
  // condition must not be the reason a delivery cannot be put away.
  const clean = {};
  for (const [k, v] of Object.entries(item)) if (v != null) clean[k] = v;
  const obj = pick({ ...clean, game, product_type: pt }, SEALED_INSERT_COLS);
  // INSERTED AT ZERO, then received. setPlacements re-mirrors through the ledger, which measures the
  // delta against this column — so a row inserted already holding its units would file no movement and
  // leave the ledger permanently behind its own cache.
  obj.quantity = 0;
  // Money is set AFTER pick, because null here is meaningful — it records that the cost is unknown,
  ...
  // Set outside pick(): po_line_id is not in SEALED_INSERT_COLS and must not be, or an API caller
  // could claim a stock row came from a purchase it did not.
  if (poLineId != null) obj.po_line_id = poLineId;
  const id = insertRow(db, 'sealed_items', obj);
  setPlacements(db, id, rows, { reason: 'receive', refKind: poLineId != null ? 'po_line' : 'item', refId: poLineId });
...
// Portfolio roll-up over sealed_items rows. Pure over (rows, upcMap) so the money maths is testable —
// it was inline in the route and wrong for two years' worth of multi-unit rows.
//
// BOTH cost and value are PER UNIT and scale by quantity. "Cost paid" sits beside a Quantity box in
// the form, so it means what ONE of them cost; summing it unscaled made a 14-box row of A$300 boxes
// contribute A$300 of cost against A$4608 of value — A$4308 of profit that does not exist.
// Sale price and sale fees are what the sale itself brought in and cost, as entered.
export function summarizeSealed(rows, upcMap = new Map()) {
  const cents = (n) => (n == null ? 0 : +n);
  const qtyOf = (x) => (x.quantity == null ? 1 : Math.max(0, x.quantity));
  ...
  // Unknown is not zero — the same counter summarizeInventory keeps, and rendered on the same tile,
  // for the same reason: cents() turns a missing cost into a 100% margin, and a barcode scan can
  // create a row without one.
  let noCostHeld = 0, noCostSold = 0;
  for (const r of rows || []) {
    counts[r.status] = (counts[r.status] || 0) + 1;
    byGame[r.game] = (byGame[r.game] || 0) + 1;
    byType[r.product_type] = (byType[r.product_type] || 0) + 1;
    const unitCost = cents(r.cost_cents) + cents(r.acq_fees_cents);
    if (r.status === 'sold') {
      // THE SAME RULE AS summarizeInventory, which this did not follow. It multiplied unitCost by
      // qtyOf(r) — the row's CURRENT quantity, which the sale had just set to zero — so the cost
      // dropped out entirely; and sale_price_cents was never written by the sale at all, so revenue was
      // zero too. An automated sealed sale moved this figure by nothing.
      //
      // sale_qty is how many units the sale covered, and both money terms are per unit, exactly as on
      // the singles side. Null is a pre-column row and reads as one.
      const soldQty = r.sale_qty != null ? Math.max(0, +r.sale_qty) : 1;
      if (r.cost_cents == null) noCostSold++;
      realizedPlCents += (cents(r.sale_price_cents) - cents(r.sale_fees_cents) - unitCost) * soldQty;
    } else {
      const qty = r.quantity || 0;
      units += qty;
      if (r.cost_cents == null) noCostHeld++;
      totalCostCents += unitCost * qty;
  ...
  return { counts, totalCostCents, realizedPlCents, valueByCurrency, byGame, byType, units, noCostHeld, noCostSold };
}
...
// The seller's OWN eBay username, so our own live listing cannot feed our own valuation. Same file and
// key lib/listings.mjs `ownSeller()` reads for the singles path; duplicated rather than imported
// because lib/listings.mjs imports PRODUCT_TYPES from THIS module and the cycle would cost more than
// three lines. Never throws (GR7): no config -> no exclusion, exactly as before.
function ownSeller() {
  try { return JSON.parse(fs.readFileSync(configFile('repricer.config.json'), 'utf8')).exclude_seller_username || null; }
  catch { return null; }
}

// LANGUAGE belongs in the QUERY, not only in the filter. eBay AU ranks English listings first, so
// "Abyss Eye Booster Box" returns a page of English boxes and a Japanese box priced off them reads far
// too low. English adds no word, because an English listing is the one that names no language. The
// dedupe below means a name that already says "Japanese" does not say it twice.
const EBAY_LANG_WORD = { JP: 'Japanese', CN: 'Chinese', TW: 'Chinese', KO: 'Korean' };

// Build an eBay AU query for a sealed item (its name usually already carries the set + product type).
export function ebayQueryFor(item) {
  const lang = EBAY_LANG_WORD[String(item.language == null ? '' : item.language).trim().toUpperCase()] || '';
  const seen = new Set();
  return [item.name || '', item.set_name || '', lang].join(' ').split(/\s+/)
...
async function resolveSealedValue(db, item, { env, base }) {
  let eb = null;
  try {
    eb = await sealedEbayValue({ base, query: ebayQueryFor(item), productType: item.product_type,
      language: item.language, excludeSeller: ownSeller() });
  } catch { eb = null; }
...
// `running` means THE TIMER IS ARMED, matching getReconcileState/getRefreshState and every other
// row on the console. It used to report _svRunning — true only during the few seconds a pass is
// actually executing — so a perfectly healthy nightly loop rendered as STOPPED for all but a
// moment of the day. The in-flight fact is still worth having; it is just a different question.
export function getSealedRefreshState() { return { ..._svState, running: !!_svTimer, in_progress: _svRunning }; }
...
function makeRouter({ db, env, base }) {
  ...
  return async (req, res) => {
    try {
      ...
      // market value (per-UPC, shared) × quantity; COST is likewise PER UNIT × quantity (fixed
      // 2026-07-26 — it used to be summed unscaled, which invented profit on every multi-unit row);
      // `units` = SUM(qty).
      if (p === '/summary' && method === 'GET') {
        // sale_qty is NOT optional — see the twin in lib/inventory.mjs. Omitting it makes every sold
        // row summarise as a single unit whatever it actually sold.
        const rows = db.prepare(`SELECT status, game, product_type, cost_cents, acq_fees_cents,
          sale_price_cents, sale_fees_cents, sale_qty, quantity, value_manual, upc, value_cents AS row_value, value_currency AS row_cur FROM sealed_items`).all();
        ...
        return send(res, 200, summarizeSealed(rows, upcMap));
      }
      ...
      // GET /locations/contents — what is actually IN each location, keyed by lowercased name. Sent
      // with the page rather than per-expand: a shop's worth of placements is a few hundred rows, and
      // one round trip beats a spinner on every disclosure triangle.
      if (p === '/locations/contents' && method === 'GET') {
        const rows = db.prepare(`SELECT pl.location AS loc, pl.quantity AS qty, i.id, i.sku, i.name, i.product_type,
            i.game, i.status, i.cost_cents, i.acq_fees_cents, i.value_cents, i.value_currency, i.value_manual, i.upc, i.image_url
          FROM sealed_placements pl JOIN sealed_items i ON i.id = pl.item_id
          WHERE pl.location IS NOT NULL AND TRIM(pl.location) <> ''
          ORDER BY i.name COLLATE NOCASE`).all();
        const upcMap = new Map(db.prepare(`SELECT upc, value_cents, currency FROM sealed_upc_prices WHERE value_cents IS NOT NULL`).all().map((r) => [r.upc, r]));
        const byLoc = {};
        for (const r of rows) {
          const key = String(r.loc).toLowerCase();
          const up = (!r.value_manual && r.upc) ? upcMap.get(normalizeUpc(r.upc)) : null;
          const unitValue = up ? up.value_cents : r.value_cents;
          const unitCost = (r.cost_cents || 0) + (r.acq_fees_cents || 0);   // per unit, like value
          (byLoc[key] = byLoc[key] || []).push({
            id: r.id, sku: r.sku, name: r.name, product_type: r.product_type, game: r.game, status: r.status,
            qty: r.qty || 0, image_url: r.image_url || null,
            unit_value_cents: unitValue == null ? null : unitValue,
            value_currency: up ? (up.currency || 'AUD') : (r.value_currency || 'USD'),
            unit_cost_cents: unitCost || null,
          });
        }
        return send(res, 200, { contents: byLoc });
      }
      ...
      if (p === '/items' && method === 'POST') {
        ...
        const seedQty = obj.quantity;   // what the CALLER asked for, read before the row is zeroed
        obj.quantity = 0;   // setPlacements below fills it through the ledger
        const id = insertRow(db, 'sealed_items', obj);
        ...
        // The seed is built from the quantity the CALLER asked for, captured before the row is
        // zeroed. setPlacements re-mirrors through the ledger by comparing the placement total against
        // the column — so a row inserted already holding its units yields a zero delta and files no
        // movement at all, leaving the ledger behind from the row's first day.
        const seed = Array.isArray(b.placements)
          ? sanitizePlacements(b.placements)
          : sanitizePlacements([{ location: obj.location != null ? obj.location : null, quantity: seedQty != null ? seedQty : 1 }]);
        setPlacements(db, id, seed, { reason: 'opening', refKind: 'item', refId: id, note: 'added by hand' });
      ...
      // ---- SEALED LISTINGS -------------------------------------------------
      // GET /listings/categories?q= — eBay's own category suggestions, so the sealed category can be
      // FOUND rather than guessed. Read-only, app token. It suggests; the owner pins one in config.
      if (p === '/listings/categories' && method === 'GET') {
        const out = await suggestCategories(env, q.get('q') || 'pokemon booster box sealed');
        return send(res, out.ok ? 200 : 502, out);
      }

      // GET /listings/aspects?category_id= — what that category will actually ask for. The FREE_TEXT
      // flag is the one to read: a required FREE_TEXT aspect takes a near-miss silently and earns no
      // facet, which is how two games sat mis-filed for months.
      if (p === '/listings/aspects' && method === 'GET') {
        const cid = q.get('category_id');
        if (!cid) return send(res, 400, { error: 'category_id required' });
        const out = await categoryAspects(env, cid, { match: q.get('match') || '' });
        return send(res, out.ok ? 200 : 502, out);
      }

      // GET /listings/pools — every pool with its live unit count and publish state.
      if (p === '/listings/pools' && method === 'GET') {
        const cfg = loadListingConfig();
        const rows = db.prepare('SELECT * FROM sealed_pools ORDER BY updated_at DESC').all().map((pl) => ({
          ...pl,
          units: poolUnits(db, pl.pool_sku),
          band: sealedBandFor(pl.product_type, cfg),
        }));
        return send(res, 200, { pools: rows });
      }

      // POST /listings/pools — upsert a pool. The pool_sku is the caller's derived key and is the eBay
      // custom label, so it is never rewritten once published: eBay binds a custom label for life.
      if (p === '/listings/pools' && method === 'POST') {
        const b = await readJson(req);
        const sku = String(b.pool_sku || '').trim().toUpperCase();
        if (!/^[A-Z0-9-]{3,44}$/.test(sku)) return send(res, 400, { error: 'pool_sku must be 3-44 chars of A-Z 0-9 and hyphen' });
        const existing = db.prepare('SELECT * FROM sealed_pools WHERE pool_sku = ?').get(sku);
        if (existing && existing.ebay_offer_id && b.pool_sku !== existing.pool_sku) {
          return send(res, 409, { error: 'this pool is already on eBay and its custom label cannot change' });
        }
        const cols = ['game','language','set_code','set_name','product_type','variant','condition','factory_sealed',
          'pack_count','title_override','desc_override','store_categories','postage_band_id','price_cents','name'];
        const vals = {};
        cols.forEach((c) => { if (b[c] !== undefined) vals[c] = b[c]; });
        if (!existing) {
          if (!vals.product_type) return send(res, 400, { error: 'product_type required' });
          const keys = Object.keys(vals);
          db.prepare(`INSERT INTO sealed_pools (pool_sku${keys.length ? ',' + keys.join(',') : ''})
                      VALUES (?${keys.map(() => ',?').join('')})`).run(sku, ...keys.map((k) => vals[k]));
        } else if (Object.keys(vals).length) {
          const keys = Object.keys(vals);
          db.prepare(`UPDATE sealed_pools SET ${keys.map((k) => k + ' = ?').join(', ')}, updated_at = datetime('now')
                      WHERE pool_sku = ?`).run(...keys.map((k) => vals[k]), sku);
        }
        // Bind the stock rows the caller nominated, so poolUnits can see them.
        if (Array.isArray(b.item_ids)) {
          const bind = db.prepare(`UPDATE sealed_items SET pool_sku = ?, updated_at = datetime('now') WHERE id = ?`);
          for (const id of b.item_ids) bind.run(sku, id);
        }
        const row = db.prepare('SELECT * FROM sealed_pools WHERE pool_sku = ?').get(sku);
        return send(res, existing ? 200 : 201, { pool: row, units: poolUnits(db, sku) });
      }


      // ---- POOL IMAGES -----------------------------------------------------
      // eBay's Inventory API will not publish an offer without at least one image, and a pool has no
      // automatic art source, so this is the route that makes a sealed listing possible at all.
      // Uploading happens HERE rather than at publish, which is what lets dry_run stay free.
      m = p.match(/^\/listings\/pools\/([A-Z0-9-]{3,44})\/images$/i);
      if (m && method === 'GET') {
        return send(res, 200, { images: sealedImageRows(db, m[1].toUpperCase()) });
      }
      if (m && method === 'POST') {
        const sku = m[1].toUpperCase();
        if (!db.prepare('SELECT 1 FROM sealed_pools WHERE pool_sku = ?').get(sku)) return send(res, 404, { error: 'no such pool' });
        const b = await readJsonBig(req);
        if (b.__oversize) return send(res, 413, { error: 'image too large' });
        const hasData = !!b.dataUrl, hasUrl = !!b.sourceUrl;
        if (hasData === hasUrl) return send(res, 400, { error: 'send exactly one of dataUrl or sourceUrl' });

        let buffer, filename, contentType, sourceUrl = null;
        if (hasData) {
          const mm = /^data:([^;]+);base64,([\s\S]+)$/.exec(String(b.dataUrl));
          if (!mm) return send(res, 400, { error: 'dataUrl must be a base64 data URL' });
          contentType = mm[1];
          buffer = Buffer.from(mm[2], 'base64');
          filename = 'sealed.' + (contentType.split('/')[1] || 'jpg').replace(/[^a-z0-9]/gi, '');
        } else {
          // Operator-typed URL that the SERVER fetches, stores and later replays, so it gets a guard
          // the browser-driven /api/img proxy does not need.
          const guard = isFetchableImageUrl(b.sourceUrl);
          if (!guard.ok) return send(res, 400, { error: guard.error });
          const got = await bytesFromUrl(guard.url);
          if (!got.ok) return send(res, 502, { error: 'could not fetch that image: ' + got.error });
          buffer = got.buffer; filename = got.filename; contentType = got.contentType; sourceUrl = guard.url;
        }

        const out = await putSealedImage(env, db, {
          poolSku: sku, buffer, filename, contentType, sourceUrl,
          kind: b.kind || (sourceUrl ? 'catalog' : 'owner'),
          position: b.position != null ? b.position : undefined,
        }, { storeOriginal: (buf, ext) => storePhotoOriginal(buf, ext) });
        if (!out.ok) return send(res, 502, { error: 'image upload failed: ' + out.error });
        return send(res, 201, { ok: true, image: out });
      }

      m = p.match(/^\/listings\/pools\/([A-Z0-9-]{3,44})\/images\/(\d+)$/i);
      if (m && method === 'DELETE') {
        // Row only. The retained original and the hosted EPS image are left alone: eBay copies bytes
        // into the offer, so removing our row cannot break a listing that is already live.
        db.prepare('DELETE FROM sealed_pool_images WHERE pool_sku = ? AND id = ?').run(m[1].toUpperCase(), Number(m[2]));
        return send(res, 200, { ok: true });
      }

      m = p.match(/^\/listings\/pools\/([A-Z0-9-]{3,44})\/images\/reorder$/i);
      if (m && method === 'POST') {
        const sku = m[1].toUpperCase();
        const b = await readJson(req);
        const order = Array.isArray(b.order) ? b.order : [];
        const tx = db.prepare('UPDATE sealed_pool_images SET position = ? WHERE pool_sku = ? AND id = ?');
        db.exec('BEGIN');
        try { order.forEach((id, i) => tx.run(i, sku, Number(id))); db.exec('COMMIT'); }
        catch (e) { db.exec('ROLLBACK'); return send(res, 500, { error: String(e?.message || e) }); }
        return send(res, 200, { images: sealedImageRows(db, sku) });
      }

      // POST /listings — publish (or revise) one pool. Idempotent on the SKU: a second call finds the
      // existing offer and revises it rather than creating a competing listing. dry_run builds and
      // validates the whole payload without touching eBay, which is the preflight the page runs.
      if (p === '/listings' && method === 'POST') {
        const b = await readJson(req);
        const sku = String(b.pool_sku || '').trim().toUpperCase();
        if (!sku) return send(res, 400, { error: 'pool_sku required' });
        const cfg = loadListingConfig();
        // image_urls is deliberately NOT taken from the body: it would let any caller put arbitrary
        // URLs on a live eBay offer. Images come from the pool's own rows, uploaded through the route
        // above. The parameter survives on the function for offline tests.
        const out = await publishSealedPool(env, db, cfg, {
          poolSku: sku, dryRun: !!b.dry_run, saveCfg: saveListingConfig,
        });
        // 200 for a clean dry run or a live publish; 409 for "you have not met the preconditions",
        // which is a different fix from an eBay error and deserves its own status.
        return send(res, out.ok ? 200 : (out.code === 'not_publishable' || out.code === 'no_pool' ? 409 : 502), out);
      }

      // POST /listings/shopify { pool_sku, dry_run } — the same pool, onto the storefront.
      //
      // It lives HERE rather than in lib/shopify.mjs's router for the reason lib/runs.mjs owns the run
      // publish: lib/sealed-shopify.mjs imports recordShopifyListing from lib/shopify.mjs, so mounting
      // the route there would close an import cycle. The switches are imported rather than re-checked,
      // so "disarmed" means the same thing on both lanes and there is one place it can change.
      if (p === '/listings/shopify' && method === 'POST') {
        const b = await readJson(req);
        const sku = String(b.pool_sku || '').trim().toUpperCase();
        if (!sku) return send(res, 400, { error: 'pool_sku required' });
        const shopCfg = loadShopifyConfig();
        const store = String(b.store || shopCfg.defaultStore || 'dev');
        const dryRun = !!b.dry_run;
        if (!guardCredentials(env, store, res)) return;
        if (!guardLiveStore(shopCfg, store, res, { dryRun })) return;
        if (!guardPins(shopCfg, store, res)) return;
        if (!dryRun && !guardPublishEnabled(shopCfg, res)) return;
        const out = await runSealedShopifyPublish({
          env, db, base, cfg: shopCfg, poolSku: sku, store, dryRun,
        });
        return send(res, out.ok ? 200 : 502, out);
      }

      // GET /listings/pushes?pool_sku= — the audit trail, newest first.
      if (p === '/listings/pushes' && method === 'GET') {
        const sku = String(q.get('pool_sku') || '').trim().toUpperCase();
        const rows = sku
          ? db.prepare('SELECT * FROM sealed_listing_pushes WHERE pool_sku = ? ORDER BY id DESC LIMIT 100').all(sku)
          : db.prepare('SELECT * FROM sealed_listing_pushes ORDER BY id DESC LIMIT 100').all();
        return send(res, 200, { pushes: rows });
      }
      ...
      if ((m = p.match(/^\/items\/(\d+)$/)) && method === 'PATCH') {
        ...
        // QUANTITY NEVER GOES IN THE WHITELIST UPDATE — not on either branch. Placements own it when
        // they are sent, and the LEDGER owns it when they are not. It used to survive into the scalar
        // branch's UPDATE, which wrote the new number directly; setPlacements then re-mirrored from the
        // column that had just been written, so the delta was zero, no movement was filed, and the
        // ledger fell permanently behind on the most-used sealed stock path.
        const wantQty = Object.prototype.hasOwnProperty.call(obj, 'quantity') ? obj.quantity : undefined;
        delete obj.quantity;
        if (hasPlacements) { delete obj.location; }   // placements own this
        const cols = Object.keys(obj);
        ...
        if (hasPlacements) {
          ...
          // The TARGET comes from the request, not from a column the request just wrote — that
          // round trip is what made the delta zero. setPlacements is handed the number the operator
          // asked for and re-mirrors onto it, filing the movement on the way.
          const cur = db.prepare(`SELECT quantity, location FROM sealed_items WHERE id = ?`).get(id);
          const target = wantQty !== undefined ? Math.max(0, Math.round(Number(wantQty) || 0)) : cur.quantity;
          const loc = Object.prototype.hasOwnProperty.call(obj, 'location') ? obj.location : cur.location;
          setPlacements(db, id, [{ location: loc, quantity: target }], { reason: 'manual', refKind: 'item', refId: id, note: 'edited by hand' });
        }
        return send(res, 200, { updated: true });
      }
      ...
      // DELETE /items/:id
      //
      // A HARD DELETE, AND NOW A GUARDED ONE. The row's valuation history cascades away with it
      // (inventory_valuations, listing_images), so deleting a SOLD row destroys the only record of what
      // it cost and what it made — realizedPlCents simply drops by that amount with nothing to say why.
      // Deleting a LISTED row is worse in the other direction: nothing delists, so the listing stays
      // buyable on a channel with no local record behind it.
      //
      // The BATCH delete two routes down has refused exactly this since it was written — `AND status =
      // 'in_stock'` — so the bulk path was already safe and the single-item path deleted anything. This
      // makes them agree. ?force=1 is the deliberate override, because "I know, remove it anyway" is a
      // real thing an owner needs and a refusal with no way past is its own bug.
      if ((m = p.match(/^\/items\/(\d+)$/)) && method === 'DELETE') {
        const id = +m[1];
        const row = db.prepare(`SELECT status, sku FROM sealed_items WHERE id = ?`).get(id);
        if (!row) return send(res, 200, { removed: true, already: true });
        // TWO INDEPENDENT GUARDS on one delete, landed here by two branches, and both are wanted: one
        // protects the cost basis of a row that has moved past in_stock, the other protects a physical
        // object a Keeper's Run has already committed to a numbered bundle.
        //
        // The run hold goes FIRST because it is the harder claim. A reservation means this unit is named
        // in a manifest that is already published and anchored, so deleting it falsifies a public
        // commitment rather than only losing local history — and blockIfHeld's message names the run and
        // bundle to release, which is more actionable than "it is listed".
        //
        // Each keeps the force test its own side was written against — `q.has('force')` here,
        // `q.get('force') === '1'` inside blockIfHeld — rather than unifying them in a merge and
        // silently changing what `?force` alone does to either guard.
        const blocked = blockIfHeld(db, { kind: 'sealed', itemId: id, force: q.get('force') === '1', action: 'delete sealed item' });
        if (blocked) return send(res, 409, blocked);
        if (!q.has('force') && row.status !== 'in_stock') {
          return send(res, 409, {
            error: `${row.sku} is ${row.status} — deleting it would destroy its cost basis and sale history`
              + (row.status === 'listed' ? ', and nothing would delist it' : '')
              + '. Pass ?force=1 to remove it anyway.',
            code: 'not_in_stock', status: row.status, sku: row.sku,
          });
        }
        db.prepare(`DELETE FROM sealed_items WHERE id = ?`).run(id);
        return send(res, 200, { removed: true });
      }
          ...
          for (const raw of rows) {
            try {
              ...
              if (existing) {
                ...
                addStock(db, existing.id, raw.location || null, qty, { reason: 'receive', refKind: 'batch', refId: batchId });   // bumps the matching placement + re-mirrors quantity
                stats.updated++;
                continue;
              }
              const obj = pick({
              ...
              obj.sku = nextSku(db, game);
              const seedQty = obj.quantity;
              obj.quantity = 0;   // setPlacements below fills it through the ledger
              const itemId = insertRow(db, 'sealed_items', obj);
              setPlacements(db, itemId, sanitizePlacements([{ location: obj.location != null ? obj.location : null, quantity: seedQty }]),
                { reason: 'opening', refKind: 'batch', refId: batchId, note: 'imported from a barcode scan' });
      ...
      if ((m = p.match(/^\/batches\/(\d+)$/)) && method === 'DELETE') {
        ...
        // The sealed twin of lib/inventory.mjs's batch delete, and it refuses for the same reason. Sealed
        // is normally the ONE case where a run hold shrinks the sellable quantity instead of blocking —
        // one sealed row genuinely supplies both a run and the shop. That answer does not exist here:
        // there is no smaller version of deleting the row the run draws its boosters from.
        if (alsoItems) {
          const scope = db.prepare(`SELECT id FROM sealed_items WHERE batch_id = ? AND status = 'in_stock'`).all(id);
          const blocked = [];
          for (const row of scope) {
            const hit = blockIfHeld(db, { kind: 'sealed', itemId: row.id, force: q.get('force') === '1', action: 'delete batch items' });
            if (hit) blocked.push(hit);
          }
          if (blocked.length) {
            return send(res, 409, {
              code: 'reserved_for_run', blocked,
              error: `${blocked.length} sealed item(s) in this batch are held for a run; release them, or pass ?force=1 to override`,
            });
          }
          db.prepare(`DELETE FROM sealed_items WHERE batch_id = ? AND status = 'in_stock'`).run(id);
        }
