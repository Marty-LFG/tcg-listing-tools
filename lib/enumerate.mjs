...
import { PRINTING_TO_FINISH, PRINTING_TO_EDITION, variantToken, formatCardNumber, pkmSetFacts, pkmRarityFor,
  ...
  async *pokemon({ env, setId, setName, filters }) {
    ...
    if (!cards || !cards.length) { yield { warning: `pokemon set ${setId}: no cards available (upstream down and nothing cached)` }; return; }
    {
      for (const c of cards) {
        seen++;
        const rarity = pkmRarityFor(c);    // corrected where upstream is wrong (the RGB Mews)
        if (!wantRarity(rarity, filters)) continue;
        // Card-exact printed collector number — era/promo/subset aware (Golden Rule 10).
        // '039a' alt-art suffixes stay verbatim (GR5). The id picks between cards sharing a number.
        const number = formatCardNumber(c.number, c.set || {}, { rarity: c.rarity, id: c.id });
        const tp = (c.tcgplayer && c.tcgplayer.prices) || null;
        // Printing matrix from the price keys; no price object → one row from the
        // rarity heuristic (still listable, market null — GR7). A holo-only set is Holo
        // before any heuristic: every card in 30th Celebration is foil, commons included.
        let keys = tp ? Object.keys(tp) : [];
        const holoSet = !!(pkmSetFacts((c.set && c.set.id) || setId) || {}).holo;
        if (!keys.length) keys = [holoSet || /holo/i.test(rarity) ? 'holofoil' : 'normal'];
        for (const k of keys) {
  ...
  async *riftbound({ setId, setName, filters }) {
    for (const { card, canonical, setMeta } of iterateRiftboundSet(setId)) {
      ...
      yield { row: {
        ...
        // The Illustrator aspect (lib/channels/ebay-map.mjs put('Illustrator', ...)) is ungated, so
        // carrying the artist here lights it up on bulk-listed rows with no change there. '' on a
        // catalog baked before the field existed.
