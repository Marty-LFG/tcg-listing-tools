...
// in a parameter (duplicatePolicyId=…). An AU account may only hold one policy per category type per
// kind, so this fires even when OUR name is unused — the id is the only way back to it.
function duplicatePolicyId(json) {
  for (const e of restErrors(json)) {
    for (const p of e.parameters) {
      const m = /^duplicatePolicyId\s*=\s*(.+)$/i.exec(String(p));
...
export async function bootstrapAccount(env, cfg) {
  ...
  for (const [k, v] of [['payment', pay], ['return', ret], ['fulfillment', ful]]) {
    ...
    else if (v.adopted) report.warnings.push(`${k} policy: reusing the account's existing policy ${v.id}${v.adoptedName ? ` ("${v.adoptedName}")` : ''} — eBay allows only one per category type, so its terms are whatever you set on eBay, not the config`);
  }
