...
import { resolveServiceLabel, curateBandServices } from './postage.mjs';
...
// immediatePay is the OWNER'S CHOICE, not an API requirement. The belief that the Sell Inventory API
// demands it is widespread and was repeated here as a hard error; it traces to a note eBay has since
// deleted from the Account API contract, written in the PayPal-only era before managed payments. The
// live contract describes immediatePay as conditional on seller intent and defaults it to FALSE, and
// the one place eBay genuinely requires it is bulkMigrateListing — converting a Trading listing into
// the Inventory model — which this repo does not do. publishOffer's own required-fields page does not
// mention immediate payment at all.
//
// It matters because it is a trade-off with a real cost on each side, and only the owner can price it:
//   ON  — the buyer pays at checkout or does not buy. Nothing is ever held against money that has not
//         arrived, but a buyer CANNOT request a combined total, so there is no invoice to discount.
//   OFF — a buyer can commit, ask for a total, and be sent one invoice with one lot of postage. The
//         cost is that single-quantity stock sits committed-but-unpaid for up to 4 days.
// Defaults to TRUE, which is the store's live state today, so nothing moves until the owner says so.
export function immediatePayWanted(cfg) {
  return (((cfg && cfg.payments) || {}).immediatePay) !== false;
}
export function paymentBody(cfg) {
  // Managed payments: NO paymentMethods. name + marketplace + categoryTypes + the owner's immediate-pay
  // choice. Nothing here posts this — it is the REFERENCE SHAPE checkPolicyConstraints reads a live
  // policy back against, so it has to describe the policy the owner actually meant to make.
  return { name: cfg.policyNames.payment, marketplaceId: cfg.marketplaceId, categoryTypes: ALL_CATS, immediatePay: immediatePayWanted(cfg) };
}
...
export function checkPolicyConstraints(kind, row, cfg, band) {
  ...
  if (kind === 'payment_policy') {
    // WARNING, not error, and the rubric above is why: eBay publishes happily either way, so this can
    // only ever be "the live policy disagrees with what the config says the owner wanted" — the same
    // shape as the returns check below. It used to be a hard error asserting the Inventory API demands
    // immediate payment, which is not true (see immediatePayWanted). accountReadyGuard never consulted
    // this, so publishing was never actually blocked by it — what the error DID do was fold into
    // accountStatus().ready and paint the Settings setup card red, telling the owner to go and undo
    // the very change that makes invoice requests possible.
    //
    // Both directions are named, because each has a consequence somebody has to have chosen.
    const wantImmediate = immediatePayWanted(cfg);
    if (row.immediatePay !== wantImmediate) {
      add('warning', 'immediatePay', wantImmediate
        ? `${who} does not require immediate payment, but the config asks for it — buyers can commit without paying, so a single-quantity card can sit unavailable for up to 4 days.`
        : `${who} requires immediate payment, but the config asks for it to be off — buyers cannot request a combined total on an immediate-payment listing, so there is never an unpaid order to invoice and invoice requests silently do nothing.`);
    }
...
export function verifyBandPolicies(bands, rows, idKey) {
  const out = { ok: true, bands: [], errors: [], warnings: [], notes: [] };
  ...
  for (const b of bands || []) {
    ...
    const e = { id: b.id, label: b.label || b.id, policyId: pid || null, policyName: row ? (row.name || null) : null,
      ...
      wantService: b.serviceCode || null, gotService: terms ? terms.serviceCode : null,
      // Is a combined-postage rule attached, and which one? A DISTINCT profile per band is worth
      // seeing: eBay groups a cart by discount profile, so three profiles means a mixed-band order is
      // three groups, and a buyer with one cheap card and one dear one very likely pays twice.
      combined: terms ? terms.combined : null, discountProfileId: terms ? terms.discountProfileId : null,
      serviceCount: terms ? terms.serviceCount : null,
      ok: false, error: null };
    ...
    if (e.ok) {
      ...
      // Not a warning any more: the description renders the whole list as a table, so a policy
      // offering a choice is described accurately rather than partially. Kept as information because
      // the SENTENCE still quotes the first service, and which one eBay puts first is eBay's call.
      if (terms.serviceCount > 1) out.notes.push(`the "${e.label}" band's policy offers ${terms.serviceCount} services — all of them are listed in the description, and the sentence quotes the first`);
      ...
      // The options TABLE quotes every service's price, not just the first, and those figures are
      // config data written at capture rather than read live at render. Only band.costCents was ever
      // checked (above), so a policy repriced in Seller Hub would leave the description quoting the
      // old amount to buyers indefinitely. Costs are never healed the way labels are: a price that
      // moved is a real change the owner should see, not something to paper over.
      const liveCost = new Map((terms.services || []).map((s) => [s.code, s.costCents]));
      for (const s of (b.services || [])) {
        if (!s || !s.code) continue;
        const name = s.label || s.code;
        if (!liveCost.has(s.code)) {
          out.warnings.push(`the "${e.label}" band lists a "${name}" option its policy no longer offers — the description is showing buyers a service they cannot pick`);
        } else if (liveCost.get(s.code) !== s.costCents) {
          const got = liveCost.get(s.code);
          out.errors.push(`the "${e.label}" band quotes A$${(s.costCents / 100).toFixed(2)} for "${name}" but its policy charges `
            + `${got == null ? 'free postage' : 'A$' + (got / 100).toFixed(2)} — the description would quote a price the checkout does not`);
          out.ok = false;
          e.ok = false;
        }
      }
...
export async function bootstrapAccount(env, cfg) {
  ...
  for (const band of bands) {
    ...
    // Capture the policy's real service list so the description's options table is built from what
    // eBay will actually charge, not from anything typed by hand. An owner-edited LABEL is preserved
    // by code: the codes and prices are eBay's truth, but the buyer-facing wording is the owner's, and
    // a re-run should not overwrite "Tracked satchel" with "AUP 500 G SATCHEL SIG".
    // The prior map is keyed on code and carries the label AND where that label came from, because
    // "keep what is there" was the original bug: the first capture on a config with no services wrote
    // prettifyServiceCode() output, and every later run then saw that as a prior worth preserving. A
    // machine label is detectable (isDerivedServiceLabel) so resolveServiceLabel repairs it, while a
    // name the owner really chose is preserved unconditionally.
    const prior = new Map((band.services || []).map((s) => [s.code, s]));
    const services = r.row
      ? fulfillmentTerms(r.row).services.map((s) => {
        const was = prior.get(s.code) || {};
        const { label, labelSource } = resolveServiceLabel(s.code, was.label, was.labelSource);
        return { code: s.code, label, labelSource, costCents: s.costCents, sortOrder: s.sortOrder };
      })
      : curateBandServices([band])[0].services || [];
    report.bands.push({ id: band.id, label, policyId: r.id, policyName: r.pinnedName || null, created: false, services });
  ...
  if (listed.ok) {
    ...
    report.notes = (report.notes || []).concat(report.bandCheck.notes || []);
  } else {
...
export async function accountStatus(env, cfg) {
  ...
  const locKey = String(((cfg && cfg.location) || {}).merchantLocationKey || '').trim();
  const [opted, sub, listed, paidL, retL, locGot] = await Promise.all([
    ...
    // The merchant location was never checked against eBay — `ready` asked only whether OUR config
    // held a non-empty string. getInventoryLocation existed but was reachable only through
    // bootstrapAccount, so a location deleted or disabled in Seller Hub left this reporting ready,
    // createOffer succeeding, and publishOffer failing with a generic error nothing could explain.
    locKey ? getInventoryLocation(env, locKey) : Promise.resolve({ ok: false, location: null, error: null }),
  ]);
  ...
  // A policy list that could not be READ is not a policy list that is fine. Until 2026-08-31 this
  // function could not tell those apart: on a failed read `bandCheck` came back null, `!bandCheck`
  // satisfied the bandsOk test, rowFor() returned null for every kind so checkPolicyConstraints had
  // nothing to object to — and `ready` came out TRUE. eBay's Account API answered 500 for the whole
  // of that morning's window and both stock tools showed green while every publish failed.
  //
  // An account we cannot read is its own state: DEGRADED. It is neither ready nor misconfigured,
  // and saying either one is a lie. `bandCheck: null` keeps meaning "unverified" — the doubt is
  // carried by `degraded` rather than smuggled into a check that looks like it passed.
  const policyReads = { fulfillment: listed.error || null, payment: paidL.error || null, return: retL.error || null };
  const policyReadError = policyReads.fulfillment || policyReads.payment || policyReads.return || null;
  // A location we could not read is degraded; a location that came back 404 is genuinely gone. Only
  // the second is a configuration fault, so only the second contradicts `ready` on its own.
  const locMissing = !!locKey && !locGot.ok && !!locGot.error && /not\s*found|\b404\b|25801|25802/i.test(String(locGot.error));
  const locUnreadable = !!locKey && !locGot.ok && !locMissing && !!locGot.error;
  const degraded = !!policyReadError || locUnreadable;
  ...
  return {
    ...
    // Verified against eBay, not just against our own config file.
    merchantLocation: {
      key: locKey || null,
      verified: !!locKey && locGot.ok,
      missing: locMissing,
      unreadable: locUnreadable,
      status: (locGot.location && locGot.location.merchantLocationStatus) || null,
      error: locGot.error || null,
    },
    // Nothing could be read from eBay, so nothing here has been confirmed. The tools pause
    // publishing on this rather than pretending the account is either good or broken.
    degraded,
    policyReadError,
    policyReads,
    ready: !degraded && optedIn && !!p.paymentPolicyId && !!p.returnPolicyId && bandsOk
      && !!loc.merchantLocationKey && !locMissing && policyCheck.ok,
  };
}
