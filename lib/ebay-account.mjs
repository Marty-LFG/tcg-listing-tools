...
// opts.pinnedId / opts.name override where the pin and the policy name come from. Fulfilment passes a
// BAND's id and name (each band owns its own policy); payment/return read cfg.policies as before.
async function ensurePolicy(env, kind, cfg, buildBody, opts = {}) {
  ...
  const name = opts.name !== undefined ? opts.name : cfg.policyNames[{ payment_policy: 'payment', return_policy: 'return' }[kind]];
...
export async function bootstrapAccount(env, cfg) {
  ...
  report.policies = {
    paymentPolicyId: pay.id, returnPolicyId: ret.id,
    created: { payment: pay.created, return: ret.created },
  };
  ...
  for (const band of bands) {
    const r = await ensurePolicy(env, 'fulfillment_policy', cfg, (c) => fulfillmentBody(c, band),
      { pinnedId: band.policyId, name: band.policyName || `TCG postage — ${band.label || band.id}` });
    report.bands.push({ id: band.id, label: band.label || band.id, policyId: r.id, policyName: r.pinnedName || r.adoptedName || null, created: !!r.created });
    if (r.error) report.errors.push(`"${band.label || band.id}" postage policy: ${r.error}`);
    else if (r.unverified) report.warnings.push(`"${band.label || band.id}" postage policy: kept your chosen policy ${r.id} unchecked — eBay's policy list was unreadable (${r.unverified})`);
    else if (r.adopted) report.warnings.push(`"${band.label || band.id}" postage policy: reusing the account's existing policy ${r.id}${r.adoptedName ? ` ("${r.adoptedName}")` : ''} — its terms are whatever you set on eBay, not what the config says`);
  }
