...
export function composeMetaFor(item = {}) {
  ...
  if (!pkm) {
    ...
    const setName = (lset && lset.name) || (mset && mset.name) || (rbset && rbset.name) || (art && art.name) || ident.name || '';
    const code = String((art && art.code) || (rbset && rbset.code) || ident.code || '').toUpperCase();
    return {
      productType,
      cardName: item.name || '',
      ...
      // beats the game logo; the game logo beats an empty rail. The asset is RAIL-ONLY.
      setLogoUrl: (art && art.logoUrl) || '',
      setLogoAsset: (art && art.logoUrl) ? '' : railLogoAsset(game),
      ...
      setAbbrev: (game === 'swu' || game === 'riftbound') ? code : '',
    };
  }
  ...
  if (intl) {
    ...
    return {
      ...
      setLogoUrl: intlLogo,
      setLogoAsset: intlLogo ? '' : railLogoAsset('pokemon'),
      setAbbrev: '',
    };
  }
  ...
  return {
    ...
    setSymbolUrl: enSym,
    setLogoUrl: enLogo,
    setLogoAsset: enLogo ? '' : railLogoAsset('pokemon'),
    setAbbrev: '',
  };
}
