...
import { readCache as readPkmSetsCache, findSet, findIntlSet, findSetSymbol } from './pkm-sets-cache.mjs';
...
export function composeMetaFor(item = {}) {
  ...
  if (intl) {
    const setName = intl.name_en || intl.name_native || item.set_name || '';
    ...
    const sym = findSetSymbol(setName) || findSetSymbol(intl.name_native);
    return {
      productType,
      ...
      // No JP logo source is baked yet — the rail simply leaves that corner empty.
      setLogoUrl: '',
    };
  }
  ...
  const enSym = (set && set.images && set.images.symbol) || (findSetSymbol(item.set_name) || {}).url || '';
  return {
    productType,
    language: ebayLanguageName(item.language),
    ...
    rarity: item.rarity || '',
    setSymbolUrl: enSym,
    setLogoUrl: (set && set.images && set.images.logo) || '',
  };
}
