...
export function composeMetaFor(item = {}) {
  ...
  if (intl) {
    ...
    const setName = intl.name_en || item.set_name || '';
    ...
    return {
      ...
      setLogoUrl: (findSetLogo(intl.code, intl.tcgdexId, setName, intl.name_native) || {}).url || '',
    };
  }
  ...
  return {
    ...
    setLogoUrl: (set && set.images && set.images.logo) || (findSetLogo(item.set_code, item.set_name) || {}).url || '',
  };
}
