...
import { toEbayListing, validateListing, loadEbayCategories, buildItemDescription, ebayLanguageName, checkStoreCategories } from './channels/ebay-map.mjs';
...
import { loadRiftboundSets, resolveRiftboundCard, riftboundDisplayName } from './riftbound-data.mjs';
...
import { compsQueryFor, compsNumberMatch, finishHint } from './stock-games.mjs';
...
import { applyMovement, setQuantity } from './stock-ledger.mjs';
// The sealed product-type taxonomy, imported rather than restated — see isSealedProductType below.
import { PRODUCT_TYPES } from './sealed.mjs';
...
const DEFAULT_CONFIG = {
  ...
  // Whether the pinned payment policy should REQUIRE IMMEDIATE PAYMENT. Not an API constraint — eBay
  // publishes either way — but it decides whether buyers can ask for a combined total at all, because
  // an immediate-payment listing never produces an unpaid order to invoice. Defaults to true, which is
  // the store's live state, so nothing changes until the owner turns it off on purpose.
  // lib/ebay-account.mjs immediatePayWanted() reads it; nothing here creates or edits an eBay policy.
  payments: { immediatePay: true },
  ...
  // sealedCategory is separate from categoryByGame because that map is keyed by GAME, and a sealed
  // box and a single share 'pokemon'. Blank by default: an unset path lets eBay's own default apply
  // rather than this file inventing a department that may not exist on the store.
  store: { defaultCategory: '', categoryByGame: {}, sealedCategory: '', sealedCategoryByType: {} },
  // RETAINED, and it must stay false. Nothing here publishes Best Offer terms any more — the shop
  // answers a haggle with an invoice (lib/deals.mjs) — so this block no longer configures the publish
  // path. It is kept as the written record of that decision, and lib/status.mjs REFUSES to save it as
  // true rather than letting a flag sit there doing nothing. The percentages remain because the
  // repricer's floor-scaling still speaks in them for the hand-made listings that do have Best Offer on.
...
export function loadConfig() {
  try {
    ...
    return {
      ...
      // curateBandServices runs on every READ, not just at capture. That is deliberate: a config
      // written before the label resolver existed holds prettified codes (a live listing rendered
      // "AUP 500 G SATCHEL SIG"), and healing on read means the buyer sees a proper name immediately
      // rather than after someone remembers to re-run bootstrap.
      shipping: { ...DEFAULT_CONFIG.shipping, ...(raw.shipping || {}), bands: curateBandServices(normalizeBands((raw.shipping || {}).bands)) },
      policies: { ...DEFAULT_CONFIG.policies, ...(raw.policies || {}) },
      payments: { ...DEFAULT_CONFIG.payments, ...(raw.payments || {}) },
...
export function guardConnected(env, res) {
...
export const OVERRIDE_FIELDS = ['character', 'card_type', 'speciality', 'illustrator', 'card_size', 'stage',
  ...
  // Non-English Pokémon provenance (lib/stock-games.mjs normalizeIntlCard -> card_facts). Undefined
  // on an English row, and pickOverrides drops undefined, so the English payload is untouched.
  'native_name', 'romaji', 'native_set', 'en_set',
  ...
  // Riftbound, and for exactly the reason above. data/riftbound.json is GITIGNORED and re-baked
  // daily, so buildRowIn's re-resolve can come back empty on a fresh clone or mid-rename — and the
  // stock tools already hold every one of these in card_facts at the moment they publish. Without
  // them here the uploader has no way to say so, and the card details table plus the Card Type,
  // domain and Character aspects all go silently empty.
  'rb_type', 'rb_domain', 'rb_tags', 'rb_e', 'rb_p', 'rb_m'];
...
// Is this row a SEALED product? `sealed_items.product_type` holds the granular taxonomy
// ('booster_box', 'elite_trainer_box', 'tin', …) and is NEVER the literal string 'sealed', so the
// old `=== 'sealed'` test could not be true for a row in EITHER table — which made the `sealed`
// profile and the `sealed` rail art unreachable from every DB-driven path. DERIVED from
// PRODUCT_TYPES, never copied, so a new product type cannot quietly fall out of the set.
//
// Provably a no-op for eBay: `inventory_items` has no product_type column at all, so the value is
// undefined on every publish-path row and includes(undefined) is false. Pinned by a test.
const isSealedProductType = (v) => v === 'sealed' || (v != null && PRODUCT_TYPES.includes(v));

// The facts a SLAB carries that a raw single does not. Deliberately absent from the rail text (see
// railText) but needed by the Shopify bands, where a slab is one of one — its cert is its SKU under
// D-012 — so there is no NM/LP pair to split into two composites.
const slabFacts = (item, productType) => (productType !== 'slab' ? {} : {
...
/**
 * @param item      the stock row
 * @param options   { productType } — an explicit override for callers that already know, because
 *                  the table you read from IS the product type. Zero schema change, no migration.
 */
export function composeMetaFor(item = {}, options = {}) {
  const graded = !!(item.grading_company || item.grade);
  const productType = options.productType
    || (isSealedProductType(item.product_type) ? 'sealed' : graded ? 'slab' : 'single');
  const slab = slabFacts(item, productType);
  ...
  if (!pkm) {
    // Every game resolves against its OWN set universe and never constructs a URL from a code:
    // svgs.scryfall.io answering 200 for a code we invented is the images.scrydex.com placeholder
    // trap over again (AGENTS.md 19), and a wrong symbol is worse than none. The `!pkm` guard is
    // load-bearing — findSet/findSetSymbol/findSetLogo match bare codes across the POKÉMON
    // universe, so an unguarded MTG 'LTR' would wear Legendary Treasures' branding (GR4 + GR10).
    //
    // Rows for swu/onepiece/riftbound carry no set_code at all and store `set_name` as
    // "Name (CODE)" — splitSetIdent recovers both identities before any lookup.
    const game = item.game;
    const ident = splitSetIdent(item.set_name, item.set_code);
    const mset = game === 'mtg' ? findMtgSet({ code: ident.code, name: ident.name }) : null;
    const lset = game === 'lorcana' ? findLorcanaSet({ code: ident.code, name: ident.name }) : null;
    const rbset = game === 'riftbound' ? findRiftboundSetMeta(ident) : null;
    // The baked per-game set-art index (lib/set-art-data.mjs): wordmark URLs for MTG + Lorcana,
    // printed denominators for Lorcana + SWU, the name↔code join for One Piece. Null on a host
    // that has not run the bakes — everything below degrades to the game logo + verbatim number.
    const art = findGameSetArt(game, ident);
    // When NOTHING resolves, the owner's stored name passes through VERBATIM — parens included.
    // The stripped ident is a lookup key, not a display value: "Weekly Play (Promo)" losing its
    // parenthetical on the rail because "(Promo)" happened to be code-shaped would be the splitter
    // rewriting identity it failed to confirm.
    const setName = (lset && lset.name) || (mset && mset.name) || (rbset && rbset.name) || (art && art.name) || item.set_name || '';
    // The code badge draws ONLY a code confirmed against the game's own roster/bake — never the
    // raw paren-derived guess. "Weekly Play (Promo)" must not wear a boxed "PROMO" the card does
    // not print (GR4); an unbaked host simply shows no badge, same degradation as the denominators.
    const confirmedCode = String((art && art.code) || (rbset && rbset.code) || '').toUpperCase();
    // Riftbound's rail carries the champion-first name the titles use (riftboundDisplayName), and
    // for the same reason: a rail reading "KEEPER OF THE HAMMER" over a card every player calls
    // Poppy. Resolved from the bake, and only when the row still carries the bake's own name.
    const rbcard = game === 'riftbound' && rbset ? resolveRiftboundCard(rbset.code, item.number) : null;
    const rbName = rbcard && item.name === rbcard.name ? riftboundDisplayName(rbcard) : '';
    return {
      productType,
      // One Piece rows carry optcgapi's digit disambiguator ("…(Bentham) (055)") — the printed
      // card does not, and the rail shows the printed name (GR10's spirit). Alias parentheticals
      // stay: they ARE printed on the card.
      cardName: game === 'onepiece' ? String(item.name || '').replace(/\s*\(\d{2,4}\)\s*$/, '') : (rbName || item.name || ''),
      language: ebayLanguageName(item.language),
      setName,
      cardNumber: gameCardNumber(game, item.number, art && art.printedTotal),
      rarity: item.rarity || '',
      setSymbolUrl: (mset && mset.icon_svg_uri) || '',
      // A real wordmark (wiki-hosted, publicly fetchable — the description masthead uses it too)
      // beats the game logo; the game logo beats an empty rail. The asset is RAIL-ONLY, and the
      // compositor falls back to it at render time if the wordmark URL stops fetching.
      setLogoUrl: (art && art.logoUrl) || '',
      setLogoAsset: railLogoAsset(game),
      // SWU and Riftbound print their set CODE on the card where Pokémon prints a symbol, so the
      // badge shows the boxed code. The other games' printed identity is already in the number
      // (OP01-120) or the wordmark, and a code they do not print would be furniture (GR4).
      setAbbrev: (game === 'swu' || game === 'riftbound') ? confirmedCode : '',
      ...slab,
    };
  }
  ...
  if (intl) {
    ...
    const intlLogo = (findSetLogo(item.language, intl.code, intl.tcgdexId, setName, intl.name_native) || {}).url || '';
    return {
      productType,
      cardName: item.name || '',
      language: ebayLanguageName(item.language),
      ...
      setLogoUrl: intlLogo,
      setLogoAsset: railLogoAsset('pokemon'),
      setAbbrev: '',
      ...slab,
    };
  }
  ...
  // index is the fallback for sets it does not carry; the game logo backstops the rail after both.
  const enLogo = (set && set.images && set.images.logo) || (findSetLogo('en', item.set_code, item.set_name) || {}).url || '';
  return {
    productType,
    cardName: item.name || '',
    language: ebayLanguageName(item.language),
...
// "Origins (OGN)" -> { name: 'Origins', code: 'OGN' }. The builders for swu/onepiece/riftbound
// (and sometimes lorcana/mtg) decorate set_name with the code because inventory_items has no
// set_code column; an explicit item.set_code (via card_facts or an override) always wins. The
// code shape is deliberately narrow — "(Alternate Art)" in a name is not a set code.
export function splitSetIdent(setName, setCode) {
  const s = String(setName == null ? '' : setName).trim();
  // {0,11}: Lorcana's set codes are single digits ('1'), so the code must be allowed to be one
  // character long — a {1,11} tail silently rejected "(1)" and cost TFC its wordmark and total.
  const m = s.match(/^(.*?)\s*\(([A-Za-z0-9][A-Za-z0-9._-]{0,11})\)$/);
  return {
    name: (m ? m[1] : s).trim(),
    code: String(setCode || (m ? m[2] : '') || '').trim(),
  };
}

// logos/rail/<game>.png — the rail-only game-logo fallback. Presence per game is pinned by
// test/invariants/rail-logos.test.mjs, so this can stay a simple derivation.
export const railLogoAsset = (game) => (GAMES.includes(game) ? `logos/rail/${game}.png` : '');

// Riftbound's set roster is already baked (data/riftbound.json — names, codes, printed totals),
// so it needs no set-art bake; this is the same match the set-art indexes do, over that roster.
function findRiftboundSetMeta({ code, name } = {}) {
  let sets = [];
  try { sets = loadRiftboundSets(); } catch { return null; }
  const nc = String(code || '').trim().toLowerCase();
  const nn = String(name || '').trim().toLowerCase();
  return sets.find((s) => nc && String(s.code || '').toLowerCase() === nc)
    || sets.find((s) => nn && String(s.name || '').toLowerCase() === nn)
    || null;
}

// The printed number for the games whose denominator the set-art bakes carry. Anything already
// slashed is printed and passes through (same guard printedCardNumber has); anything non-numeric
// (OP01-120, SWSH039) is its own printed identity and passes through too.
export function gameCardNumber(game, number, printedTotal) {
  const raw = String(number == null ? '' : number).trim();
  if (!raw || raw.includes('/') || !/^\d+$/.test(raw) || !printedTotal) return raw;
  const n = parseInt(raw, 10);
  // Lorcana prints an UNPADDED numerator over a 3-digit denominator: 42/204. SWU pads the
  // numerator to three: 010/252. Verified against the cards themselves (GR10).
  if (game === 'lorcana') return `${n}/${String(printedTotal).padStart(3, '0')}`;
  if (game === 'swu') return `${String(n).padStart(3, '0')}/${printedTotal}`;
  return raw;
}
...
export function printedCardNumber(item = {}, set = null) {
  ...
  // and printed total. Lorcana and SWU get their denominators from their OWN baked totals through
  // gameCardNumber above, never through this formatter; MTG genuinely has none (Scryfall's
  ...
  try {
    return formatCardNumber(raw, {
      ...
      // The identity key IS the pokemontcg.io card id, which is what tells the three Classic
      // Collection #106s apart (and names the set, when the row carries no set object).
    }, { rarity: item.rarity || '', id: item.identity_key || '' }) || raw;
  } catch { return raw; }
}
...
export async function runPublish(env, db, cfg, saveCfg, { itemId, overrides = {}, photoPaths = [], dryRun = false, compose = null }) {
  const built = itemToListing(db, itemId, overrides);
  if (built.error) return { ok: false, error: built.error };
  const { listing, item, merged } = built;

  // Reserved for a Keeper's Run? Refuse before anything is built, uploaded or sent. This covers
  // /preview and /publish in one line because both arrive here, and it is deliberately BEFORE
  // validateListing so a reserved card reports why it is refused rather than a list of copy warnings.
  const held = assertNotReserved(db, 'inventory', item.id);
  if (held) return { ok: false, error: held.error, code: held.code, reservation: held };
  ...
  // A staged row carries a provisional STG-* sku; its real shelf label is reserved HERE, on the way to
  // a live listing, and committed further down only once eBay accepts it. eBay keys inventory by SKU
  // and binds the Custom label to a listing for life, so the swap has to happen before the first API
  // call — there is no renaming it afterwards.
  //
  // The peek/commit rule and the reasons for it now live in lib/shelf-label.mjs, because the label is
  // spent by the first real publish on ANY channel and Shopify publishes first. A dry run still
  // reserves nothing; the provisional inventory-item record that leaves behind on eBay is tidied up
  // below, on the real publish.
  const reserved = reserveShelfLabel(db, item, { dryRun, channel: 'ebay' });
  if (!reserved.ok) return { ok: false, error: reserved.error, validation: v };
  const pendingLabel = reserved.reservation;
  listing.sku = reserved.sku;
  ...
  const res = await publishListing(env, { listing, cfg, imageUrls: img.imageUrls, conditionDescriptors: cd.descriptors, existingOfferId, dryRun });
  ...
  if (res.ok && !dryRun && pendingLabel) {
    const commit = commitShelfLabel(db, item, pendingLabel, { channel: 'eBay' });
    if (commit.committed) {
      ...
      // Only on a successful commit — a failed one leaves the row on the provisional, which must
      // therefore keep its eBay record.
      deleteInventoryItem(env, commit.provisional).catch(() => {});
    }
  }
  ...
  try {
    db.prepare(`INSERT INTO listing_pushes (item_id, sku, action, offer_id, listing_id, status, error, request, response)
      ...
      JSON.stringify({ title: listing.title, price_cents: listing.price_cents, aspects: listing.aspects, conditionDescriptors: cd.descriptors, descriptorSource: cd.source, imageUrls: img.imageUrls, inventoryItem: res.requestBody || null, offer: res.offerBody || null }),
  ...
  if (res.ok && !dryRun) writeBackListed(db, cfg, item, listing, res);
  return { ...res, sku: item.sku, itemId: item.id, title: listing.title, price_cents: listing.price_cents, imageUrls: img.imageUrls, imageWarnings: img.warnings, aspects: listing.aspects, descriptionHtml: listing.descriptionHtml, storeCategoryNames, conditionDescriptors: cd.descriptors, descriptorSource: cd.source, validation: v };
}
...
function writeBackListed(db, cfg, item, listing, res) {
  ...
  try {
    db.prepare(`INSERT INTO ebay_listings (sku, marketplace, offer_id, listing_id, item_id, game, category_id, price_cents, currency, available_qty, listing_status, best_offer_enabled, auto_accept_cents, auto_decline_cents, listing_url, last_synced_at, raw)
      ...
      .run(item.sku, cfg.marketplaceId, res.offerId || null, res.listingId || null, item.id, item.game, listing.categoryId, listing.price_cents,
        // Always 0/null: nothing this tool publishes carries Best Offer. The COLUMNS stay because
        // they are also written by the Trading mirror for the ~103 hand-made listings that do have it
        // on, and the repricer reads them before it widens a floor.
        listing.quantity, 0, null, null, url);
...
// The last thing accountStatus told us, remembered so surfaces that must not spend an eBay call can
// still say whether the account was readable. Preflight is the one that matters: it is FREE by
// design and that invariant is worth keeping, but "3 rows publishable" printed during an eBay
// outage is a promise it cannot keep. Echoing a known-stale fact, stamped with its age, is honest;
// making preflight phone eBay to find out would not be.
let _acctHealth = null;
function noteAccountHealth(st) {
  _acctHealth = { degraded: !!(st && st.degraded), ready: !!(st && st.ready), error: (st && st.policyReadError) || null, at: new Date().toISOString() };
}
export function lastAccountHealth() { return _acctHealth; }

const BATCH_MAX = 500;   // runaway guard, far above any real batch — not a product limit
// How many consecutive eBay server errors it takes to call a run an incident and stop it.
// Three, because two in a row is bad luck on a big batch and four is a minute of wasted work.
const INCIDENT_STREAK = 3;
...
export async function runBatchPublish(env, db, cfg, saveCfg, { itemIds, overridesById = {}, releasedIds = [], compose = null, emit, shouldCancel }) {
  const released = new Set((releasedIds || []).map(Number));
  const rows = batchRows(db, itemIds, overridesById);
  ...
  // A RUN of eBay 5xx is an incident at their end, not a property of these cards. On 2026-08-31
  // every publish answered 500 [25002] "System error"; ebayRest had already retried each one four
  // times, and the batch ground on regardless. Two rows that morning — but this loop is built for
  // 100+, and every attempt strands an offer nothing in this repo can delete. A 4xx is still this
  // row's own problem and never trips it; only a server error counts, and only consecutively.
  let systemFails = 0;
  ...
  for (const r of rows) {
    ...
    try {
      const out = await runPublish(env, db, cfg, saveCfg, {
        itemId: r.id, overrides: r.overrides, dryRun: false,
      ...
      if (out.ok) {
        ...
        stats.failed++;
        if (Number(out.httpStatus) >= 500) {
          systemFails++;
          if (systemFails >= INCIDENT_STREAK) {
            aborted = 'eBay incident: ' + systemFails + ' publishes in a row came back as a server error (HTTP '
              + out.httpStatus + ') — that is their end, not these cards. Nothing else was attempted; resume when eBay recovers.';
          }
        } else systemFails = 0;
...
// TW is Traditional Chinese and has its OWN lane: without the entry a TW row fell through to 'en'
// and was priced against English-only comps — a confident number for a card you do not own,
// arriving through a missing map key rather than a bad rule (GR4).
const LANG_DATA = { EN: 'en', JP: 'ja', JA: 'ja', ZH: 'zh-cn', CN: 'zh-cn', TW: 'zh-tw', KO: 'ko' };
// The comps query, its number filter and the foil/nonfoil hint now live in lib/stock-games.mjs
// beside the other per-game facts (the browser needs the same query for its ↗ links, and the
// arbitrage engine needs the same finish hint). Re-exported here because this is where every
// existing caller and test looks for them.
export { compsQueryFor, compsNumberMatch, finishHint };
...
export async function reviseTradingListing(env, db, { listingId, priceCents, availableQty, force = false, expectPriceCents = null, bestOffer = 'leave' } = {}) {
  ...
  if (after.ok) {
    ...
    if (row.item_id && after.available_qty != null) {
      // Through the ledger, because this is the one place stock moves on eBay's word rather than ours:
      // the number is eBay's read-back after a revise, so the movement says 'correction' and points at
      // the listing that supplied it. A quantity that changed because a marketplace said so is exactly
      // the kind nobody can explain a month later.
      // GUARDED, because this replaced a bare UPDATE that was a silent no-op on a missing row.
      // setQuantity THROWS instead — and ebay_listings.item_id can dangle, since the stock delete
      // leaves it behind. Throwing here would 500 a revise that eBay has ALREADY applied, turning a
      // stale local pointer into a failed call against a listing that did change.
      try {
        setQuantity(db, {
          kind: 'inventory', itemId: row.item_id, quantity: after.available_qty,
          reason: 'correction', refKind: 'listing', refId: id,
          note: "set from eBay's available quantity after a revise",
        });
      } catch (e) { console.warn('[listings] revise: could not record the stock movement —', e?.message || e); }
    }
  }
  return {
      ...
      if (existing) itemId = existing.id;
        ...
        const r = db.prepare(`INSERT INTO inventory_items
          ...
          .run(identity, parsed.name || row.title || 'Card', set ? set.name : (parsed.setName || null),
            ...
            0,   // the units are filed as a movement below; a row that already held them would file none
            row.state === 'sold' ? 'sold' : 'in_stock',
        ...
        // The INSERT above wrote the quantity, so the row is zeroed and the units re-filed as a
        // movement. setQuantity alone would have measured the target against a cache already holding
        // it, computed a delta of zero and filed nothing — the ledger silently behind from the start.
        // 'opening' is the honest reason: these units were not received here, they were found on a
        // listing that already existed.
        try {
          const adopted = Math.max(1, row.quantity || 1);
          applyMovement(db, {
            kind: 'inventory', itemId, delta: adopted, reason: 'opening',
            refKind: 'listing', refId: row.listing_id, note: 'adopted from an existing eBay listing',
          });
        } catch { /* GR7: a ledger hiccup must not lose the link this route exists to make */ }
        out.created++;
      }
    }
  ...
  return async (req, res) => {
    try {
      ...
      if (p === '/store/categories' && method === 'GET') {
        ...
        // Every configured department, checked against what the store really has. This is the probe
        // that catches a path going stale after the owner reorganises their storefront.
        const stale = checkStoreCategories(loadConfig(), r.categories);
        const body = { ok: true, storeName: r.storeName || null, categories: r.categories, max: STORE_CATEGORY_MAX, stale };
      ...
      // POST /preview { itemId, price_cents?, photoPaths?, ...richAspects } — dry-run:
      ...
      if (p === '/preview' && method === 'POST') {
        ...
        const out = await runPublish(env, db, cfg, saveConfig, {
          itemId: b.itemId, overrides: pickOverrides(b), photoPaths: b.photoPaths || [], dryRun: true,
        });
        return send(res, out.ok ? 200 : 422, out);
      }

      // POST /publish { itemId, price_cents?, photoPaths?, ...richAspects } — create/revise
      ...
      if (p === '/publish' && method === 'POST') {
        ...
        if (b.itemId == null) return send(res, 400, { error: 'itemId required' });
        const out = await runPublish(env, db, cfg, saveConfig, {
          itemId: b.itemId, overrides: pickOverrides(b), photoPaths: b.photoPaths || [], dryRun: false,
          // A listing DECISION — not a card fact, so it rides on the request rather
      ...
      if (p === '/batch/preflight' && method === 'GET') {
        ...
        return send(res, 200, { ...batchPreflight(db, cfg, { itemIds: ids, releasedIds: released }), account: lastAccountHealth() });
      }

      // POST /batch/preflight/canary { item_ids[], overrides_by_id? } — a REAL dry run
      ...
      if (p === '/batch/preflight/canary' && method === 'POST') {
        ...
        for (const r of picked) {
          try {
            const one = await runPublish(env, db, cfg, saveConfig, { itemId: r.id, overrides: r.overrides, dryRun: true });
      ...
      // POST /batch { item_ids[], overrides_by_id?, released_ids[] } → NDJSON.
      ...
      if (p === '/batch' && method === 'POST') {
        ...
        if (!ids.length) return send(res, 400, { error: 'item_ids (non-empty array) required' });
        if (ids.length > BATCH_MAX) return send(res, 400, { error: 'too many items (' + ids.length + '); the cap is ' + BATCH_MAX, code: 'batch_too_large' });
        // Reserved rows are refused UP FRONT, not row by row. runPublish guards them anyway, but a
        // batch that fails on card 63 of 100 has already spent 62 eBay writes to tell the operator
        // something this query knew before the first one — and the fix is to deselect, not to retry.
        // Every offender is named, because the operator wants one trip back to the grid, not sixty.
        const reserved = ids.map((id) => {
          const r = assertNotReserved(db, 'inventory', id);
          return r ? { item_id: id, ...r } : null;
        }).filter(Boolean);
        if (reserved.length) {
          return send(res, 409, {
            code: 'reserved_for_run', reserved,
            error: `${reserved.length} of ${ids.length} selected item(s) are reserved for a run; deselect them and try again`,
          });
        }
        ...
        const started = startBatchJob(env, db, cfg, saveConfig, {
          itemIds: pending, overridesById: b.overrides_by_id || {},
      ...
      if ((m = p.match(/^\/(\d+)\/revise-price$/)) && method === 'POST') {
        ...
        const out = await runPublish(env, db, cfg, saveConfig, { itemId: +m[1], overrides: { price_cents: b.price_cents }, dryRun: false });
        return send(res, out.ok ? 200 : 422, out);
      }
