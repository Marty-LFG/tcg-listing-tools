...
import { getStoreCategories, getSellerSkus, getSellerListings, getListingState, buildReviseInventoryStatusInner, tradingCall,
...
import { readCache as readPkmSetsCache, findSet, findIntlSet } from './pkm-sets-cache.mjs';
...
import { loadConfig as loadImageConfig, composeVersion as currentComposeVersion, resolveVariant as resolveRailVariant } from './listing-image-config.mjs';
...
export function composeMetaFor(item = {}) {
  ...
  if (intl) {
    return {
      ...
      setName: intl.name_en || intl.name_native || item.set_name || '',
      ...
      rarity: item.rarity || '',
      // TCGdex has symbols for only a handful of JP sets today (4 of 277). No symbol is correct
      // here — borrowing the English set's would put the wrong symbol on a Japanese card.
      setSymbolUrl: (intl.symbol && String(intl.symbol).trim()) || '',
    };
  }

  // The English set's cached record carries the SYMBOL plus the era fields Golden Rule 10's
  // formatter reads. A cold cache or a set it does not know simply means no symbol.
  ...
  return {
    ...
    rarity: item.rarity || '',
    setSymbolUrl: (set && set.images && set.images.symbol) || '',
  };
}
...
export async function reviseTradingListing(env, db, { listingId, priceCents, availableQty, force = false } = {}) {
  ...
  const before = { price_cents: live.price_cents, available_qty: live.available_qty, sold_qty: live.sold_qty };
  const inner = buildReviseInventoryStatusInner({ itemId: id, priceCents, availableQty });
  const res = await tradingCall(env, 'ReviseInventoryStatus', inner);
  ...
  return {
    ...
    after: after.ok ? { price_cents: after.price_cents, available_qty: after.available_qty, sold_qty: after.sold_qty } : null,
    verified: after.ok,
...
export async function importSellerListings(env, db, { marketplaceId = 'EBAY_AU' } = {}) {
  ...
  const up = db.prepare(`INSERT INTO ebay_seller_listings
      (listing_id, sku, title, price_cents, currency, quantity, available_qty, sold_qty, listing_type, state, listing_url, created_via, item_id, identity_key, last_seen_at)
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?, datetime('now'))
  ...
  for (const row of r.listings) {
    ...
    up.run(row.listing_id, row.sku, row.title, row.price_cents, row.currency || 'AUD', row.quantity,
      ...
      ours, item ? item.id : null, identity);
    imported++;
  }
