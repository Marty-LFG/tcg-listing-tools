...
export async function runPublish(env, db, cfg, saveCfg, { itemId, overrides = {}, bestOfferSpec, photoPaths = [], dryRun = false }) {
  ...
  try {
    db.prepare(`INSERT INTO listing_pushes (item_id, sku, action, offer_id, listing_id, status, error, request, response)
      ...
      JSON.stringify({ title: listing.title, price_cents: listing.price_cents, aspects: listing.aspects, conditionDescriptors: cd.descriptors, descriptorSource: cd.source, imageUrls: img.imageUrls, bestOffer, inventoryItem: res.requestBody || null }),
