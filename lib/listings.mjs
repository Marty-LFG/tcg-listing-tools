...
export function composeMetaFor(item = {}) {
  ...
  if (intl) {
    ...
    const sym = findSetSymbol(setName, intl.name_native, intl.code, intl.tcgdexId);
    return {
      productType,
  ...
  const enSym = (set && set.images && set.images.symbol) || (findSetSymbol(item.set_name, item.set_code) || {}).url || '';
  return {
    productType,
    language: ebayLanguageName(item.language),
