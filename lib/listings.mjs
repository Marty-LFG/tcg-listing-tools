...
export function composeMetaFor(item = {}) {
  ...
  if (!pkm) {
    ...
    const mset = item.game === 'mtg'
      ...
      : null;
    // Lorcana resolves against its OWN set list too — but purely to CONFIRM the set, not to fetch
    // art: Lorcast publishes no set symbol and no wordmark, and neither does any other keyless
    // source. So both slots stay empty and the masthead falls back to the set name, exactly as it
    // does for Magic's missing wordmark. Constructing a symbol URL out of the set code instead is
    // the images.scrydex.com placeholder trap (AGENTS.md 19) — a URL that 200s with the wrong
    // picture is worse than no picture (GR4).
    //
    // The lookup still earns its keep: it is what pins the OWNER'S stored set name to a real set,
    // and being in this branch at all is what stops a Lorcana row falling into the Pokémon
    // set-identity path below, where a bare code like '1' or 'P1' could seat a Pokémon symbol, a
    // Pokémon logo and a fabricated 'n/total' onto a Disney card.
