  ...
  for (const l of links || []) {
    ...
    if (!itemId) {
      ...
      if (existing) itemId = existing.id;
        ...
        // The row was inserted holding its units, so the ledger is squared to it rather than adding on
        // top. 'opening' is the honest reason: these units were not received here, they were found on a
        // listing that already existed.
        try {
          setQuantity(db, {
            kind: 'inventory', itemId, quantity: Math.max(1, row.quantity || 1), reason: 'opening',
