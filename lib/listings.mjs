...
// The comps query for one row, mirroring each builder's own findEbay(). This used to prepend the
// literal 'Pokemon' to EVERY game, so a Magic search read "Pokemon Smaug the Magnificent 249 The
// Hobbit (HOB)" — a query that matches nothing, which reads as "no comps" rather than as a bug.
//
