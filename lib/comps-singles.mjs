...
// recommended list price = undercut the cheapest IN-cluster listing by 1c, snapped DOWN to a price
// ending, floored at $0.50. Verbatim port of extras.js analyzeComps line 518.
export function recommendedFromCluster(cluster) {
  const under = Math.round((cluster.cheapestInCluster - 0.01) * 100) / 100;
  return Math.max(0.5, snapToEnding(under, 'down'));
}
