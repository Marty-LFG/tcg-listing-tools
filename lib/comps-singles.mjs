...
export function buildNumberRe(num) {
  ...
  // N[a]/M — the letter suffix is part of the IDENTITY, not decoration: 030a/298 is the alt art and
  // 030/298 is the base card, and they do not trade at the same price. Building /\b0*30\b/ from the
  // former (what this used to do) both failed to match the card's own title and matched the base
  // card's, which is how an alt art got priced off the wrong listings.
  const m = s.match(/^(\d{1,4})([A-Za-z])?\s*\/\s*(\d{1,4}|[A-Za-z]{2,5})$/);
  if (m) {
    const lhs = '0*' + String(+m[1]) + (m[2] || '');
    // A lettered denominator (Star Wars Unlimited writes 107/SOR) is a set code, so it is matched
    // literally rather than zero-stripped.
    const rhs = /^\d+$/.test(m[3]) ? '0*' + String(+m[3]) : m[3];
    return new RegExp(`\\b${lhs}\\s*/\\s*${rhs}\\b`, 'i');
  }
  // Bare, optionally lettered (Riftbound runes print 214a). Same rule: the letter distinguishes cards.
  const b = s.match(/^(\d{1,4})([A-Za-z])?$/);
  if (b) return new RegExp(`\\b0*${+b[1]}${b[2] || ''}\\b`, 'i');
  const n = s.match(/\d{1,4}/); return n ? new RegExp(`\\b0*${+n[0]}\\b`, 'i') : null;
}
...
export async function singlesEbayValue(opts = {}) {
  const {
    base, query, numberMatch, lang, finish, excludeSeller, graded = false, minComps = 4, timeoutMs = 12000,
    // The Browse query string to send INSTEAD of the one built below, so a caller that already
    // has a structured plan (lib/ebay-query.mjs — category_ids, aspect_filter, conditionIds) can
    // use it without this function learning about aspects. Everything after the '?'. Default null
    // keeps the historical construction byte-for-byte, which the unit tests pin.
    //
    // It exists so ebay-testbed.html can compare query strategies against THIS engine rather than
    // against a second copy of the clustering maths (GR9): a testbed that reimplements the thing it
    // is measuring is measuring the wrong thing.
    browseQuery = null,
  } = opts;
  ...
  if (!rows || !rows.length) {
    const ask = await get('/api/ebay/buy/browse/v1/item_summary/search?' + (browseQuery
      || ('limit=200&q=' + enc + '&filter=' + encodeURIComponent(askFilter('buyingOptions:{FIXED_PRICE}')))));
