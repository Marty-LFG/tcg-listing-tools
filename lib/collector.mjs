...
import { mapPrice, pricePath, toAUD } from './normalize.mjs';
import { scrubSecrets } from './logbuffer.mjs';
import { BYPASS_HEADER } from './set-cache.mjs';
...
// Every request this file makes carries the cache-bypass header. The card routes it fetches
// (lib/normalize.mjs pricePath) are now answered from stored sets for the builders' benefit, and
// that is exactly wrong here: a price history assembled from a cached copy would record the same
// number until the copy changed, and the buy signals built on it would be reading their own echo.
// See lib/set-cache.mjs BYPASS_HEADER.
const FRESH = { [BYPASS_HEADER]: '1' };
async function jfetch(url, { retry429 = true } = {}) {
  let r = await fetch(url, { headers: FRESH });
  if (r.status === 429 && retry429) { await sleep(1500); r = await fetch(url, { headers: FRESH }); }
  return r;
}
...
// Riftbound prices come from TCGplayer, not Scrydex: that subscription lapsed (402
// SUBSCRIPTION_INACTIVE) and it was the game's only price source, which left price_snapshots empty
// for EVERY game because the whole watchlist is Riftbound. See lib/riftbound-prices.mjs.
const SOURCE_BY_GAME = { riftbound: 'tcgplayer', mtg: 'scryfall', pokemon: 'pokemontcg', swu: 'swudb', lorcana: 'lorcast' };
...
async function collectCard(db, base, card, rates) {
  const path = pricePath(card.game, card.identity_key);   // NOT lookupPath — riftbound prices are keyless now
  ...
  if (!r.ok) {
    // Riftbound now resolves through the keyless /api/riftbound/prices index, which answers 503 when it has
    // never been baked. The Scrydex codes below are retained for the reactivated-subscription case
    // (401/403 = key missing/invalid; 402 = valid key, no active plan — Scrydex has no free tier).
    let err;
    if (card.game === 'riftbound' && r.status === 503) err = 'rb_index_missing';
    else if (card.game === 'riftbound' && (r.status === 401 || r.status === 403)) err = 'scrydex_unauthorized';
...
export function computeSignals(db, cardId) {
  ...
  // A card whose AUD price is unknown (FX was down when the snapshot was taken) is NOT eligible for a
  // buy 'opportunity' alert — FAIL the gate rather than defaulting to pass, so sub-threshold noise
  // can't fire during an FX outage. Only gates 'opportunity'; momentum/downtrend don't check price.
