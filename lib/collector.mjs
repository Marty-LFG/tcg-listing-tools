...
const SOURCE_BY_GAME = { riftbound: 'scrydex', mtg: 'scryfall', pokemon: 'pokemontcg', swu: 'swudb' };
...
// Runs one collection pass over active cards (or a single id). Returns {checked, snapshots}.
export async function runPass({ db, base, onlyId = null } = {}) {
  ...
  for (const card of rows) {
    try { if ((await collectCard(db, base, card, rates)) === 'snapshot') snapshots++; }
    catch (e) { console.error('[collector] card', card.id, e?.message || e); }
  ...
  return { checked: rows.length, snapshots };
}
...
// ---- scheduler -------------------------------------------------------------
// Singleton + HMR guard so a dev reload never stacks intervals.
export function startCollector({ db, base, cadenceHours = 24 }) {
  if (globalThis.__tcgCollectorTimer) return;
  const tick = () => runPass({ db, base }).catch((e) => console.error('[collector]', e?.message || e));
  // one pass shortly after boot, then on the cadence
  const boot = setTimeout(tick, 30_000);
  if (boot.unref) boot.unref();
  const timer = setInterval(tick, Math.max(1, cadenceHours) * 3600_000);
