...
import { buildProductSetInput, productSetProduct, setAvailableQty, publishToChannel, upsertIdentity }
  from './channels/shopify-product-api.mjs';
import { firstErrorText } from './channels/shopify-admin.mjs';
import { PRODUCT_TYPES, TAXONOMY, dispatchWeightGrams, slug, setIdentityHandleFor } from './channels/shopify-map.mjs';
import { buildSealedTitle, buildSealedPitch, sealedCondText } from './sealed-copy.mjs';
import { SHOPIFY_CATALOGUE_ART } from './listing-copy.mjs';
...
import { recordShopifyListing, pinsFor, publishStatus, rebuildSetIdentity } from './shopify.mjs';
 ...
 *
 * So: the same two-sentence shape buildShopifyDescription uses for a single (2026-09-13). Identity
 * first, because it carries what the title cannot; then how it is pictured. The brand line and the
 * parcel sentence are the theme's trust rows on the same page, said once, not here as well.
 */
export function buildSealedShopifyDescription(pool) {
  ...
  return [`<p>${esc(identity)}</p>`, `<p>${esc(SHOPIFY_CATALOGUE_ART)}</p>`].join('\n');
}
...
export function toSealedShopifyProduct(pool, { units = 0, status = 'ACTIVE', collections = [] } = {}) {
  ...
  return {
    ...
    identity: null,
    // The SET identity, which sealed DOES join — it is the whole point of showing a booster box on a
    // card's page and the set's cards on the box's. Same handle rule as the singles lane, so both
    // sides of the relationship land on one metaobject rather than two that nearly match.
    setIdentity: setIdentityHandleFor(pool)
      ? {
        handle: setIdentityHandleFor(pool),
        type: 'bk_set_identity',
        fields: {
          display_name: [pool.set_name, lang !== 'EN' ? `(${lang})` : ''].filter(Boolean).join(' '),
          game: pool.game || 'pokemon',
          set_code: pool.set_code || '',
          set_name: pool.set_name || '',
          language: lang,
...
export async function publishSealedProduct(env, {
  ...
  // The set identity first, so its GID can be spliced into bkc.set. Non-fatal: a box with no set
  // relationship is a thinner page, not a wrong one, and every store looks like this until
  // bk_set_identity is defined on it.
  ...
  if (product.setIdentity) {
    ...
    steps.push({ step: 'set_identity', ok: sident.ok, error: sident.ok ? null : (firstErrorText(sident.res) || 'refused'), handle: product.setIdentity.handle });
    if (sident.ok) setIdentityGid = sident.gid;
    else v.warnings.push('published WITHOUT a set identity — this box will not appear in the related-items row for its set');
  }

  const input = buildProductSetInput(product, { fileGids, ogFileGid, identityGid: null, setIdentityGid });
  if (dryRun) {
...
export async function runSealedShopifyPublish({
  ...
  // The set's related-items lists, recomputed whole now that this box is live. Same non-fatal
  // treatment as the singles lane: the product is already sellable, and a thin related row is not a
  // reason to report a failed publish.
  if (result.ok && product.setIdentity) {
    const set = await rebuildSetIdentity(env, db, {
      game: pool.game || 'pokemon', setCode: pool.set_code, language: pool.language || 'EN',
