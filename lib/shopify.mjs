...
export function shopifyBatchPreflight(db, cfg, { itemIds = [], store = 'dev' } = {}) {
  ...
  for (const id of ids) {
    ...
    try { peek = reserveShelfLabel(db, item, { dryRun: false, channel: 'shopify' }); } catch { /* GR7 */ }
  ...
  for (const c of collisions) {
    for (const r of rows) {
      if (r.handle === c.handle) {
        ...
        r.warnings = [...r.warnings, `${c.count} rows in this batch map to the same product handle "${c.handle}" — they would overwrite one another`];
      }
    }
  }
...
export async function runShopifyBatchPublish({
  ...
  emit({ start: { total: ids.length, store, dryRun, status: publishStatus(cfg), truncated: (itemIds || []).length > ids.length } });
