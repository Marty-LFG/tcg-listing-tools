...
const CONFIG_PATH = path.join(ROOT, 'data', 'shopify.config.json');
...
export function loadConfig() {
  ...
  catch { /* a missing or broken config means "not configured", which the guards report properly */ }
...
const SECRET_KEYS = /^(signature|policy|x-goog-signature|x-goog-credential|GoogleAccessId|acl|key|success_action_status|content-type)$/i;
export function scrubForAudit(v) {
  if (Array.isArray(v)) return v.map(scrubForAudit);
  if (v && typeof v === 'object') {
    return Object.fromEntries(Object.entries(v).map(([k, x]) => [k, SECRET_KEYS.test(k) ? '[scrubbed]' : scrubForAudit(x)]));
  }
  return v;
}
...
export function recordShopifyListing(db, row) {
  db.prepare(`
    ...
    ON CONFLICT(sku) DO UPDATE SET
      kind = excluded.kind, item_id = excluded.item_id, product_gid = excluded.product_gid,
      variant_gid = excluded.variant_gid, inventory_gid = excluded.inventory_gid,
      location_gid = excluded.location_gid, identity_gid = excluded.identity_gid,
      identity_handle = excluded.identity_handle, handle = excluded.handle, state = excluded.state,
      published_at = COALESCE(excluded.published_at, shopify_listings.published_at),
      price_cents = excluded.price_cents, available_qty = excluded.available_qty,
      last_synced_at = excluded.last_synced_at, error = excluded.error, raw = excluded.raw,
...
export async function runShopifyPublish({ env, db, base, cfg, item, store = 'dev', dryRun = false, fetchImpl } = {}) {
  ...
  const row = { ...item, sku: held.sku };
  steps.push({ step: 'shelf_label', ok: true, sku: held.sku, willSpend: !!held.reservation });

  // 2 — the frames. A publish with no images is not a product anyone would buy, but losing a LATER
  ...
  // 3 — the mapping, then the sequence.
  ...
  return async (req, res) => {
    try {
      ...
      if (p === '/preview' && method === 'POST') {
        ...
        const row = { ...item, sku: peek.sku || item.sku };
        const product = toShopifyProduct(row, { collections: collectionGidsFor(cfg, row) });
