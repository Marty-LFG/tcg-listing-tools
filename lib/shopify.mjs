...
export async function rebuildIdentity(env, db, { identityHandle, store = 'dev', fetchImpl } = {}) {
  ...
  const rows = db.prepare(`
    SELECT sl.sku, sl.product_gid, i.condition
     ...
     WHERE sl.kind = 'inventory' AND sl.identity_handle = ?
       AND sl.product_gid IS NOT NULL AND sl.state = 'live'
  `).all(identityHandle);
...
export function shopifyBatchPreflight(db, cfg, { itemIds = [], store = 'dev' } = {}) {
  ...
  for (const id of ids) {
    ...
    // not use: a row that already claimed one reuses it, and no new label is assigned.
    const claim = db.prepare(
    ...
    const mirror = db.prepare('SELECT sku, state, handle, product_gid FROM shopify_listings WHERE kind = ? AND item_id = ? ORDER BY updated_at DESC LIMIT 1').get('inventory', id);
  ...
  for (const r of rows) {
    ...
    const owner = db.prepare(
      `SELECT sku, item_id, product_gid FROM shopify_listings
        WHERE handle = ? AND item_id IS NOT NULL AND item_id != ? AND state = 'live'
        ORDER BY updated_at DESC LIMIT 1`).get(r.handle, r.item_id);
    if (!owner) continue;
    r.ok = false;
...
export async function runShopifyBatchPublish({
  ...
  for (const id of ids) {
    ...
    if (!force && !dryRun) {
      const mirror = db.prepare('SELECT sku, state, handle FROM shopify_listings WHERE kind = ? AND item_id = ? ORDER BY updated_at DESC LIMIT 1').get('inventory', id);
