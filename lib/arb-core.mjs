...
//
// THE BUYER'S BASIS. Live: buyerPct of today's market at the live rate. Reference: buyerPct of the
// market as of the buyer's own stale snapshot (refMarketUsdCents), converted at the buyer's own
// flat factor (storeFactor) — what a shop that priced its shelf off late-July TCGplayer at ×1.5 and
// never repriced actually pays. The "too good" ratio stays against TODAY's market at the live rate
// either way: it is a wrong-card detector, and the wrong card is wrong whichever basis pays for it.
export function hitMaths({ priceCents, shipCents, marketUsdCents, fx, buyerPct, feeMode = 'feeAU', refMarketUsdCents = null, storeFactor = null }) {
  ...
  const reference = refMarketUsdCents != null && refMarketUsdCents > 0 && storeFactor > 0;
  const buyerAudCents = reference
    ? Math.round(refMarketUsdCents * storeFactor * buyerPct)
    : Math.round((+marketUsdCents || 0) * fx * buyerPct);
  const profitCents = buyerAudCents - deliveredCents;
  return {
    feeCents, deliveredCents, marketAudCents, buyerAudCents, profitCents,
    buyerBasis: reference ? 'reference' : 'live',
...
// ---------------------------------------------------------------------------
// The reference date — which stored figure stands for "the market as of <date>"
// ---------------------------------------------------------------------------

const dayMs = 86_400_000;
export const isoDay = (v) => String(v || '').slice(0, 10);

// From a list of dated figures ([{ day, market_usd_cents, source }], any order), the one that best
// stands for the reference day: the latest ON OR BEFORE it (a change-only history holds the price
// that was in force), else the earliest after it, either within `maxGapDays`. Exact beats near,
// and a typed-in figure beats a recorded one on the same day. Null when nothing is close enough.
export function nearestReference(rows, refDay, maxGapDays) {
  const ref = Date.parse(isoDay(refDay));
  if (!isFinite(ref)) return null;
  const gap = (+maxGapDays >= 0 ? +maxGapDays : 0) * dayMs;
  const rank = (r) => (r.source === 'manual' ? 0 : r.source === 'snapshot' ? 1 : 2);
  let best = null;
  for (const r of rows || []) {
    const t = Date.parse(isoDay(r.day));
    if (!isFinite(t) || !(r.market_usd_cents > 0)) continue;
    const before = t <= ref;
    const dist = Math.abs(ref - t);
    if (dist > gap + 1) continue;
    const cand = { ...r, day: isoDay(r.day), gap_days: Math.round(dist / dayMs), before };
    if (!best) { best = cand; continue; }
    // prefer on-or-before; then the smaller gap; then the better source
    if (cand.before !== best.before) { if (cand.before) best = cand; continue; }
    if (cand.gap_days !== best.gap_days) { if (cand.gap_days < best.gap_days) best = cand; continue; }
    if (rank(cand) < rank(best)) best = cand;
  }
  return best;
}

export const dropPct = (refCents, nowCents) => (refCents > 0 && nowCents != null ? Math.round(((refCents - nowCents) / refCents) * 1000) / 10 : null);
...
export function warningsFor(h, ctx = {}) {
  ...
  if (ctx.noReference) out.push({ k: 'no_reference', why: 'no market figure for the buyer’s reference date — scored on today’s market at the live rate instead' });
  if (ctx.noLiveMarket) out.push({ k: 'no_live_market', why: 'TCGplayer has no market for this printing today — the buyer’s reference priced it, and the too-good check could not run' });
  if (ctx.refGapDays != null && ctx.refGapDays > 0) out.push({ k: 'ref_gap', why: 'the reference figure is from ' + ctx.refGapDays + ' day' + (ctx.refGapDays === 1 ? '' : 's') + ' ' + (ctx.refBefore ? 'before' : 'after') + ' the reference date' });
  return out;
}
