...
const COND_RULES = [
  ...
  ['lp', /\blight(?:ly)?[\s-]*played\b|\blp\b|\bexcellent\b|\bwhitening\b|\bscratch(?:es|ed)?\b|\bedge\s*wear\b|\bwear\b/i],
  ['mp', /\bmod(?:erately)?[\s-]*played\b|\bmp\b|\bvery\s*good\b|(?<!un)\bplayed\b/i],
];
...
export function classifyTitleCondition(title, condDescriptors) {
  ...
  const t = String(title || '').replace(HP_STAT_RE, ' ');
...
export function titleFinish(title) {
  ...
  if (/\bnon[\s-]?(?:holo|foil)\b|\bnonholo\b|\bnonfoil\b|\bnormal\b|\bregular\b|\bstandard\b/i.test(t)) return 'normal';
...
export function filterListing(row, ctx) {
  ...
  if (cond.cls !== 'nm' && cond.cls !== 'unstated') return { ok: false, reason: 'cond_' + cond.cls, cond };
  ...
  if (ctx.sellerMinPct != null && row.sellerFbPct != null && row.sellerFbPct < ctx.sellerMinPct) return { ok: false, reason: 'seller_fb' };
  if (ctx.sellerMinScore != null && row.sellerFbScore != null && row.sellerFbScore < ctx.sellerMinScore) return { ok: false, reason: 'seller_fb' };
