...
export function parseSubmissionIds(raw) {
  ...
  try {
    ...
    return v.map((n) => Math.round(+n)).filter((n) => Number.isFinite(n) && n > 0);
  } catch { return []; }
}
...
export function resolveLink(db, line) {
  ...
  // A row that has left stock is NOT a merge target. The card can sell while the restock is still on
  // the water, and merging six boxes onto a row marked 'sold' hides them from every in_stock view
  // while summarizeSealed keeps counting them as sold. identityMatches already filters on
  // status='in_stock' and lib/sealed.mjs:1417 refuses the same case ("listed/sold: hands off") —
  // only this direct path was open. Not alive, so the caller falls through to an identity match or
  // creates a fresh row, and the preview says which.
  if (row.status !== 'in_stock') return { alive: false, reason: 'not_in_stock', current: row };
...
export function reconcileGate(lines) {
  const blocking = [];
  for (const l of lines) {
    ...
    if (l.qty_received > 0) {
      ...
      if (l.line_kind === 'grading' && !parseSubmissionIds(l.submission_ids).length) {
...
export function buildPlan(db, order, lines, opts = {}) {
  ...
  const gate = reconcileGate(lines);
...
// Same product, still in stock — the fallback when a linked row has been deleted. Deliberately
...
function identityMatches(db, kind, snap) {
  if (kind === 'sealed') {
    return db.prepare(`SELECT id, sku, quantity, cost_cents, acq_fees_cents FROM sealed_items
      WHERE status = 'in_stock' AND name = ? AND IFNULL(game,'') = IFNULL(?,'')
        AND IFNULL(set_name,'') = IFNULL(?,'') AND IFNULL(language,'') = IFNULL(?,'')
        AND IFNULL(product_type,'') = IFNULL(?,'')`)
      .all(snap.name, snap.game, snap.set_name, snap.language, snap.product_type);
  }
  return db.prepare(`SELECT id, sku, quantity, cost_cents, acq_fees_cents FROM inventory_items
    WHERE status = 'in_stock' AND name = ? AND IFNULL(game,'') = IFNULL(?,'')
      AND IFNULL(set_name,'') = IFNULL(?,'') AND IFNULL(language,'') = IFNULL(?,'')
      AND IFNULL(condition,'') = IFNULL(?,'') AND IFNULL(grading_company,'') = IFNULL(?,'')`)
    .all(snap.name, snap.game, snap.set_name, snap.language, snap.condition, snap.grading_company);
}
      ...
      if (step.action === 'fee') {
        ...
        for (const sid of ids) {
          db.prepare(`UPDATE grading_submissions
                        SET grading_cost_cents = COALESCE(grading_cost_cents, 0) + ?,
  ...
  return async (req, res) => {
    try {
      ...
      if (p === '/orders' && method === 'POST') {
        ...
        if (b.status !== undefined && !CREATE_STATUSES.includes(b.status)) {
