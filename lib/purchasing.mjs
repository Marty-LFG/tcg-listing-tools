  ...
  try {
    ...
    for (const step of plan.steps) {
      ...
      if (step.action === 'fee') {
        ...
        const ids = line.identity_key ? String(line.identity_key).split(',').map((s) => +s.trim()).filter(Boolean) : [];
      ...
      result.push({
        ...step, item_id: out.id, sku: out.sku, created: out.created, quantity_after: out.quantity,
        // The pre-blend figures, so a weighted average that moved an existing cost basis is
        // auditable and reversible by hand.
        blend_before: step.action === 'merge'
          ? { quantity: step.target_qty_before, cost_cents: step.blend_before_cost ?? null }
          : null,
      });
    }
  ...
  return async (req, res) => {
    try {
      ...
      if ((m = p.match(/^\/lines\/(\d+)\/count$/)) && method === 'POST') {
        ...
        if (b.lot_units !== undefined) fields.lot_units = b.lot_units == null ? null : Math.max(0, Math.round(+b.lot_units));
        patchRow(db, 'purchase_lines', lineId, fields);

        if (Array.isArray(b.placements)) {
          const rows = sanitizePlacements(b.placements);
