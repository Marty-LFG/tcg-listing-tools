...
function identityMatches(db, kind, snap) {
  if (kind === 'sealed') {
    return db.prepare(`SELECT id, sku, quantity, cost_cents, acq_fees_cents FROM sealed_items
      WHERE status IN ('in_stock','listed') AND name = ? AND IFNULL(game,'') = IFNULL(?,'')
    ...
    try {
      ...
      if (p === '/submissions' && method === 'GET') {
        const rows = db.prepare(`SELECT id, name, set_name, number, grading_company, tier, status,
          ...
          WHERE status IN ('draft','submitted','received') AND promoted_item_id IS NULL
          ORDER BY submitted_at DESC, id DESC LIMIT 200`).all();
      ...
      if (p === '/stock' && method === 'GET') {
        ...
        // Math.max(0, NaN) is NaN, and node:sqlite throws "datatype mismatch" the moment a NaN is
        // bound to LIMIT ? — so a garbage query string turned the picker's hot path into a 500.
        const num = (v, dflt) => { const n = +v; return Number.isFinite(n) ? n : dflt; };
