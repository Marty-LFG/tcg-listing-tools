...
import { storeCategoryList } from './channels/ebay-map.mjs';
import { resolveSealedImageUrls } from './sealed-images.mjs';
import { reservedPoolUnits } from './runs-reserve.mjs';
...
// Sealed needs its OWN storefront department: cfg.store.categoryByGame is keyed by GAME, and sealed
// shares 'pokemon' with singles, so falling through to it would file a booster box under Pokemon
// Singles. Read live from the store on 2026-08-19, the real departments are
// /Trading Card Games/Pokemon TCG/Pokemon Sealed and .../Pokemon Singles.
//
// Precedence mirrors resolveStoreCategoryNames: the pool's own pick wins, then per-type config, then
// a single sealed default, then nothing (which lets the game fallback take over rather than inventing
// a path). A path that is not a live store category is passed through UNREPAIRED, exactly as the
// singles path does, because silently fixing a typo turns it into a plausible WRONG department.
export function sealedStoreCategories(pool, cfg) {
  const s = (cfg || {}).store || {};
  const own = storeCategoryList(pool && pool.store_categories);
  if (own.length) return own;
  const byType = (s.sealedCategoryByType || {})[pool && pool.product_type];
  return storeCategoryList(byType || s.sealedCategory || '');
}
...
export function poolUnits(db, poolSku) {
  ...
  const onHand = Number(r && r.n) || 0;

  // Units this pool has promised to a Keeper's Run. Sealed is the ONE guard that subtracts rather than
  // refuses: a single sealed_items row genuinely supplies both a run and the shop, so blocking the
  // listing outright would take a whole pool off sale to protect three boosters. A fully-reserved pool
  // yields 0 and the existing zero-quantity path takes over unchanged.
  //
  // The state vocabulary is deliberately NOT restated here — reservedPoolUnits owns it, and it excludes
  // 'consumed' because those units have already been drawn off the placements the sum above reads.
  return Math.max(0, onHand - reservedPoolUnits(db, poolSku));
}
...
export function toSealedListing(pool, { units, cfg, band } = {}) {
  ...
  return {
    ...
    // resolveStoreCategoryNames reads storeCategoryNames, NOT store_categories, and falls back to
    // cfg.store.categoryByGame[listing.game]. Sealed was emitting the wrong field name AND never
    // setting game, so every sealed listing resolved to nothing and eBay filed it under the store's
    // default department. Both are fixed here.
    game: pool.game || 'pokemon',
    storeCategoryNames: sealedStoreCategories(pool, cfg),
    graded: false,
  };
}
...
//
// Both enums have now been read: 261044 offers only "Box" and 183456 only "Pack", one member each.
// Anything NOT read off a live enum stays omitted rather than guessed.
const CATEGORY_ASPECTS = {
  '261044': { configuration: 'Box', packsAspect: null },            // CCG Sealed Boxes
  '183456': { configuration: 'Pack', packsAspect: 'Number of Packs' }, // CCG Sealed Packs
};
...
export function validateSealedListing(listing, pool, cfg) {
  ...
  // eBay will not publish an offer with no image, so this is a REFUSAL and not a warning. Singles
  // only warns, because a single always has catalog art from the card cache and a missing image is
  // an anomaly there. A pool has no automatic art source at all, so no-image is the ordinary state
  // of a new pool and has to be caught before the eBay round trip rather than after it.
  //
  // The generic follow-us banner must never satisfy this. resolveSealedImageUrls only appends it
  // when a real image exists, so an empty pool arrives here as [] and refuses.
  //
  // When scope widens past factory-sealed New (the condition gate above), eBay's stock-photo rule
  // returns and this must tighten to: at least one row of kind='owner'.
  if (!(listing.imageUrls && listing.imageUrls.length)) {
    errors.push('no image — eBay will not publish an offer without one; add an owner photo or the catalog art to this pool first');
  }
 ...
 */
export async function publishSealedPool(env, db, cfg, { poolSku, dryRun = false, imageUrls, saveCfg } = {}) {
  ...
  // Setting it ON the listing, not just passing it along: buildInventoryItemPayload falls back to
  // listing.imageUrls, so the payload stays fully assertable offline, which is what the unit tests
  // are built on. An explicit imageUrls argument still wins, and the two agree.
  const img = imageUrls ? { imageUrls, warnings: [] }
    : await resolveSealedImageUrls(env, db, cfg, saveCfg, poolSku);
  listing.imageUrls = img.imageUrls;
