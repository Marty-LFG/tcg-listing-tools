...
export function toSealedListing(pool, { units, cfg, band } = {}) {
  ...
  return {
    ...
    quantity: units,
    // NEVER a hardcoded default. eBay AU's sealed category is not the singles one (183454) and nothing
    // in this repo has ever read it, so it is pinned in config after a live lookup. Publishing into a
    // guessed category is how a booster box ends up filed as a single card (GR4).
    categoryId: ((cfg || {}).sealed || {}).categoryId || null,
    conditionId: SEALED_CONDITION_ID,
    aspects: sealedAspects(pool, cfg),
 ...
 */
export function sealedAspects(pool, cfg) {
  ...
  put('Configuration', ({ booster_box: 'Booster Box', booster_bundle: 'Booster Bundle', booster_pack: 'Booster Pack' })[pool.product_type]);
  put('Number of Packs', pool.pack_count);
...
export function validateSealedListing(listing, pool, cfg) {
  ...
  if (!listing.categoryId) {
    errors.push('no sealed eBay category is pinned — look one up with GET /api/sealed/listings/categories?q=pokemon+booster+box and save it as sealed.categoryId');
  }
