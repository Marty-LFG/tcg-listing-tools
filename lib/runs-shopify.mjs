 ...
 */
/**
 * The alternate PDP template a run renders through: templates/product.keepers-run.json.
 *
 * Shopify resolves `product.<suffix>.json`, so this string and that filename are one fact in two
 * places. A suffix naming a template that does not exist falls back to product.json silently, which is
 * exactly the failure this constant exists to make greppable.
 */
export const RUN_TEMPLATE_SUFFIX = 'keepers-run';

/**
 * The guarantee, as the product description.
 *
 * Escaped rather than trusted: the sentence is generated from committed claims, but it lands in
 * descriptionHtml and a storefront is not the place to discover that something upstream let a bracket
 * through. Empty in, empty out - a run with no sentence gets no description rather than an empty tag.
 */
export function guaranteeHtml(run) {
  const text = String(run?.guarantee_text ?? '').trim();
  if (!text) return '';
...
export function buildRunProductSetInput(run, bundles, {
  ...
  const input = {
    ...
    // THE GUARANTEE IS THE DESCRIPTION, unless a caller supplies its own.
    //
    // It is the one sentence that says what a buyer is actually getting, it is generated from the
    // run's own claims rather than typed, and it is inside headerDigest - so the page cannot drift
    // from what was anchored. Published with an empty description the product said nothing at all
    // about its contents, which for a mystery bundle is the only thing there is to say.
    descriptionHtml: descriptionHtml || guaranteeHtml(run),
    productType: PRODUCT_TYPES.bundle,
    vendor: 'Binders Keepers',
    status,
    tags: ['keepers-run', `run-${String(run.public_id).toLowerCase()}`, ...tags],

    // A run is not a single, and the singles PDP has no way to pick a bundle NUMBER - which §5.6.3
    // makes a property of the product rather than a nicety, because the visible remaining set is what
    // makes "7 is gone" checkable instead of asserted. Without this a run rendered through
    // templates/product.json: no picker, a condition ladder with nothing to choose between, and trust
    // copy about penny sleeves and a Near Mint scale.
    templateSuffix: RUN_TEMPLATE_SUFFIX,
...
export function recordRunListing(db, row) {
  ...
  // Refused here as well as in recordShopifyListing, so the error names the function the caller actually
  // called. The mirror is keyed (sku, store) and a run row filed against the wrong one is the same defect
  // as a single's: the cutover skips a bundle that was never published on the store it is reading.
  if (!row.store) throw new Error("recordRunListing: store is required (dev|live)");
  return recordShopifyListing(db, { ...row, kind: 'run' });
}

// STORE-SCOPED, like every other read of this mirror. shopify_listings is keyed (sku, store), so a
// store-blind read returns the dev rehearsal's row for a live run and vice versa — and these two feed
// setBundleAvailable, which then writes a quantity against another store's inventory item.
export const runListings = (db, runId, store = 'dev') =>
  db.prepare(`SELECT sl.* FROM shopify_listings sl
                JOIN run_bundles b ON b.id = sl.item_id
               WHERE sl.kind = 'run' AND sl.store = ? AND b.run_id = ? ORDER BY b.bundle_no`).all(store, +runId);

export const runListingFor = (db, runId, bundleNo, store = 'dev') =>
  db.prepare(`SELECT sl.* FROM shopify_listings sl
                JOIN run_bundles b ON b.id = sl.item_id
               WHERE sl.kind = 'run' AND sl.store = ? AND b.run_id = ? AND b.bundle_no = ?`)
    .get(store, +runId, +bundleNo);
...
export function sellableSet(db, run) {
  const derived = availabilityFrom(run.unit_count, asEntries(ledgerEntries(db, run.id)));
  // REFUSE RATHER THAN STOCK FROM A CHAIN THAT DOES NOT HOLD. `available` is a best effort once the
  // walk has found a problem, and the failure this guards is the expensive direction: a chain claiming
  // to sell one parcel twice derives an availability indistinguishable from selling it once, so a
  // republish would cheerfully put a number back on the shelf that somebody already owns. Our own
  // appendSale refuses every case this can catch, so if it ever fires the database is telling us
  // something worse than a publish failure.
  if (derived.problems.length) {
    throw new Error(`run ${run.public_id}'s ledger does not hold, so what is still for sale cannot be `
      + `derived from it: ${derived.problems[0]}`);
  }
  return new Set(derived.available);
}
...
export async function publishRunProduct(env, db, {
  ...
  for (const s of stocked) {
    recordRunListing(db, {
      sku: s.sku, store, item_id: byNo.get(s.bundleNo).id, product_gid: set.productGid, variant_gid: s.variantGid,
...
export async function setBundleAvailable(env, db, {
  ...
  const row = runListingFor(db, run.id, bundleNo, store);
  ...
  recordRunListing(db, { sku: row.sku, store, item_id: row.item_id, available_qty: quantity });
...
export const mirrorAvailability = (db, runId, store = 'dev') =>
  Object.fromEntries(runListings(db, runId, store).map((r) => [parseRunSku(r.sku)?.bundleNo, r.available_qty]));
