...
import { ns, sha256Prefixed } from './runs-canonical.mjs';
...
export function chaseLadderCanonical(ladder = []) {
  ...
  ordered.forEach((e, i) => {
    ...
    out += ns(String(e.rank)) + ns(String(e.card_name)) + ns(String(e.set_code))
      + ns(String(e.card_number)) + ns(String(e.language))
      + ns(String(e.grading_company)) + ns(String(e.grade));
  });
  return out;
}
