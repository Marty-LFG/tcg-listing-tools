...
import { resolveShop } from './channels/shopify-admin.mjs';
import { ingestOrder } from './keepers-ingest.mjs';
import { sweepOrders, driftReport } from './keepers-reconcile.mjs';
import { projectDirty, projectOrderAwards, projectionQueueDepth, projectionFor, isUnchanged, metafieldInputs } from './keepers-project.mjs';
...
export async function runKeepersPass(env, db, { trigger = 'schedule', nowMs = Date.now() } = {}) {
  ...
  // BACKFILL THE CODES OBSERVE MODE COULD NOT MINT.
  //
  // customers/create is recorded but not acted on while observing — the delivery is stamped 'skipped'
  // and the drain only ever re-reads 'received', so a customer who joined during a soak and never
  // ordered would come out the far side with no referral code at all. Nothing else would ever revisit
  // them: the reconcile sweep pages ORDERS, not customers.
  //
  // Cheap, bounded, and idempotent — ensureReferralCode returns any existing code untouched, so this
  // is a no-op the moment every customer has one. It runs only in a pass, which already requires
  // mode 'apply'.
  let mintedCodes = 0;
  if (cfg.mode === 'apply') {
    try {
      const missing = db.prepare(
        'SELECT customer_gid FROM keepers_customers WHERE referral_code IS NULL LIMIT 200',
      ).all();
      for (const r of missing) {
        try {
          withTransaction(db, () => { ensureReferralCode(db, r.customer_gid); });
          mintedCodes++;
        } catch (e) { console.warn('[keepers] could not mint a referral code —', e?.message || e); }
      }
    } catch (e) { console.warn('[keepers] referral backfill failed —', e?.message || e); }
  }
  ...
  if (cfg.project?.enabled && cfg.mode === 'apply') {
    ...
    // The per-order award, behind the SAME two switches, because it is the same act: ledger-derived
    // numbers written to Shopify. A separate call rather than folded into projectDirty because it
    // writes a different OWNER and needs no rank table — an unreadable rank table must stop customer
    // levels being written without also stopping order awards, which do not depend on it.
    orderAwards = await projectOrderAwards(env, db, {
      store: cfg.store, limit: cfg.project_limit, nowMs, allowLive: cfg.project.allowLive,
    });
  }
  ...
  return {
    ...
    drained, projected, orderAwards,
    // Reported, not just counted: a backfill that quietly does work nobody can see is how the gap it
    // exists to close got there in the first place.
    mintedCodes,
 ...
 */
// vite's loadEnv() READS .env and hands the result back as an object — it never puts anything into
// process.env. So `process.env` here is the SHELL's environment, and on this box that holds no
// SHOPIFY_SHOP and no SHOPIFY_DEV_SHOP at all. shopMismatchProblem defaulted to it, resolveShop threw
// ShopifyNotConfigured, the catch below returned null — and the DOMAIN half of the pair check never
// ran. Not "ran and passed": never ran, on every /state read since it shipped. The label half needs
// no env and worked fine, which is exactly why nothing looked wrong — the loud half worked, and the
// quiet half, the half-done edit the docblock calls the likeliest mistake, was silent.
//
// Remembered from the plugin, the way lib/shopify-hooks.mjs remembers its own. loadEnv with an empty
// prefix returns process.env's keys too, so the remembered object is a superset and there is nothing
// to merge; process.env stays the fallback for callers that never had a plugin — scripts and tests.
let _env = {};
export function keepersEnv() { return _env && Object.keys(_env).length ? _env : process.env; }
      ...
      server.middlewares.use('/api/keepers', async (req, res) => {
...

        {
          ...
          if ((mint || revoke) && req.method === 'POST') {
            ...
            const target = revoke
              ? d.prepare('SELECT discount_gid, status, code FROM keepers_redemptions WHERE id = ?').get(Number(revoke[1]))
              : null;
            // A WEDGED ROW REACHES THE STORE TOO, now that revoke can recover one. It has no
            // discount_gid — that is what wedged means — but it carries the code, and revokeRedemption
            // searches Shopify for that code before deciding anything. Judging "touches the store" on
            // the gid alone would let exactly that search run against live with allowLive unset, which
            // is the one thing this gate exists to prevent.
            const wedged = Boolean(target && !target.discount_gid && target.status === 'minting' && target.code);
            const touchesStore = Boolean(mint) || Boolean(target && target.discount_gid) || wedged;
...
/**
 * shopMismatchProblem — the receiver and the engine must be pointed at the SAME store.
 *
 * TWO CONFIG FILES, ONE DECISION, AND NOTHING COMPARED THEM. `shopify-hooks.config.json` decides
 * which store's deliveries are ACCEPTED (`shop`, and the `expect_shop_domain` that actually gates
 * them). `keepers.config.json` decides which store the engine READS BACK from (`store`). Going live
 * means editing both, by hand, on the box — and nothing refused, warned, or noticed when only one of
 * them moved.
 *
 * The failure is quiet, which is what earns it a check. Point the receiver at live while the engine
 * still says dev and every live order is accepted, stored and drained — then re-read against the DEV
 * store, where that order id does not exist. Nothing is written wrongly; the ingest just fails, over
 * and over, and the symptom is "orders are arriving and nothing happens". Reverse the two and the
 * receiver 401s every delivery instead, which Shopify retries for 48 hours while sig_failures climbs
 * and poisons the one counter that means "the app secret rotated".
 *
 * THE DOMAIN IS CHECKED, NOT JUST THE LABEL. `shop: 'live'` is a label; `expect_shop_domain` is the
 * string every delivery is compared against. Setting the first and forgetting the second is the most
 * likely way to get this half-done, and comparing labels alone would call that healthy.
 *
 * Returns null when the receiver is disabled — nothing arrives, so nothing can disagree — and when the
 * domain cannot be resolved from .env, because an unconfigured store is a different problem that
 * already reports itself elsewhere.
 */
export function shopMismatchProblem(env = keepersEnv(), keepersCfg = null, hooksCfg = null) {
  const k = keepersCfg || loadConfig();
  let h = hooksCfg;
  if (!h) { try { h = loadHooksConfig(); } catch { return null; } }
  if (!h || h.enabled === false) return null;

  const engineStore = String(k?.store || '').trim().toLowerCase();
  const receiverStore = String(h?.shop || '').trim().toLowerCase();
  if (!engineStore || !receiverStore) return null;

  if (engineStore !== receiverStore) {
    return `the receiver accepts deliveries from the ${receiverStore} store (shopify-hooks.config.json `
      + `"shop") while the engine reads the ${engineStore} store (keepers.config.json "store") — going `
      + 'live means changing both together';
  }

  // The labels agree; now the string that actually gates every delivery.
  const want = String(h?.expect_shop_domain || '').trim().toLowerCase();
  if (!want) return null;            // validateShopifyHooksConfig already refuses this when enabled
  let expected;
  try { expected = resolveShop(env, engineStore).toLowerCase(); } catch { return null; }
  if (want !== expected) {
    return `expect_shop_domain is ${want}, but the ${engineStore} store is ${expected} — the label and `
      + 'the domain disagree, and the domain is what every delivery is checked against';
  }
  return null;
}
...
export function getKeepersEngineState(db) {
  ...
  return {
    ...
    // Separate from config_problem because it is a problem with the PAIR, not with this file: the
    // keepers config can be perfectly valid on its own while pointing somewhere the receiver is not.
    shop_mismatch: shopMismatchProblem(),
