      ...
      server.middlewares.use('/api/keepers', async (req, res) => {
...

        {
          ...
          if ((mint || revoke) && req.method === 'POST') {
            ...
            // The live seatbelt, on BOTH. Minting needs it because the code is spendable; revoking
            // needs it because deactivating reaches the same store. project.allowLive is the
            // store-level "I meant live" switch and it is reused rather than duplicated.
            if (cfg.store === 'live' && cfg.project?.allowLive !== true) {
              return send(409, {
                ok: false,
