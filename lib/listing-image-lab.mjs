...
function collectSamples(game) {
  ...
  if (game === 'pokemon') {
    ...
    for (const f of listJsonFiles(path.join(ROOT, 'data', dirs[game]))) {
      ...
      for (const c of (doc && doc.cards) || []) push(sampleFromCard(game, c, { setId, setName }));
    }
  }
  return out;
}

export function gameSamples(game, q = '', limit = 24) {
      ...
      server.middlewares.use('/api/listing-image', async (req, res) => {
        try {
          ...
          if (p === '/preview' && method === 'POST') {
            ...
            if (b && b.stockRow && !b.dataUrl && b.stockRow.game === 'onepiece') {
              const clean = cleanOnepieceArt(b.stockRow);
              if (clean) b.url = clean;
            }
            ...
            const meta = b.meta && typeof b.meta === 'object' ? b.meta
              : b.stockRow && typeof b.stockRow === 'object' ? composeMetaFor(b.stockRow)
                : {};
          ...
          if (p === '/samples' && method === 'GET') {
            ...
            const r = gameSamples(game, q, limit);
