...
export function parseCatch(line, opts) {
  ...
  for (const t of toks) {
    ...
    if (Object.prototype.hasOwnProperty.call(PRINTING_TOKENS, lower)) { out.printing = PRINTING_TOKENS[lower]; continue; }
...
export function rowKey(row) {
  return [
    // A no-op today — stock-runner's makeRow carries no `game` — but this mirrors stockKey() in
    // lib/inventory.mjs, which reads row.game, and the moment a second game reaches the runner a
    // hardcoded segment would silently merge two games' rows under one key.
    row.game || 'pokemon',
