...
// Lorcana's three foil-only chase rarities → the finish that names the foil row.
//
// COUNTED, not assumed (docs/DATA_SOURCES.md, 3,192 cards across all 22 Lorcast sets): every one of
// the 324 Enchanted / Epic / Iconic cards is foil-only — not a single one carries a `usd` price. So
// for these there is no plain printing to sell, and the foil row is not "the foil version of a
// card", it IS the card.
//
// PROMO IS DELIBERATELY ABSENT. It looks like it belongs and does not: of 193 promos, 12 are
// non-foil, 6 carry both prices and 66 carry no price at all. Treating it as foil-only would title
// and price a plain promo as a foil.
//
// Shared by lorcanaPrintingsFor below and ENUMERATORS.lorcana (lib/enumerate.mjs), which used to
// carry its own private `enchanted` test — two ladders that had already drifted apart by two whole
// rarities.
export function lorcanaSpecialFinish(rarity) {
  const r = String(rarity || '');
  if (/enchanted/i.test(r)) return 'Enchanted';
  if (/iconic/i.test(r)) return 'Iconic';
  if (/epic/i.test(r)) return 'Epic';
  return null;
}

// The printing matrix for Lorcana, off Lorcast's two price fields. Same
// { key, finish, edition, variant, marketUsd } shape as printingsFor / mtgPrintingsFor, so the
// runner's row code never branches.
//
// Unlike Magic's surge foil, marketUsd here is TRUSTWORTHY on the special rarities: Lorcast has no
// second foil product to confuse them with, so `usd_foil` on an Iconic is the Iconic's own figure
// (Ariel 10/241 reads US$1,344, not a base-card price). No marketUnreliable flag is warranted.
//
// There is no Cold Foil row. TCGplayer sells Normal / Cold Foil / Holofoil, but no keyless source
// labels cold foil per card, so it exists only as an operator-chosen finish in the uploader and is
// never derived here (GR4).
export function lorcanaPrintingsFor(card) {
  const p = (card && card.prices) || {};
  const special = lorcanaSpecialFinish(card && card.rarity);
  const usd = toNum(p.usd), foil = toNum(p.usd_foil);
  const out = [];
  // A special rarity has no plain printing, so its `usd` is suppressed rather than sold as a base
  // card. Counted at zero occurrences — if that ever changes, the probe in docs/DATA_SOURCES.md is
  // what catches it.
  if (usd != null && !special) out.push({ key: 'usd', finish: 'Normal', edition: '', variant: variantToken('', 'Normal'), marketUsd: usd });
  if (foil != null) { const fin = special || 'Foil'; out.push({ key: 'usd_foil', finish: fin, edition: '', variant: variantToken('', fin), marketUsd: foil }); }
  // Nothing priced at all — 66 promos are in exactly this state. Still listable, market null, the
  // owner prices it by hand (GR7). 'Normal' is the under-promising answer for a card we cannot see
  // the finish of; the uploader's dropdown is where it gets corrected.
  if (!out.length) {
    const fin = special || 'Normal';
    out.push({ key: special ? 'usd_foil' : 'usd', finish: fin, edition: '', variant: variantToken('', fin), marketUsd: null });
  }
  out.sort((a, b) => printingOrder(a.key) - printingOrder(b.key));
  return out;
}

// The printing matrix for Riftbound. Same { key, finish, edition, variant, marketUsd } shape as its
// three siblings, so the runner's row code never branches — but it returns EXACTLY ONE entry, and
// that is not a simplification, it is what the game is.
//
// data/riftbound.json is single-printing per card: an Alternate Art, an Overnumbered, a Signature
// and an SP promo are each their OWN baked entry with their own collector number, not a second
// finish of the number beside them. And the finish is a deterministic function of the treatment
// (finishOf, lib/riftbound-data.mjs) — Epic and every Showcase treatment are foil-only, everything
// else is non-foil. So there is never a choice to offer.
//
// ⚠ `variant` IS THE TREATMENT, NOT variantToken(). For every other game `variant` names the
// printing ('Holo', 'Reverse Holo', 'Etched Foil'); for Riftbound it names 'Alternate Art' /
// 'Overnumbered' / 'Signature' / 'Ultimate', which is what ENUMERATORS.riftbound writes and what
// titleParts turns into the '(Signature)' token at priority 82 (lib/listing-copy.mjs) — above the
// rarity token and the condition code, because it is a 10-100x price differentiator. Returning
// variantToken('', 'Foil') here would collapse all four onto 'Foil', which (a) silently deletes
// '(Signature)' from the title of the most valuable cards in the game and (b) yields a different
// stockKey from the row the bulk enumerator produces for the SAME physical card, so the duplicate
// check misses it. Nothing is lost by carrying the treatment instead: single-printing means there
// is never a foil and a non-foil of one identity_key to keep apart on
// UNIQUE(game, identity_key, variant).
//
// finishFromRarity must NEVER run for Riftbound: its regex matches a bare "rare", and there are 335
// Rares in the catalog that would each come back Holofoil, feed finishHint('foil') into the comps
// search, and return a confident price for a card you do not own. The adapter's declared fallback
// is a plain Non-foil, and it is unreachable while every baked card carries a finish.
export function riftboundPrintingsFor(card) {
  const finish = (card && card.finish) || 'Non-foil';
  const key = finish === 'Foil' ? 'foil' : 'normal';
  return [{
    key,
    finish,
    edition: '',
    variant: (card && card.variant) || 'Base',
    // Joined onto the card by lib/riftbound-cards.mjs from the keyless TCGplayer index. Null is a
    // real answer — a Vendetta alt-art with no sales yet genuinely has no market price, and the row
    // then carries the `unverified` flag rather than a made-up figure (GR4).
    marketUsd: toNum(card && card.marketUsd),
  }];
}
...
// Lorcana's printing keys ARE Lorcast's two price fields, so the tokens map onto those rather than
// onto finish names. `h` is kept as an alias for foil the way Magic keeps it — muscle memory from
// the Pokémon pile, and safe because COND_TOKENS is matched first so `hp` is still Heavily Played.
//
// There is deliberately NO token for Enchanted, Epic or Iconic. Like Magic's etched and surge, each
// is its OWN collector number (Epic occupies 205-225, Iconic 241-245), never a second finish of the
// number beside it — so it is typed as its own number and a token could only ever match nothing.
export const LORCANA_PRINTING_TOKENS = { n: 'usd', f: 'usd_foil', h: 'usd_foil' };
// Riftbound has NO printing tokens, and the empty object is the point rather than an omission.
// Every baked card is a single printing whose finish follows its treatment (riftboundPrintingsFor
// above), so a printing letter would be a control with exactly one answer. parseCatch treats an
// empty table as "no token matches" and falls through to the number branch, and applyGameChrome
// drops the grammar chip entirely — the same rule the intl lanes' empty rarityOptionsFor follows:
// a control that cannot change anything disappears rather than lying (GR4).
export const RIFTBOUND_PRINTING_TOKENS = {};
...
//
//   125            the card number (25, 25/197, 199a, SWSH039, TG01, R/RGB)
...
export function parseCatch(line, opts) {
  ...
  for (const t of toks) {
    ...
    // A lettered number, typed the way it is printed: 30th Celebration's three Mews are R/RGB, G/RGB
    // and B/RGB. Bare it would be lost — `r` is the reverse-holo token — so the slash is what says
    // "number". numKeys drops the part after it, leaving the R that pokemontcg.io stores.
    if (/^[A-Za-z]+\/[A-Za-z0-9]+$/.test(t)) { out.num = t; continue; }
    out.unknown.push(t);
  }
  return out;
}
...
export function deriveState(row, medianAud, cfg) {
  if (!row) return 'EYES';
  // shopPublished / shopFailed are the Shopify lane's twins of published / failed. They were written
  // in seven places and read in none, so a Shopify run painted nothing on the grid while the status
  // bar underneath it said "N held — fix and press Shopify again". An eBay row carries neither field,
  // so this changes nothing on that lane.
  if (row.listingUrl || row.published || row.shopPublished) return 'LIVE';
  if (row.failed || row.shopFailed) return 'FAILED';
