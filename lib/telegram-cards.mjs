...
/**
 * AN EBAY INBOX MESSAGE. The one card that fires for a message no other card claimed.
 *
 * Before this, a message reached a person only if it was a reply from a KNOWN buyer with a prior sent
 * message, or a deal ask with deals.enabled on. A pre-sale question from a stranger was written to
 * member_messages and read by nobody. eBay scores sellers on response time, so silence is not free.
 *
 * THE PREVIEW IS THE POINT. There is no public per-message deep link (see lib/ebay-links.mjs), so the
 * card cannot hand you the message — it has to BE the message. Show enough that the common case
 * ("yes, it's the alt art") can be answered from the notification without opening anything, and let
 * the buttons handle the rest.
 *
 * The caller strips quoted history (stripQuotedHistory, lib/deals.mjs) before passing `preview`; the
 * truncation happens here, because the character budget is this file's business.
 */
export function renderInboxMessage({ senderId, subject, preview, itemTitle, status, receivedText,
  unread, previewChars = 400 } = {}, decided) {
  let s = `📬 <b>eBay MESSAGE</b>  ·  @${escapeHtml(senderId || 'unknown')}\n`;
  if (itemTitle) s += `<i>re: ${escapeHtml(shortTitle(itemTitle))}</i>\n`;
  if (subject) s += `<b>${escapeHtml(subject)}</b>\n`;
  const body = clip(preview, previewChars);
  if (body) s += `<i>“${escapeHtml(body)}”</i>\n`;
  // The status line is what says whether this still needs you. 'Unanswered' is eBay's own word and
  // only clears when a reply goes out on eBay, so it is the honest signal rather than our guess.
  const bits = [];
  if (status) bits.push(String(status).toLowerCase());
  if (unread === true) bits.push('unread');
  if (receivedText) bits.push(receivedText);
  if (bits.length) s += `<i>${escapeHtml(bits.join(' · '))}</i>`;
  return trimTo(s) + renderDigestFooter(decided);
}

/**
 * THE SECOND ASK. Same message, still unanswered, hours later.
 *
 * Deliberately thinner than the first card: the body has already been shown once, and a nag that
 * repeats the whole thing reads as a duplicate and trains you to swipe both away. What it adds is the
 * one fact the first card could not carry — how long it has been sitting there.
 */
export function renderInboxNag({ senderId, subject, itemTitle, waitingText, nagCount = 1 } = {}, decided) {
  let s = `⏰ <b>STILL UNANSWERED</b>${nagCount > 1 ? ` · <i>${nagCount}${ordinalSuffix(nagCount)} nudge</i>` : ''}\n`;
  s += `@${escapeHtml(senderId || 'unknown')}`;
  if (waitingText) s += ` · waiting ${escapeHtml(waitingText)}`;
  s += '\n';
  if (subject) s += `<b>${escapeHtml(subject)}</b>\n`;
  if (itemTitle) s += `<i>re: ${escapeHtml(shortTitle(itemTitle))}</i>\n`;
  s += '<i>Reply on eBay, or tap Handled to stop the reminders.</i>';
  return trimTo(s) + renderDigestFooter(decided);
}

// Collapse whitespace and cut on a word boundary when one is close enough — a preview that stops
// mid-word reads as a rendering bug rather than a deliberate excerpt.
...
// Every builder here owes Telegram the same 4096-character promise; MAX_TEXT is the shared floor.
function trimTo(s) {
  return s.length <= MAX_TEXT ? s : s.slice(0, MAX_TEXT - 24).trimEnd() + '… <i>(truncated)</i>';
}

/**
 * AN ARBITRAGE HIT — one eBay AU listing the local buyer's 80%-of-market price beats (lib/arbitrage.mjs).
 *
 * The three numbers that decide the purchase are the whole card: what it costs LANDED (item + postage
 * + the buyer-protection fee assumed), what the buyer pays, and the gap. Cents in, dollars out; the
 * currency is AUD on every line, and the TCGplayer figure is shown in USD with the rate that joined
 * them, because a hit re-read next week has to explain its own arithmetic.
 *
 * Warnings ride along in full. The alert path already withholds the two that mean "probably wrong
 * card" (arb-core ALERT_BLOCKERS); the rest — unstated condition, an ambiguous printing, a coupon —
 * are things to know before clicking Buy, not reasons not to look.
 */
export function renderArbHit(hit, { dashboardUrl } = {}) {
  const aud = (c) => 'A$' + ((+c || 0) / 100).toFixed(2);
  const usd = (c) => 'US$' + ((+c || 0) / 100).toFixed(2);
  const number = hit.number ? `#${hit.number}` : '';
  const printing = hit.printing_key === 'reverseHolofoil' ? 'Reverse Holo' : hit.printing_key === 'holofoil' ? 'Holo' : hit.printing_key === 'normal' ? 'Normal' : (hit.printing_key || '');
  let s = `💰 <b>ARBITRAGE HIT</b>  ·  <b>${aud(hit.profit_cents)}</b> profit`;
  if (hit.margin_pct != null) s += ` (${Math.round(hit.margin_pct)}% on outlay)`;
  s += `\n<b>${escapeHtml(hit.name || 'card')}</b> ${escapeHtml(number)}${printing ? ` · ${escapeHtml(printing)}` : ''}`;
  if (hit.set_id) s += ` · <code>${escapeHtml(hit.set_id)}</code>`;
  s += `\n<i>${escapeHtml(shortTitle(hit.title) || '')}</i>`;
  s += `\n\nLanded  <b>${aud(hit.delivered_cents)}</b>  <i>${aud(hit.price_cents)} + ${aud(hit.ship_cents)} post${hit.fee_cents ? ` + ${aud(hit.fee_cents)} fee` : ''}</i>`;
  if (hit.buyer_basis === 'reference') {
    s += `\nBuyer pays  <b>${aud(hit.buyer_aud_cents)}</b>  <i>${Math.round((hit.buyer_pct || 0) * 100)}% of ${usd(hit.ref_market_usd_cents)} (their ${escapeHtml(hit.ref_date || '')} figure) × ${hit.store_factor}</i>`;
    if (hit.market_usd_cents != null) s += `\n<i>market today ${usd(hit.market_usd_cents)} @ ${(+hit.fx_usd_aud || 0).toFixed(4)} = ${aud(Math.round(hit.market_usd_cents * (+hit.fx_usd_aud || 0)))}</i>`;
  } else {
    s += `\nBuyer pays  <b>${aud(hit.buyer_aud_cents)}</b>  <i>${Math.round((hit.buyer_pct || 0) * 100)}% of ${usd(hit.market_usd_cents)} market @ ${(+hit.fx_usd_aud || 0).toFixed(4)}</i>`;
  }
  if (hit.seller) s += `\nSeller  @${escapeHtml(hit.seller)}${hit.seller_fb_pct != null ? ` · ${hit.seller_fb_pct}%` : ''}${hit.seller_fb_score != null ? ` (${hit.seller_fb_score})` : ''}`;
  for (const w of hit.warnings || []) s += `\n⚠️ ${escapeHtml(w.why || w.k || String(w))}`;
  const buttons = [];
  if (hit.url) buttons.push([{ text: 'Open on eBay', url: hit.url }]);
  if (dashboardUrl) buttons.push([{ text: 'All hits', url: dashboardUrl }]);
  return { text: trimTo(s), buttons };
}
