...
async function maybeCompose(bytes, compose, warnings) {
  if (!compose || !compose.enabled) return { ...bytes, composeHash: null, composeVersion: null };
  try {
    const r = await composeListingImage(bytes.buffer, compose.meta || {}, compose.options || {});
...
export async function buildOfferImageUrls(env, { db, itemId, sources = [], cfg, saveCfg, compose = null } = {}) {
  ...
  for (const s of sources) {
    ...
    const done = await maybeCompose(raw, compose, warnings);
