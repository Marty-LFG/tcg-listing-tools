...
function cachedEps(db, { itemId, sourceUrl, localPath }) {
  ...
  if (itemId == null) {
    ...
    row = db.prepare(`SELECT * FROM listing_images WHERE source_url = ? AND eps_url IS NOT NULL ORDER BY id DESC LIMIT 1`).get(sourceUrl);
  } else if (localPath) {
    // An OWNER PHOTO is of one physical card. It stays item-scoped, or one card's photo would end
    // up on another card's listing.
    row = db.prepare(`SELECT * FROM listing_images WHERE item_id = ? AND local_path IS ? ORDER BY id DESC LIMIT 1`).get(itemId, localPath);
  } else return null;
