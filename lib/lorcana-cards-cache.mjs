...
// One card out of a set, by its collector number. Lorcast keys these as strings that may be padded
// ('7' vs '007'), so the compare is on the number, not the text.
const sameNumber = (a, b) => {
  ...
  return Number.isFinite(ia) && Number.isFinite(ib) && ia === ib && /^\d+$/.test(na.replace(/^0+/, '') || '0') && /^\d+$/.test(nb.replace(/^0+/, '') || '0');
};
export function findCardInSet(cards, num) {
  return (cards || []).find((c) => c && sameNumber(c.collector_number, num)) || null;
}
