...
import { reactToOrderEvents, reactToMessageEvents } from './ebay-notify-react.mjs';
...
export const TOPIC_ACTIONS = {
  ...
  // The buyer pressed "Request total from seller" in their cart. eBay's own spec for this topic says
  // it fires for auctions and for "fixed-price listings that do NOT require immediate payment" — so
  // on an immediate-payment listing it can never arrive, which is why payments.immediatePay decides
  // whether this whole lane exists (see lib/ebay-account.mjs immediatePayWanted).
  BUYER_REQUESTED_PURCHASE_QUOTE: 'deal_request',
...
export function recordEvent(db, envelope, { postsaleEnabled = true } = {}) {
  ...
  else if (!postsaleEnabled && (action === 'order_by_id' || action === 'message_poll' || action === 'deal_request')) {
...
async function handleRequest(env, req, res) {
  ...
  // 'message_poll' joins 'order_by_id' here. Until now it was a dead action string: recordEvent
  // stamped it and nothing ever selected it, so subscribing NEW_MESSAGE bought exactly nothing.
  if (out.stored && (out.action === 'order_by_id' || out.action === 'message_poll')) {
...
export async function runReactPass({ trigger = 'schedule' } = {}) {
  ...
  const run = (async () => {
    ...
    try {
      ...
      // Message events drain in POLL MODE ONLY. Observe mode's whole promise is that it changes
      // nothing, and a member-message poll writes rows and sends cards - it is not an observation.
      let messages = null;
      if (mode === 'poll') {
        try { messages = await reactToMessageEvents(_env, _db, {}); }
        catch (e) { console.warn('[ebay-notify] message react failed —', e?.message || e); }
      }
      _react.last_run = { at: started.toISOString(), trigger, mode, ...r, messages };
      return { ...r, messages };
    } catch (e) {
      ...
      server.middlewares.use('/api/ebay-notify', async (req, res) => {
        ...
        if (p === '/test' && req.method === 'POST') {
          ...
          if (!id) {
            ...
            if (!subs.ok) return send(502, { error: subs.error, used_token: subs.usedToken });
            const cfg = loadConfig();
            const first = subs.items.find((s) => (cfg.topics || []).includes(s.topicId));
            // TWO VERY DIFFERENT SITUATIONS, and telling them apart is the entire point of this branch.
            //
            // This used to answer both with "nothing subscribed yet — run POST /reconcile?apply=1",
            // which was observed on the live account while an ENABLED ORDER_CONFIRMATION subscription
            // existed: the reconcile it recommends then reports zero changes, so the operator is left
            // with two tools flatly contradicting each other and no way to tell which one is lying.
            // Advice asserted from an absence is worse than no advice, because a reconcile that finds
            // nothing to do reads as confirmation that everything is fine.
            //
            // An empty list is NOT evidence of an unsubscribed account: ebay-notify-admin.mjs
            // documents that GET /subscription on the wrong token answers 200 {"total":0} rather
            // than 403, so "none" and "we asked wrongly" arrive identically. Say which one we saw,
            // report the token the read used, and let the operator look before creating anything.
            if (!first) {
              return send(409, subs.items.length ? {
                error: 'topic_not_subscribed',
                detail: `eBay returned ${subs.items.length} subscription(s), none of them for ${(cfg.topics || []).join(', ')}`,
                subscribed_topics: [...new Set(subs.items.map((s) => s.topicId))],
                wanted_topics: cfg.topics || [], used_token: subs.usedToken,
              } : {
                error: 'no_subscriptions_returned',
                detail: 'eBay returned an EMPTY subscription list. That is what a genuinely unsubscribed account looks like, and also what a read on the wrong token looks like — they are indistinguishable from here. Check GET /subscriptions and POST /reconcile (dry run) before creating anything.',
                wanted_topics: cfg.topics || [], used_token: subs.usedToken, total: subs.total ?? null,
              });
            }
        ...
        // GET /subscriptions — what eBay says we are subscribed to, read the same way /test reads it.
        //
        // It exists because until now the ONLY way to exercise that lookup was POST /test, which asks
        // eBay to send a real notification: a diagnostic you cannot run without causing the event you
        // are trying to diagnose. When /test claimed nothing was subscribed, there was no way to ask
        // "what did you actually see?" without firing another one.
        //
        // `used_token` is the field to look at first. Subscriptions are user-token-only, and a read on
        // the application token comes back 200 with an empty list rather than an error.
        if (p === '/subscriptions' && req.method === 'GET') {
          const auth = diagOk(env, req, url); if (!auth.ok) return send(auth.code, { error: auth.error });
          const subs = await getSubscriptions(env);
          if (!subs.ok) return send(502, { error: subs.error, used_token: subs.usedToken });
          const cfg = loadConfig();
          const wanted = cfg.topics || [];
          return send(200, {
            count: subs.items.length,
            total_reported_by_ebay: subs.total ?? null,
            used_token: subs.usedToken,
            wanted_topics: wanted,
            missing_topics: wanted.filter((t) => !subs.items.some((s) => s.topicId === t)),
            items: subs.items.map((s) => ({
              id: s.subscriptionId, topicId: s.topicId, status: s.status,
              destinationId: s.destinationId, schemaVersion: s.payload?.schemaVersion ?? null,
            })),
          });
        }
