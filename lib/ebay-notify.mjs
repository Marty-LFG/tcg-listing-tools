...
export const DEFAULT_CONFIG = {
  ...
  //             This is what makes the soak meaningful: it measures how far ahead of the poll push
  //             actually is, on real sales, while changing nothing.
  //   off     — receive and record the notification, look at nothing.
  // Driving the pipeline is a later mode, and one to enable on evidence from this one.
  react: { mode: 'observe', quiet_ms: 5000, max_wait_ms: 60000, min_gap_ms: 5000 },
};
export const REACT_MODES = ['off', 'observe'];
...
export function getNotifyState() {
  ...
  return {
    ...
    keys: keyCacheStats(),
    // What a notification is allowed to do, and what it has actually done. `mode: 'observe'` means
    // READ-ONLY: orders are read back from eBay and written down, never ingested or acted on.
    react: { mode: cfg.react?.mode || 'observe', ...getObserveState() },
  };
}
...
async function handleRequest(env, req, res) {
  ...
  endNoBody(res, 204);
  // Only a genuinely new order event is worth a look — a redelivery has already been observed, and
  // scheduling on it would spend an API call to re-learn the same thing.
  if (out.stored && out.action === 'order_by_id') {
    try { scheduleObserve(); } catch (e) { console.warn('[ebay-notify] could not schedule an observe pass —', e?.message || e); }
  }
  return undefined;
}
...
// at once should cost one GetOrders call, not five. Coalescing is safe here precisely because the
// worker asks eBay about a SET of order ids — waiting a moment only makes the batch bigger.
//
...
export function getObserveState() {
  return { runs: _react.runs, pending: !!_react.timer, last_run: _react.last_run };
}

export async function runObservePass({ trigger = 'schedule' } = {}) {
  const cfg = loadConfig();
  if ((cfg.react?.mode || 'observe') === 'off') return { ok: true, skipped: 'react_off' };
  if (!_db) return { ok: false, error: 'no database' };
  // One pass at a time. The pass reads a batch and stamps it handled, so two overlapping runs would
  // read the same rows and spend the same API calls twice.
  if (_react.inFlight) return _react.inFlight;
  _react.inFlight = (async () => {
    const started = new Date();
    try {
      const r = await observeOrderEvents(_env, _db, {});
      _react.last_run = { at: started.toISOString(), trigger, ...r };
      return r;
    } catch (e) {
      _react.last_run = { at: started.toISOString(), trigger, ok: false, error: String(e?.message || e) };
      return _react.last_run;
    } finally { _react.inFlight = null; _react.lastRunAt = Date.now(); _react.runs++; }
  })();
  return _react.inFlight;
}

function scheduleObserve() {
  const cfg = loadConfig();
  if ((cfg.react?.mode || 'observe') === 'off') return;
  const k = cfg.react || {};
  ...
  _react.timer = setTimeout(() => {
    _react.timer = null;
    runObservePass({ trigger: 'notification' }).catch((e) => console.warn('[ebay-notify] observe pass failed —', e?.message || e));
  }, Math.max(0, delay));
...
export function stopNotifyListener() {
  ...
  // A pending observe pass belongs to the listener that scheduled it. Leaving it armed across a
  ...
  return {
    ...
    configureServer(server) {
      ...
      server.middlewares.use('/api/ebay-notify', async (req, res) => {
        ...
        // POST /observe — run an observe pass now instead of waiting for the debounce. READ-ONLY:
        // it re-reads orders from eBay and records what it found, ingesting nothing.
        if (p === '/observe' && req.method === 'POST') {
          const auth = diagOk(env, req, url); if (!auth.ok) return send(auth.code, { error: auth.error });
          try { return send(200, await runObservePass({ trigger: 'manual' })); }
