...
export function ensureReferralCode(db, customerGid, { prefix = 'BK', attempts = 8 } = {}) {
  ...
  if (row.referral_code) {
    // Already minted — but minted is not the same as DELIVERED. This returned early without marking
    // dirty, and projectDirty only ever selects `WHERE dirty = 1`, so a code that existed locally and
    // had never reached Shopify was never re-queued by anything. The customer keeps seeing
    // /pages/refer's "give it a few minutes" while the code sits in SQLite.
    if (String(row.referral_code_written || '') !== String(row.referral_code)) markDirty(db, customerGid);
