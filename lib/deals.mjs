 ...
 * separators go.
 */
// Everything below the buyer's own words: the quoted history eBay staples under a reply. Split out
// of normaliseForMatch so the ALERT preview and the CLASSIFIER strip the same thing - a card that
// showed the quoted tail would lead with our own last message quoted back at us, and a classifier
// that read it would fire on our own "ask us about a bundle" line. One implementation, two readers.
...
export function normaliseForMatch(subject, body) {
  ...
  return stripQuotedHistory(raw).replace(/\s+/g, ' ').trim().toLowerCase();
}
...
export function dealSummary({ lines, shipping, costBasis } = {}) {
  ...
  return {
    ...
    // Which bound chose the band — 'line', 'subtotal' or 'slab'. Carried through so a card can explain
    // itself: "signature, because the order totals $280" beats leaving the owner to wonder why a pile
    // of cheap commons is going signed-for.
    boundBy: post.boundBy,
