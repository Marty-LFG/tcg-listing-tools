...
/**
 * Hex back to bytes, refusing anything that is not exactly `n` bytes of LOWERCASE hex.
 *
 * Strict on case because §4.2 makes hex lowercase everywhere it is encoded or compared, and strict on
 * length because every hex value in this system is a fixed-width digest or key - a short one reaching a
 * hash function silently produces a different, valid-looking digest.
 */
export function fromHex(hex, n = 32) {
  const want = new RegExp(`^[0-9a-f]{${n * 2}}$`);
  if (typeof hex !== 'string' || !want.test(hex)) {
    throw new TypeError(`expected ${n} bytes as ${n * 2} lowercase hex characters; got ${JSON.stringify(hex)}`);
  }
