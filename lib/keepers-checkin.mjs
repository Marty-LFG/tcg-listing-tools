...
export function checkIn(db, { customerGid, show, rules = {}, nowMs = Date.now() }) {
  ...
  // THE MASTER SWITCH, honoured here as it is on every other earn path.
  //
  // snippets/bk-keepers-on.liquid tells the reader that one flag "stops the storefront promising and
  // the engine accruing together", and keepers-ingest.mjs:183 refuses an order accrual on exactly
  // this test. The check-in did neither, which made it the one automatic, customer-triggered path
  // that could write to the ledger with the programme switched off.
  //
  // The comparison is coercion-tolerant on purpose: every metaobject field arrives as a STRING, so
  // the string "false" is the shape this actually takes in production and a bare falsiness test would
  // read it as true. Same reasoning, same expression, as the ingest path.
  if (rules.enabled === false || String(rules.enabled ?? '').trim().toLowerCase() === 'false') {
    return { ok: false, reason: 'programme_disabled' };
  }
...
export function makeCheckinHandler(getConfig) {
  return async function handleCheckin(env, { customerGid, url, method }) {
    ...
    const { mode = 'apply', rules = {}, shows = [] } = cfg || {};
    const now = Date.now();
    ...
    // A CLAIM ONLY WRITES IN 'apply'. The mode is the engine's promise about whether anything is
    // appended at all, and this was the one path that ignored it — the GET status read above is
    // harmless in any mode, but the POST below appends. Under 'observe' it would have quietly
    // contaminated the very ledger the soak exists to keep empty, with nothing in the observations
    // table to show for it, and the only thing standing in the way was an empty `shows` list.
    //
    // Answered as a warm 200 rather than an error: the person is standing at a table holding a phone,
    // and this is our configuration problem, not something they did. `would_award` is the observation
    // in spirit — the soak can still see what a real check-in would have been worth.
    if (mode !== 'apply') {
      return {
        status: 200,
        body: {
          ...
          reason: 'not_applying',
          mode,
          would_award: Math.trunc(Number(rules.checkin_xp)) || 0,
          show: show.slug,
          message: "We're not handing out show XP just yet — grab us at the table and we'll sort you out.",
        },
      };
    }
