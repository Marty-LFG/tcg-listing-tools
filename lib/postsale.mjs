...
async function onDigestAction(env, db, cq, cfg, action, date, who) {
  ...
  if (action === 'dn') {                        // backed out of the confirm
    ...
    return editDigestMessage(env, db, date, 'dispatch');
  }

  if (action === 'dq') {                        // are you sure?
    const n = digestOrders(db, date).length;
    ...
    return editDigestMessage(env, db, date, 'dispatch', {
      ...
      buttons: [[{ text: `⚠️ Yes, dispatch ${n}`, data: `psdy:${date}` }, { text: 'Cancel', data: `psdn:${date}` }]],
    });
  }

  // action === 'dy' — confirmed.
  const orders = digestOrders(db, date);
  ...
  if (getMeta(db, 'digest_dispatching:' + date)) return answer('Already dispatching, hang on');
  ...
  await editDigestMessage(env, db, date, 'dispatch', {
    done: { dispatch: { icon: '⏳', status: `Dispatching ${orders.length}…`, who } }, buttons: [],
  });

  const r = await dispatchAllInDigest(env, db, date, cfg, who);
  if (!r.claimed) {
    return editDigestMessage(env, db, date, 'dispatch', {
  ...
  return editDigestMessage(env, db, date, 'dispatch', { done: { dispatch: done }, buttons: [] });
}
...
async function onPostsaleUpdate(env, db, u) {
  ...
  const dg = (cq.data || '').match(/^ps(p|dq|dy|dn):(\d{4}-\d{2}-\d{2})$/);
...
// The orders a given day's digest covered, with line items + locations resolved.
//
// Scoped by orders.pack_digest_date rather than by "everything unshipped right now", because the
// digest's buttons act on the set the message actually LISTED. Someone tapping "mark all shipped"
// three hours later must not sweep in two orders that landed since and nobody has looked at.
//
// An order still unshipped tomorrow gets re-stamped with tomorrow's date, which also means yesterday's
// buttons quietly stop matching anything. That is the right failure: a stale button does nothing
// rather than acting on a list its message no longer describes.
export function digestOrders(db, date, { unshippedOnly = true, lookup } = {}) {
  const rows = db.prepare(`SELECT * FROM orders WHERE pack_digest_date = ?
                           ${unshippedOnly ? "AND shipped_status = 'unshipped'" : ''}
                           ORDER BY paid_time ASC`).all(date);
...
export function buildDigestView(db, date, { done } = {}) {
  const lookup = buildInventoryLookup();
  const orders = digestOrders(db, date, { lookup });
  ...
  return {
    ...
    pullButtons: toPick ? [[{ text: `✅ Mark all ${toPick} picked`, data: `psp:${date}` }]] : [],
    // A real eBay write per order, so this one asks first (see onPostsaleUpdate).
    dispatchButtons: orders.length ? [[{ text: `🚚 Mark all ${orders.length} shipped`, data: `psdq:${date}` }]] : [],
  };
}
...
export async function runPackDigest(env, db, { force = false } = {}) {
  ...
  const ids = db.prepare(`SELECT order_id FROM orders WHERE shipped_status = 'unshipped'`).all().map((r) => r.order_id);
  if (!ids.length) { setMeta(db, 'last_pack_digest_date', date); return { ok: true, count: 0 }; }
  // Stamp the set FIRST. This is what the message's buttons act on, so it has to be recorded before
  // anything is sent — a button whose set is decided at tap time would act on whatever happens to be
  // unshipped by then, not on the orders the message actually listed.
  const stamp = db.prepare('UPDATE orders SET pack_digest_date = ? WHERE order_id = ?');
  db.exec('BEGIN');
  try { for (const id of ids) stamp.run(date, id); db.exec('COMMIT'); }
  catch (e) { try { db.exec('ROLLBACK'); } catch {} throw e; }
  ...
  const view = buildDigestView(db, date);
  const chatId = (env.TELEGRAM_CHAT_ID || '').trim();
  ...
  // Remember where each message landed so a button tap can edit the RIGHT one of the two. A send that
  // somehow came back without a message id just means that message can't be edited later; the action
  // itself still runs, so this must never throw.
  if (r.ok && r.result) setMeta(db, 'digest_pull_msg:' + date, `${chatId}:${r.result.message_id}`);
  if (r2.ok && r2.result) setMeta(db, 'digest_dispatch_msg:' + date, `${chatId}:${r2.result.message_id}`);
  if (r.ok) setMeta(db, 'last_pack_digest_date', date);
  _packDigest.last_run = { at: new Date().toISOString(), count: view.orders.length, ok: !!r.ok, summary_ok: !!r2.ok };
  return { ok: !!r.ok, count: view.orders.length, lines: view.orders.reduce((n, o) => n + o.items.length, 0) };
}
...
async function editDigestMessage(env, db, date, which, { done, buttons } = {}) {
  const at = getMeta(db, `digest_${which}_msg:` + date);
  if (!at) return { ok: false, skipped: 'no_message' };
  const [chatId, messageId] = at.split(/:(?=\d+$)/);
  const view = buildDigestView(db, date, { done });
...
// Mark every order on a digest as pulled + packed. Local only: nothing is written to eBay and the
// orders stay in the queue to dispatch, they just drop off the pull list.
export function pickAllInDigest(db, date, who) {
  const r = db.prepare(`UPDATE orders SET picked_at = ? WHERE pack_digest_date = ?
                        AND shipped_status = 'unshipped' AND picked_at IS NULL`).run(nowSql(), date);
  console.log(`[postsale/digest] ${r.changes} order(s) marked picked by ${who} (${date})`);
  return r.changes || 0;
}
...
// queue rather than half-hiding them. The claim is a single atomic INSERT on the meta table, so two
// people tapping at once cannot both start dispatching the same day's orders.
export async function dispatchAllInDigest(env, db, date, cfg, who) {
  const claimed = db.prepare(`INSERT INTO meta (key, value) VALUES (?, ?) ON CONFLICT(key) DO NOTHING`)
    .run('digest_dispatching:' + date, nowSql()).changes > 0;
  ...
  try {
    ...
    console.log(`[postsale/digest] dispatched ${ok}/${orders.length} by ${who} (${date})${failed.length ? ' — ' + failed.length + ' failed' : ''}`);
    return { claimed: true, total: orders.length, ok, failed };
  } finally {
    // Always release, so a crash mid-run doesn't wedge the button forever.
    db.prepare('DELETE FROM meta WHERE key = ?').run('digest_dispatching:' + date);
  }
}
  ...
  return async (req, res) => {
    try {
      ...
      if (p === '/orders' && method === 'GET') {
        ...
        const conds = [];
        if (status === 'unshipped') conds.push(`shipped_status='unshipped'`);
        else if (status === 'shipped') conds.push(`shipped_status='shipped'`);
      ...
      if (p === '/picksheet' && method === 'GET') {
        ...
        if (idsParam) {
          ...
          const conds = [];
          if (status !== 'all') conds.push(`shipped_status='unshipped'`);
