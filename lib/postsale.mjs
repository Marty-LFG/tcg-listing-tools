...
export async function pollOrders(env, db, { trigger = 'schedule' } = {}) {
  ...
  try {
    ...
    // Only a window we read to the end may move the cursor.
    if (!truncated) setMeta(db, 'orders_cursor', modTimeTo);
    else {
    ...
    return finishPoll(started, trigger, { ok: true, seen, ingested, refreshed, tracking_found: trackingFound,
      ...
      holds, sweep, truncated, window: { from: modTimeFrom, to: modTimeTo } });
  } catch (e) {
...
export async function dispatchOrder(env, db, id, cfg = DEFAULT_CONFIG, { dispatch: doEbay = true, tracking, carrier } = {}) {
  ...
  //
  // Not while dry_run is on, because then `shipped` records a REHEARSAL rather than anything eBay was
  // told, and a gate that trusted it would make the first live tap after switching over a no-op. In
  // dry run the fall-through costs nothing: the branch below still makes no call.
  if (prev && prev.shipped_status === 'shipped' && !tracking && !cfg.dry_run) {
  ...
  return async (req, res) => {
    try {
      ...
      if (p === '/orders/shipped' && method === 'POST') {
        ...
        return send(res, 200, { ok: !failed.length, requested: ids.length, shipped, failed, held, cancelled, missing,
          dry_run: !!cfg.dry_run });
      }
