...
import { breakOversoldReservations } from './runs-reserve.mjs';
...
// ONE TRACKER-SIDE TRANSACTION around a stock write and the movement that explains it, so the two
// cannot separate. These are the only stock writes in this file that open one, and it does not disturb
// the cross-DB ordering documented below: it spans a single database and completes entirely before the
// postsale stamp. Without it a crash between the UPDATE and the movement leaves a row marked sold whose
// ledger still says it is on the shelf — drift reconcile would report, that nobody asked for.
//
// REENTRANT, because "callers are never already inside a tracker transaction" stopped being true.
//
// That sentence stood here and was correct while applyStockDecrements was the only caller — its three
// BEGINs are all on postsale.db, so tdb was never mid-transaction. Merging Keeper's Runs added a second
// caller that is: consumeReservation opens a tracker BEGIN and calls decrementSealedItem inside it, so
// that the decrement, the reservation's state change and the audit row cannot separate. SQLite has no
// nested BEGIN, so the two correct designs collided as "cannot start a transaction within a
// transaction" — three tests in runs-reserve, and a pack-time consume that could never have worked.
//
// Joining the outer transaction rather than opening one loses nothing the wrapper existed to protect:
// the point is that a stock write and the movement explaining it commit together or not at all, and an
// enclosing transaction is by definition at least as wide. A throw still unwinds both, because
// consumeReservation rolls back on any error. SAVEPOINT would add partial rollback that no caller here
// wants, at the cost of changing the eBay path's hot loop.
function stockTxn(tdb, fn) {
  if (tdb.isTransaction) return fn();
  tdb.exec('BEGIN');
  try { const r = fn(); tdb.exec('COMMIT'); return r; }
  catch (e) { try { tdb.exec('ROLLBACK'); } catch { /* already unwound */ } throw e; }
}

export function decrementInventoryItem(tdb, itemId, qtySold, saleUnitCents, orderRef = null) {
  ...
  if (newQty <= 0) {
    ...
    stockTxn(tdb, () => {
      tdb.prepare(`UPDATE inventory_items SET status = 'sold', channel_status = 'ended',
      ...
      // The whole remaining balance leaves, not `qty` — an order line for 3 against a row holding 2
      // sells the row out, and the ledger has to end at zero rather than at minus one.
      applyMovement(tdb, { kind: 'inventory', itemId, delta: -before.quantity, reason: 'sale', refKind: 'order', refId: orderRef });
    });
    return { ok: true, sold: true, newQty: 0, effect: mkEffect(true) };
  }
  stockTxn(tdb, () => applyMovement(tdb, { kind: 'inventory', itemId, delta: -qty, reason: 'sale', refKind: 'order', refId: orderRef }));
...
export function decrementSealedItem(tdb, itemId, qtySold, saleUnitCents = null, orderRef = null) {
  ...
  if (places.length) {
    // Wrapped for the same reason the inventory twin is, and it matters MORE here: the draw-down is
    // several statements across two tables before the movement lands, so a crash mid-way would leave
    // placements emptied with nothing in the ledger to say the units left.
    return stockTxn(tdb, () => {
    const touched = [];
    ...
    tdb.prepare(`UPDATE sealed_items SET location = ?, status = ?, channel_status = ?,
      ...
      .run(firstLoc, sold ? 'sold' : item.status, sold ? 'ended' : item.channel_status,
        sold ? 1 : 0, sold ? 1 : 0, saleUnitCents != null ? Math.round(saleUnitCents) : null,
        sold ? 1 : 0, qty, itemId);
    // The placements are already drawn down, so the ledger is told the TOTAL they now sum to rather
    // than a delta — the same shape recomputeItemStock uses on the sealed module's own side.
    setQuantity(tdb, { kind: 'sealed', itemId, quantity: total, reason: 'sale', refKind: 'order', refId: orderRef, location: firstLoc });
  ...
  const sold = newQty <= 0;
  return stockTxn(tdb, () => {
    tdb.prepare(`UPDATE sealed_items SET status = ?, channel_status = ?,
                 sold_at = CASE WHEN ? THEN COALESCE(sold_at, datetime('now')) ELSE sold_at END,
                 sale_price_cents = CASE WHEN ? THEN COALESCE(sale_price_cents, ?) ELSE sale_price_cents END,
                 sale_qty = CASE WHEN ? THEN COALESCE(sale_qty, 0) + ? ELSE sale_qty END,
                 updated_at = datetime('now') WHERE id = ?`)
      .run(sold ? 'sold' : item.status, sold ? 'ended' : item.channel_status,
        sold ? 1 : 0, sold ? 1 : 0, saleUnitCents != null ? Math.round(saleUnitCents) : null,
        sold ? 1 : 0, qty, itemId);
    setQuantity(tdb, { kind: 'sealed', itemId, quantity: newQty, reason: 'sale', refKind: 'order', refId: orderRef });
...
export function applyStockDecrements(pdb, tdbIn) {
  ...
  const lines = pdb.prepare(`SELECT li.id, li.order_id, li.ebay_item_id, li.sku, li.quantity, li.unit_price_cents
                             ...
                             WHERE li.stock_applied_at IS NULL AND ${PAID_O} AND ${NOT_CANCELLED_O}
                               -- A Keeper's Run bundle is not stock this sweep owns. Its units were
                               -- drawn down at PACK time by consumeReservation off run_reservations, so
                               -- a decrement here would be the SECOND draw on one physical object.
                               --
                               -- Excluded rather than left to fall through the if (!m) continue below,
                               -- which never stamps a row and therefore re-selects it on every sweep
                               -- forever - turning matched:0 / pending:N, the one diagnostic this
                               -- function offers, into noise.
                               --
                               -- The prefix is a literal here rather than an import: lib/postsale.mjs
                               -- importing a runs module would add a fourth edge to the
                               -- listings -> runs-reserve -> postsale -> listings cycle that
                               -- consumeReservation's dynamic import already exists to avoid. A test
                               -- asserts this literal against RUN_SKU_PREFIX in lib/runs-shopify.mjs.
                               AND (li.sku IS NULL OR li.sku NOT LIKE 'BK-RUN-%')`).all();
  ...
  for (const li of lines) {
    ...
    if (!m) continue;
    matched++;
    const r = m.kind === 'sealed'
        ? decrementSealedItem(tdb, m.id, li.quantity, li.unit_price_cents, li.order_id)
        : decrementInventoryItem(tdb, m.id, li.quantity, li.unit_price_cents, li.order_id);
    if (r.ok) {
      ...
      // A Keeper's Run reservation is NOT allowed to block the decrement above — the sale is real, and
      // refusing it would leave the shop believing it owns a card someone has paid for. This is the
      // other half of that decision: the stock moves, and then the run is told. The reservation flips to
      // `broken`, which is never deleted because it IS the incident record, and lockRun refuses while
      // one stands. Golden Rule 7 — degrade visibly, never silently.
      //
      // Wrapped because a runs-module failure must never take down the sweep that keeps stock honest.
      try {
        for (const b of breakOversoldReservations(tdb, m.kind, m.id, `sold on order ${li.order_id}`)) {
          broken.push(b);
          console.warn(`[postsale] RESERVATION BROKEN — ${m.kind} item ${m.id} sold on order ${li.order_id} `
            + `while held for ${b.bundle ? 'bundle ' + b.bundle : b.run ? 'run ' + b.run : 'a run'}; `
            + 'that run cannot lock until someone resolves it');
        }
      } catch (e) { console.error('[postsale] could not record a broken reservation:', e?.message || e); }
    }
  }
  ...
  return { considered: lines.length, matched, applied, sold, broken_reservations: broken };
}
...
function reverseInventoryEffect(tdb, eff) {
  ...
  if (!eff.after.sold) {
    ...
    setQuantity(tdb, { kind: 'inventory', itemId: eff.item_id, quantity: eff.before.quantity, reason: 'cancel', refKind: 'order', refId: eff.order_id ?? null });
  ...
  // sale_qty is cleared alongside sale_price_cents: they are one fact about a sale that did not
  // happen, and leaving the count behind would make the next roll-up scale a null price by it.
  tdb.prepare(`UPDATE inventory_items SET status = 'in_stock', channel_status = 'ended',
               sold_at = NULL, sale_price_cents = NULL, sale_qty = NULL, updated_at = datetime('now') WHERE id = ?`)
    .run(eff.item_id);
  setQuantity(tdb, { kind: 'inventory', itemId: eff.item_id, quantity: eff.before.quantity, reason: 'cancel', refKind: 'order', refId: eff.order_id ?? null });
...
function reverseSealedEffect(tdb, eff) {
  ...
  tdb.prepare(`UPDATE sealed_items SET location = ?, status = ?, channel_status = ?,
               ...
               sale_price_cents = CASE WHEN ? THEN NULL ELSE sale_price_cents END,
               sale_qty = CASE WHEN ? THEN NULL ELSE sale_qty END,
               updated_at = datetime('now') WHERE id = ?`)
    .run(loc, eff.after.sold ? 'in_stock' : eff.before.status,
      eff.after.sold ? 'ended' : eff.before.channel_status,
      eff.after.sold ? 1 : 0, eff.after.sold ? 1 : 0, eff.after.sold ? 1 : 0, eff.item_id);
  setQuantity(tdb, { kind: 'sealed', itemId: eff.item_id, quantity: qty, reason: 'cancel', refKind: 'order', refId: eff.order_id ?? null, location: loc });
