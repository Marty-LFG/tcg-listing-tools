...
import { getOrders, geteBayOfficialTime, sendBuyerMessage, getMemberMessages } from './ebay-trading.mjs';
...
export function ingestOrder(db, o, cfg = DEFAULT_CONFIG) {
  ...
  try {
    ...
    db.prepare(`INSERT INTO postsale_messages (order_id, buyer_id, ebay_item_id, is_repeat_buyer, status)
                VALUES (?,?,?,?,'pending') ON CONFLICT(order_id) DO NOTHING`)
...
export async function pollOrders(env, db, { trigger = 'schedule' } = {}) {
  ...
  try {
    ...
    if (!watermark) {
      ...
      if (!t.ok || !t.timestamp) return finishPoll(started, trigger, { ok: false, error: 'eBay time/auth failed', ack: t.ack, errors: t.errors });
    ...
    const msg = await processMessages(env, db, cfg);
...
function makeRouter({ env, db }) {
  return async (req, res) => {
    try {
      ...
      // GET /orders?limit= — recent ingested orders + their line items.
      if (p === '/orders' && method === 'GET') {
        const limit = Math.min(500, parseInt(url.searchParams.get('limit') || '100', 10) || 100);
        const orders = db.prepare(`SELECT * FROM orders ORDER BY COALESCE(paid_time, created_time, ingested_at) DESC LIMIT ?`).all(limit);
        const itemsBy = {};
        for (const it of db.prepare(`SELECT * FROM order_line_items WHERE order_id IN (SELECT order_id FROM orders ORDER BY COALESCE(paid_time, created_time, ingested_at) DESC LIMIT ?)`).all(limit)) {
          (itemsBy[it.order_id] ||= []).push(it);
        }
        return send(res, 200, { orders: orders.map((o) => ({ ...o, items: itemsBy[o.order_id] || [] })) });
      }
      ...
      // POST /orders/:id/shipped — mark an order packed/shipped so it drops off the "to pack" digest.
      const shipM = p.match(/^\/orders\/(.+)\/shipped$/);
      if (shipM && method === 'POST') {
        const id = decodeURIComponent(shipM[1]);
        const r = db.prepare(`UPDATE orders SET shipped_status='shipped', shipped_time=COALESCE(shipped_time, datetime('now')) WHERE order_id = ?`).run(id);
