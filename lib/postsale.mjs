...
const DEFAULT_CONFIG = {
  ...
  // How often to re-read the orders we still consider open, BY ID. The ModTime cursor is a window and
  // a window can be missed; this is the backstop that means a state change cannot be lost forever. 0 = off.
...
function holdAlertText(db, row, { kind, state, restock, watching, lookup } = {}, decided) {
  ...
  return renderHoldAlert({
    ...
    reason: row.cancel_reason_detail || row.cancel_reason, initiator: row.cancel_initiator,
...
export async function pollOrders(env, db, { trigger = 'schedule' } = {}) {
  ...
  try {
    ...
    //
    // Scheduled ticks only. pollOrders is also what the dashboard's ↻ calls, and a person waiting on
    // that button should not additionally wait on three ReturnAll GetOrders they did not ask for.
    const sweepMin = trigger === 'schedule' ? Math.max(0, +cfg.sweep_interval_min || 0) : 0;
    let sweep = null;
    if (sweepMin) {
      const last = getMeta(db, 'last_sweep_at');
      if (!last || Date.now() - new Date(last).getTime() >= sweepMin * 60_000) {
        try {
    ...
    try {
      ...
      if (p === '/picksheet' && method === 'GET') {
        ...
        const holds = orders.filter((o) => isOnHold(o) || isCancelled(o)).map((o) => ({
          ...
          cancel_reason: o.cancel_reason_detail || o.cancel_reason || null,
