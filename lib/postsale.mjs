...
import {
  draftMessage, guardrailScrub, nextBusinessDay, fallbackDraft,
...
function renderApprovalCard({ buyerUsername, repeat, itemsText, subject, body, dryRun }, decided) {
  let s = `💌 <b>@${escapeHtml(buyerUsername || '')}</b>${repeat ? '  ⭐ <i>repeat buyer</i>' : ''}\n`;
...
async function draftAndRoute(env, db, msg, cfg, { pushCard = true } = {}) {
  ...
  if (!use && cfg.fallback_enabled !== false) {
    const fb = fallbackDraft({ order, items, cfg, shipBy });
    if (fb.ok && guardrailScrub(fb.body).clean) use = fb;
  }
  if (!use) {
...
async function stampTelegramCard(env, db, msg, decided) {
  ...
  await editCard(env, { chatId: msg.telegram_chat_id, messageId: msg.telegram_message_id,
    text: renderApprovalCard({ buyerUsername: order?.buyer_username, repeat: !!msg.is_repeat_buyer, itemsText: cardsText(db, msg.order_id), subject: msg.subject, body: msg.body }, decided),
...
async function onPostsaleUpdate(env, db, u) {
  ...
  // message ids exist. Empty allowlist denies everyone (see isAllowedUser).
...
export async function pollOrders(env, db, { trigger = 'schedule' } = {}) {
  ...
  try {
    ...
    return finishPoll(started, trigger, { ok: true, seen, ingested, skipped_unpaid: skippedUnpaid, skipped_pre_watermark: skippedPreWatermark, stock, messages: msg, window: { from: modTimeFrom, to: modTimeTo } });
  } catch (e) {
...
export async function runPackDigest(env, db, { force = false } = {}) {
  ...
  const rows = db.prepare(`SELECT o.order_id, o.buyer_username, o.ship_city, o.ship_state, o.paid_time, o.picked_at
  ...
  // Two messages, because this is two jobs. The PULL LIST runs through exactly the same pipeline as
  // GET /picksheet — buildInventoryLookup → attachLocations → buildPickSheet — so the phone walks the
  // shelf in the same order as the printed sheet. Inventing a second ordering here would be a way to
  // pull the wrong card confidently.
  ...
  attachLocations(db, orders, lookup);

  // Already-pulled orders drop off the PULL list (that work is done) but stay in the dispatch
  // summary flagged, because they still have to be posted. Same rule as the pick sheet's default.
  const pickRows = [];
  for (const o of orders) {
  ...
  const r = await sendMessage(env, { chatId, text: renderPullList(ps.groups, {
    orderCount: rows.length, itemCount: ps.rows.length, unitCount: ps.unit_count,
  }) });
  ...
  const r2 = await sendMessage(env, { chatId, text: renderDispatchSummary(orders) });
  if (r.ok) setMeta(db, 'last_pack_digest_date', date);
  _packDigest.last_run = { at: new Date().toISOString(), count: rows.length, ok: !!r.ok, summary_ok: !!r2.ok };
  return { ok: !!r.ok, count: rows.length, lines: ps.rows.length };
}
    ...
    try {
      ...
      if (p === '/picksheet' && method === 'GET') {
        ...
        if (idsParam) {
          ...
          const ph = ids.map(() => '?').join(',');
          orders = db.prepare(`SELECT order_id, buyer_username, sales_record_number, paid_time FROM orders WHERE order_id IN (${ph}) ORDER BY COALESCE(paid_time, created_time) ASC`).all(...ids);
        } else {
          ...
          const where = conds.length ? 'WHERE ' + conds.join(' AND ') : '';
          orders = db.prepare(`SELECT order_id, buyer_username, sales_record_number, paid_time FROM orders ${where} ORDER BY COALESCE(paid_time, created_time) ASC`).all();
        }
        const lookup = buildInventoryLookup();
        const rows = [];
        for (const o of orders) {
      ...
      if (editM && method === 'POST') {
        ...
        if (fresh.telegram_chat_id && fresh.telegram_message_id) {
          ...
          const { only } = cardItems(db, fresh.order_id);
          await editCard(env, { chatId: fresh.telegram_chat_id, messageId: fresh.telegram_message_id, text: renderApprovalCard({ buyerUsername: order?.buyer_username, repeat: !!fresh.is_repeat_buyer, itemsText: cardsText(db, fresh.order_id), subject: fresh.subject, body: fresh.body, dryRun: loadConfig().dry_run }), buttons: approvalButtons(fresh.id, loadConfig(), only), photo: !!(only && only.image_url) }).catch(() => {});
        }
        return send(res, 200, { ok: true, message: fresh });
      }
