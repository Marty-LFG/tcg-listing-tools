...
// When a sold line item can't be matched to a stored inventory location, fall back to grouping by its
// SKU prefix — many sellers encode storage in the SKU (e.g. AAC-012 → bin "AAC"). Far better than one
// big "Unsorted" pile; labelled "SKU …" so it isn't mistaken for a resolved inventory shelf record.
export function skuGroupLabel(sku) {
  if (!sku) return null;
  const s = String(sku).trim();
  if (!s) return null;
  const prefix = s.replace(/[-_\s]*\d+[A-Za-z]?$/, '').trim();   // strip a trailing number: AAC-012 → AAC
  return 'SKU ' + (prefix || s);
}
