...
import { shortTitle, renderPackDigest, renderSaleAlert } from './telegram-cards.mjs';
...
export async function runPackDigest(env, db, { force = false } = {}) {
  ...
  // Line items per order, not a comma-joined blob: the digest lists them one per line so a ten-card
  // order reads as ten things to pull rather than one paragraph. Titles carry commas, so splitting a
  // group_concat back apart was never safe anyway.
  const liStmt = db.prepare('SELECT title, quantity FROM order_line_items WHERE order_id=?');
  // Already-picked orders stay in the digest — they're packed but still need posting — just flagged so
  // the list reads as "these two are in the satchel, the rest need pulling".
  const orders = rows.map((r) => ({ ...r, items: liStmt.all(r.order_id) }));
  const text = renderPackDigest(orders);
  const r = await sendMessage(env, { chatId: (env.TELEGRAM_CHAT_ID || '').trim(), text });
