...
export async function runPackDigest(env, db, { force = false } = {}) {
  ...
  const lines = rows.slice(0, 40).map((r) => `${r.picked_at ? '✅' : '•'} ${escapeHtml(r.items || r.order_id)} — ${escapeHtml([r.ship_city, r.ship_state].filter(Boolean).join(', ') || '—')} · @${escapeHtml(r.buyer_username || '')}`);
  const more = rows.length > 40 ? `\n…and ${rows.length - 40} more` : '';
  const packed = rows.filter((r) => r.picked_at).length;
  const text = `📦 <b>To pack today</b> (${rows.length}${packed ? ` · ${packed} already packed` : ''})\n${lines.join('\n')}${more}`;
  const r = await sendMessage(env, { chatId: (env.TELEGRAM_CHAT_ID || '').trim(), text });
