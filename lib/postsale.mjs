...
export function decrementInventoryItem(tdb, itemId, qtySold, saleUnitCents, orderRef = null) {
  ...
  // ONE TRACKER-SIDE TRANSACTION around the row write and its movement, so the two cannot separate.
  //
  // This is the only stock write in this file that opens one, and it does not disturb the cross-DB
  // ordering documented below: it spans a single database and completes entirely before the postsale
  // stamp. Without it a crash between the UPDATE and the movement would leave a row marked sold whose
  // ledger still says it is on the shelf — drift the reconcile would report but nobody asked for.
  const txn = (fn) => {
    tdb.exec('BEGIN');
  ...
  txn(() => applyMovement(tdb, { kind: 'inventory', itemId, delta: -qty, reason: 'sale', refKind: 'order', refId: orderRef }));
