...
export async function backfillOrders(env, db, { months = 24 } = {}) {
  ...
  const floorMs = Date.now() - Math.max(1, months) * 30 * 24 * 3600_000;
  let toMs = Date.now(), ingested = 0, seen = 0, windows = 0, emptyStreak = 0;
  try {
    while (toMs > floorMs && emptyStreak < 2) {
      ...
      while (page <= 100) {
        ...
        if (!res.ok) { _backfill.last_run = { at: started.toISOString(), ok: false, error: 'GetOrders failed', ack: res.ack, errors: res.errors, ingested, seen }; return _backfill.last_run; }
        for (const o of res.orders) {
          seen++;
          if (!o.paid) continue;
    ...
    _backfill.last_run = { at: started.toISOString(), finished_at: new Date().toISOString(), ok: true, ingested, seen, windows, months };
    console.log(`[postsale] backfill: ${ingested} historical order(s) ingested (${seen} seen, ${windows} windows) — messages closed`);
