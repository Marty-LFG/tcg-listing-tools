...
import { shortTitle, renderPullList, renderDispatchSummary, renderSaleAlert, renderHoldAlert, renderDealCard } from './telegram-cards.mjs';
import { getOrders, geteBayOfficialTime, sendBuyerMessage, getMemberMessages, completeSale, getItem, sendInvoice, siteId } from './ebay-trading.mjs';
...
import { classifyDealAsk, dealSummary, checkDiscount } from './deals.mjs';
...
export function refreshOrder(db, o, cfg = DEFAULT_CONFIG) {
  ...
  if (becameShipped) {
    ...
    // TWO DIFFERENT EVENTS ARRIVE THROUGH THE SAME FLAG, and telling them apart is the whole of this.
    //
    //   a label was BOUGHT   — eBay flips the order the instant the postage label is paid for, while
    //                          the cards are still on the shelf. The parcel has NOT gone, so the order
    //                          must keep its place in the pack queue. That is what label_bought_at is.
    //   "mark as dispatched" — the seller bulk-ticking orders in Seller Hub for parcels that ALREADY
    //                          went. There is no work left. Stamping label_bought_at here parks the
    //                          order in the queue forever, because only a human tap ever clears it —
    //                          which is exactly how eight posted orders ended up on a morning digest.
    //
    // A bought label carries a tracking number (firstTracking, lib/ebay-trading.mjs); a bulk tick sends
    // ShippedTime and nothing else. The postage tier is the second half of the same question, because
    // a tracked or express service the buyer PAID for will get a label even before the number reaches
    // GetOrders. So: HOLD ON DOUBT, SETTLE ONLY ON EVIDENCE OF ABSENCE. An express order held a poll
    // too long gets looked at; an untracked letter settled wrongly is invisible until someone counts.
    //
    // postage_tracked is always present on next — postageColumns writes `p.tracked ? 1 : 0` and the
    // never-blank loop above deliberately keeps the 0.
    //
    // Note the asymmetry with the transition rule: an order settled here is NOT re-evaluated if a
    // postage.services override later declares its service tracked, because becameShipped will not
    // fire twice. That is the rule working as designed — settleLabelBought is the tool for correcting
    // a batch, and it re-derives through the same classifyPostage.
    const labelLooksBought = !!(next.tracking_number || prev.tracking_number)
      || !!(next.postage_tracked ?? prev.postage_tracked);
    if (!prev.dispatch_source && !prev.picked_at && !prev.label_bought_at && labelLooksBought) {
      next.label_bought_at = nowSql();
    }
    // else: there is nothing more to write, and that is the point. shipped_status='shipped' with no
  ...
  if (!changed.length) {
    return { updated: false, changed: [], gotTracking: false, gotDelivered: false, becameShipped: false,
      labelBought: false, settledViaEbay: false,
  ...
  return { updated: true, changed, gotTracking, gotDelivered, becameShipped, queued,
    labelBought: !!next.label_bought_at, settledViaEbay: becameShipped && !next.label_bought_at,
...
async function onPostsaleUpdate(env, db, u) {
  ...
  if (!m && !dg && !ack && !dl) return;               // not ours — another handler will claim it
...
export async function pollMemberMessages(env, db, { trigger = 'schedule' } = {}) {
  ...
  try {
    ...
    let page = 1, seen = 0, replies = 0, deals = 0;
    while (page <= MAX_PAGES) {
      ...
      for (const mm of res.messages) {
        ...
        db.prepare(`INSERT INTO member_messages (message_id, message_type, sender_id, ebay_item_id, subject, body, status, creation_time)
          ...
          .run(mm.messageId || synthMessageId(mm), 'AskSellerQuestion', mm.senderId, mm.itemId, mm.subject, mm.body, mm.status, mm.creationTime);
        ...
        try {
          const dealId = recordDealFromMessage(db, { ...mm, messageId: mm.messageId || synthMessageId(mm) }, cfg);
          if (dealId) {
            deals++;
    ...
    _msgPoll.last_run = { at: started.toISOString(), finished_at: new Date().toISOString(), trigger, ok: true, seen, replies, deals };
    return { ok: true, seen, replies, deals };
  } catch (e) {
...
export function runMemberMessagePoll(env, db, { trigger = 'schedule' } = {}) {
  if (_msgPollInFlight) return _msgPollInFlight;
  _msgPollInFlight = Promise.resolve(pollMemberMessages(env, db, { trigger })).finally(() => { _msgPollInFlight = null; });
...
/**
 * Settle orders wrongly parked in label_bought, by the SAME rule refreshOrder now applies live.
 *
 * These rows are not history that turned out differently — the value was WRONG WHEN IT WAS WRITTEN.
 * eBay never bought a label for them; the seller bulk-ticked parcels that had already gone, and the
 * old code could not tell the two apart. So this CLEARS label_bought_at rather than adding a "settled"
 * column: a third term in the queue predicate, carried forever, to remember that a stamp should not
 * have been there is a worse trade than removing the stamp.
 *
 * Classification runs through attachPostage — deliberately the same classifyPostage that
 * postageColumns calls — so this and the live path cannot disagree about the one order it must not
 * touch. A tracked order is never cleared, and the UPDATE says so a second time in SQL: whatever the
 * classification decided a moment ago, a row carrying a tracking number is out of scope.
 *
 * Dry-run by default, and exposed as POST /diag/settle-label-bought rather than run on boot. A boot
 * pass would sweep `orders` unattended on every restart, on every HMR reload and inside the
 * integration suite, which is precisely when nobody is watching. The shape here is the one
 * /api/ebay-notify/reconcile already established: look at the plan, then ?apply=1.
 *
 * Idempotent by construction — a cleared row no longer matches LABEL_BOUGHT_SQL, so a second run
 * settles nothing, forever.
 */
export function settleLabelBought(db, cfg = loadConfig(), { apply = false } = {}) {
  const rows = db.prepare(`SELECT * FROM orders WHERE ${LABEL_BOUGHT_SQL}`).all();
  attachPostage(rows, cfg);
  const held = rows.filter((o) => o.tracking_number || (o.postage && o.postage.tracked));
  const settle = rows.filter((o) => !o.tracking_number && !(o.postage && o.postage.tracked));
  let updated = 0;
  if (apply && settle.length) {
    const ph = settle.map(() => '?').join(',');
    updated = db.prepare(`UPDATE orders SET label_bought_at = NULL
                          WHERE order_id IN (${ph}) AND tracking_number IS NULL`)
      .run(...settle.map((o) => o.order_id)).changes || 0;
    // Named, not counted — the ring buffer behind /api/status/logs is the only record of a one-shot
    // correction, and "cleared 8" is not something anybody can check afterwards.
    console.log(`[postsale] settle-label-bought: cleared ${updated} order(s) eBay had merely marked `
      + `dispatched — ${settle.map((o) => o.order_id).join(', ')}`);
  }
  const view = (o) => ({
  ...
  return { ok: true, dry_run: !apply, checked: rows.length, updated,
    settled: settle.map(view), held: held.map(view) };
}
...
function makeRouter({ env, db }) {
  return async (req, res) => {
    try {
      ...
      // POST /diag/settle-label-bought[?apply=1] — apply the label-bought-vs-marked-dispatched rule to
      // rows that were stamped before the rule existed. Dry-run by default; the response names every
      // order on both sides, because "8 settled" is not a thing anyone can check.
      if (p === '/diag/settle-label-bought' && method === 'POST') {
        const auth = diagOk(env, req, url); if (!auth.ok) return send(res, auth.code, { error: auth.error });
        const result = settleLabelBought(db, loadConfig(), { apply: url.searchParams.get('apply') === '1' });
        return send(res, 200, { triggered: 'settle-label-bought', result });
      }
