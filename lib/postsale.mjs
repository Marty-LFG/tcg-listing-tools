...
export function recordDealFromMessage(db, mm, cfg = DEFAULT_CONFIG) {
  ...
  if (!deals.enabled || deals.detect_from_messages === false) return false;
  const cls = classifyDealAsk(mm.subject, mm.body);
  if (!cls.ask) return false;
  ...
  try {
    ...
    while (page <= MAX_PAGES) {
      ...
      for (const mm of res.messages) {
        ...
        try { if (recordDealFromMessage(db, { ...mm, messageId: mm.messageId || synthMessageId(mm) }, cfg)) deals++; }
