 ...
 * have caught.
 */
export async function reconcileRedemptions(env, db, { rules = {}, store = 'dev', nowMs = Date.now(), limit = 100, graphql = shopifyGraphQL } = {}) {
  ...
  let used = 0; let expired = 0; let refunded = 0; let checked = 0; let awaitingGrace = 0;
  const disagreements = [];

  for (const r of rows) {
    if (!r.discount_gid) continue;
    checked++;
    const res = await graphql(env, READ_QUERY, { id: r.discount_gid }, { store });
    if (!res.ok) continue;
    ...
    if (count > 0 && !r.used_order_gid) {
      ...
      // and WARNED, because an array returned to a caller that discards it is not an alarm. The row
      // this produces is status='used' with used_order_gid NULL, and driftReport counts exactly that
      // shape: the STATE is the durable half of the signal, still there long after a log line has
      // scrolled out of the ring buffer.
      disagreements.push({ id: r.id, code: r.code, asyncUsageCount: count, note: 'used on an order we never recorded' });
      console.warn(`[keepers] redemption ${r.id} (${r.code}) was spent on Shopify but no order of ours claims it `
        + `— asyncUsageCount ${count}. Either a webhook was lost, or the code reached someone it was not minted for.`);
    }
    if (count > 0) {
    ...
    // EXPIRY WAITS OUT A GRACE WINDOW, and the waiting is the fix rather than a delay bolted onto it.
    //
    // asyncUsageCount is asynchronous — the name says so — and this scan runs every 30 minutes. A
    // code spent shortly before endsAt can therefore still read 0 on the first sweep after it. Acting
    // on that reading marked the redemption 'expired' and handed the points back for a code that had
    // been SPENT, and nothing could ever correct it afterwards: 'expired' is excluded from this scan
    // (status='active') AND from markUsedByOrder's update (status IN ('active','used')), so neither
    // of the two readings the design calls its alarm could still reach the row. Silent, irreversible,
    // and wrong in the customer's favour twice over — they kept the discount and got the points back.
    //
    // Staying 'active' through the window is what makes it self-correcting instead of merely later:
    // the row remains in this scan, so a lagging asyncUsageCount is still seen, and remains in
    // markUsedByOrder's range, so a late webhook still lands. The refund happens only once neither
    // reading has said anything for the whole window. Nothing about the customer's experience moves —
    // the code is already dead on Shopify at endsAt; this is our bookkeeping catching up honestly.
    const graceMin = Number(rules.redemption_expiry_grace_min);
    const graceMs = (Number.isFinite(graceMin) && graceMin >= 0 ? graceMin : 60) * 60000;
    const expiresMs = r.expires_at ? Date.parse(r.expires_at) : NaN;
    if (Number.isFinite(expiresMs) && expiresMs + graceMs <= nowMs) {
      ...
      // Past endsAt but inside the window. Counted so it reads as deliberate patience rather than a
      // sweep that keeps re-reading the same rows and never finishing with them.
      awaitingGrace++;
    }
  }
  return { checked, used, expired, refunded, awaitingGrace, disagreements };
}
