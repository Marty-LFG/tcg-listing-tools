...
const CONFIG_PATH = configFile('bulk-pricing.config.json');
...
// Order matters, for the same reason ebayFinish (lib/channels/ebay-map.mjs) carries the note:
// "Non-holo" contains "holo" and "Nonfoil" contains "foil", so the NEGATIONS have to be tested
// before the holo/foil branches or a plain card is floored at the holo/foil tier. Both spellings
// are live — the uploader's dropdown offers "Non-holo", Scryfall's own finish vocabulary is
// "nonfoil" — and a mis-tiered common is a 3-4x over-floor (GR4: floors are owner-set, and an
// owner-set floor for a base card is the BASE floor).
export function finishClass(finish) {
  const f = (finish || '').toLowerCase();
  if (/reverse/.test(f)) return 'Reverse Holo';
  if (/non[-\s]?(holo|foil)/.test(f)) return 'Base';
  if (/holo/.test(f)) return 'Holo';
  // Lorcana's foil-only chase rarities each get their own tier, because one floor cannot serve a
  // US$4 Enchanted and a US$3,632 Iconic. 'Cold Foil' contains "foil", so like 'Surge Foil' it has
  // to sit above the generic branch or it silently prices as an ordinary foil.
  if (/enchanted/.test(f)) return 'Enchanted';
  if (/iconic/.test(f)) return 'Iconic';
  if (/epic/.test(f)) return 'Epic';
  if (/cold\s*foil/.test(f)) return 'Cold Foil';
