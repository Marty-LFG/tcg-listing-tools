...
/**
 * What we already have ON THIS STORE for these exact bytes, if anything.
 *
 * The store predicate is the whole point of the lookup, not decoration. `url` and `file_gid` are a
 * `gid://shopify/MediaImage/<n>` and a CDN link that exist on exactly one store, so a store-blind hit
 * hands a live publish a dev file — which `productSet` refuses outright with INVALID_INPUT, and which
 * the adopt-before-uploading path above would then "recover" from on every artifact, for ever. This
 * module was written while shopify_files was keyed on content_hash alone; it is keyed
 * (content_hash, store) now.
 */
export function cachedArtifact(db, sha, store = 'dev') {
  return db.prepare("SELECT * FROM shopify_files WHERE content_hash = ? AND store = ? AND status = 'ready'")
    .get(sha, store);
}
...
export async function uploadArtifact(env, db, {
  ...
  const hit = cachedArtifact(db, sha, store);
  ...
  db.prepare(`INSERT INTO shopify_files (content_hash, store, status, filename, bytes, mime_type)
              VALUES (?, ?, 'staged', ?, ?, ?)
              ON CONFLICT(content_hash, store) DO UPDATE SET status='staged', filename=excluded.filename,
                bytes=excluded.bytes, mime_type=excluded.mime_type`)
    .run(sha, store, filename, bytes.length, mimeType);

  const staged = await gql(STAGE, {
    input: [{ filename, mimeType, resource: 'FILE', httpMethod: 'POST', fileSize: String(bytes.length) }],
  }, 10);
  if (!staged.ok) return fail(db, sha, `stagedUploadsCreate: ${firstErrorText(staged) || `HTTP ${staged.httpStatus}`}`, store);
  const ue = staged.data?.stagedUploadsCreate?.userErrors || [];
  if (ue.length) return fail(db, sha, `stagedUploadsCreate: ${ue.map((e) => e.message).join('; ')}`, store);
  const target = staged.data.stagedUploadsCreate.stagedTargets?.[0];
  if (!target?.resourceUrl) return fail(db, sha, 'stagedUploadsCreate returned no target', store);

  const put = await postToStagedTarget(target, bytes, { filename, mimeType, fetchImpl });
  if (!put.ok) return fail(db, sha, put.error || 'staged upload failed', store);

  const created = await gql(CREATE, {
    files: [{ originalSource: target.resourceUrl, contentType: 'FILE', alt: `bkr1 ${filename}` }],
  }, 10);
  if (!created.ok) return fail(db, sha, `fileCreate: ${firstErrorText(created) || `HTTP ${created.httpStatus}`}`, store);
  const cerr = created.data?.fileCreate?.userErrors || [];
  if (cerr.length) return fail(db, sha, `fileCreate: ${cerr.map((e) => e.message).join('; ')}`, store);
  const fileGid = created.data.fileCreate.files?.[0]?.id;
  if (!fileGid) return fail(db, sha, 'fileCreate returned no file id', store);
  ...
  db.prepare("UPDATE shopify_files SET file_gid = ?, status = 'staged' WHERE content_hash = ? AND store = ?")
    .run(fileGid, sha, store);
  ...
  for (const wait of POLL_STEPS_MS) {
    ...
    if (node?.fileErrors?.length) {
      return fail(db, sha, `file processing: ${node.fileErrors.map((e) => e.message).join('; ')}`, store, fileGid);
    }
    if (node?.fileStatus === 'READY' && node.url) {
      db.prepare("UPDATE shopify_files SET status='ready', url=?, ready_at=datetime('now') WHERE content_hash=? AND store=?")
        .run(node.url, sha, store);
      return { ok: true, fileGid, url: node.url, sha256: sha, adopted: false };
    }
  }
  return fail(db, sha, 'file never became READY', store, fileGid);
}

// `store` occupies what used to be a dead `_sha` slot: every call passed sha a second time and nothing
// read it. Required rather than defaulted, because a failure row written against the wrong store is the
// same defect as a cache hit from one.
function fail(db, sha, error, store, fileGid = null) {
  try {
    db.prepare("UPDATE shopify_files SET status='failed', error=?, file_gid=COALESCE(?, file_gid) WHERE content_hash=? AND store=?")
      .run(String(error).slice(0, 500), fileGid, sha, store);
