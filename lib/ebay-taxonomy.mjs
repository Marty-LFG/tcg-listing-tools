...
export async function categoryAspects(env, categoryId) {
  try {
    ...
    const r = await appGet(env, url);
    if (!r.ok || !r.json) return { ok: false, error: 'HTTP ' + r.status };
    const aspects = (r.json.aspects || []).map((a) => {
      ...
      return {
        ...
        values: (function(){
          var all = (a.aspectValues || []).map(function(v){ return v.localizedValue; });
          if (!needle) return all.slice(0, 25);
          return all.filter(function(v){ return String(v).toLowerCase().includes(needle); }).slice(0, 40);
        })(),
      };
    });
