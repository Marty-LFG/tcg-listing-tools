...
// Look a label up in a live value-name map, case- and whitespace-insensitively. eBay returns sentence
// case ('Near mint or better') while our semantic descriptors carry eBay's own title-case enum ('Near
// Mint or Better'), so an exact match NEVER hit — the live map was dead weight and every listing
// silently used baked IDs, which is precisely how a stale baked ID would ship unnoticed.
function lookupLabel(map, label) {
  if (!map || label == null) return null;
  const want = String(label).trim().toLowerCase();
  if (!want) return null;
  for (const [name, id] of Object.entries(map)) if (String(name).trim().toLowerCase() === want) return id;
  return null;
}
...
export async function resolveConditionDescriptorIds(env, semantic, { graded, categoryId = '183454' } = {}) {
  ...
  for (const d of semantic || []) {
    ...
    // An unmappable descriptor NAME used to be dropped silently, which shipped a trading-card
    // condition with a missing (or absent) descriptor container. Surface it so publish blocks (GR4).
    if (!nameId) { unresolved.push(d.name + ' (unknown descriptor name)'); continue; }
    ...
    if (d.name === 'Professional Grader') {
      ...
      valueId = lookupLabel(live && live.grader, longGraderName(code));
      ...
      valueId = lookupLabel(live && live.cardCondition, d.value);
...
// Category discovery
// ---------------------------------------------------------------------------
// Every category id in this repo was 183454, the singles category, hardcoded per game. Sealed product
// does not belong there, and nothing here could ever FIND another one: both Taxonomy calls above take
// a category id as INPUT. So a sealed listing had no honest category to go to, and guessing one is how
// a booster box gets filed as a single card.
//
// getCategorySuggestions is eBay's own answer to "what category is this thing". Read-only, app token,
// so it is safe to call from a settings probe. The result is a RANKED SUGGESTION, never a decision:
// the caller shows it to the owner, who pins one into config. We do not auto-adopt the top hit.
export async function suggestCategories(env, q) {
  const query = String(q || '').trim();
  if (!query) return { ok: false, error: 'no query' };
  try {
    const url = `${TAXO}/category_tree/${TREE_ID}/get_category_suggestions?q=${encodeURIComponent(query)}`;
    const r = await appGet(env, url);
    if (!r.ok || !r.json) return { ok: false, error: 'HTTP ' + r.status, raw: r.json || null };
    const rows = (r.json.categorySuggestions || []).map((s) => {
      const c = s.category || {};
      const path = (s.categoryTreeNodeAncestors || []).slice().reverse().map((a) => a.categoryName);
      return {
        categoryId: c.categoryId,
        categoryName: c.categoryName,
        path: path.concat(c.categoryName || '').filter(Boolean).join(' > '),
        leaf: s.categoryTreeNodeLevel != null ? s.categoryTreeNodeLevel : null,
        relevancy: s.relevancy || null,
      };
    });
    return { ok: true, treeId: TREE_ID, marketplace: MARKET, query, suggestions: rows };
  } catch (e) { return { ok: false, error: String(e?.message || e) }; }
}

// Full aspect dump for a category the owner is considering. The FREE_TEXT flag is the one that matters:
// a required FREE_TEXT aspect accepts a near-miss silently and simply earns no facet, which is how
// Lorcana and Riftbound sat mis-filed for months.
// Shape one get_item_aspects_for_category response. PURE and exported so it can be unit tested:
// categoryAspects itself needs a live eBay call, which is exactly how a ReferenceError in here shipped
// once and passed the whole suite. `match` searches the FULL value list rather than the 25-value
// sample, because "is 'Pokemon TCG' really in this enum" is not a question a sample can answer.
export function shapeAspects(json, match) {
  const needle = String(match == null ? '' : match).trim().toLowerCase();
  return ((json && json.aspects) || []).map((a) => {
    const c = a.aspectConstraint || {};
    const all = (a.aspectValues || []).map((v) => v.localizedValue);
    const hits = needle ? all.filter((v) => String(v).toLowerCase().includes(needle)) : null;
    return {
      name: a.localizedAspectName,
      required: !!c.aspectRequired,
      mode: c.aspectMode || null,                     // FREE_TEXT | SELECTION_ONLY
      cardinality: c.itemToAspectCardinality || null,
      valueCount: all.length,
      matched: hits ? hits.length : undefined,
      values: hits ? hits.slice(0, 40) : all.slice(0, 25),
    };
  });
}

export async function categoryAspects(env, categoryId, opts = {}) {
  try {
    const url = `${TAXO}/category_tree/${TREE_ID}/get_item_aspects_for_category?category_id=${encodeURIComponent(categoryId)}`;
    const r = await appGet(env, url);
    if (!r.ok || !r.json) return { ok: false, error: 'HTTP ' + r.status };
    const aspects = shapeAspects(r.json, opts.match);
    return { ok: true, categoryId: String(categoryId), required: aspects.filter((a) => a.required), aspects };
  } catch (e) { return { ok: false, error: String(e?.message || e) }; }
}
