...
// Repair mojibake: some UPCItemDB titles are UTF-8 bytes stored as Latin-1 ("PokÃ©mon" for "Pokémon"),
// which garbles both display and matching. Only re-decode when the telltale "Ã"/"Â" markers are present
// AND doing so cleanly reduces them (never touches a correct "Pokémon" or plain ASCII). Exported for tests.
export function demojibake(s) {
  const str = String(s == null ? '' : s);
  if (!/[ÃÂ]/.test(str)) return str;
  try {
    const fixed = Buffer.from(str, 'latin1').toString('utf8');
    if (!fixed.includes('�') && (fixed.match(/[ÃÂ]/g) || []).length < (str.match(/[ÃÂ]/g) || []).length) return fixed;
  } catch { /* leave as-is */ }
  return str;
}
...
export async function lookupUpcName({ upc, key, keyType, enabled = true } = {}) {
  ...
  try {
    ...
    const title = item && demojibake(String(item.title || '').trim())
    if (!title) return { matched: false }
    return {
      matched: true,
      title,
      name: cleanSealedTitle(title),
      brand: demojibake((item.brand || '').trim()),
      category: demojibake((item.category || '').trim()),
