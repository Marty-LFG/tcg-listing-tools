 ...
 *
 * Exported for lib/ebay-notify-react.mjs, which asks the same question either side of a poll. Sharing
 * it rather than copying it keeps "do we have this order" one definition — the two modes disagreeing
 * about that would make their measurements incomparable, which is the only thing observe mode is for.
 */
export function knownOrderIds(db, ids) {
 ...
 * compare against.
 *
 * The lead means two different things, and which one depends on the mode the row was written in.
 * In OBSERVE mode nothing acted on the notification, so the lead is how far ahead of the schedule
 * push was: the case for turning poll mode on. In POLL mode the notification IS what ran the poll,
 * so the same number becomes reaction time — notification to order-in-hand, seconds rather than
 * minutes. Both are worth watching and neither needed its own field; what they share is "how long
 * after eBay told us did this order become real to us", which is the question either way.
 */
export function observationSummary(db, { days = 7 } = {}) {
  try {
    ...
    let ahead = 0, matched = 0, tests = 0, unresolved = 0;
    const leads = [];
    for (const r of rows) {
      let obs = null; try { obs = JSON.parse(r.observation || 'null'); } catch { /* older row */ }
      // A poll-mode row that never resolved to an order is either eBay's test payload or a sale eBay
      // announced and then would not return. Neither is evidence about latency, so both are counted
      // on their own rather than folded into the median — and separately from `tests`, because in
      // poll mode the two are indistinguishable from here and pretending otherwise would be a guess.
      if (obs?.mode === 'poll' && obs.order_adopted === false && !r.order_ingested_at) { unresolved++; continue; }
      // eBay's testSubscription sends a payload of literal placeholders — the orderId is the string
      // "string" — so it never resolves to an order. Counting those as "push beat the poll" would
      // inflate exactly the number this whole mode exists to measure.
      if (obs && obs.found_on_ebay === false && !r.order_ingested_at) { tests++; continue; }
      if (r.order_ingested_at) {
        ...
        if (Number.isFinite(lead)) {
          leads.push(lead);
          // Derived from the two RECORDED timestamps rather than from the observation snapshot.
          // order_known_at_receipt is evaluated when the pass runs, so a backlog drained hours late
          // reads as "the poll already had it" for orders push genuinely announced first. The
          // timestamps do not drift like that.
          if (lead > 0) ahead++;
        }
      }
    }
    ...
    return {
      ...
      test_notifications: tests,        // eBay's own testSubscription payloads, excluded from the rest
      unresolved: unresolved,           // poll mode: announced by eBay, never returned by it
      ahead_of_poll: ahead,
