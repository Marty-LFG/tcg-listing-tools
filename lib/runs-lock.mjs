...
export function deriveChases(bundles, ladder, specs, byBundle) {
  ...
  for (const entry of ladder) {
    if (!claimed.has(entry.rank)) {
      ...
      //
      // A GRADED CARD IN THIS SYSTEM HAS NO SET CODE. inventory_items carries set_NAME and no set_CODE, so
      // every graded line commits set_code = '' — and a ladder entry stating a set code can therefore never
      // match one, however right the card is. Leave set_code empty for an inventory-sourced chase and the
      // remaining fields identify it; a set code will only work once inventory_items has that column.
