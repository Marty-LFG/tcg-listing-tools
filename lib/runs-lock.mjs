...
/**
 * §10.4 step 2: randomise which bundles hold the chases.
 *
 * Fisher-Yates over `crypto.randomInt`, NEVER Math.random — a predictable placement is a predictable
 * product, and this is the single draw the whole thing turns on.
 *
 * PINNED BUNDLES ARE EXCLUDED from the draw and keep exactly what was assigned to them, which is what lets
 * one bundle deliberately hold the best option in every slot at once.
 *
 * Because this runs AFTER the manifest is assembled, everything assigned earlier — the rarity spread, the
 * pack mix — is automatically uncorrelated with chase placement. That independence is free and needs no
 * separate mechanism, which is the reason placement is last rather than first.
 */
export function placeChases(bundles, ladder, rng = randomInt) {
  const pinnedChases = bundles.filter((b) => b.pinned && b.is_chase);
  const eligible = bundles.filter((b) => !b.pinned).map((b) => b.bundle_no);
  const need = ladder.length - pinnedChases.length;
  if (need < 0) throw new Error('more pinned chase bundles than the chase ladder declares');
  if (need > eligible.length) {
    throw new Error(`the ladder declares ${ladder.length} chases but only ${eligible.length + pinnedChases.length} bundles can hold one`);
  }
  ...
  let rank = 0;
  for (const b of pinnedChases) placement.set(b.bundle_no, ladder[rank++] ?? null);
  for (const no of eligible.slice(0, need)) placement.set(no, ladder[rank++] ?? null);
  return placement;
}
 ...
 *
 * The injectable `codes`, `salts`, `nonceFor` and `rng` exist so the specification's published vectors are
...
export async function lockRunPhase1(db, runId, {
  actor = null, now = null, codes = null, salts = null, nonceFor = null, rng = randomInt,
} = {}) {
  ...
  if (!problems.length) {
    try {
      placement = placeChases(bundles, ladder, rng);
  ...
  try {
    ...
    const setBundle = db.prepare(`UPDATE run_bundles
      SET salt_hex = ?, verify_code = ?, leaf_hash = ?, is_chase = ?, chase_tier_id = ?, updated_at = ?
      WHERE id = ?`);
    ...
    prepared.forEach((p, i) => {
      ...
      setBundle.run(p.salt, p.code, tree.leaves[i], p.tier ? 1 : 0, p.tier ? p.tier.id : null,
        stamp, p.bundle.id);
    });
...
export async function lockRunPhase2(db, runId, { publisher, anchorer, actor = null, now = null } = {}) {
  ...
  const stamp = now || new Date().toISOString();
  const published = await publisher({ db, run });
