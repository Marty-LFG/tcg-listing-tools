...
//
// The one import is deliberate: lookupPath() has to recognise a namespaced non-English Pokémon
// identity key, and that rule belongs in exactly one place (lib/pokemon-intl.mjs) rather than as a
// regex copied into every file that touches an identity key.
import { intlKeyLang } from './pokemon-intl.mjs';
...
const MAPPERS = {
  riftbound(j, variant) {
    const c = unwrap(j);
    if (!c) return null;
    // Keyless TCGplayer lane (/api/rbp, lib/riftbound-prices.mjs) — a FLAT row, already one price.
    // Riftbound foils aren't separate products (finish follows rarity: finishOf() in
    // riftbound-data.mjs), so there is no normal/foil pair to choose between and `variant` is moot.
    // No trend deltas either — the graph reconstructs them from price_snapshots history instead.
    if (!c.variants && c.market != null) {
      return { market: +c.market, low: c.low != null ? +c.low : null, currency: c.currency || 'USD', source: c.source || 'tcgplayer' };
    }
    // Scrydex lane (/api/rb) — kept so a reactivated subscription needs no code change, and so
    // anything still reading the Scrydex shape (card_cache payloads) maps identically.
    if (!c.variants) return null;
  ...
  mtg(j, variant) {
    ...
    // "nonfoil" CONTAINS "foil" — and it is Scryfall's own finishes value as well as the MTG
    // builder's default dropdown option, so an unguarded .includes('foil') seats the FOIL price on
    // a non-foil card. On a chase print that is a multiple, not a rounding error (GR4).
    const nonFoil = /non[-\s]?foil/.test(fin);
    if (nonFoil) market = pr.usd || pr.usd_foil || pr.usd_etched;
    else if (fin.includes('etched')) market = pr.usd_etched || pr.usd_foil || pr.usd;
  ...
  lorcana(j, variant) {
    ...
    //
    // Enchanted, Epic and Iconic have to be named here: they are foil-only printings whose words
    // contain neither "foil" nor "holo", so a bare /foil/ test reads them as NON-foil and reaches
    // for `usd` first. That happens to fall through to usd_foil today only because their `usd` is
    // null — one upstream change and an Iconic quotes a base-card price.
    const foil = /foil|enchanted|iconic|epic/i.test(variant || '');
...
// Game-specific identity_key -> proxy path for the card RECORD (name + images + metadata).
// NOTE: this is not the price path — see pricePath() below. inventory.mjs (card images via
// imageFrom) and collectr-resolve.mjs (name-overlap guard) both need the full record, so this must
// keep pointing at a source that carries name/images.
export function lookupPath(game, identityKey) {
  switch (game) {
    ...
    case 'pokemon':
      // A non-English key is namespaced with its TCGdex lane ('ja:m5-102', lib/pokemon-intl.mjs)
      // and pokemontcg.io holds no such card, so there is no record path — the documented `null`
      // contract, same as `default` below. Returning the /api/pkm URL anyway would only buy a
      // guaranteed 404 per card, and it makes pricePath() null too, which is what keeps the
      // collector from filing an intl row as a price failure it can never fix. Intl images resolve
      // through resolveIntlImage() in lib/inventory.mjs instead, which reads the TCGdex shape.
      if (intlKeyLang(identityKey)) return null;
...
// Path the collector fetches for a PRICE. Same as lookupPath() for every game except riftbound,
// whose Scrydex price lane went 402 SUBSCRIPTION_INACTIVE — /api/riftbound/prices serves keyless
// TCGplayer market prices from the baked index instead (lib/riftbound-prices.mjs).
//
// Split from lookupPath() deliberately: that path carries no images and no card record, so
// repointing lookupPath() would have broken inventory card images and collectr-resolve's
// name-overlap guard, which both fetch it for reasons other than price.
//
// NOT '/api/rb…': the '/api/rb' proxy in vite.config.js prefix-matches anything starting with it.
export function pricePath(game, identityKey) {
  if (game === 'riftbound') return '/api/riftbound/prices/' + encodeURIComponent(identityKey);
  return lookupPath(game, identityKey);
}
