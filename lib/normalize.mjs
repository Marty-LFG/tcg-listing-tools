...
// here without wiring its data pipeline.
export const GAMES = ['riftbound', 'mtg', 'pokemon', 'swu', 'lorcana'];

// Games we can hold in INVENTORY (graded + sealed stock) — a SUPERSET of the card-data GAMES. One Piece
// has no card API wired up, so it's stockable (recorded manually / priced by sealed barcode via
// PriceCharting) but is deliberately kept OUT of GAMES so no card-lookup/tracker path claims to support
// it. The graded + sealed inventory validate against this list; the tracker/builders use GAMES.
export const STOCK_GAMES = [...GAMES, 'onepiece'];
