...
//   GATED    /:id/manifest and /:id/lock, which do. The manifest is the pre-sale answer sheet — anyone
//            holding it knows which numbered bundle has the chase before a single one is sold — and the
//            lock route MINTS THE VERIFICATION CODES, which are bearer secrets: whoever reads one can
//            decrypt that bundle's record. Everything else in this app on the private network is
//            commercial data; these are the product, so they sit behind the same DIAG_TOKEN check as the
//            diagnostics routes. The codes are withheld even there unless ?codes=1 is asked for.
//
...
//
// LOCK IS HERE; PUBLISH AND ANCHOR ARE NOT YET. Phase 1 of §10.4 runs through lib/runs-lock.mjs, which
// freezes the manifest and generates every committed value. Phase 2 — uploading the blob file, publishing
// the commitment and submitting the timestamp — is R2-4 and R2-5. Every write goes through
...
import { lockRunPhase1 } from './runs-lock.mjs';
import { commitment } from './runs-public.mjs';
import { submitAnchor, upgradeAnchor, anchorsFor, publicAnchor, saleGate } from './runs-anchor.mjs';
import { startRunsJobs, runAnchorSweepNow, runOrdersPollNow, getRunsJobsState } from './runs-jobs.mjs';
import { UNSOLD_POLICY } from './runs-policy.mjs';
import { lockRunPhase2 } from './runs-lock.mjs';
import { makeArtifactPublisher, makeAnchorer, artifactsFor, publishAnchorReceipt } from './runs-artifacts.mjs';
import { armingState } from './runs-arm.mjs';
import { publishRunProduct, runListings, mirrorAvailability, runProductHandle, setBundleAvailable } from './runs-shopify.mjs';
import {
  publicLedger, availability, appendSale, appendCancel, appendReprice, appendPause, appendResume,
  appendOpenedLive, verifyChain as verifyLedgerChain, ledgerEntries, asEntries,
} from './runs-ledger.mjs';
import { consumeReservation } from './runs-reserve.mjs';
import { ingestRunOrders, availabilityDrift } from './runs-orders.mjs';
import { closeOutBundles, discloseTiers, verifyDisclosure } from './runs-disclose.mjs';
import { publicContents } from './runs-public.mjs';
import { uploadArtifact, verifyReadback, recordArtifact, artifactFilename, ARTIFACT_MIME } from './runs-artifacts.mjs';
import { loadConfig as loadShopifyConfig, pinsFor } from './shopify.mjs';
...
// Section 5.5: shipped is not delivered. Publishing assignments while parcels are in transit
// identifies who is receiving a valuable one, so the close-out waits out a delivery grace period past
// the LAST dispatch. Twenty-one days is the specification's own figure, and section 12 records it as a
// guess that should exceed the slowest realistic delivery.
const GRACE_DAYS = 21;

function graceState(db, run) {
  const last = db.prepare('SELECT MAX(shipped_at) m FROM run_bundles WHERE run_id = ?').get(run.id)?.m;
  const from = last || run.closed_at;
  if (!from) return { elapsed: false, ends_at: null, last_dispatch: null, days: GRACE_DAYS };
  const ends = new Date(new Date(from).getTime() + GRACE_DAYS * 86400_000).toISOString();
  return { elapsed: Date.now() >= Date.parse(ends), ends_at: ends, last_dispatch: last || null, days: GRACE_DAYS };
}

// Every reason this run cannot close, as data. Section 5.6.6: a run cannot close until every bundle
// number appears in the published ledger, which is what makes the chase count arithmetic rather than a
// promise.
function closeCheck(db, run) {
  const reasons = [];
  const full = db.prepare('SELECT * FROM runs WHERE id = ?').get(run.id);
  const a = availability(db, run.id);
  const unsold = a.available;
  if (full.status === 'closed' || full.status === 'disclosed') reasons.push(`the run is already ${full.status}`);
  if (full.status === 'draft' || full.status === 'abandoned') reasons.push(`the run is ${full.status}`);
  // ACCOUNTED FOR, NOT SOLD, AND THAT DISTINCTION IS DELIBERATE.
  //
  // An audit asked whether a run closing with ZERO sales - every number opened live - should be
  // refused. It should not, and the answer is written here so the question is not re-opened as a gap.
  // Unsold-policy v2 exists precisely to break the deadlock where an unsellable bundle would hold a run
  // open for ever: withdrawal and self-purchase are both forbidden, so a number nobody wants is opened
  // on a public stream and RECORDED as opened. Nothing about that reasoning changes if the number of
  // such bundles is all of them. Such a run closes honestly - the commitment held, the chase count is
  // still provable over every bundle, and the contents are disclosed - it just sold nothing, which is a
  // commercial outcome and not a correctness one.
  if (unsold.length) {
    reasons.push(`${unsold.length} number(s) are still unaccounted for: ${unsold.map(pad3).join(', ')}`);
  }
  if (a.paused) reasons.push('an event session is still open; resume before closing');
  if (full.sales_close_at && Date.now() < Date.parse(full.sales_close_at)) {
    reasons.push(`sales do not close until ${full.sales_close_at}`);
  }
  if (full.close_by && Date.now() > Date.parse(full.close_by)) {
    // Recorded rather than fatal: the date has passed and that is a fact about the run, not a reason
    // to refuse the close it was meant to force.
    reasons.push(`NOTE the close-by date ${full.close_by} has passed`);
  }
  const blocking = reasons.filter((r) => !r.startsWith('NOTE'));
  return { can_close: !blocking.length, reasons, unsold, paused: a.paused, grace: graceState(db, full) };
}
...
export function makeRunsRouter({ env, db }) {
  return async (req, res, next) => {
    try {
        ...
        try {
          db.prepare(`INSERT INTO runs (public_id, edition, name, mode, unit_count, unit_price_cents,
            ...
            .run(publicId, Math.round(+b.edition) || 1, String(b.name).trim(), mode, unitCount,
              ...
              // DEFAULTED, not left to the caller. The policy is inside headerDigest and is anchored
              // for the life of the run, so two runs making the same promise in slightly different
              // words would commit two different digests for one policy. A caller may still supply its
              // own text; what it may not do is leave it blank and get a run that cannot lock.
              String(b.unsold_policy || '').trim() || UNSOLD_POLICY, b.notes || null);
      ...
      // TWO SEGMENTS, NOT ONE, AND NOT UNLIMITED.
      //
      // This was `(\/[a-z-]+)?` and `[a-z-]` cannot match a `/`, so every two-segment tail fell straight
      // through to next() and 404'd. `/anchor/upgrade` and `/print/mark` were written, gated, and dead —
      // and because §5.7.5's upgrade is what moves an anchor from `submitted` to `confirmed`, and the
      // sale gate refuses while any header anchor is unconfirmed, the unreachable route meant NO RUN
      // COULD EVER OPEN FOR SALE. Neither route had an integration test, which is why it went unnoticed.
      //
      // Bounded at 2 rather than `*` on purpose: it admits exactly the depth this file uses and refuses a
      // third, so a typo'd path still falls through to next() instead of being swallowed as a run id.
      // A three-segment route added later fails loudly here rather than silently 404ing again.
      const runM = p.match(/^\/([A-Za-z0-9-]+)((?:\/[a-z-]+){0,2})$/);
      ...
      // --- the published artifacts. UNGATED, deliberately, because they contain nothing ---------------
      //
      // These are what a buyer's page fetches. The commitment carries hashes and counts and names no card;
      // the blob file is ciphertext whose keys live only on printed inserts. Gating them would protect
      // nothing and would make verification depend on our being reachable, which §5.3.2 is built to avoid.

      // GET /:id/commitment — §5.2, published once at lock and never republished.
      if (run && tail === '/commitment' && method === 'GET') {
        if (run.status === 'draft') {
          return send(res, 409, { error: `run ${run.public_id} is a draft; nothing is committed yet`, code: 'not_locked' });
        }
        return send(res, 200, commitment(db, run.id, {
          rarityTable: await rarityTable(),
          anchors: anchorsFor(db, run.id, { scope: 'header' }),
        }));
      }

      // GET /:id/blob — the raw blob container. Served with permissive CORS because §5.3.2 requires an
      // INDEPENDENTLY HOSTED verifier to be able to fetch it: a buyer must never have to trust our page.
      if (run && tail === '/blob' && method === 'GET') {
        const row = db.prepare('SELECT bytes, sha256 FROM run_blobs WHERE run_id = ?').get(run.id);
        // 409, not 404, and deliberately the same shape as /commitment's refusal above. A 404 here
        // would be indistinguishable from "no such route" — which is the precise confusion that let
        // two genuinely unreachable routes sit unnoticed.
        if (!row) {
          return send(res, 409, { error: `run ${run.public_id} is not locked; it has no blob file`, code: 'not_locked' });
        }
        res.statusCode = 200;
        res.setHeader('content-type', 'application/octet-stream');
        res.setHeader('access-control-allow-origin', '*');
        res.setHeader('x-bkr1-blob-sha256', row.sha256);
        return res.end(Buffer.from(row.bytes));
      }

      // GET /:id/anchors — pending or confirmed, said honestly, plus the sale gate's own reasons.
      if (run && tail === '/anchors' && method === 'GET') {
        return send(res, 200, { anchors: anchorsFor(db, run.id), gate: saleGate(db, run.id) });
      }

      // POST /:id/anchor — submit the header digest for timestamping.
      if (run && tail === '/anchor' && method === 'POST') {
        const b = await readJson(req);
        const cfg = loadRunsConfig();
        const mode = b.mode || cfg?.anchor?.mode || 'opentimestamps';
        if (!run.header_digest) {
          return send(res, 409, { error: `run ${run.public_id} is not locked`, code: 'not_locked' });
        }
        return send(res, 201, publicAnchor(await submitAnchor(db, {
          runId: run.id, digest: run.header_digest, scope: 'header', mode, actor: b.actor || null,
        })));
      }

      // POST /:id/anchor/upgrade — §5.7.5. Until this succeeds the anchor is pending, not confirmed.
      if (run && tail === '/anchor/upgrade' && method === 'POST') {
        const rows = db.prepare(`SELECT id FROM run_anchors WHERE run_id = ? AND scope = 'header'`).all(run.id);
        const out = [];
        for (const r of rows) out.push(publicAnchor(await upgradeAnchor(db, r.id)));
        return send(res, 200, { anchors: out, gate: saleGate(db, run.id) });
      }

      // POST /:id/lock — §10.4 phase 1. GATED, and gated for a reason the other routes are not: it mints
      // the verification codes, which are BEARER SECRETS. Whoever reads one can decrypt that bundle's
      // record and learn its contents before the run closes.
      //
      // The codes are therefore NOT in the response by default. They reach paper through the insert print
      // (R2-6) and nowhere else. `?codes=1` exists for the operator who is about to print, and is the one
      // place in the app that will ever serve them.
      if (run && tail === '/lock' && method === 'POST') {
        const auth = diagTokenCheck(env, bearer(req, url));
        if (!auth.ok) return send(res, auth.code, { error: auth.error, code: 'manifest_gated' });
        const b = await readJson(req);
        let locked;
        try {
          locked = await lockRunPhase1(db, run.id, { actor: b.actor || null });
        } catch (e) {
          // A refusal is not an error condition, it is the answer: the operator gets every problem at
          // once, each with the bundle and cert that caused it, rather than one per attempt.
          if (e.code === 'lock_refused') return send(res, 409, { error: e.message, code: e.code, problems: e.problems });
          if (e.code === 'manifest_changed') return send(res, 409, { error: e.message, code: e.code });
          throw e;
        }
        const wantCodes = url.searchParams.get('codes') === '1';
        return send(res, 200, {
          run: runByIdOrPublicId(db, run.id),
          header_digest: locked.headerDigest,
          run_root: locked.runRoot,
          codes_commit: locked.codesCommit,
          blob_hash: locked.blobHash,
          blob_length: locked.blobLength,
          guarantee: locked.guaranteeText,
          leaves: locked.leaves,
          code_leaves: locked.codeLeaves,
          bundles: locked.bundles.map((x) => ({
            bundle_no: x.bundle_no, label: x.label, is_chase: x.is_chase, leaf: x.leaf,
            ...(wantCodes ? { code: x.code } : {}),
          })),
        });
      }

      // GET /:id/print — everything the packing bench needs, in one gated read.
      //
      // GATED, and it is the most sensitive read in the app: it returns every verification code AND the
      // full contents of every bundle. That is the pre-sale answer sheet plus the bearer secrets, which is
      // why it exists only as a print feed and is never rendered on a screen someone might walk past.
      //
      // THE ORDER MATTERS AND IT IS NOT THE OBVIOUS ONE. The insert carrying the code goes INSIDE the
      // parcel, so the code must exist before sealing; codes are minted in lock phase 1, because
      // codesCommit is a Merkle root over them and sits inside headerDigest. So nothing here can be
      // printed before the run locks — which is exactly what this route refuses.
      if (run && tail === '/print' && method === 'GET') {
        const auth = diagTokenCheck(env, bearer(req, url));
        if (!auth.ok) return send(res, auth.code, { error: auth.error, code: 'manifest_gated' });
        if (run.status === 'draft') {
          return send(res, 409, {
            error: `run ${run.public_id} is a draft; verification codes are minted at lock, so there is nothing to print yet`,
            code: 'not_locked',
          });
        }
        const bundles = db.prepare(`SELECT id, bundle_no, label, verify_code, seal_serial, is_chase
                                      FROM run_bundles WHERE run_id = ? ORDER BY bundle_no`).all(run.id);
        const slots = db.prepare(`SELECT s.bundle_id, s.slot, s.seq, s.display_name, s.set_code, s.set_name,
                                         s.card_number, s.rarity, s.language, s.grading_company, s.grade,
                                         s.cert_number, s.upc, s.product_type, s.qty
                                    FROM run_bundle_slots s
                                    JOIN run_bundles b ON b.id = s.bundle_id
                                   WHERE b.run_id = ? ORDER BY b.bundle_no, s.slot, s.seq`).all(run.id);
        return send(res, 200, {
          run: {
            public_id: run.public_id, name: run.name, unit_count: run.unit_count,
            header_digest: run.header_digest, guarantee: run.guarantee_text,
          },
          specs: specsFor(db, run.id).map((s) => ({ slot: s.slot, label: s.label, qty_per_bundle: s.qty_per_bundle })),
          bundles: bundles.map((b) => ({
            bundle_no: b.bundle_no, label: b.label, code: b.verify_code, seal_serial: b.seal_serial,
            is_chase: b.is_chase,
            lines: slots.filter((s) => s.bundle_id === b.id).map(({ bundle_id, ...rest }) => rest),
          })),
        });
      }

      // POST /:id/print/mark { bundle_no } — record that an insert was printed.
      if (run && tail === '/print/mark' && method === 'POST') {
        const auth = diagTokenCheck(env, bearer(req, url));
        if (!auth.ok) return send(res, auth.code, { error: auth.error, code: 'manifest_gated' });
        const b = await readJson(req);
        const bundle = db.prepare('SELECT id, label, packed_at FROM run_bundles WHERE run_id = ? AND bundle_no = ?')
          .get(run.id, Math.round(+b.bundle_no));
        if (!bundle) return send(res, 404, { error: `run ${run.public_id} has no bundle ${b.bundle_no}` });
        // The schema CHECK refuses insert_printed_at without packed_at, so this stamps both: printing the
        // insert IS the last step before the parcel is closed, and a bundle cannot be packed without one.
        const stamp = new Date().toISOString();
        db.prepare(`UPDATE run_bundles SET packed_at = COALESCE(packed_at, ?), insert_printed_at = ?,
                      updated_at = ? WHERE id = ?`).run(stamp, stamp, stamp, bundle.id);
        audit(db, { runId: run.id, bundleId: bundle.id, entity: 'run_bundles', entityId: bundle.id,
          action: 'insert_printed', actor: b.actor || null });
        return send(res, 200, { bundle: bundle.label, insert_printed_at: stamp });
      }

      // POST /:id/publish - §10.4 phase 2. Uploads the blob file and the commitment to Shopify Files,
      // reads both back from the CDN, submits the header digest for timestamping, and only then moves
      // the run to locked_published.
      //
      // IDEMPOTENT BY CONSTRUCTION, and that is why phase 1 built the blob rather than this route:
      // revision 3 encrypted here, so a retry minted fresh nonces and produced ciphertext that no
      // longer hashed to the blob_hash already inside the anchored digest. A retry can only republish
      // the SAME bytes if the same bytes still exist, so phase 1 persists them and this reads them.
      if (run && tail === '/publish' && method === 'POST') {
        const auth = diagTokenCheck(env, bearer(req, url));
        if (!auth.ok) return send(res, auth.code, { error: auth.error, code: 'manifest_gated' });
        const b = await readJson(req);
        const cfg = loadRunsConfig();

        // The positive arming guard. The store comes from CONFIG, never from the request, and for a
        // live target the shop itself is asked what it is before a byte is written.
        let shopCfg = null;
        try { shopCfg = loadShopifyConfig(); } catch { shopCfg = null; }
        const arm = armingState(db, { runsCfg: cfg, shopifyCfg: shopCfg, run, store: b.store || null });
        if (!arm.armed) {
          return send(res, 409, {
            error: `run ${run.public_id} is not armed to publish to "${arm.target}"`,
            code: 'not_armed', target: arm.target, reasons: arm.reasons,
          });
        }

        try {
          const out = await lockRunPhase2(db, run.id, {
            publisher: makeArtifactPublisher({ env, store: arm.target }),
            anchorer: makeAnchorer({ mode: cfg.anchor.mode, actor: b.actor || null }),
            actor: b.actor || null,
          });
          // The receipt is published separately and is the ONE artifact §5.7.6 allows to be revised,
          // which is why the commitment above froze with an empty anchors array.
          let receipt = null;
          if (!out.alreadyDone) {
            try { receipt = await publishAnchorReceipt(env, db, { runId: run.id, store: arm.target, actor: b.actor || null }); }
          ...
          return send(res, 200, {
            ...
            ...out,
            receipt,
            artifacts: artifactsFor(db, run.id).map((x) => ({
              kind: x.kind, seq: x.seq, sha256: x.sha256, bytes: x.byte_length, url: x.url,
              superseded_at: x.superseded_at,
            })),
            gate: saleGate(db, run.id),
          });
        } catch (e) {
          const msg = String(e?.message || e);
          return send(res, /phase 2 follows|no stored blob|not locked|did not read back|hashes to/.test(msg) ? 409 : 502,
            { error: msg, code: 'publish_failed' });
        }
      }

      // POST /:id/publish-product - list the run on the storefront: ONE product, one variant per
      // bundle number, one unit each.
      //
      // §5.6.3: the buyer picks their own number, and the storefront's own inventory is what stops a
      // number selling twice and shows the true remaining set. That is why this is variants rather than
      // one product per bundle.
      //
      // A CONFIRMED ANCHOR IS REQUIRED, not merely a submitted one. §5.7.7: sales open when the
      // commitment provably predates them, and a pending calendar attestation proves nothing to an
      // outsider. The gate is asked here rather than restated.
      if (run && tail === '/publish-product' && method === 'POST') {
        const auth = diagTokenCheck(env, bearer(req, url));
        if (!auth.ok) return send(res, auth.code, { error: auth.error, code: 'manifest_gated' });
        const b = await readJson(req);
        const cfg = loadRunsConfig();
        ...
        try { shopCfg = loadShopifyConfig(); } catch { shopCfg = null; }
        const arm = armingState(db, { runsCfg: cfg, shopifyCfg: shopCfg, run, store: b.store || null });
        // A dry run builds the input and calls nothing, so it is exempt from arming the same way the
        // singles preview is - but never from the gate below, which is about the commitment, not us.
        if (!arm.armed && !dryRun) {
          return send(res, 409, { error: `run ${run.public_id} is not armed to publish to "${arm.target}"`,
            code: 'not_armed', target: arm.target, reasons: arm.reasons });
        }
        ...
        if (!gate.open && !dryRun) {
          return send(res, 409, {
            error: `run ${run.public_id} may not go on sale yet`, code: 'sale_gate', reasons: gate.reasons,
          });
        }

        const bundles = db.prepare(`SELECT id, bundle_no, label, status FROM run_bundles
                                      WHERE run_id = ? ORDER BY bundle_no`).all(run.id);
        const full = db.prepare('SELECT * FROM runs WHERE id = ?').get(run.id);
        let pins = { locationGid: null, publicationGid: null };
        try { pins = pinsFor(shopCfg || loadShopifyConfig(), arm.target); } catch { /* surfaced as a validation error below */ }

        try {
          const out = await publishRunProduct(env, db, {
            run: full, bundles, store: arm.target, dryRun, actor: b.actor || null,
            locationGid: pins.locationGid, publicationGid: pins.publicationGid,
            status: b.status || 'ACTIVE', descriptionHtml: b.description_html || '',
          });
          if (!out.ok) {
            return send(res, 409, { error: out.error || 'the product could not be published',
              code: 'product_refused', errors: out.errors || [], steps: out.steps });
          }
          return send(res, 200, { ...out, availability: mirrorAvailability(db, run.id, arm.target) });
        } catch (e) {
          return send(res, 502, { error: String(e?.message || e), code: 'product_failed' });
        }
      }

      // GET /:id/storefront - the mirror: what Shopify last told us is available, per number.
      if (run && tail === '/storefront' && method === 'GET') {
        // WHICH storefront: the mirror holds a row per store, so this names the configured publish
        // target rather than folding two stores' answers into one list.
        const sfStore = loadRunsConfig().publish.store;
        return send(res, 200, {
          store: sfStore,
          handle: runProductHandle(run),
          listings: runListings(db, run.id, sfStore).map((x) => ({
            sku: x.sku, variant_gid: x.variant_gid, state: x.state, available_qty: x.available_qty,
          })),
          availability: mirrorAvailability(db, run.id, sfStore),
        });
      }

      // GET /:id/artifacts - what has been published, and where. Open: it is a list of public URLs.
      if (run && tail === '/artifacts' && method === 'GET') {
        return send(res, 200, {
          artifacts: artifactsFor(db, run.id).map((x) => ({
            kind: x.kind, seq: x.seq, sha256: x.sha256, bytes: x.byte_length, mime_type: x.mime_type,
            url: x.url, published_at: x.published_at, readback_at: x.readback_at, superseded_at: x.superseded_at,
          })),
        });
      }

      // POST /:id/poll - read storefront orders now, without waiting for the interval. GATED: it writes
      // ledger entries, which move real numbers out of the available set.
      if (run && tail === '/poll' && method === 'POST') {
        const auth = diagTokenCheck(env, bearer(req, url));
        if (!auth.ok) return send(res, auth.code, { error: auth.error, code: 'manifest_gated' });
        const cfg = loadRunsConfig();
        try {
          const out = await ingestRunOrders(env, db, { runId: run.id, store: cfg.publish.store, actor: 'manual' });
          return send(res, 200, { ...out, available: availability(db, run.id).available, drift: availabilityDrift(db, run.id, cfg.publish.store) });
        } catch (e) { return send(res, 502, { error: String(e?.message || e) }); }
      }

      // GET /:id/ledger - the published chain, plus what it says is available.
      //
      // OPEN, and that is the point. Section 5.6.3: publishing the chain is what makes availability
      // checkable rather than asserted. A buyer told "7 is gone" can see that 7 was sold to an earlier
      // ordinal, and one shown a restricted set can compare it against what remained.
      if (run && tail === '/ledger' && method === 'GET') {
        const pub = publicLedger(db, run.id);
        const chain = await verifyLedgerChain(run.public_id, pub.entries);
        return send(res, 200, { ...pub, verified: chain.ok, errors: chain.errors });
      }

      // GET /:id/available - the derived set, for a storefront section or an operator at a table.
      if (run && tail === '/available' && method === 'GET') {
        const a = availability(db, run.id);
        return send(res, 200, {
          available: a.available, paused: a.paused, present: a.present,
          sold: [...a.sold.keys()].sort((x, y) => x - y),
        });
      }

      // POST /:id/ledger - append an entry by hand: an in-person sale, a cancellation, a reprice, or
      // an event pause/resume. GATED, because it moves stock and money in the real world.
      if (run && tail === '/ledger' && method === 'POST') {
        const auth = diagTokenCheck(env, bearer(req, url));
        if (!auth.ok) return send(res, auth.code, { error: auth.error, code: 'manifest_gated' });
        const b = await readJson(req);
        const actor = b.actor || null;
        try {
          let entry = null;
          switch (b.kind) {
            case 'sale_in_person':
              entry = await appendSale(db, run.id, {
                bundleNo: b.bundle_no, channel: 'sale_in_person', detail: b.detail || '', actor,
                order: b.order || null,
              });
              break;
            case 'opened_live':
              entry = await appendOpenedLive(db, run.id, { bundleNo: b.bundle_no, detail: b.detail || '', actor });
              break;
            case 'cancel':
              entry = await appendCancel(db, run.id, { token: b.token, reason: b.detail || '', actor });
              break;
            case 'reprice':
              entry = await appendReprice(db, run.id, { note: b.detail || 'run-wide price change', actor });
              break;
            case 'pause':
              entry = await appendPause(db, run.id, { eventId: b.event_id, actor });
              break;
            case 'resume':
              entry = await appendResume(db, run.id, { eventId: b.event_id, actor });
              break;
            default:
              return send(res, 400, { error: `unknown ledger kind "${b.kind}"` });
          }

          // THE LEDGER IS WRITTEN FIRST, THEN THE STOREFRONT. Section 5.6.3 makes the ledger the
          // availability proof and the storefront a mirror of it. Reversed, a network failure after
          // the storefront write would leave a number nobody can buy and no record of why; in this
          // order it leaves one sold in the ledger and still purchasable online, which the next poll
          // finds and refuses by name. Loud beats silent, so a mirror failure is REPORTED, never fatal.
          let mirror = null;
          const affects = ['sale_in_person', 'opened_live', 'cancel'].includes(b.kind);
          if (affects && b.mirror !== false) {
            const cfg = loadRunsConfig();
            try {
              mirror = await setBundleAvailable(env, db, {
                run, bundleNo: entry.bundleNo ? Number(entry.bundleNo) : Number(b.bundle_no),
                quantity: b.kind === 'cancel' ? 1 : 0, store: cfg.publish.store, actor,
              });
            } catch (e) { mirror = { ok: false, error: String(e?.message || e) }; }
          }
          return send(res, 201, { entry, mirror, available: availability(db, run.id).available });
        } catch (e) {
          const msg = String(e?.message || e);
          const code = e.code || null;
          return send(res, /already_sold|sale_gate|ledger_moved/.test(String(code)) || /already|must name|has shipped|not in this run/.test(msg) ? 409 : 500,
            { error: msg, code });
        }
      }

      // POST /:id/pack { bundle_no } - consume the reservations for one bundle at pack time.
      //
      // THIS IS WHERE A RUN'S STOCK IS ACTUALLY DRAWN DOWN, and until now consumeReservation had no
      // caller at all - a live run's stock would never have moved. It is also why run SKUs are excluded
      // from applyStockDecrements: the units come off here, once, and an order line must never take a
      // second bite.
      //
      // A dev run is a logged no-op inside consumeReservation, so a rehearsal never touches real stock.
      if (run && tail === '/pack' && method === 'POST') {
        const auth = diagTokenCheck(env, bearer(req, url));
        if (!auth.ok) return send(res, auth.code, { error: auth.error, code: 'manifest_gated' });
        const b = await readJson(req);
        const bundle = db.prepare('SELECT id, label, packed_at FROM run_bundles WHERE run_id = ? AND bundle_no = ?')
        ...
        const rows = db.prepare(`SELECT reservation_id FROM run_bundle_slots WHERE bundle_id = ?`).all(bundle.id);
        if (!rows.length) return send(res, 409, { error: `bundle ${bundle.label} has no frozen slots; lock the run first`, code: 'not_locked' });
        const out = [];
        try {
          for (const r of rows) out.push(await consumeReservation(db, r.reservation_id, { actor: b.actor || null }));
        } catch (e) {
          return send(res, 409, { error: String(e?.message || e), consumed: out });
        }
        const stamp = new Date().toISOString();
        db.prepare("UPDATE run_bundles SET packed_at = COALESCE(packed_at, ?), status = 'packed', updated_at = ? WHERE id = ?")
          .run(stamp, stamp, bundle.id);
        audit(db, { runId: run.id, bundleId: bundle.id, entity: 'run_bundles', entityId: bundle.id,
          action: 'packed', actor: b.actor || null, after: { consumed: out.length } });
        return send(res, 200, { bundle: bundle.label, packed_at: stamp, consumed: out });
      }

      // POST /:id/dispatch { bundle_no } - the parcel has left.
      //
      // WHY THIS EXISTS AT ALL. run_bundles.shipped_at had three readers and no writer: graceState takes
      // MAX(shipped_at) to start the delivery grace clock, the bundle SELECT surfaces it, and
      // appendCancel refuses to cancel a sale after dispatch. With nothing ever writing it, graceState
      // silently fell back to the run's closed_at - so the 21-day wait before disclosure started at the
      // wrong moment, and a sale could be cancelled after the parcel was gone.
      //
      // Deliberately AFTER packing. Packing is what draws the stock down and freezes the parcel's
      // contents; dispatch is a claim about the physical world made after that, and a dispatch of a
      // bundle nobody packed is a mistake worth refusing rather than recording.
      if (run && tail === '/dispatch' && method === 'POST') {
        const auth = diagTokenCheck(env, bearer(req, url));
        if (!auth.ok) return send(res, auth.code, { error: auth.error, code: 'manifest_gated' });
        const b = await readJson(req);
        const bundle = db.prepare('SELECT id, label, packed_at, shipped_at FROM run_bundles WHERE run_id = ? AND bundle_no = ?')
          .get(run.id, Math.round(+b.bundle_no));
        if (!bundle) return send(res, 404, { error: `run ${run.public_id} has no bundle ${b.bundle_no}` });
        if (!bundle.packed_at) {
          return send(res, 409, { error: `bundle ${bundle.label} has not been packed yet`, code: 'not_packed' });
        }
        // Idempotent by COALESCE rather than by refusing: a second click on a dispatch button is a
        // slip, not an error, and the first timestamp is the true one.
        const stamp = new Date().toISOString();
        db.prepare("UPDATE run_bundles SET shipped_at = COALESCE(shipped_at, ?), status = 'shipped', updated_at = ? WHERE id = ?")
          .run(stamp, stamp, bundle.id);
        const after = db.prepare('SELECT shipped_at FROM run_bundles WHERE id = ?').get(bundle.id);
        audit(db, { runId: run.id, bundleId: bundle.id, entity: 'run_bundles', entityId: bundle.id,
          action: 'dispatched', actor: b.actor || null, after: { shipped_at: after.shipped_at } });
        return send(res, 200, { bundle: bundle.label, shipped_at: after.shipped_at, already: bundle.shipped_at ? true : false });
      }

      // GET /:id/close-check - why this run can or cannot close, as a list.
      if (run && tail === '/close-check' && method === 'GET') {
        return send(res, 200, closeCheck(db, run));
      }

      // POST /:id/close - every number accounted for, sales closed, and the close-by date not passed.
      //
      // Section 5.6.6: a run cannot close until every bundle number appears in the published ledger.
      // That is what makes the chase count arithmetic rather than a promise - and it is why unsold-
      // policy v2 added the opened_live disposition, without which one unsellable bundle would have
      // deadlocked a run permanently.
      if (run && tail === '/close' && method === 'POST') {
        const auth = diagTokenCheck(env, bearer(req, url));
        if (!auth.ok) return send(res, auth.code, { error: auth.error, code: 'manifest_gated' });
        const b = await readJson(req);
        const check = closeCheck(db, run);
        if (!check.can_close) {
          return send(res, 409, { error: `run ${run.public_id} cannot close yet`, code: 'close_blocked', ...check });
        }
        const stamp = new Date().toISOString();
        db.prepare("UPDATE runs SET status = 'closed', closed_at = ?, updated_at = ? WHERE id = ?")
          .run(stamp, stamp, run.id);
        audit(db, { runId: run.id, entity: 'runs', entityId: run.id, action: 'closed', actor: b.actor || null,
          after: { closed_at: stamp } });
        return send(res, 200, { run: runByIdOrPublicId(db, run.id), closed_at: stamp, ...closeCheck(db, run) });
      }

      // POST /:id/disclose { tiers } - build, SELF-VERIFY, publish, then transition.
      //
      // The self-verification is the load-bearing step, not a formality. Section 5.5.2 exists because a
      // producer can open a committed grade 9 honestly, with a correct salt and correct proofs, against
      // a published claim of gte 10 and pass every structural check. Running verifyDisclosure on our
      // OWN artifact before uploading means the only way to publish a false disclosure is to publish
      // one we have already been told is false.
      if (run && tail === '/disclose' && method === 'POST') {
        const auth = diagTokenCheck(env, bearer(req, url));
        if (!auth.ok) return send(res, auth.code, { error: auth.error, code: 'manifest_gated' });
        const b = await readJson(req);
        const full = db.prepare('SELECT * FROM runs WHERE id = ?').get(run.id);
        if (!full.closed_at) return send(res, 409, { error: `run ${run.public_id} has not closed`, code: 'not_closed' });

        const grace = graceState(db, full);
        if (!grace.elapsed && b.force !== true) {
          // Section 5.5: shipped is not delivered, and publishing assignments while parcels are in
          // transit identifies who is receiving a valuable one.
          return send(res, 409, {
            error: `the delivery grace period ends ${grace.ends_at}`, code: 'grace_period', ...grace,
          });
        }

        const cfg = loadRunsConfig();
        const tiers = Array.isArray(b.tiers) && b.tiers.length ? b.tiers : ['A', 'B', 'C'];
        try {
          const specs = specsFor(db, run.id);
          const claims = publishedClaims(db, run.id);
          const artifact = await discloseTiers({
            run: full, specs, claims, ladder: ladderRows(db, run.id),
            bundles: closeOutBundles(db, run.id), tiers,
          });
          const table = await rarityTable();
          const commit = commitment(db, run.id, { rarityTable: table, anchors: [] });
          const check = await verifyDisclosure(artifact, commit, { rarityTable: table });
          if (!check.ok) {
            return send(res, 409, {
              error: 'the disclosure does not verify against its own commitment; nothing was published',
              code: 'disclosure_invalid', errors: check.errors,
            });
          }
          if (b.dry_run === true) return send(res, 200, { dry_run: true, tiers, verified: true, openings: artifact.openings.length });

          const bytes = new TextEncoder().encode(JSON.stringify(artifact, null, 2));
          const up = await uploadArtifact(env, db, {
            bytes, filename: artifactFilename(full, 'disclosure'), mimeType: ARTIFACT_MIME.disclosure,
            store: cfg.publish.store,
          });
          if (!up.ok) return send(res, 502, { error: `publishing the disclosure: ${up.error}`, code: 'publish_failed' });
          const back = await verifyReadback(up.url, { expectSha256: up.sha256 });
          if (!back.ok) return send(res, 502, { error: `the published disclosure did not read back: ${back.error}` });

          recordArtifact(db, {
            runId: run.id, kind: 'disclosure', sha256: up.sha256, byteLength: bytes.length,
            mimeType: ARTIFACT_MIME.disclosure, body: bytes, fileGid: up.fileGid, url: up.url,
            store: cfg.publish.store,
          });
          const stamp = new Date().toISOString();
          db.prepare("UPDATE runs SET status = 'disclosed', disclosed_at = ?, updated_at = ? WHERE id = ?")
            .run(stamp, stamp, run.id);
          audit(db, { runId: run.id, entity: 'runs', entityId: run.id, action: 'disclosed',
            actor: b.actor || null, after: { tiers, url: up.url, openings: artifact.openings.length } });
          return send(res, 200, { tiers, url: up.url, openings: artifact.openings.length, verified: true });
        } catch (e) {
          return send(res, 500, { error: String(e?.message || e), code: 'disclose_failed' });
        }
      }

      // GET /:id/disclosure - the published artifact. Open: it IS the public record.
      if (run && tail === '/disclosure' && method === 'GET') {
        const row = db.prepare("SELECT * FROM run_artifacts WHERE run_id = ? AND kind = 'disclosure' ORDER BY seq DESC").get(run.id);
        if (!row) return send(res, 409, { error: `run ${run.public_id} has not disclosed`, code: 'not_disclosed' });
        res.statusCode = 200;
        res.setHeader('content-type', 'application/json');
        res.setHeader('access-control-allow-origin', '*');
        return res.end(Buffer.from(row.body));
      }

      // GET /:id/contents - what was in every bundle. Refuses before close, in the module itself.
      if (run && tail === '/contents' && method === 'GET') {
        try {
          return send(res, 200, { contents: publicContents(db, run.id) });
        } catch (e) {
          return send(res, e.code === 'not_closed' ? 409 : 500, { error: String(e?.message || e), code: e.code || null });
        }
      }
...
export function runsPlugin(env) {
  return {
    ...
    configureServer(server) {
      ...
      // The anchor upgrade sweep. Without it nothing ever moves from `submitted` to `confirmed`, and
      // §5.7.7's sale gate refuses while any header anchor is unconfirmed — so no run could open.
      startRunsJobs(env, db);
    },
  };
}
