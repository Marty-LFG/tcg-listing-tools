...
// The mat between the card and everything around it, as a fraction of the frame's SHORT edge —
// 0.032 of 1512 is 48px, exactly the mat the eBay square puts between card and rail. Without it the
// card's edge sits hard against the band's hairline, which reads as a printing error rather than a
// frame. A code constant rather than a setting: it is the band's counterpart to cardPaddingX, and
// nobody needs to tune it independently of bandFraction.
export const CARD_MAT_FRACTION = 0.032;
...
export function resolveBandGeometry(frame, fraction = DEFAULT_BAND_FRACTION) {
  ...
  const mat = Math.round(Math.min(frame.width, frame.height) * CARD_MAT_FRACTION);
  const cardBox = { width: frame.width - 2 * mat, height: frame.height - 2 * bandH - 2 * mat };
  if (cardBox.height < 100 || cardBox.width < 100) {
    throw new Error(
      `bands leave no room for the card: ${frame.height} - 2x${bandH} - 2x${mat} = ${cardBox.height} `
      + `(shopify.bandFraction ${fraction} is too thick for a ${frame.width}x${frame.height} frame)`);
  }
  return { bandH, mat, cardBox };
}
