...
//
// Lifted VERBATIM from the old landing-page calculator so the forward (feeAU) and
// inverse (listForTarget) stay in sync in a single place (AGENTS.md §7). The dashboard
// dropped that calculator; this file is now the only home for the maths.
// Consumers: bulk-listing-builder.html + stock-runner.html (<script type="module">),
// lib/listings.mjs, lib/pricing.mjs (target-net back-out + grid fee footer).
// `pcSolve` has no UI caller left and is held by scripts/check-pricing.mjs +
// test/unit/fees.test.mjs. Dual-target: no DOM, no fetch, no deps.
//
