...
const BAKED_PRICES = path.join(ROOT, 'data', 'riftbound-prices.json');
// RIFTBOUND_PRICES_PATH — the fixture hook, twin of RIFTBOUND_DATA_PATH in lib/riftbound-data.mjs,
// and read per call for the same reason (an import is hoisted above anything a test could set).
export const pricesPathDefault = () => (process.env.RIFTBOUND_PRICES_PATH
  ? path.resolve(process.env.RIFTBOUND_PRICES_PATH)
  : BAKED_PRICES);
export const PRICES_PATH = BAKED_PRICES;
...
export function loadRiftboundPrices(dataPath = pricesPathDefault()) {
  try {
