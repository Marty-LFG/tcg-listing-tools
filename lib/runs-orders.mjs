 ...
 */
export function availabilityDrift(db, runId, store = 'dev') {
  const a = availability(db, runId);
  // Scoped, because "what the storefront says" is a different answer per store and comparing the ledger
  // against the wrong one manufactures drift that is not there — the loudest possible false alarm, on
  // the one check whose whole job is to be trusted when it fires.
  const mirror = db.prepare(`SELECT sl.sku, sl.available_qty FROM shopify_listings sl
                               JOIN run_bundles b ON b.id = sl.item_id
                              WHERE sl.kind = 'run' AND sl.store = ? AND b.run_id = ?`).all(store, +runId);
  const drift = [];
