...
// Returns ONE LINE PER ELEMENT, not a joined string. Two short lines beat one long one down a
// 300px rail: the type is sized to the longest line, so "JAPANESE · ABYSS EYE" as a single run had
// to shrink to fit where "JAPANESE" over "ABYSS EYE" does not. Rotated onto the rail they read as
// two stacked columns.
export function railText(meta = {}, layout = DEFAULT_LAYOUT) {
  ...
  if (lang && !isEnglish(lang) && isRailDrawable(lang)) lines.push(lang.toUpperCase());
  if (set && isRailDrawable(set)) lines.push(set.toUpperCase());
  // Truncate per line. The language is one short token, so only a set name ever hits this — and
  // clipping the language would turn "JAPANESE" into "JAPANES", which reads as a typo rather than
  // an abbreviation.
  const max = layout.text.maxChars;
  return lines.map((s) => (s.length > max ? s.slice(0, max - 1).trimEnd() + '…' : s));
}
