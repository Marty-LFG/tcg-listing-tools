...
import { DEFAULT_BAND_FRACTION } from './listing-image-targets.mjs';
...
export const DEFAULT_CONFIG = Object.freeze({
  ...
  // The Shopify frames (lib/listing-image-bands.mjs). A SECOND master switch, off by default, so
  // turning the compositor on for eBay does not silently start emitting a second channel's images.
  // The frame SIZES are not here — they are the storefront's grid contract and live in
  // lib/listing-image-targets.mjs beside ASSET_VERSION. These are the numbers with a taste
  // component.
  shopify: Object.freeze({
    enabled: false,
    quality: 88,
    // Band thickness as a fraction of the frame's long edge. 0.093 of 2112 is 196px, which leaves
    // a 1512x1720 card box — a 63:88 card lands at 1230x1720 with plum on all four sides.
    bandFraction: DEFAULT_BAND_FRACTION,
    // Advisory only. A portrait source this far off 63:88 is a bad trim or a photo of something
    // that is not a card. Nothing is cropped either way, so this changes no pixels — it just says
    // so. Landscape sources are exempt; they are legitimately sideways cards.
    aspectWarnPct: 8,
    og: Object.freeze({ railWidth: 300, cardPaddingY: 38, cardPaddingX: 36 }),
  }),
});

export const SHOPIFY_OVERRIDE_KEYS = Object.freeze(['enabled', 'quality', 'bandFraction', 'aspectWarnPct']);
export const SHOPIFY_OG_OVERRIDE_KEYS = Object.freeze(['railWidth', 'cardPaddingY', 'cardPaddingX']);
...
export function loadConfig() {
  try {
    ...
    return {
      ...
      // `og` gets its OWN spread. Without it, a config file written before `og` existed — or one
      // carrying only `og.railWidth` — would replace the whole block and leave the paddings
      // undefined, which is a hole rather than a default.
      shopify: {
        ...DEFAULT_CONFIG.shopify, ...(raw.shopify || {}),
        og: { ...DEFAULT_CONFIG.shopify.og, ...((raw.shopify && raw.shopify.og) || {}) },
      },
    };
...
function freshDefaults() {
  return {
    ...DEFAULT_CONFIG,
    applyTo: { ...DEFAULT_CONFIG.applyTo },
    font: { ...DEFAULT_CONFIG.font },
    layoutOverrides: {}, textOverrides: {}, badgeOverrides: {}, variantOverrides: {},
    shopify: { ...DEFAULT_CONFIG.shopify, og: { ...DEFAULT_CONFIG.shopify.og } },
  };
}
...
// The owner-editable cfg.shopify values that CHANGE PIXELS and were never in the key.
//
// composeHash's contract above says it hashes every INPUT. These broke it: `bandFraction` reaches the
// band geometry (listing-image-bands.mjs:537), `quality` reaches both jpeg encoders (:496, :624), and
// `shopify.og.*` reaches resolveOgGeometry — none of them through `layout`, which was the only
// geometry composeHash ever saw. Measured: bandFraction 0.093 vs 0.12, and quality 40, produced
// 176,285 / 180,556 / 57,488 bytes under ONE contentHash.
//
// It was invisible while storePut overwrote its destination unconditionally — a config change
// re-rendered, the new bytes replaced the old, and the stale key never showed. Now that storePut
// RETURNS an already-stored file instead of overwriting it, a hash ignoring these would pin the FIRST
// rendering permanently: tuning bandFraction would do nothing to any card already composed, and the
// store would end up half-framed one way and half the other — the exact outcome the fixed-rail
// decision at the top of this file exists to prevent.
//
// APPENDED ONLY WHERE A VALUE DIFFERS FROM ITS DEFAULT, the same trick as `flags` and `target`. At
// stock settings it returns '' and every hash in existence is byte-for-byte unchanged: no orphaned
// store entry, no forced re-upload, and listing-image-hash-pin.test.mjs keeps its literal pins.
export function shopifyPixelKey(cfg, kind) {
  const s = (cfg && cfg.shopify) || {};
  const d = DEFAULT_CONFIG.shopify;
  const out = [];
  if (s.quality != null && s.quality !== d.quality) out.push(`q=${s.quality}`);
  if (kind === 'og') {
    const og = s.og || {};
    for (const k of SHOPIFY_OG_OVERRIDE_KEYS) {
      if (og[k] != null && og[k] !== d.og[k]) out.push(`og.${k}=${og[k]}`);
    }
  } else if (s.bandFraction != null && s.bandFraction !== d.bandFraction) {
    out.push(`bf=${s.bandFraction}`);
  }
  return out.join(',');
}

export function composeHash({ sourceBytes, layout, variant, textLines = [], assetDigest = '', badge = '', flags = '', target = '', shopify = '' }) {
  const h = crypto.createHash('sha256')
    ...
    .update(SEP + badge);
  // Pixel-changing switches that are not layout (today: 'notrim' on catalog art). APPENDED only
  // when set, same trick as the badge's abbrev segment, so every composite keyed before a flag
  // existed keeps its exact hash.
  if (flags) h.update(SEP + 'flags:' + flags);
  // The OUTPUT FRAME (lib/listing-image-targets.mjs). Same append-only trick, and here it is the
  // whole guarantee that this change costs nothing: targetFingerprint() returns '' for the eBay
  // square, so the segment is never written for it and every image already hosted on a live listing
  // keeps its exact key. test/unit/listing-image-hash-pin.test.mjs pins that directly.
  if (target) h.update(SEP + 'target:' + target);
  // Owner-editable pixel settings that never reached `layout` — see shopifyPixelKey. Append-only and
  // empty at stock settings, so this segment is written for a tuned store and for nothing else.
  if (shopify) h.update(SEP + 'shopify:' + shopify);
  return h.digest('hex');
}
...
// The target id is APPENDED and only for non-default targets, so the eBay token stays exactly
// `v1/default/62bbecce`. It carries the READABLE id rather than the fingerprint: this token exists
// for a human answering "which listings are still on the old art?", which a hash cannot answer.
export function composeVersion(variant, assetDigest = '', targetId = '') {
  return `${ASSET_VERSION}/${variant}`
    + (assetDigest ? '/' + assetDigest.slice(0, 8) : '')
    + (targetId && targetId !== 'ebay-square' ? '/' + targetId : '');
}
