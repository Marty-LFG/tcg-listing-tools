...
// Test seam: pre-load a key so the integration suite can exercise the real listener end to end with a
// locally generated keypair. Production never calls this — a key only ever arrives from getPublicKey.
export function __seedKey(kid, pem, { algorithm = 'ECDSA', digest = 'SHA1' } = {}) {
  _keys.set(String(kid), { pem: toPem(pem), algorithm, digest });
}
