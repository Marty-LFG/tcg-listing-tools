...
//
// AND THE AUDIENCE IS NOT A SHOPPER ON OUR OWN SITE. bk-theme/templates/product.json contains no
// product-description block — the PDP is built from bk-product-header / bk-condition-tiles / bk-buy /
// bk-product-facts and never renders this text at all. It reaches the SEO snippet, Shopify Catalog, the
// Shop app and the Google & YouTube feed instead. D-020 is why that matters: agents cannot filter on
// trading-card attributes, so title and description TEXT are the only route by which set, number,
// rarity and language reach one.
export const SHOPIFY_WYSIWYG = 'What you see is what you get — we photograph the actual card.';
