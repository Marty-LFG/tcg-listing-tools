...
// pokemon-listing-builder.html genTitle() rarity abbreviations.
const PKM_RAB = { 'special illustration rare': 'SIR', 'illustration rare': 'IR', 'ultra rare': 'UR', 'hyper rare': 'HR', 'double rare': 'RR', 'secret rare': 'Secret', 'rare secret': 'Secret', 'amazing rare': 'AR', 'radiant rare': 'Radiant', 'rare rainbow': 'Rainbow' };
...
export function pokemonPitch(f) {
  ...
  return `${f.name} from the Pokémon TCG ${f.set} set${f.stage ? ', ' + f.stage : ''}. ${f.rarity || ''} rarity.`;
}
...
export function buildDescription(game, f, opts) {
  ...
  const rows = [['Set', f.set], ['Card number', f.num], ['Rarity', f.rarity], ['Pokémon', f.poke], ['Stage', f.stage]];
  if (game !== 'pokemon') { rows.splice(3, 2); }         // poke/stage rows are Pokémon-only
  if (f.type) rows.push(['Type', f.type]);
  if (f.edition) rows.push(['Edition', f.edition]);
  rows.push(['Finish', f.finish]); rows.push(['Language', f.lang]);
  // Native metadata (JP/CN/KO): original name + set for searchability, plus the English-equivalent set.
  const nlab = (f.lang || '').replace(/\s*\(.*$/, '').trim() || 'Original';
  if (f.nativeName) rows.push([nlab + ' name', f.nativeName]);
  if (f.nativeSet) rows.push([nlab + ' set', f.nativeSet]);
  ...
  <div style="background:#1c2b5e;border-radius:10px 10px 0 0;padding:22px 24px;border-bottom:3px solid #f0c020;">
    ...
    <div style="color:#9aa6cf;font-size:14px;margin-top:4px;">${esc(f.num)} &middot; ${esc(f.set)} &middot; ${esc(f.rarity)}</div>
  </div>
  <div style="padding:20px 24px 4px;"><p style="margin:0 0 16px;">${esc(f.pitch)}</p></div>
  <div style="padding:0 24px;">
