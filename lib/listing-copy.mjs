...
// SEALED wording, for the same reason the LEGO and Funko builders keep their own (GR6): a booster box
// does not ship in a penny sleeve and toploader, and saying so would be a promise about a parcel nobody
// packs that way. The eBay sealed lane composes its own paragraph in lib/sealed-copy.mjs and folds the
// POSTAGE BAND into it; this sentence deliberately carries no figure, timeframe or region, because the
// Shopify theme owns all three from schema settings.
export const SEALED_PROTECTION = 'Packed with padding so it reaches you the way it left here.';

// SHOPIFY wording. Frozen on arrival like everything above (GR6), and living HERE rather than in
// lib/channels/shopify-map.mjs so that ONE file still owns every frozen sentence and the anti-mirror
// rule holds for the second channel too.
//
// WHY THE eBAY SENTENCES DO NOT ALL CARRY OVER. CARD_FOOTER ends "Thanks for looking" and
// SLAB_CONDITION_SUFFIX points at "item specifics" — both eBay-marketplace idiom. A Google snippet or
// a Shop card is not a place where somebody is looking at one listing among many, and "item specifics"
// names an eBay UI element that does not exist on Shopify. CARD_PROTECTION and SLAB_PROTECTION do carry
// over unchanged: they describe the parcel rather than the marketplace, and neither holds a figure,
// timeframe or region that could drift out of step with the theme's schema-driven shipping block.
//
// THE AUDIENCE INCLUDES A SHOPPER ON OUR OWN SITE. An earlier version of this comment said the opposite
// — that templates/product.json has no description block and "never renders this text at all", so the
// audience was only SEO snippets, Shopify Catalog, the Shop app and the Google & YouTube feed. That was
// false: product.json defines `bk_description` bound to `closest.product.description` and lists it in
// block_order. The text below is on the page a customer reads. The mistake mattered — the photography
// sentence was reasoned about as machine-readable metadata rather than a claim a buyer relies on, and
// it shipped asserting something untrue of most of the catalogue (D-030).
//
// The feed audience is still real and still the reason the TITLE carries set/number/rarity/language
// (D-020: agents cannot filter on trading-card attributes, so the text is the only route in).

// PHOTO PROVENANCE — two sentences, and which one you get is a fact about the product, not a house
// style (bk-shopify D-030). Most cards are listed on catalogue art: the game API's scan, composed into
// our frame. What we actually sell on those is the CONDITION CALL, so that is what the sentence claims.
// The old single sentence asserted photography for everything, which was false for most of the
// catalogue and is the defect D-030 exists to close.
//
// ⚠ SHOPIFY_WYSIWYG IS CURRENTLY UNREACHABLE, DELIBERATELY. No photograph can reach a Shopify product
// yet — composeFrames sends no image bytes, so the compositor falls back to catalogue art every time
// (PHOTO-PROVENANCE.md §1.1). Until that lands, buildDescription emits SHOPIFY_CATALOGUE_ART
// unconditionally. Shipping a conditional whose true branch cannot fire is the same class of mistake
// as the claim it replaces.
export const SHOPIFY_CATALOGUE_ART = 'Pictured with stock artwork; the condition above is graded by eye, not guessed.';
export const SHOPIFY_WYSIWYG = 'Photographed as it is — the card in the pictures is the one that ships.';
// Spec §10's "Run by collectors, not a warehouse." with invariant 6's Newcastle folded in, so the
// origin appears exactly once and costs no sentence of its own. Approved as this variant 2026-08-23.
// Deliberately NOT a shipping sentence: no rate, no threshold, no timeframe, no carrier, no region.
export const SHOPIFY_PROVENANCE = 'Run by collectors in Newcastle, not a warehouse.';
...
export function formatCardNumber(number, set, opts) {
  ...
  if (!raw) return '';
  // The printed numbers no era rule can rebuild (pkmSetFacts below). opts.id is the pokemontcg.io
  // card id, and it matters: three Classic Collection cards are all "106". Never on the intl lanes,
  // whose sets are keyed differently and whose numbers TCGdex already returns card-correct.
  var cut = String(opts.id || '').lastIndexOf('-');
  var facts = opts.source === 'tcgdex' ? null : pkmSetFacts(set.id || (cut > 0 ? String(opts.id).slice(0, cut) : ''));
  if (facts && facts.cards) {
    var hit = [];
    for (var i = 0; i < facts.cards.length; i++) {
      if (opts.id ? facts.cards[i][2] === opts.id : facts.cards[i][0] === raw) hit.push(facts.cards[i][1]);
    }
    if (hit.length === 1) return hit[0];
    // A number several cards share and nothing saying which one: the numerator alone, never
    // another card's denominator. A reprint subset never takes its own total either.
...
// ---------------------------------------------------------------------------
// pkmSetFacts — per-set printing facts no upstream carries. Every value was checked against
// TCGplayer's own product list for the set (2026-09-24). Keyed by pokemontcg.io set id, or by
// `<lang>:<CODE>` for the intl lanes.
//   holo    every card in the set is holofoil, commons included. A new set carries no TCGplayer
//           prices on pokemontcg.io for a week or more, and until they land every tool guessed the
//           finish off the rarity word: 71 of 30th Celebration's 161 cards came out non-holo (GR5).
//   own     a reprint subset: each card keeps its ORIGINAL printed number (Charizard 4/102 under
//           a 30 stamp), so the subset's own total is never a denominator. Without it the Classic
//           Collection printed 004/030, a number on no card (GR10).
//   cards   [raw upstream number, printed number, pokemontcg.io id] for each card the era rule
//           cannot rebuild. The id is there because subsets reuse numbers: three Classic
//           Collection cards are all #106 (106/105, 106/106, 106/160) and only the id says which.
//   rarity  pokemontcg.io id -> the rarity TCGplayer sells the card under, where upstream is
//           wrong. The three RGB Mews arrive as "Common".
// MIRROR: extras.js TCG.pkmSetFacts holds the same table, because a classic script cannot import
// this one; pkmIdForPrinted / pkmSharedCount / pkmRarityFor below are mirrored the same way.
// scripts/check-listing-copy.mjs compares them, and `'*'` lists the keys so it can check them all.
// ---------------------------------------------------------------------------
export function pkmSetFacts(key) {
  var T = {
    me55: {
      holo: true,
    ...
    // The Japanese and Korean 30th CELEBRATION: every card holo ("C/Uを含めて全部キラ"). The
    // Classic Collection is numbered INSIDE the set there (136/103-165/103), so nothing is `own`.
    'ja:M6A': { holo: true },
    'ko:M6A': { holo: true },
  };
...
// The pokemontcg.io id a typed number names, in a set pkmSetFacts carries card rows for:
// '106/106' is Palkia LV.X, '106/105' is Shining Celebi, and a bare '106' is all three. A bare
// number counts only when one card carries it ('4' in Celebrations is cel25c-4_A, an id nothing
// typed could otherwise reach). '' unless exactly one card matches. Padding-insensitive, so
// '50/185' finds '050/185'.
export function pkmIdForPrinted(setId, typed) {
  var f = pkmSetFacts(setId), k = cardNumberKey(typed), hit = [];
  if (!f || !f.cards || !k) return '';
  for (var i = 0; i < f.cards.length; i++) {
    var e = f.cards[i];
    if (cardNumberKey(e[1]) === k || (k.indexOf('/') < 0 && cardNumberKey(e[0]) === k)) hit.push(e[2]);
  }
  return hit.length === 1 ? hit[0] : '';
}

// How many cards in the set carry this bare number — so a tool can say "3 cards are #106" rather
// than silently opening the first. 0 for a set pkmSetFacts has no card rows for.
export function pkmSharedCount(setId, typed) {
  var f = pkmSetFacts(setId), k = cardNumberKey(String(typed == null ? '' : typed).split('/')[0]), n = 0;
  if (!f || !f.cards || !k) return 0;
  for (var i = 0; i < f.cards.length; i++) if (cardNumberKey(f.cards[i][0]) === k) n++;
  return n;
}

// A pokemontcg.io card's rarity, corrected where pkmSetFacts knows upstream is wrong.
export function pkmRarityFor(card) {
  var c = card || {}, f = pkmSetFacts((c.set && c.set.id) || '');
...
// ---------------------------------------------------------------------------
// Grader labels reach us spaced ('NM - MT+', 'EX - MT') from a Collectr/PriceCharting export and
// tight ('NM-MT') off a slab. The SPACING is normalised by RULE below, not by enumeration: a table
// only ever caught the forms someone happened to list, which is how 'CGC 8.5 NM - MT+' reached
// buyers raw while 'CGC 8 NM-MT' beside it was tightened. What is left here is the one rung the
// graders themselves print long, so no grade word is invented (GR4).
// 'MINT+' is deliberately NOT flattened to 'MINT': CGC and SGC both print a 9.5 as 'Mint+', and
// dropping the plus misstates the grader's own label on the slab in the photos.
const GRADE_LABEL_NORMALISE = { 'GEM MT': 'GEM MINT', 'GEM-MT': 'GEM MINT' };
export function gradeTitleToken(company, grade, label) {
  ...
  lab = lab.replace(/\s*-\s*/g, '-').replace(/\s+/g, ' ');
...
// The bare abbreviation, exported so the Shopify sink can put it in PRODUCT TAGS without keeping a
// second copy of the map (GR9 — a duplicated vocabulary is a vocabulary that drifts).
//
// Why tags: Shopify's predictive-search endpoint ANDs its tokens, so a search for "SIR 186/159"
// returns NOTHING when the rarity abbreviation lives only in a metafield — one unmatched word is
// enough to empty the result set. Carrying the abbreviation in a tag is the durable fix, and it costs
// nothing here because the map already exists.
export function pkmRarityAbbrev(r) {
  const ab = PKM_RAB[String(r || '').toLowerCase()];
  return ab && ab.toLowerCase() !== String(r || '').toLowerCase() ? ab : '';
}
...
export function pokemonPitch(f) {
  ...
  return `${f.name} from the Pokémon TCG ${set} set${f.stage ? ', ' + f.stage : ''}.${f.rarity ? ' ' + opRarName(f.rarity) + ' rarity.' : ''}`;
}
...
// rbMapRarity flattens Alternate Art / Overnumbered / Signature / Ultimate into ONE catalogue word,
// 'Showcase'. That is TCGplayer's tier name, not anything printed on the card, and the treatments it
// hides are far apart in price, so the row leads with the treatment a buyer can actually see and
// keeps Showcase as the tier behind it. Nothing is invented (GR4): the underlying rarity of a premium
// print is NOT recoverable, because the bake already stores 'Showcase' in its place.
function rbRarDisplay(r, v) { return v && v !== r ? (r === 'Showcase' ? v + ' (Showcase)' : r + ', ' + v) : (r || ''); }

// The champion on a Riftbound Unit — eBay's Character aspect, and the best facet the game has.
//
// Riot's card gallery names a champion card "<Champion>, <Epithet>": 'Darius, Trifarian',
// 'Kai'Sa, Survivor'. That is NOT the " - " form championTag (lib/riftbound-data.mjs) splits on —
// that one is riftscribe's and TCGplayer's shape, and against the baked catalog it hits 4 of 1173
// names, all of them "… - Starter". The comma form hits 297 names / 99 distinct champions.
//
// GATED ON TYPE, and the gate is why this is a derivation rather than a guess. Counted over the
// bake (2026-08-25): 296 of those 297 comma names are Units. The one that is not —
// 'Heisho, Shell of the World', a Battlefield — has a PLACE before the comma, not a champion. So
// an ungated split would put a location in the Character aspect. Legends carry the champion's
// TITLE with no name in the string at all ('Hand of Noxus') and correctly yield nothing.
//
// ⚠ This is a NEW export, not a change to championTag, which is under the GR9 mirror with
// riftbound-listing-builder.html and pinned by test/invariants/riftbound-mirror.test.mjs.
export function riftboundCharacter(name, type) {
  if (!/^\s*unit\s*$/i.test(String(type || ''))) return '';
  const s = String(name || '');
  const i = s.indexOf(', ');
...
function optionsTable(rows, zebra, pal) {
  // Defaults are the sans frames' own table ink and label width (lorcana/swu/onepiece/riftbound,
  // plus the funko/lego mirrors), so only the two serif frames have to say anything. Without this
  // the postage table rendered in a different typeface and a grey belonging to neither frame.
  const p = pal || {};
  const label = p.label || '#6b6b7e', value = p.value || '#1a1a22', width = p.width || '38%';
...
// One Piece prints a rarity CODE on the card, and that code is what optcgapi returns, so the details
// row read "Rarity | SEC". Buyers read words. These nine are the COMPLETE printed vocabulary,
// enumerated from Bandai's own English card list across all 59 series, with Bandai's own names from
// its per-product rarity breakdowns. optcgapi writes 'PR' where Bandai prints 'P'. An unlisted code
// renders UNCHANGED rather than being glossed with a guess (GR4).
const OP_RAR = { C: 'Common', UC: 'Uncommon', R: 'Rare', SR: 'Super Rare', SEC: 'Secret Rare', L: 'Leader', TR: 'Treasure Rare', SP: 'Special Card', P: 'Promotion Card', PR: 'Promotion Card' };
function opRarKey(r) { return ('' + (r == null ? '' : r)).trim().toUpperCase().replace(/^SP CARD$/, 'SP'); }
function opRarName(r) { const k = opRarKey(r); return OP_RAR[k] || (r || ''); }
function opRarDisplay(r) { const k = opRarKey(r); return OP_RAR[k] ? OP_RAR[k] + ' (' + k + ')' : (r || ''); }
export function onepiecePitch(f) {
  ...
  return `${name}, a ${f.color ? f.color + ' ' : ''}${type} from the One Piece Card Game's ${setName} set.${f.rarity ? ' ' + opRarName(f.rarity) + ' rarity.' : ''}`;
}
...
export function buildDescription(game, f, opts) {
  ...
  if (game === 'lorcana') {
    ...
    // The LABEL follows the values, the same way the One Piece 'Stats' row below does. A fixed
    // four-slot header forced a placeholder into every gap, and the row-level blank filter never
    // caught it: it only drops the row when all four are blank, so an Action or an Item, Cost and
    // nothing else, went out reading '7 / - / - / -'. Nothing is missing from those cards; the stats
    // simply do not exist on that card type, so neither should the columns.
    const stat = [['Cost', f.cost], ['Strength', f.strength], ['Willpower', f.willpower], ['Lore', f.lore]].filter((s) => s[1] !== '' && s[1] != null);
    if (stat.length) rows.push([stat.map((s) => s[0]).join(' / '), stat.map((s) => s[1]).join(' / ')]);
  ...
  <div style="background:#1a1326;border-radius:10px 10px 0 0;padding:22px 24px;border-bottom:3px solid #c9a24b;">
    ...
    <div style="color:#9a91ad;font-size:14px;margin-top:4px;">${[f.num, f.set, f.rarity].filter((x) => x != null && String(x).trim() !== '').map(esc).join(' &middot; ')}</div>
  </div>
  ...
  if (game === 'swu' || game === 'onepiece') {
    ...
    const rows = op
      ? [['Set', f.set], ['Card code', f.num], ['Rarity', opRarDisplay(f.rarity) + (chase && f.variant !== f.rarity ? ', ' + f.variant : '')], ['Card type', f.type], ['Colour', f.color]]
      : [['Set', f.set], ['Card number', f.num], ['Rarity', f.rarity + (f.variant && f.variant !== 'Standard' && f.variant !== f.rarity ? ', ' + f.variant : '')], ['Card type', f.type], ['Aspects', f.aspect]];
    if (op) {
      if (f.attr) rows.push(['Attribute', f.attr]);
      if (f.traits) rows.push(['Types', f.traits]);
  ...
  <div style="background:#0b0d12;border-radius:10px 10px 0 0;padding:22px 24px;border-bottom:3px solid ${accent};">
    ...
    <div style="color:#ffffff;font-size:22px;font-weight:700;line-height:1.25;">${esc(f.name)}${vSpan}</div>
    <div style="color:#8a8f9c;font-size:14px;margin-top:4px;">${[f.num, f.set, op ? opRarDisplay(f.rarity) : f.rarity].filter((x) => x != null && String(x).trim() !== '').map(esc).join(' &middot; ')}</div>
  </div>
  <div style="padding:20px 24px 4px;"><p style="margin:0 0 16px;">${esc(f.pitch)}</p></div>
  <div style="padding:0 24px;">
  ...
  if (game === 'riftbound') {
    ...
    const rows = [['Set', f.set], ['Card number', f.num], ['Rarity', rbRarDisplay(f.rarity, f.variant)], ['Card type', f.type], ['Domain', f.domain]];
  ...
  <div style="background:#091428;border-radius:10px 10px 0 0;padding:22px 24px;border-bottom:3px solid #c8aa6e;">
    ...
    <div style="color:#ffffff;font-size:22px;font-weight:700;line-height:1.25;">${esc(f.name)}${vSpan}</div>
    <div style="color:#7e8db0;font-size:14px;margin-top:4px;">${[f.num, f.set, rbRarDisplay(f.rarity, f.variant)].filter((x) => x != null && String(x).trim() !== '').map(esc).join(' &middot; ')}</div>
  </div>
  <div style="padding:20px 24px 4px;"><p style="margin:0 0 16px;">${esc(f.pitch)}</p></div>
  <div style="padding:0 24px;">
  ...
  <div style="padding:22px 26px 0;">
    ...
    ${optionsTable(cp.options, '#faf8f4', { label: '#7c6f5c', value: '#1f1a13', width: '42%', font: SANS_UI })}
  </div>
  ...
  // The generic side of this frame has no game to name, and 'Trading Card' was a GUESS presented as
  // fact (GR4): the rows that reach here are the ones whose `game` did not resolve, which is exactly
  // when we do not know the thing IS a trading card. Every game with a display name already has a
  // frame of its own above, so there is no map to look a real label up in. The generic masthead is
  // the store name alone. A caller that genuinely knows the label may still pass f.gameLabel.
  const eyebrow = game === 'pokemon' ? 'Binders Keepers &middot; Pok&eacute;mon TCG'
    : (f.gameLabel ? 'Binders Keepers &middot; ' + esc(f.gameLabel) : 'Binders Keepers');
  ...
  // rarDisplay()'s table is PKM_RAB, POKEMON rarity vocabulary, so it is gated exactly the way the
  // species/stage rows below are. Another product type's "Art Rare" is not Pokemon's Art Rare, and
  // glossing it "(AR)" states a fact about a card nobody checked (GR4). No mirror is needed:
  // pokemon-listing-builder.html is a Pokemon-only tool with no `game` in scope.
  const rarCell = game === 'pokemon' ? rarDisplay(f.rarity) : (f.rarity || '');
  rows.push(['Rarity', rarCell]);
  ...
  const artHtml = f.img
    ...
    <div style="display:inline-block;vertical-align:middle;width:210px;max-width:100%;padding:0 22px 0 0;box-sizing:border-box;">
      ...
      <div style="font-family:${SANS_UI};font-size:10.5px;letter-spacing:.2em;text-transform:uppercase;color:#7a6f85;font-weight:700;margin:14px 0 9px;">${esc(rarCell)}</div>
  ...
  <div style="padding:22px 26px 0;">
    ...
    <p style="margin:0;font-size:16px;">${cp.postage}</p>
    ${optionsTable(cp.options, '#faf8fb', { label: '#7a6f85', value: '#1b1420', width: '42%', font: SANS_UI })}
  </div>
