...
export function titleParts(game, f) {
  ...
  if (game === 'riftbound') {
    ...
    var altParen = (rvar === 'Signature') ? '(Signature)' : (rvar === 'Overnumbered') ? '(Overnumbered)' : (isAlt ? '(Alt Art)' : '');
    // Signature/Overnumbered is a 10-100x price differentiator (299*/298 US$2739 vs 299/298 US$296),
    // so it outranks the rarity token and the condition code when fitTitle has to shed parts (GR5) —
    // an 85-char Signature title used to drop the word "(Signature)" and keep "M/NM". (Alt Art) keeps
    // its old priority: the trailing letter in the collector number already carries it.
    var varPrio = (rvar === 'Signature' || rvar === 'Overnumbered') ? 82 : 60;
...
function rbMapRarity(r) { return (r === 'Alternate Art' || r === 'Overnumbered' || r === 'Signature') ? 'Showcase' : (r || ''); }
