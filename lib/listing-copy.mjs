...
// ---------------------------------------------------------------------------
import { bandForListing, postagePhrase, shippingOf } from './shipping-bands.mjs';
...
export function mtgPitch(f) {
  ...
  if (chase) return `${f.name} — ${f.treat && f.treat !== 'Normal' ? f.treat + ' ' : ''}${f.rarity} from ${f.set}. A standout pickup for collectors and players.`;
  return `${f.name} — a ${f.colour || ''} ${f.type || 'card'} from ${f.set}. ${f.rarity || ''}${f.finish && f.finish !== 'Nonfoil' ? ', ' + f.finish.toLowerCase() : ''}.`;
}
...
export function swuPitch(f, rawRarity, setName) {
  ...
  if (chaseVariant || chaseRarity) {
    return `The ${vWord}${name} ${type} from Star Wars: Unlimited — ${setName}. A ${asp ? asp + '-aspect ' : ''}standout chase card prized by collectors and players alike.`;
  }
  return `${name} — a ${asp ? asp + '-aspect ' : ''}${type.toLowerCase()} from Star Wars: Unlimited's ${setName} set. ${rawRarity || ''} rarity${v && v !== 'Standard' ? ', ' + v.toLowerCase() + ' variant' : ''}.`;
}
...
export function buildDescription(game, f, opts) {
  ...
  if (game === 'swu' || game === 'onepiece') {
    ...
    const rows = op
      ? [['Set', f.set], ['Card code', f.num], ['Rarity', f.rarity + (chase ? ' — ' + f.variant : '')], ['Card type', f.type], ['Colour', f.color]]
      : [['Set', f.set], ['Card number', f.num], ['Rarity', f.rarity + (f.variant && f.variant !== 'Standard' ? ' — ' + f.variant : '')], ['Card type', f.type], ['Aspect(s)', f.aspect]];
    if (op) {
