...
CREATE TABLE IF NOT EXISTS purchase_orders (
  ...
  arrived_at TEXT, received_at TEXT,
...
CREATE TABLE IF NOT EXISTS purchase_lines (
  ...
  received_kind TEXT,                             -- 'sealed'|'inventory'|'batch'|'none'
  received_item_id INTEGER,                       -- the stock row the units landed in (idempotency guard)
  received_sku TEXT,                              -- denormalised, so the trail survives a deleted stock row
  received_batch_id INTEGER REFERENCES bulk_batches(id) ON DELETE SET NULL,  -- 'lot' lines seed a batch
  received_at TEXT,
  notes TEXT,
  created_at TEXT NOT NULL DEFAULT (datetime('now')),
  updated_at TEXT NOT NULL DEFAULT (datetime('now'))
...
CREATE TABLE IF NOT EXISTS purchase_payments (
  ...
  amount_cents INTEGER NOT NULL,                  -- in the currency below — NOT necessarily the order's:
  currency TEXT NOT NULL,                         -- a deposit can settle in AUD on a USD order (GR3)
  aud_cents INTEGER,                              -- what the bank actually took, when known
...
function migratePurchasing(db) {
  ...
  addColumnIfMissing(db, 'purchase_lines', 'received_batch_id', 'INTEGER REFERENCES bulk_batches(id) ON DELETE SET NULL');
