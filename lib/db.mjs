...
function migrateStockLedger(db) {
  db.exec(`
...
function initDb(dbPath) {
  ...
  try {
    ...
    // LAST, and that is load-bearing. Its opening backfill reads whatever quantity each stock row holds
    // once every earlier migration has finished with it — including the sealed_placements seed in
    // migrateSealed, which is what sets sealed_items.quantity on a legacy DB in the first place.
    migrateStockLedger(db);
