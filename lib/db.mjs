...
function migrateBulk(db) {
  ...
  // fine. It is not fine for Keeper's Runs: `set_code` is one of the fifteen §4.4 fields hashed into a
  // bundle's commitment, and the comment two lines above says card_facts is "rewritten wholesale from a
  // card lookup". A hashed manifest field that a routine re-lookup can silently rewrite between draft
  // and lock is the exact drift the freeze-at-lock design exists to prevent.
  //
  // It also makes the graded and sealed sides symmetric — sealed_items.set_code has always been a real
  // column, which is why boundLines could read one for a booster and never for a slab.
