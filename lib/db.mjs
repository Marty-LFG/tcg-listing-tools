...
function migrateBulk(db) {
  ...
  // HOW MANY UNITS THE SALE COVERED, on both stock tables.
  //
  // sale_price_cents is PER UNIT, like cost_cents and acq_fees_cents beside it — but the sale also
  // sets quantity to 0, so after it there was nothing left saying how many units the money was for.
  // summarizeInventory multiplied by `r.quantity || 1`, which on a sold row is 1; summarizeSealed
  // multiplied by Math.max(0, quantity), which on a sold row is 0 and dropped the cost entirely. Two
  // twin functions, two different wrong answers, both caused by asking a zeroed column a question it
  // could no longer answer.
  //
  // NULL means a row sold before this column existed. Every reader falls back to 1, which is what both
  // summaries effectively assumed and is correct for every qty-1 slab and single — the overwhelming
  // majority of sold rows. Nothing is backfilled: the true figure for an old multi-unit sale is not
  // recoverable from a row whose quantity was zeroed.
  addColumnIfMissing(db, 'inventory_items', 'sale_qty', 'INTEGER');
  addColumnIfMissing(db, 'sealed_items', 'sale_qty', 'INTEGER');
  // The printed set code, PROMOTED OUT OF card_facts into a column of its own.
  //
  // TWO INDEPENDENT REASONS arrived at this from different directions, and both are worth keeping
  ...
  //
  // Keeper's Runs makes the same blob a correctness hazard rather than only an indexing one: `set_code`
  // is one of the fifteen §4.4 fields hashed into a bundle's commitment, so a routine re-lookup that
  // rewrites card_facts between draft and lock would silently move a hashed manifest field. That is
  // precisely the drift the freeze-at-lock design exists to prevent.
  //
...
function migratePregrade(db) {
  ...
  // THE CURRENCY grading_cost_cents IS IN — Golden Rule 3's "record the currency alongside it", which
  // this column has been missing since it existed.
  //
  // It has three writers and they do not agree. card-grader.html writes the fee straight off
  // data/grading.config.json, whose own note says "Only PCG quotes in AUD; every other company's
  // figures below are native USD and are NOT converted" — so a PSA submission lands in USD. The
  // inventory edit modal writes AUD. A purchase receive writes AUD via toAudCents. And the receive
  // then does `grading_cost_cents = COALESCE(grading_cost_cents,0) + ?`, which sums two currencies
  // into one number that is neither. inventory.html has been rendering the result with centsAud()
  // throughout, so a USD fee has been displayed as though it were dollars of the wrong kind.
  //
  // NULL means "written before this column existed". Those rows are ambiguous by construction and are
  // deliberately not guessed at: nothing backfills a currency onto them, because the only honest value
  // would come from knowing which of the three writers produced each one.
  addColumnIfMissing(db, 'grading_submissions', 'grading_cost_currency', 'TEXT');
...
//
// Every table above answers "what do I hold". None of them answers "how did it get to 3", and until now
// nothing did: each change to inventory_items.quantity or sealed_items.quantity was a destructive
// UPDATE that overwrote the previous number without leaving anything behind. inventory_valuations and
// sealed_valuations look like history and are not — they record VALUE, never a quantity, a delta or a
// reason.
//
// So this is the append-only side of stock. One row per movement, signed, with why it happened and what
// it happened because of. quantity on the stock row becomes a maintained CACHE of SUM(delta) — the same
// relationship sealed_items.quantity already has with sealed_placements, and lib/stock-ledger.mjs owns
// the invariant the way lib/sealed.mjs's four helpers own that one.
//
// NO FOREIGN KEY ON item_id, deliberately, for the reason migrateSealedListing and migrateShopify both
// document: inventory_items.id and sealed_items.id are INDEPENDENT sequences with PRAGMA foreign_keys
// ON, so a sealed id that happens to exist in inventory_items would PASS an FK check and file a sealed
// movement against an unrelated graded card. `kind` is the discriminator that makes the pair mean
// something.
//
// sku IS DENORMALISED ON PURPOSE. A hard delete takes the stock row and cascades its valuation history
// away; the movements survive it, and without the sku on the row the trail would name an id that no
// longer resolves to anything.
function migrateStockLedgerTable(db) {
  db.exec(`
    CREATE TABLE IF NOT EXISTS stock_movements (
      id         INTEGER PRIMARY KEY AUTOINCREMENT,
      kind       TEXT NOT NULL CHECK (kind IN ('inventory','sealed')),
      item_id    INTEGER NOT NULL,          -- tracker id within its kind; no FK, see above
      sku        TEXT,                      -- denormalised; outlives a deleted stock row
      delta      INTEGER NOT NULL,          -- SIGNED. +3 received, -1 sold, -2 written off
      qty_after  INTEGER NOT NULL,          -- the cache's value once this movement landed
      reason     TEXT NOT NULL CHECK (reason IN (
                   'opening','receive','sale','cancel','return','writeoff',
                   'damage','loss','stocktake','transfer','convert','manual','correction')),
      ref_kind   TEXT,                      -- 'order'|'po_line'|'batch'|'item' — what caused it
      ref_id     TEXT,
      location   TEXT,
      note       TEXT,
  ...
  `);
  // Inside the function, not the DDL template: db.exec(DDL) runs before this table exists.
  db.exec(`CREATE INDEX IF NOT EXISTS idx_move_item ON stock_movements(kind, item_id, at);`);
  db.exec(`CREATE INDEX IF NOT EXISTS idx_move_ref  ON stock_movements(ref_kind, ref_id);`);
  db.exec(`CREATE INDEX IF NOT EXISTS idx_move_at   ON stock_movements(at);`);
}

// The eBay AU → local-buyer arbitrage finder (lib/arbitrage.mjs, arbitrage.html). Four tables:
// what we watch, what we found, each scan, and the day's eBay call count.
//
// MONEY IS INTEGER CENTS IN THE CURRENCY IT WAS SOURCED IN (GR3). An eBay AU row is AUD; a TCGplayer
// figure is USD; the fx rate that joined them is stored on the same row (fx_usd_aud) so a hit can be
// re-read a month later and still explain its own arithmetic. Nothing here is pre-converted.
//
// arb_hits is keyed on the listing (card + printing + eBay item id), not on the scan: a listing that
// is still there tomorrow is the same hit with fresh figures, which is what lets `status` mean
// something across days — `new` fires an alert exactly once, `gone` records that the market moved on.
function migrateArbitrage(db) {
  db.exec(`
    CREATE TABLE IF NOT EXISTS arb_watch (
      id                    INTEGER PRIMARY KEY AUTOINCREMENT,
      game                  TEXT NOT NULL DEFAULT 'pokemon',
      set_id                TEXT NOT NULL,                 -- 'sv3'
      card_id               TEXT NOT NULL,                 -- 'sv3-125'
      number                TEXT NOT NULL,                 -- raw upstream number (GR10: display via TCG.formatCardNumber)
      name                  TEXT NOT NULL,
      set_name              TEXT,
      printed_total         INTEGER,
      ptcgo_code            TEXT,
      printing_key          TEXT NOT NULL,                 -- tcgplayer.prices key: normal | reverseHolofoil | holofoil | ...
      active                INTEGER NOT NULL DEFAULT 1,
      added_at              TEXT NOT NULL DEFAULT (datetime('now')),
      last_scanned_at       TEXT,
      last_scan_id          TEXT,
      last_market_usd_cents INTEGER,
      last_market_currency  TEXT NOT NULL DEFAULT 'USD',
      last_market_at        TEXT,
      last_rows             INTEGER,
      last_kept             INTEGER,
      last_hits             INTEGER,
      UNIQUE(game, card_id, printing_key)
    );
    CREATE TABLE IF NOT EXISTS arb_hits (
      id                  INTEGER PRIMARY KEY AUTOINCREMENT,
      watch_id            INTEGER REFERENCES arb_watch(id) ON DELETE SET NULL,   -- NULL for an ad-hoc scan
      game                TEXT NOT NULL DEFAULT 'pokemon',
      ...
      scored_printing_key TEXT,                          -- the printing the money was scored against (may differ when ambiguous)
      printing_ambiguous  INTEGER NOT NULL DEFAULT 0,
      item_id             TEXT NOT NULL,                 -- eBay Browse item id ('v1|287460438921|0')
      legacy_item_id      TEXT,
      price_cents         INTEGER NOT NULL,
      price_currency      TEXT NOT NULL DEFAULT 'AUD',
      ship_cents          INTEGER NOT NULL,
      fee_cents           INTEGER NOT NULL DEFAULT 0,    -- buyer-protection fee assumed (lib/fees.mjs), 0 when buyer_fee_mode is none
      delivered_cents     INTEGER NOT NULL,              -- price + ship + fee, same currency as price
      market_usd_cents    INTEGER,                       -- NULL when the printing has no live market and the buyer's reference stood in
      market_currency     TEXT NOT NULL DEFAULT 'USD',
      market_at           TEXT,                          -- tcgplayer.updatedAt as received
      fx_usd_aud          REAL NOT NULL,
      fx_date             TEXT,
      buyer_pct           REAL NOT NULL,
      buyer_aud_cents     INTEGER NOT NULL,
      profit_cents        INTEGER NOT NULL,
      margin_pct          REAL,                          -- profit / delivered × 100
      ratio               REAL,                          -- delivered / market_aud
      title               TEXT,
      url                 TEXT,
      image               TEXT,
      seller              TEXT,
      seller_fb_pct       REAL,
      seller_fb_score     INTEGER,
      cond                TEXT,
      cond_id             TEXT,
      cond_class          TEXT,                          -- nm | unstated (anything else never becomes a hit)
      cond_evidence       TEXT,
      match_tier          TEXT,                          -- strict | set
      coupon              INTEGER NOT NULL DEFAULT 0,
      best_offer          INTEGER NOT NULL DEFAULT 0,
      warnings            TEXT NOT NULL DEFAULT '[]',    -- JSON array of { k, why }
      listed_at           TEXT,
      first_seen          TEXT NOT NULL,
      last_seen           TEXT NOT NULL,
      last_scan_id        TEXT,
      status              TEXT NOT NULL DEFAULT 'new',   -- new | seen | bought | dismissed | gone
      gone_reason         TEXT,                          -- not_listed | below_floor
      notified_at         TEXT,
      qty_available       INTEGER,                       -- filled only by the explicit check-qty call
      UNIQUE(card_id, printing_key, item_id)
    );
    CREATE INDEX IF NOT EXISTS idx_arb_hits_status ON arb_hits(status, profit_cents DESC);
    CREATE INDEX IF NOT EXISTS idx_arb_hits_card ON arb_hits(card_id, printing_key);
    CREATE TABLE IF NOT EXISTS arb_scans (
      id          TEXT PRIMARY KEY,
      mode        TEXT NOT NULL,                         -- adhoc | watch
    ...
    -- eBay's daily quota resets on UTC midnight, so the day is UTC. Attempts are counted, not
    -- successes: a 4xx still spent a call.
    CREATE TABLE IF NOT EXISTS arb_budget (
      day        TEXT PRIMARY KEY,
      calls      INTEGER NOT NULL DEFAULT 0,
      updated_at TEXT NOT NULL
    );
    -- The newly-listed sweep: one row per set being swept. watermark is the itemCreationDate of
    -- the newest listing a pass has seen, so the next pass pages newest-first only until it reaches
    -- a listing it has already read — one or two calls a day per set.
    CREATE TABLE IF NOT EXISTS arb_sweep_sets (
      set_id        TEXT PRIMARY KEY,                    -- 'sv3'
      ...
      last_run_at   TEXT,
      last_scan_id  TEXT,
      last_total    INTEGER,                             -- eBay's total for the broad query, so the page can show how busy the set is
      last_calls    INTEGER,
      last_rows     INTEGER,
      last_new_rows INTEGER,
      last_matched  INTEGER,
      last_hits     INTEGER,
      last_hits_new INTEGER,
      last_dropped  TEXT                                 -- JSON { reason: n } of the last pass
    );
  `);
  // Where a hit came from: a card the operator named ('scan') or the newly-listed sweep ('sweep').
  addColumnIfMissing(db, 'arb_hits', 'source', "TEXT NOT NULL DEFAULT 'scan'");

  // THE STORE'S STALE REFERENCE. The local buyer prices off a TCGplayer Market snapshot it took
  // weeks ago (late July 2026, measured off its shelf: label = July market × 1.5, never repriced),
  // and its 80% is 80% of THAT. So a card that has fallen since the snapshot is worth more to the
  // buyer than to the market, and the finder needs the market as of the reference date, not today.
  //
  // Nobody stored July: the tracker watched one Pokémon card, added in August. Three sources, best
  // first — a figure the owner typed off TCGplayer's own chart (arb_reference_prices), the daily
  // set-wide history this module now records for itself (arb_market_snapshots, change-only so a
  // card that does not move costs no rows), and the tracker's price_snapshots for anything it
  // happened to watch. Money is USD cents; the store's conversion factor is on the hit row.
  db.exec(`
    CREATE TABLE IF NOT EXISTS arb_market_snapshots (
      day              TEXT NOT NULL,                     -- UTC 'YYYY-MM-DD'
      card_id          TEXT NOT NULL,
      printing_key     TEXT NOT NULL,
      market_usd_cents INTEGER NOT NULL,
      market_at        TEXT,                              -- tcgplayer.updatedAt as received
      PRIMARY KEY (card_id, printing_key, day)
    );
    CREATE INDEX IF NOT EXISTS idx_arb_snap_day ON arb_market_snapshots(day);
    CREATE TABLE IF NOT EXISTS arb_reference_prices (
      id               INTEGER PRIMARY KEY AUTOINCREMENT,
      card_id          TEXT NOT NULL,
      printing_key     TEXT NOT NULL,
      ref_date         TEXT NOT NULL,                     -- the day the figure was true, 'YYYY-MM-DD'
      market_usd_cents INTEGER NOT NULL,
      note             TEXT,
      added_at         TEXT NOT NULL DEFAULT (datetime('now')),
      UNIQUE(card_id, printing_key, ref_date)
    );
  `);
  // market_usd_cents was NOT NULL for the first day of this table's life. A card the buyer's stale
  // reference can price but pokemontcg.io cannot (the newest set has no prices for weeks) needs a
  // NULL there, and SQLite cannot drop a NOT NULL in place — so a table that still carries it is
  // rebuilt once, columns copied by name, indexes recreated by the block above on the next boot.
  const mkt = db.prepare('PRAGMA table_info(arb_hits)').all().find((c) => c.name === 'market_usd_cents');
  if (mkt && mkt.notnull) {
    const cols = db.prepare('PRAGMA table_info(arb_hits)').all().map((c) => c.name);
    const ddl = db.prepare("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'arb_hits'").get().sql
      .replace(/market_usd_cents\s+INTEGER\s+NOT NULL/, 'market_usd_cents INTEGER')
      .replace(/CREATE TABLE (IF NOT EXISTS )?arb_hits/, 'CREATE TABLE arb_hits_rebuild');
    db.exec('BEGIN');
    try {
      db.exec(ddl);
      db.exec(`INSERT INTO arb_hits_rebuild (${cols.join(', ')}) SELECT ${cols.join(', ')} FROM arb_hits`);
      db.exec('DROP TABLE arb_hits');
      db.exec('ALTER TABLE arb_hits_rebuild RENAME TO arb_hits');
      db.exec('CREATE INDEX IF NOT EXISTS idx_arb_hits_status ON arb_hits(status, profit_cents DESC)');
      db.exec('CREATE INDEX IF NOT EXISTS idx_arb_hits_card ON arb_hits(card_id, printing_key)');
      db.exec('COMMIT');
      console.log('[db] arb_hits rebuilt — market_usd_cents is now nullable');
    } catch (e) { db.exec('ROLLBACK'); console.error('[db] arb_hits rebuild failed —', e?.message || e); }
  }
  addColumnIfMissing(db, 'arb_hits', 'buyer_basis', "TEXT NOT NULL DEFAULT 'live'");   // live | reference
  addColumnIfMissing(db, 'arb_hits', 'ref_market_usd_cents', 'INTEGER');
  addColumnIfMissing(db, 'arb_hits', 'ref_date', 'TEXT');
  addColumnIfMissing(db, 'arb_hits', 'ref_source', 'TEXT');                             // manual | snapshot | tracker
  addColumnIfMissing(db, 'arb_hits', 'store_factor', 'REAL');
  // How much of the set TCGplayer priced when it was last swept, so the page can say "0 of 84 cards
  // priced" on the row instead of a sweep that silently matches everything and keeps nothing.
  addColumnIfMissing(db, 'arb_sweep_sets', 'last_priced_cards', 'INTEGER');
  addColumnIfMissing(db, 'arb_sweep_sets', 'last_total_cards', 'INTEGER');
}

// OPENING BALANCES, separated from the table above so a failure here cannot take the table with it.
function migrateStockLedgerBackfill(db) {
  // OPENING BALANCES. Every row that predates the ledger gets one movement equal to what it holds now,
  // so SUM(delta) == quantity is true for the whole table from the first boot rather than only for rows
  // created afterwards. It is explicitly NOT a reconstruction: nothing here pretends to know when those
  // units arrived or why, and 'opening' is the reason precisely so nobody later reads it as one.
  //
  // WHERE NOT EXISTS makes it a no-op on every boot after the first, and scopes it to rows that have no
  // movements at all — a row the ledger has been maintaining is never re-opened.
  for (const [kind, table] of [['inventory', 'inventory_items'], ['sealed', 'sealed_items']]) {
    db.exec(`INSERT INTO stock_movements (kind, item_id, sku, delta, qty_after, reason, note)
             SELECT '${kind}', t.id, t.sku, COALESCE(t.quantity, 0), COALESCE(t.quantity, 0), 'opening',
                    'balance carried in when the ledger was introduced'
               FROM ${table} t
              WHERE NOT EXISTS (SELECT 1 FROM stock_movements m WHERE m.kind = '${kind}' AND m.item_id = t.id)`);
  }
}
...
function migrateShopifyStore(db) {
  ...
  // IMMEDIATE, not deferred: the write lock is taken now rather than lazily at the first CREATE,
  // so a second process holding it fails here and rolls back cleanly instead of half-way through a
  // DROP/RENAME. busy_timeout does not reliably retry a deferred lock upgrade.
...
function migrateRuns(db) {
  db.exec(`
    ...
    -- Every artifact this run has PUBLISHED, content-addressed, with what the CDN gave back.
    --
    -- The superseded_at CHECK is §5.7.6 as a constraint rather than a comment: the anchor receipt is
    -- the ONLY published artifact that may ever be revised. The commitment is published exactly once,
    -- because reissuing it would let an observer diff successive versions and identify which bundle
    -- changed — and shipping order correlates with sale order.
    --
    -- That is also why the commitment publishes with an EMPTY anchors array: the anchor's state flips
    -- from pending to confirmed, so a commitment carrying it inline would have to be republished, which
    -- §5.2 forbids in the same breath that it lists the field. The receipt lives in its own artifact.
    CREATE TABLE IF NOT EXISTS run_artifacts (
      id            INTEGER PRIMARY KEY AUTOINCREMENT,
      run_id        INTEGER NOT NULL REFERENCES runs(id) ON DELETE CASCADE,
      kind          TEXT NOT NULL,              -- commitment|blob|anchor|disclosure|ledger
      seq           INTEGER NOT NULL DEFAULT 1, -- amendments (§10.5) and revised receipts (§5.7.6)
      sha256        TEXT NOT NULL,
      byte_length   INTEGER NOT NULL,
      mime_type     TEXT,
      body          BLOB,                       -- NULL for 'blob'; those bytes live in run_blobs
      file_gid      TEXT, url TEXT, store TEXT,
      published_at  TEXT NOT NULL DEFAULT (datetime('now')),
      readback_at   TEXT,                       -- when the CDN copy was last proven byte-for-byte
      superseded_at TEXT,
      UNIQUE (run_id, kind, seq),
      CHECK (kind IN ('commitment','blob','anchor','disclosure','ledger')),
      CHECK (superseded_at IS NULL OR kind = 'anchor')
    );
    CREATE INDEX IF NOT EXISTS idx_run_artifact_run ON run_artifacts(run_id, kind, seq);

    -- Small per-run key/value, for job state like the orders cursor.
    --
    -- Deliberately NOT a column on runs: a cursor is something a background job writes every few
    -- minutes, and a job writing to the row that carries the commitment is how a status flip acquires
    -- an accidental UPDATE next to it.
    ...
    CREATE TABLE IF NOT EXISTS run_ledger (
      ...
      -- 'opened_live' is the v2 unsold-policy disposition (lib/runs-policy.mjs). It accounts for a
      -- bundle number WITHOUT being a sale, so it deliberately does NOT consume a sale ordinal or move
      -- a unit: the two CHECKs below stay tied to the sale kinds alone, which is what keeps the
      -- specification's published ledger vectors reproducible.
      CHECK (kind IN ('sale_online','sale_in_person','opened_live','cancel','reprice','pause','resume')),
      CHECK (seq >= 0),
      ...
      -- A disposition that names no bundle accounts for nothing, and the availability set derived from
      -- this chain would silently disagree with the storefront.
      CHECK (kind NOT IN ('sale_online','sale_in_person','opened_live') OR bundle_no IS NOT NULL)
    );
    ...
    -- The PRIVATE binding from a ledger entry to the order that produced it.
    --
    -- A SEPARATE TABLE precisely because run_ledger is PUBLISHED IN FULL. Section 5.6.2 makes ref a
    -- random 128-bit token specifically so a storefront order number is never recoverable from the
    -- chain: order numbers are short and sequential, so hashing one would leave a preimage space of a
    -- few thousand values and anyone could map ordinal to order. Putting the order id on the ledger row
    -- would undo that in a single column.
    --
    -- The UNIQUE index is the orders poll's idempotency key - the analogue of
    -- order_line_items.stock_applied_at, and for the same reason: re-reading a window of orders must
    -- append nothing the second time.
    CREATE TABLE IF NOT EXISTS run_ledger_orders (
      entry_id   INTEGER PRIMARY KEY REFERENCES run_ledger(id) ON DELETE CASCADE,
      channel    TEXT NOT NULL,               -- 'shopify' | 'in_person'
      store      TEXT,                        -- dev|live, so a rehearsal is never read as a real sale
      order_ref  TEXT NOT NULL,
      order_name TEXT,                        -- '#1001' - operator-facing only, never published
      line_ref   TEXT NOT NULL,
      buyer_ref  TEXT,
      created_at TEXT NOT NULL DEFAULT (datetime('now'))
    );
    CREATE UNIQUE INDEX IF NOT EXISTS uq_run_ledger_order_line
      ON run_ledger_orders(channel, line_ref);
  ...
  // The two lifecycle stamps section 10.1 names. closed_at starts the delivery grace period that
  // section 5.5 requires before a close-out may publish; disclosed_at records that it did.
  addColumnIfMissing(db, 'runs', 'closed_at', 'TEXT');
  addColumnIfMissing(db, 'runs', 'disclosed_at', 'TEXT');
  // shopify_files was built for images, which carry neither of these: an image is addressed by its
  // MediaImage gid and its type is implied. A GenericFile — the commitment JSON and the blob container —
  // is fetched by URL and served with a declared content type, and §5.3.2 requires both to be checkable.
  addColumnIfMissing(db, 'shopify_files', 'url', 'TEXT');
  addColumnIfMissing(db, 'shopify_files', 'mime_type', 'TEXT');

  // The ledger's kind vocabulary gained 'opened_live' with unsold-policy v2 (lib/runs-policy.mjs).
  // CREATE TABLE IF NOT EXISTS will not update a CHECK on a database that already has the table, and
  // SQLite cannot alter a constraint in place, so a stale one has to be rebuilt.
  //
  // ONLY WHEN IT IS EMPTY. A ledger with rows is an append-only hash chain that is published and
  // anchored; dropping it would destroy the one artifact a buyer checks availability against. If a
  // stale table ever holds rows the migration REFUSES and says so, rather than choosing for someone.
  // Safe today because nothing has ever written to this table.
  try {
    const ddl = db.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='run_ledger'").get();
    if (ddl && !String(ddl.sql).includes('opened_live')) {
      const rows = db.prepare('SELECT COUNT(*) n FROM run_ledger').get().n;
      if (rows > 0) {
        console.warn(`[db] run_ledger predates the opened_live disposition and holds ${rows} row(s) — `
          + 'refusing to rebuild an append-only chain. Migrate it deliberately.');
      } else {
        db.exec('DROP TABLE run_ledger;');
        migrateRuns(db);   // recreate from the one DDL above, so there is no second copy to drift
      }
    }
  } catch { /* table absent on a fresh DB; the DDL above just made it */ }
}
...
function initDb(dbPath) {
  ...
  // THE STOCK LEDGER'S TABLE COMES FIRST AND IS NOT OPTIONAL.
  //
  // Everything below runs inside one swallowed try, so the FIRST throw skips every migration after it
  // and still hands back a usable handle. That is survivable for an additive column. It is not
  // survivable for stock_movements: every receive, every hand edit and every sale decrement goes
  // through applyMovement, which throws without the table — so a transient SQLITE_BUSY inside an
  // unrelated migration would boot an app that cannot take stock in or sell it.
  //
  // migrateShopify's two table REBUILDS are the realistic thrower (DROP + RENAME under a deferred
  // BEGIN, on a file other processes also open), and they sit upstream of everything new. So the table
  // is created before the chain rather than at the end of it.
  try { migrateStockLedgerTable(db); }
  catch (e) { console.error('[db] FATAL: the stock ledger table could not be created —', e?.message || e); throw e; }
  ...
  try {
    ...
    migrateRuns(db);
    migrateArbitrage(db);
    // The BACKFILL stays last, and that is load-bearing: it reads whatever quantity each stock row
    // holds once every earlier migration has finished with it — including the sealed_placements seed in
    // migrateSealed, which is what sets sealed_items.quantity on a legacy DB in the first place. If it
    // is skipped the ledger simply starts empty and applyMovement's catch-up carries each balance in on
    // first touch, which is a degradation rather than a failure.
    //
    // migrateRuns therefore goes ABOVE it rather than after, so that "last" keeps meaning last as more
    // migrations arrive.
    migrateStockLedgerBackfill(db);
...
/**
 * Closes the process singleton and forgets it, so a later openDb() opens a fresh handle.
 *
 ...
 *
 * Clearing _db is the part that matters. Closing the handle without it would leave every later
 * openDb() returning a CLOSED database, which fails in a way nobody would connect to a teardown.
 */
