...
// Which subsystem a line came from — the leading [tag] nearly everything in this codebase prefixes
// its output with. Untagged lines share one bucket rather than each becoming their own.
const TAG_RE = /^\[([^\]]{1,40})\]/;
export function tagOf(msg) {
  const m = TAG_RE.exec(String(msg || ''));
  return m ? m[1] : '(untagged)';
}

/**
 * Make room WITHOUT letting one chatty source evict every other.
 *
 * A plain oldest-first ring answers "what happened most recently", which sounds right and is the
 * wrong question for this buffer. Measured on the live box: 190 of 214 buffered lines were
 * `[api/ebay]` search URLs from a single comps scan, leaving ONE [ebay-notify] line and no
 * [postsale] lines at all. One more scan would have evicted those too. So the buffer was busy
 * remembering three hundred near-identical price lookups and forgetting the sale that did not alert
 * — which is precisely the incident it exists for.
 *
 * Instead, drop the oldest line belonging to whichever tag currently occupies the MOST space. That
 * is max-min fairness: a quiet subsystem keeps its handful of lines however loud its neighbour gets,
 * while a lone noisy source may still use the whole buffer when nothing else is competing (so
 * nothing is wasted on idle reservations). With only one tag present it degrades exactly to the
 * old oldest-first behaviour.
 *
 * Deliberately NOT solved by suppressing [api/ebay]: a burst of comp searches is itself worth
 * seeing when the question is "why is the repricer hammering eBay". The volume was never the
 * problem — the eviction was.
 */
function evict(b) {
  while (b.length > MAX) {
    const counts = new Map();
    for (const e of b) counts.set(e.tag, (counts.get(e.tag) || 0) + 1);
    let fattest = null, most = -1;
    for (const [tag, n] of counts) if (n > most) { most = n; fattest = tag; }
    const i = b.findIndex((e) => e.tag === fattest);
    b.splice(i < 0 ? 0 : i, 1);
  }
}
...
function push(level, args) {
  ...
  const b = buf();
  const clipped = msg.slice(0, 2000);
  b.push({ t: new Date().toISOString(), level, msg: clipped, tag: tagOf(clipped) });
  if (b.length > MAX) evict(b);
}
...
// tag = keep only this subsystem ('postsale', 'ebay-notify', …), which is how you read past a busy
// neighbour without having to eyeball a page of price lookups.
// Messages are sanitized on READ too, so any line buffered by an older instance (pre-fix, still
// carrying ANSI/control bytes) is served clean regardless of when it entered the buffer. `tag` is
// recomputed on read for the same reason: lines buffered before tagging existed still filter.
export function getLogs({ tail = 200, level = null, tag = null } = {}) {
  const b = buf();
  const min = level && RANK[level] != null ? RANK[level] : 0;
  let filtered = min ? b.filter((e) => RANK[e.level] >= min) : b;
  if (tag) filtered = filtered.filter((e) => (e.tag ?? tagOf(e.msg)) === tag);
  const n = Math.max(1, Math.min(MAX, (tail | 0) || 200));
  return filtered.slice(-n).map((e) => ({ ...e, tag: e.tag ?? tagOf(e.msg), msg: sanitizeMsg(e.msg) }));
}

/** What is actually in the buffer, by subsystem — the index for a `?tag=` read. */
export function logTags() {
  const counts = new Map();
  for (const e of buf()) {
    const t = e.tag ?? tagOf(e.msg);
