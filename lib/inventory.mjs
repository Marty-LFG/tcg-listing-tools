...
import { resolveSetCode } from './set-code.mjs';
...
// DELIBERATELY STORE-BLIND, and it is the one query against shopify_listings that is. That table is
// keyed (sku, store) so every other consumer filters on it — but "has this label been handed out" must
// not depend on who is asking: the label is a number written on the card in the box. A label claimed by
// a dev rehearsal is spent in the real world, and re-issuing it because live has not seen it yet would
// put two physical cards under one number. test/invariants/shopify-store-scoping.test.mjs carries this
// as a named exemption rather than an oversight.
...
// Fill card_facts.set_code on a row about to be written, when the set can be resolved from the name the
// row already carries. `bkc.set_code` is REQUIRED on the bk_card_identity metaobject, so a row without
// one gets no identity and can never appear in the PDP condition selector (bk-shopify D-026).
//
// Only lib/stock-games.mjs ever wrote card_facts. The three insert paths in this file never did, so
// every row they created has been invisible to the selector since the paths existed — 24 of them found
// on 2026-08-25. Unresolvable names are deliberately LEFT BLANK: a wrong set code silently files the
// card into the wrong automated per-set collection (D-016/D-027), which reads as intentional and is
// therefore worse than an obvious gap. scripts/backfill-set-code.mjs reports what this cannot resolve.
// WRITES BOTH PLACES, and the column is the authoritative one. card_facts keeps its copy because the
// Shopify identity mapper reads it through mergeCardFacts and nothing is served by changing that path
// here; the COLUMN exists because Keeper's Runs hashes set_code into a bundle's commitment and
// card_facts is rewritten wholesale by any later card lookup. Writing both in one function is what
// stops them drifting — there is no second place that stamps one without the other.
function stampSetCode(obj, src) {
  try {
    let facts = {};
    if (obj.card_facts) { try { facts = JSON.parse(obj.card_facts); } catch { facts = {}; } }
    const existing = String(facts.set_code || '').trim();
    if (existing) {
      // Already resolved into the blob by an earlier path; mirror it onto the column rather than
      // returning, or a row inserted before this migration keeps an empty column forever.
      if (!String(obj.set_code || '').trim()) obj.set_code = existing;
      return;
    }
    const { code } = resolveSetCode({
      game: obj.game || src?.game,
      set_name: obj.set_name || src?.set_name,
      language: obj.language || src?.language,
    });
    if (!code) return;
    facts.set_code = code;
    obj.card_facts = JSON.stringify(facts);
    obj.set_code = code;
  } catch { /* never block an insert over a lookup (GR7) */ }
}

/**
 * Split one multi-quantity row into that many rows of quantity 1.
 *
 * WHY THIS EXISTS. `holdForRun` refuses a quantity above 1 with "split it into single rows before
 * holding it for a run" — because a partial reservation against a qty-3 row means the database can no
 * longer answer "this exact physical object is in bundle 7", which is the entire verification claim of
 * Keeper's Runs. That instruction was unfollowable: nothing in this codebase could split a row. Twenty-
 * five art cards arriving as one bulk line had no path into a manifest at all.
 *
 * The ORIGINAL row keeps its id and its SKU and drops to quantity 1. That matters more than it looks:
 * ebay_listing_id, valuations, batch membership and any published listing all point at that id, so
 * minting a fresh row for the original and deleting it would orphan every one of them. The new siblings
 * are the copies.
 *
 * Refused on a reserved row unless forced, on a sold row always, and on anything already at 1.
 */
export function splitInventoryItem(db, itemId, { actor = null, note = null } = {}) {
  const row = db.prepare('SELECT * FROM inventory_items WHERE id = ?').get(+itemId);
  if (!row) throw new Error(`no such inventory item: ${itemId}`);
  const qty = row.quantity ?? 1;
  if (qty <= 1) throw new Error(`inventory item ${itemId} is already a single row`);
  if (row.status === 'sold') throw new Error(`inventory item ${itemId} is sold; splitting it would invent stock`);

  // Copy every column except the ones that identify THIS row or THIS row's listing. A sibling must not
  // inherit an eBay listing id — two rows pointing at one live listing is how a single card gets sold
  // twice — nor a shelf label, which is a physical location claim.
  const skip = new Set(['id', 'sku', 'quantity', 'ebay_listing_id', 'ebay_offer_id', 'created_at', 'updated_at']);
  ...
  db.exec('BEGIN');
  try {
    // BORN EMPTY, then the unit MOVES in. The row used to be inserted at quantity 1, which wrote stock
    // into existence without a movement to explain it. Inserting at 0 and applying +1 costs one extra
    // statement and makes the split legible in the ledger from both ends — and it is the difference
    // between the child ending at 1 and ending at 2, because applyMovement adds to the cache rather
    // than asserting it. Nothing outside the transaction ever observes the zero.
    const insert = db.prepare(`INSERT INTO inventory_items (sku, quantity, ${cols.join(', ')})
                               VALUES (?, 0, ${cols.map(() => '?').join(', ')})`);
    for (let i = 1; i < qty; i++) {
      const sku = nextSku(db, row.game);
      insert.run(sku, ...cols.map((c) => row[c]));
      const madeId = db.prepare('SELECT last_insert_rowid() id').get().id;
      made.push({ id: madeId, sku });
      applyMovement(db, { kind: 'inventory', itemId: madeId, delta: 1, reason: 'convert',
        refKind: 'item', refId: String(row.id), sku,
        note: `split off ${row.sku} (was ${qty})` });
    }
    // THROUGH THE LEDGER, not a bare UPDATE.
    //
    // This wrote `SET quantity = 1` directly, which predates lib/stock-ledger.mjs and trips its
    // single-writer invariant. A split conserves stock — one row of N becomes N rows of one — so it is
    // recorded as a conversion on both sides and the movements net to zero. Getting that wrong is not
    // cosmetic: the ledger is what answers "how did it get to 3", and a silent drop from N to 1 reads
    // as N-1 units vanishing with no reason attached.
    setQuantity(db, { kind: 'inventory', itemId: row.id, quantity: 1, reason: 'convert',
      refKind: 'item', refId: String(row.id),
      note: `split into ${qty} single rows` });
    db.exec('COMMIT');
  } catch (e) { db.exec('ROLLBACK'); throw e; }

  return { id: row.id, sku: row.sku, was: qty, created: made, actor, note };
}
...
export function summarizeInventory(rows) {
  ...
  // UNKNOWN IS NOT ZERO. cents() coerces a null cost to 0, which reads as a 100% margin — and the two
  // paths most likely to produce one leave it null by default (promote starts its cost field blank, a
  // bulk import has no cost at all). The arithmetic below is unchanged, because a sum of known costs
  // is still the right sum; what was missing is any way to know how much of it is missing. Counted
  // here and rendered on the Realized P/L tile, so it says how much of the figure is missing rather
  // than looking exact (GR4).
  ...
  for (const r of rows) {
    ...
    if (r.status === 'sold') {
      // Scaled by sale_qty, NOT by quantity — the sale zeroed quantity, so `r.quantity || 1` said one
      // unit for a lot of fifty and charged COGS once for the whole lot. sale_qty is null on rows sold
      // before the column existed; 1 is what this expression already effectively assumed, and is right
      // for every qty-1 slab and single.
      const soldQty = r.sale_qty != null ? Math.max(0, +r.sale_qty) : 1;
      if (r.cost_cents == null) noCostSold++;
      realizedPlCents += (cents(r.sale_price_cents) - cents(r.sale_fees_cents) - cents(r.cost_cents) - cents(r.acq_fees_cents)) * soldQty;
    } else {
      units += qty;
  ...
  return { counts, units, totalCostCents, realizedPlCents, valueByCurrency, byGame, byCompany, noCostHeld, noCostSold };
}
...
// A Keeper's Run reservation is a hard hold: the item is promised to a bundle whose manifest may
// already be published, and the published record names this exact physical object. So the destructive
// edits — marking it sold, shrinking the quantity, deleting the row — are refused here.
//
// `?force=1` is the only way past, and it PROCEEDS while recording why. Quietly dropping a hold the
// owner placed on purpose is the wrong failure; so is having no override at all, because the physical
// world does occasionally win. Returns a 409 body when the caller should stop, or null to continue.
// A thin adapter: the refusal itself, and the ?force=1 audit write, live in lib/runs-reserve.mjs so the
// sealed twin of every route below shares one implementation rather than a second copy that drifts.
const reservationBlock = (db, itemId, q, action) =>
  blockIfHeld(db, { kind: 'inventory', itemId, force: q.get('force') === '1', action });
const BATCH_COLS = [
...
const SUB_COLS = [
  'game', 'identity_key', 'name', 'set_name', 'number', 'variant', 'language',
  'grading_company', 'tier', 'declared_value_cents', 'grading_cost_cents', 'grading_cost_currency',
...
export function receiveInventory(db, opts = {}) {
  ...
  if (itemId != null) {
    ...
    // The MONEY and the location, which are this statement's own business. The QUANTITY is not
    // here any more: it moves through the ledger below, so the units arriving have a reason and a
    // purchase-order line attached to them rather than appearing as a bigger number.
    db.prepare(`UPDATE inventory_items SET location = COALESCE(location, ?),
    ...
    const moved = applyMovement(db, {
      kind: 'inventory', itemId, delta: addQty, reason: 'receive',
      refKind: poLineId != null ? 'po_line' : 'item', refId: poLineId, location: loc,
    });
    return { id: itemId, sku: cur.sku, created: false, quantity: moved.quantity };
  }
  ...
  // INSERTED AT ZERO, then received. The ledger is the authority for quantity, so a new row starts
  // empty and its units arrive as a movement — otherwise the insert writes addQty, the movement adds
  // addQty on top, and a lot of seven lands as fourteen.
  const obj = pick({ ...clean, quantity: 0, location: loc }, ITEM_INSERT_COLS);
  ...
  // A NEW row's units are a movement too, and the insert above deliberately left it at zero: writing
  // the quantity there AND filing a movement for the same units lands a lot of seven as fourteen,
  // because applyMovement's catch-up reads the cache as stock the ledger has not seen yet.
  applyMovement(db, {
    kind: 'inventory', itemId: id, delta: addQty, reason: 'receive',
    refKind: poLineId != null ? 'po_line' : 'item', refId: poLineId, location: loc, sku: obj.sku,
  });
  ...
  return async (req, res) => {
    try {
      ...
      if (p === '/summary' && method === 'GET') {
        // sale_qty is NOT optional here. summarizeInventory scales every sold row's money by it, and a
        // column the SELECT omits arrives as undefined and falls back to 1 — which silently collapsed a
        // ten-unit lot's revenue to one unit's worth the moment sale_price_cents became per-unit.
        const rows = db.prepare(`SELECT status, game, grading_company, cost_cents, acq_fees_cents,
          sale_price_cents, sale_fees_cents, sale_qty, value_cents, value_currency, quantity FROM inventory_items`).all();
      ...
      if (p === '/items' && method === 'POST') {
        ...
        try {
          ...
          obj.sku = sku;
          stampSetCode(obj, b);
          // Insert at zero and file the units as a movement, the same shape receiveInventory uses:
          // writing the quantity here AND filing a movement would double it, and writing it here and
          // filing nothing leaves the ledger behind its own cache from the row's first day.
          const wantQty = Math.max(0, Math.round(Number(obj.quantity != null ? obj.quantity : 1) || 0));
          obj.quantity = 0;
          id = insertRow(db, 'inventory_items', obj);
          if (wantQty > 0) {
            applyMovement(db, { kind: 'inventory', itemId: id, delta: wantQty, reason: 'opening',
              refKind: 'item', refId: id, sku, note: 'added by hand' });
          }
      ...
      // GET /match?identity_key=&variant=&condition=&language=&game=&graded=&grading_company=&grade=&number=
      ...
      if (p === '/match' && method === 'GET') {
        ...
        // With no identity key (a PriceCharting-only set such as the JP 30th CELEBRATION) the name is
        // all there is to go on, and a name is not a card: that set alone has thirty different
        // Pikachu, so every one of them matched every other as "already in stock". A number, when
        // the caller sends one, narrows the name to the card. A stored row with no number stays in,
        // because it cannot be ruled out.
        const wantNum = cardNumberKey(want.number);
        const rows = want.identity_key
          ...
          : db.prepare('SELECT * FROM inventory_items WHERE game = ? AND lower(name) = lower(?) ORDER BY id').all(want.game, want.name)
            .filter((r) => !wantNum || !r.number || cardNumberKey(r.number) === wantNum);
      ...
      // POST /items/:id/split — one qty-N row becomes N rows of 1.
      //
      // Guarded like every other destructive edit in this file: a reserved row refuses unless ?force=1,
      // which proceeds and records why. Splitting is not destructive in the usual sense — no stock is
      // lost — but it CHANGES WHICH ROW a run's reservation points at, and a run's whole claim is that a
      // named row is a named physical object.
      if ((m = p.match(/^\/items\/(\d+)\/split$/)) && method === 'POST') {
        const id = +m[1];
        const blocked = reservationBlock(db, id, q, 'split into single rows');
        if (blocked) return send(res, 409, blocked);
        const b = await readJson(req);
        try {
          return send(res, 200, splitInventoryItem(db, id, { actor: b.actor || null, note: b.note || null }));
        } catch (e) {
          const msg = String(e?.message || e);
          return send(res, /no such|already a single|is sold/.test(msg) ? 409 : 500, { error: msg });
        }
      }
      ...
      if ((m = p.match(/^\/items\/(\d+)$/)) && method === 'PATCH') {
        ...
        // THE RESERVATION GUARD RUNS FIRST, AND THE ORDER IS LOAD-BEARING. `shrinks` reads
        // obj.quantity, and the ledger routing below DELETES obj.quantity before it updates anything —
        // so guarding after it would silently read undefined and never refuse a shrink. Only the
        // destructive shapes are guarded: fixing a typo in the card name of a reserved item is fine and
        // common; selling it or shrinking it out from under the run is not.
        const cur = db.prepare('SELECT status, quantity FROM inventory_items WHERE id = ?').get(id);
        const goesSold = obj.status === 'sold' && cur && cur.status !== 'sold';
        const shrinks = obj.quantity != null && cur && +obj.quantity < (cur.quantity ?? 1);
        if (goesSold || shrinks) {
          const blocked = reservationBlock(db, id, q, goesSold ? 'patch status=sold' : 'patch quantity down');
          if (blocked) return send(res, 409, blocked);
        }
        // QUANTITY LEAVES THIS STATEMENT. Everything else here is a description of the card and
        // belongs in a blind whitelist update; quantity is stock, and a hand edit that moves it is the
        // one kind of movement nothing else records — no order, no purchase order, just somebody
        // typing a different number. Routing it through the ledger is what makes it explicable later.
        const wantQty = Object.prototype.hasOwnProperty.call(obj, 'quantity') ? obj.quantity : undefined;
        delete obj.quantity;
        const rest = Object.keys(obj);
        if (!rest.length && wantQty === undefined) return send(res, 400, { error: 'nothing to update' });
        db.exec('BEGIN');
        try {
          if (rest.length) {
            const sets = rest.map((c) => c + ' = ?').concat([`updated_at = datetime('now')`]);
            db.prepare(`UPDATE inventory_items SET ${sets.join(', ')} WHERE id = ?`).run(...rest.map((c) => obj[c]), id);
          }
          if (wantQty !== undefined) {
            setQuantity(db, { kind: 'inventory', itemId: id, quantity: wantQty, reason: 'manual', refKind: 'item', refId: id,
              note: 'edited by hand' });
          }
          db.exec('COMMIT');
        } catch (e) { db.exec('ROLLBACK'); throw e; }
      ...
      if ((m = p.match(/^\/items\/(\d+)$/)) && method === 'DELETE') {
        const id = +m[1];
        const row = db.prepare(`SELECT status, sku FROM inventory_items WHERE id = ?`).get(id);
        if (!row) return send(res, 200, { removed: true, already: true });
        // Two independent guards, as on the sealed delete: the run hold first because it is the harder
        // claim — a reservation names this row in a manifest that is already published and anchored —
        // and its message names the run and bundle to release, which is more actionable than "it is
        // listed". The status guard then protects the cost basis of anything past in_stock.
        const blocked = reservationBlock(db, id, q, 'delete item');
        if (blocked) return send(res, 409, blocked);
          ...
          for (const raw of rows) {
            try {
              ...
              if (existing) {
                ...
                // Quantity is always defined on this object (it defaults to 1), so leaving it in the
                // whitelist did two wrong things at once: it reset an existing row's stock to 1 on
                // every re-import, and it did so without a movement — permanent drift, on a row that
                // already has movements so the boot backfill will never repair it.
                const reQty = obj.quantity;
                delete obj.quantity;
                ...
                if (reQty !== undefined) {
                  setQuantity(db, { kind: 'inventory', itemId: existing.id, quantity: reQty,
                    reason: 'correction', refKind: 'batch', refId: batchId, note: 're-imported from the source batch' });
                }
                stats.updated++;
              } else {
                obj.sku = graded ? nextSku(db, raw.game) : nextBulkSku(db, raw.game);
                const newQty = Math.max(0, Math.round(Number(obj.quantity != null ? obj.quantity : 1) || 0));
                obj.quantity = 0;
                const newId = insertRow(db, 'inventory_items', obj);
                if (newQty > 0) {
                  applyMovement(db, { kind: 'inventory', itemId: newId, delta: newQty, reason: 'opening',
                    refKind: 'batch', refId: batchId, sku: obj.sku, note: 'imported from a bulk batch' });
                }
                stats.inserted++;
              }
      ...
      if ((m = p.match(/^\/batches\/(\d+)$/)) && method === 'DELETE') {
        ...
        // THE EIGHTH DISPOSING SURFACE, and the one the first pass of these guards missed. A reservation
        // is orthogonal to lifecycle by design — holding a card never changes its status — so a reserved
        // row is still 'in_stock' and sits squarely inside this WHERE clause. Deleting it leaves the
        // reservation dangling with no foreign key to catch it, and the manifest's LEFT JOIN then renders
        // a nameless slot while still claiming the bundle is packed: a lie no amount of hashing detects.
        //
        // Guarded row by row rather than in the SQL, so the refusal can name which cards and for which
        // run — and so ?force=1 leaves an audit trail, which the bare DELETE never could.
        if (alsoItems) {
          const scope = db.prepare(`SELECT id FROM inventory_items WHERE batch_id = ? AND status = 'in_stock'`).all(id);
          const blocked = [];
          ...
          if (blocked.length) {
            return send(res, 409, {
              ...
              error: `${blocked.length} item(s) in this batch are reserved for a run; release them, or pass ?force=1 to override`,
            });
          }
      ...
      if ((m = p.match(/^\/batches\/(\d+)\/mark$/)) && method === 'POST') {
        ...
        // A bulk UPDATE, so the reservation check has to run over every row it would touch — one
        // reserved slab inside a hundred-row batch would otherwise sell silently. Reported together
        // rather than one at a time, because the operator wants the whole list to go and deselect.
        if (to === 'sold') {
          const scope = db.prepare(`SELECT id FROM inventory_items
                                     WHERE batch_id = ?${inClause} AND status <> 'sold'`).all(...args);
          const blocked = [];
          for (const row of scope) {
            const hit = reservationBlock(db, row.id, q, 'batch mark sold');
            if (hit) blocked.push(hit);
          }
          if (blocked.length) {
            return send(res, 409, {
      ...
      if ((m = p.match(/^\/submissions\/(\d+)\/promote$/)) && method === 'POST') {
        ...
        // A null currency is legacy — written before the column existed, when every reader assumed AUD.
        const feeCur = String(sub.grading_cost_currency || 'AUD').toUpperCase();
        const gradingFeeIsAud = feeCur === 'AUD';
        const item = pick({
          ...
          // Grading cost is part of cost basis; the client adds the purchase cost separately.
          //
          // CARRIED ONLY WHEN IT IS ALREADY AUD. inventory_items has no currency column — every amount
          // on it is AUD by convention, which is exactly why the receive converts before writing — so
          // copying a USD grading fee here would make it silently claim to be dollars it is not
          // (GR3). card-grader.html seeds the fee in the company's NATIVE currency, and for everyone
          // except PCG that is USD. A null currency is legacy and treated as AUD, which is what every
          // reader already assumed, so existing rows behave exactly as before.
          acq_fees_cents: b.acq_fees_cents != null ? +b.acq_fees_cents : (gradingFeeIsAud ? sub.grading_cost_cents : null),
        ...
        // Said out loud rather than swallowed: the slab is real and in stock, but its cost basis is
        // short the grading fee and nothing on the row would show that. Naming the amount and its
        // currency is what lets the operator type the AUD figure instead of discovering the gap in a
        // margin number months later (GR4).
        const warnings = [];
        if (!gradingFeeIsAud && sub.grading_cost_cents != null && b.acq_fees_cents == null) {
          warnings.push(`the grading fee is recorded as ${feeCur} ${(sub.grading_cost_cents / 100).toFixed(2)} and stock cost basis is AUD, `
            + 'so it was NOT carried onto the slab. Enter the AUD fee on the item to complete its cost basis.');
        }
        return send(res, 201, { item_id: itemId, sku, warnings });
      }
