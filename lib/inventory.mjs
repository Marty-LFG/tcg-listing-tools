...
import { liveReservation, audit as runAudit } from './runs-reserve.mjs';
...
function reservationBlock(db, itemId, q, action) {
  const held = liveReservation(db, 'inventory', +itemId);
  if (!held) return null;
  const where = held.bundle_label ? `bundle ${held.bundle_label}`
    : held.run_public_id ? `run ${held.run_public_id}` : 'a run';
  if (q.get('force') !== '1') {
    return {
      code: 'reserved_for_run', item_id: +itemId, reservation_id: held.id,
      run: held.run_public_id || null, bundle: held.bundle_label || null,
      error: `item ${itemId} is reserved for ${where}; release the reservation, or pass ?force=1 to override`,
    };
  }
  runAudit(db, {
    runId: held.run_id, bundleId: held.bundle_id, entity: 'inventory_item', entityId: +itemId,
    action: 'reservation_override', before: { reservation_id: held.id, state: held.state },
    note: `${action} forced while reserved for ${where}`,
  });
  return null;
}
const BATCH_COLS = [
