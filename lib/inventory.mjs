...
export function splitInventoryItem(db, itemId, { actor = null, note = null } = {}) {
  ...
  try {
    ...
    for (let i = 1; i < qty; i++) {
      ...
      made.push({ id: db.prepare('SELECT last_insert_rowid() id').get().id, sku });
    }
    db.prepare("UPDATE inventory_items SET quantity = 1, updated_at = datetime('now') WHERE id = ?").run(row.id);
    db.exec('COMMIT');
  } catch (e) { db.exec('ROLLBACK'); throw e; }
