...
function pickPkmCard(arr, item) {
  ...
  if (num) {
    ...
    // Disambiguate same-number cards across sets by the slab's set name when we have one.
...
async function resolveImage(db, base, item) {
  ...
  let url = null, resolvedKey = null, resolvedLang = null;
  ...
  if (item.game === 'pokemon' && INTL_LANGS[lang]) {
    ...
    url = await resolveIntlImage(base, item, lang);
    if (url) resolvedLang = lang;   // a TCGdex match proves the language — correct a stale EN row.
  } else {
  ...
  if (url) {
    ...
    if (resolvedKey && !item.identity_key) sets.identity_key = resolvedKey;
    if (resolvedLang && resolvedLang !== String(item.language || '').toUpperCase()) sets.language = resolvedLang;
