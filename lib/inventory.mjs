...
export function nextStockLabel(db) {
  ...
  for (let guard = 0; guard < 200; guard++) {
    ...
    if (!db.prepare('SELECT 1 FROM inventory_items WHERE sku = ?').get(label)) return label;
    // Taken (a hand-entered label, or a seed that was set too low). Skip it — never reuse.
  }
  return null;
}
  ...
  return async (req, res) => {
    try {
      ...
      if (p === '/match' && method === 'GET') {
        ...
        return send(res, 200, { key, exact, near, held: rows.length });
      }
      ...
      if (p === '/labels' && method === 'GET') {
        ...
        return send(res, 200, { ...st, inUse: used.length, highestInDb: labelFor(maxLabelSeq(used)) || null });
      }
