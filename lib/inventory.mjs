...
// A label counts as TAKEN if it is on a stock row OR on any listing eBay knows about. The mirror
// matters: most of this seller's labels live only on hand-made listings that were never stock, so a
// stock-only check happily re-issues one that is on a live listing.
export function labelTaken(db, label) {
  if (db.prepare('SELECT 1 FROM inventory_items WHERE sku = ?').get(label)) return true;
  try { return !!db.prepare('SELECT 1 FROM ebay_seller_listings WHERE sku = ?').get(label); }
...
// figures. This is the OPPOSITE of sealed.mjs's per-ROW convention (money as entered = the row
// total) — do NOT unify them. Graded slabs + manual adds are quantity 1 (column DEFAULT 1, never
...
const INTL_LANGS = { JP: ['ja'], KO: ['ko'], ZH: ['zh-tw', 'zh-cn'] };
...
// Effective language: an explicit intl code (JP/KO/ZH) wins; else sniff the set name so a row
...
// TCGdex card ids zero-pad the number to 3 (SV4M-077); try that first, then bare, then 2-pad.
function intlNumCandidates(num) {
  const n = String(num || '').split('/')[0].trim();
  const out = [];
  if (/^\d+$/.test(n)) { const i = parseInt(n, 10); out.push(('00' + i).slice(-3), String(i), ('0' + i).slice(-2)); }
  else if (n) out.push(n, n.toUpperCase());
    ...
    try {
      ...
      if (p === '/match' && method === 'GET') {
        ...
        const slim = (r) => ({
          ...
          listingUrl: r.ebay_listing_id ? 'https://www.ebay.com.au/itm/' + r.ebay_listing_id : null,
        });
      ...
      if (p === '/labels' && method === 'GET') {
        ...
        // series happily hands out a label that is on a live hand-made listing.
        let nextFree = null;
        if (st.seeded) {
          for (let s = st.seq + 1, i = 0; i < 200; s++, i++) {
            const l = labelFor(s);
            if (!l) break;
            if (!labelTaken(db, l)) { nextFree = l; break; }
          }
        }
        return send(res, 200, { ...st, next: nextFree, inUse: used.length + onEbay.length,
          highestInDb: labelFor(maxLabelSeq(used.concat(onEbay))) || null });
      }

      // POST /labels/seed { label } | { seq } — move the series to just past a known label, e.g. the
      // highest custom label already on eBay. Only ever moves FORWARD (never reuse a retired number).
