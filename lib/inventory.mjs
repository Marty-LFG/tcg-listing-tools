...
export function upcomingStockLabels(db, want) {
  const row = db.prepare('SELECT seq FROM sku_counter WHERE namespace = ?').get(LABEL_NS);
  if (!row) return [];                                                   // unseeded — caller falls back
  ...
  for (let seq = row.seq + 1, guard = 0; out.length < n && guard < n + 200; seq++, guard++) {
    const label = labelFor(seq);
  ...
  return async (req, res) => {
    try {
      ...
      if (p === '/labels' && method === 'GET') {
        ...
        const upcoming = upcomingStockLabels(db, peek);
