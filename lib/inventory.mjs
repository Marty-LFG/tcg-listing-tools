...
export function summarizeInventory(rows) {
  ...
  // here so the page can say so rather than presenting a confident wrong number (GR4).
...
export function receiveInventory(db, opts = {}) {
  ...
  // A NEW row's units are a movement too. The insert already carries the quantity, so this is recorded
  // as the opening delta rather than added on top — applyMovement derives qty_after from the ledger and
  // writes the cache to match, which lands on exactly the number the insert set.
  applyMovement(db, {
