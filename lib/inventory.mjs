...
// See the twin in lib/sealed.mjs: a null on either side is not zero, so an unknown old cost adopts
// the new figure and an unknown new cost leaves the old one alone.
...
export function receiveInventory(db, opts = {}) {
  const { itemId = null, item = {}, quantity = 1, location = null,
