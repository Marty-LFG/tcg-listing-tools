...
import { scanListings, startRepricerScan } from './repricer-scan.mjs';
// Promotion lives in its own module with its I/O injected, so the lifecycle rules (stale price,
// duplicate, strand) are testable without eBay or Telegram. It cannot import this file back.
import { promoteFromScan, reconcileApplying } from './repricer-propose.mjs';
import { openDb } from './db.mjs';
import { configFile } from './config-paths.mjs';

// The runtime config moves with TCG_CONFIG_DIR (so tests get a disposable copy); the tracked example
// is always read from the repo's own data/, matching lib/postsale.mjs.
const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const CONFIG_PATH = configFile('repricer.config.json');
const CONFIG_EXAMPLE_PATH = path.join(ROOT, 'data', 'repricer.config.example.json');
...
const DEFAULT_CONFIG = {
  ...
  guardrails: {
    ...
    // 'medium', not 'high', and that is not a softening. eBay denies this app the
    // buy.marketplace.insights scope, so sold comps 403 and every lookup falls back to ASKING
    // prices — which costs 2 confidence points (lib/comps-singles.mjs), capping the achievable
    // score at 4 when 'high' needs 5. Demanding 'high' would mean the scan proposes nothing,
    // forever, with no error to explain it. The trust is restored by ALSO requiring the comps
    // engine's own `reliable` flag and min_comparable, which is what actually rejects a thin set.
    required_confidence: 'medium', max_increase_pct_per_run: 40,
    proposal_ttl_hours: 24, never_decrease: true,
    // Which competitor to undercut. 'cheapest_n' beats the Nth cheapest AU listing outright, which is
    // what keeps us on the first page when a buyer sorts by price; 'cluster' beats the cheapest
    // listing in the densest band, which on a card with a cheap tail prices us into the middle of the
    // pack (measured: it would have moved Forest of Vitality from 7th cheapest to 18th). N is 3 rather
    // than 1 so one lowball — a damaged card, a mis-title, someone dumping stock — cannot set the
    // price for the whole shelf.
    target_anchor: 'cheapest_n', anchor_n: 3,
    // Third place is enough to stay on page one, but when the top of the market is bunched together
    // third buys almost nothing and CHEAPEST wins the click. Within this much of the cheapest
    // listing, undercut the cheapest instead — the larger of the two applies, so one rule works on a
    // A$3 card and a A$300 one.
    beat_cheapest_within_aud: 0.5, beat_cheapest_within_pct: 5,
    // Every comp is an ASKING price — eBay's sold API is entitlement-gated (lib/ebay-token.mjs) — so a
    // raise wants a second opinion from something that actually transacted. 'require' refuses one that
    // nothing seconds, which is what a A$44.98 proposal on a card that last sold for A$25.00 needed.
    corroboration: 'require', corroboration_tolerance_pct: 30,
    // How many cards one promotion pass may put in the chat. A first run over a whole catalogue
    // could otherwise post dozens at once — past Telegram's ~20/min group limit, and past what
    // anyone will actually read before tapping.
    max_cards_per_run: 5,
    // Phase 5, off until the ReviseFixedPriceItem write is proven on a real listing. MUST be listed
    // here and not only in the .example: ensureConfigSeeded backfills from THIS object, and a
    // guardrail missing from it renders blank in settings and gets saved over.
    best_offer_scaling: false,
  },
  // PriceCharting's ungraded rung is eBay-sold-derived but USD and the US market, so it is a
  // corroborator rather than an anchor — and it costs a scrape per listing, hence opt-in.
  corroborate_pricecharting: false,
...
// data/repricer.config.json becomes gitignored (server-owned) so a settings save on the always-on box
// stops dirtying `git status` — a tracked runtime config is what forced skip-worktree onto
// refresh.config.json and then blocked every pull (.gitignore). Re-seeded on boot from the tracked
// example, and back-filled with keys added by a later release: loadConfig() merges DEFAULT_CONFIG so
// the server behaves correctly regardless, but the settings dashboard renders the FILE — a
// default-only key shows as an empty box, and saving that form would persist the blank over the
// default. Only ever ADDS absent keys; an existing value is never touched. (Mirrors lib/postsale.mjs.)
export function ensureConfigSeeded() {
  try {
    if (!fs.existsSync(CONFIG_PATH) && fs.existsSync(CONFIG_EXAMPLE_PATH)) {
      fs.copyFileSync(CONFIG_EXAMPLE_PATH, CONFIG_PATH);
      console.log('[repricer] seeded data/repricer.config.json from example');
    }
    if (fs.existsSync(CONFIG_PATH)) {
      const raw = JSON.parse(fs.readFileSync(CONFIG_PATH, 'utf8'));
      const missing = Object.keys(DEFAULT_CONFIG).filter((k) => !(k in raw));
      // Nested groups matter as much as top-level ones — a new guardrail is exactly the kind of key
      // that would otherwise render blank and be saved over.
      for (const group of ['guardrails', 'telegram']) {
        if (raw[group] && typeof raw[group] === 'object') {
          for (const k of Object.keys(DEFAULT_CONFIG[group])) {
            if (!(k in raw[group])) { raw[group][k] = DEFAULT_CONFIG[group][k]; missing.push(group + '.' + k); }
          }
        }
      }
      if (missing.length) {
        for (const k of missing) if (!k.includes('.')) raw[k] = DEFAULT_CONFIG[k];
        fs.writeFileSync(CONFIG_PATH, JSON.stringify(raw, null, 2) + '\n');
        console.log('[repricer] config backfilled with new defaults: ' + missing.join(', '));
      }
    }
  } catch (e) { console.warn('[repricer] config seed failed —', e?.message || e); }
}
...
// Money is stored as integer cents (GR3); this is the one place it becomes a string for a human.
const moneyC = (c, cur = 'AUD') => money((Number(c) || 0) / 100, cur);

// DIAG_TOKEN gate (inlined to avoid a lib/status.mjs <-> lib/repricer.mjs import cycle; same
// contract as lib/postsale.mjs: unset => 503 disabled, missing => 401, wrong => 403).
...
function renderProposalCard(p, decided) {
  ...
  s += `${moneyC(p.from_price_cents, p.currency)}  →  <b>${moneyC(p.to_price_cents, p.currency)}</b>`;
  if (p.from_price_cents > 0) s += `   <b>+${Math.round(((p.to_price_cents - p.from_price_cents) / p.from_price_cents) * 100)}%</b>`;
  s += '\n';
...
export async function applyReprice(env, p, cfg) {
  // Straight reads now — these are integer cents in the table, so there is no float to round out of
  // and no drift to make expectPriceCents refuse for reasons nobody can see.
  const fromCents = p.from_price_cents || 0;
  const toCents = p.to_price_cents || 0;
  ...
  // Phase 5: when the owner has turned floor-scaling on, a raise carries the Best Offer auto-accept
  // up with it by the same ratio — otherwise a successful raise just deepens the discount. The write
  // reverts itself if eBay takes the price but not the floor, so a `failed` card here means nothing
  // moved rather than half of it did.
  const r = await reviseTradingListing(env, openDb(), {
    listingId: p.item_id, priceCents: toCents, expectPriceCents: fromCents,
    bestOffer: (cfg.guardrails && cfg.guardrails.best_offer_scaling) ? 'scale' : 'leave',
  });
...
export function repricerPlugin(env) {
  return {
    ...
    configureServer(server) {
      ...
      ensureConfigSeeded();   // recreate/backfill the gitignored runtime config a deploy may have removed
      const cfg = loadConfig();
      // GR1: the comps engine reaches eBay through this server's own /api/ebay proxy, so the scan
      // self-fetches rather than duplicating the token plumbing.
      ...
      const router = async (req, res) => {
        try {
          ...
          // GET /config — non-secret status (never returns the bot token). Reads config LIVE rather
          // than the copy captured at configureServer: a settings save changed behaviour everywhere
          // else (the callback handler and /propose both re-read) while this endpoint kept reporting
          // the values from boot, which is how you end up debugging the wrong number.
          ...
          // Body (all optional): { title, from_price_cents, to_price_cents, summary, chat_id }
          if (p === '/test-alert' && method === 'POST') {
            ...
            const from = b.from_price_cents != null ? Math.round(+b.from_price_cents) : 1200;
            const to = b.to_price_cents != null ? Math.round(+b.to_price_cents) : 1849;
          ...
          if (p === '/propose' && method === 'POST') {
            ...
            const ins = db.prepare(`INSERT INTO reprice_proposals
              ...
              .run(listingId, live.title || listingId, fromCents, toCents, evidence, String(chatId), `+${ttlH} hours`);
            const id = ins.lastInsertRowid;
            const row = db.prepare('SELECT * FROM reprice_proposals WHERE id = ?').get(id);
            ...
            db.prepare('UPDATE reprice_proposals SET telegram_message_id=? WHERE id=?').run(r.result.message_id, id);
            return send(res, 201, { ok: true, id, message_id: r.result.message_id, title: live.title, from_price_cents: fromCents, to_price_cents: toCents, photo: !!r.photo });
          }

          // POST /propose-from-scan?scan_id=&limit= — promote a scan's `raise` verdicts into pending
          // proposals and put a card in Telegram for each.
          //
          // Deliberately SEPARATE from /scan, which stays read-only by contract (its integration test
          // asserts it sends no write). This is the step that commits, so it is its own decision:
          // pricing the store and offering to change prices are different acts, and only one of them
          // needs a human ready to look at a phone.
          if (p === '/propose-from-scan' && method === 'POST') {
            const auth = diagOk(env, req, url); if (!auth.ok) return send(res, auth.code, { error: auth.error });
            const c = loadConfig();
            const chatId = url.searchParams.get('chat_id') || (env.TELEGRAM_CHAT_ID || '').trim() || null;
            const canSend = telegramEnabled(env) && !!chatId;
            const max = Math.max(1, parseInt(url.searchParams.get('limit') || '', 10) || (c.guardrails && c.guardrails.max_cards_per_run) || 5);

            // Hazard 3, before anything new is created: a row stranded in 'applying' by a restart is
            // still holding its listing's uniqueness slot, so resolving it first is what lets that
            // listing be proposed again in this very pass.
            const strands = await reconcileApplying(db, { getLive: (id) => getListingState(env, id) });

            const out = await promoteFromScan({
              db, cfg: c, scanId: url.searchParams.get('scan_id') || null, chatId, max,
              getLive: (id) => getListingState(env, id),
              getImage: async (id) => { try { const im = await getItem(env, id); return im.ok ? (im.imageUrl || null) : null; } catch { return null; } },
              sendCard: canSend ? (async (row) => {
                const r = await sendCard(env, {
                  chatId: row.telegram_chat_id, photo: (safeParse(row.evidence) || {}).image_url,
                  text: renderProposalCard(row), buttons: proposalButtons(row.id, row),
                });
                return { ok: !!r.ok, message_id: r.ok ? r.result.message_id : null, description: r.description };
              }) : null,
            });
            if (!canSend) out.note = telegramEnabled(env)
              ? 'no chat_id — proposals created but no cards sent (set TELEGRAM_CHAT_ID or pass ?chat_id=)'
              : 'TELEGRAM_BOT_TOKEN not set — proposals created but no cards sent';
            return send(res, out.error ? 409 : 200, { triggered: 'repricer-propose', reconciled: strands, result: out });
          }

          // POST /scan — one read-only pass: refresh the mirror, price every active listing against
          // the market, write a price_checks row each. Writes NOTHING to eBay and creates no
          // proposals; this is shadow mode's evidence, available before any delivery surface exists.
          // DIAG-gated because it spends real API budget (one GetItem + one Browse per candidate).
          // ?limit=N caps the pass for a quick look.
          if (p === '/scan' && method === 'POST') {
            const auth = diagOk(env, req, url); if (!auth.ok) return send(res, auth.code, { error: auth.error });
            const limit = Math.max(0, parseInt(url.searchParams.get('limit') || '0', 10) || 0);
            const result = await scanListings({ env, rdb: db, tdb: openDb(), cfg: loadConfig(), base, limit });
            return send(res, result.ok ? 200 : 502, { triggered: 'repricer-scan', result });
          }
          // GET /price-checks?scan_id=&verdict=&limit= — read the audit trail back.
          if (p === '/price-checks' && method === 'GET') {
            const conds = [], args = [];
            const scanId = url.searchParams.get('scan_id'); if (scanId) { conds.push('scan_id = ?'); args.push(scanId); }
            const verdict = url.searchParams.get('verdict'); if (verdict) { conds.push('verdict = ?'); args.push(verdict); }
            const limit = Math.min(1000, parseInt(url.searchParams.get('limit') || '200', 10) || 200);
            // `1=1` so the optional filters can always append with AND — no branching on whether a
            // WHERE clause exists, which is where this kind of query builder usually goes wrong.
            const where = 'WHERE 1=1' + conds.map((c) => ' AND ' + c).join('');
            const rows = db.prepare(`SELECT * FROM price_checks ${where} ORDER BY id DESC LIMIT ?`).all(...args, limit);
            const byVerdict = db.prepare(`SELECT verdict, COUNT(*) n FROM price_checks ${where} GROUP BY verdict ORDER BY n DESC`).all(...args);
            const byCode = db.prepare(`SELECT code, COUNT(*) n FROM price_checks ${where} AND code IS NOT NULL GROUP BY code ORDER BY n DESC`).all(...args);
            return send(res, 200, { rows, byVerdict, byCode });
          }
      ...
      // The scan loop. loadCfg (not a config snapshot) so a settings save is picked up on the next
      // tick, and so SETTINGS.repricer.apply() can re-arm it without server context of its own.
      startRepricerScan({ env, rdb: db, tdb: openDb(), base, loadCfg: loadConfig });
