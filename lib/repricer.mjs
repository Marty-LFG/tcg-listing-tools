...
import { getUser, getListingState } from './ebay-trading.mjs';
  ...
  return {
    ...
    configureServer(server) {
      ...
      async function handleCallback(cq) {
        ...
        // Real reprice. A refusal is recorded as `failed` with eBay's own words, so a card that
        // didn't apply says why on its face instead of just losing its buttons.
        const ap = await applyReprice(env, p, liveCfg);
        if (!ap.ok) {
          db.prepare("UPDATE reprice_proposals SET status='failed', decided_by=?, decided_at=datetime('now'), error=? WHERE id=?").run(who, ap.error, id);
          await answerCallbackQuery(env, { id: cq.id, text: String(ap.error).slice(0, 190), showAlert: true });
          return editMessageText(env, { chatId: p.telegram_chat_id, messageId: p.telegram_message_id, text: renderProposalCard(p, { status: 'failed', who }) + '\n<i>' + escapeHtml(String(ap.error)) + '</i>', clearButtons: true });
        }
        const nowCents = (ap.after && ap.after.price_cents) != null ? ap.after.price_cents : Math.round(p.to_price * 100);
        db.prepare("UPDATE reprice_proposals SET status='applied', decided_by=?, decided_at=datetime('now'), applied_at=datetime('now') WHERE id=?").run(who, id);
        await answerCallbackQuery(env, { id: cq.id, text: 'Applied — eBay now says A$' + (nowCents / 100).toFixed(2) });
        return editMessageText(env, { chatId: p.telegram_chat_id, messageId: p.telegram_message_id, text: renderProposalCard(p, { status: 'applied', who }), clearButtons: true });
      }
      ...
      const router = async (req, res) => {
        try {
          ...
          if (p === '/propose' && method === 'POST') {
            ...
            const evidence = JSON.stringify({ summary: b.summary || 'Manual proposal — no comps run (the Phase 3 scan is not built)' });
            const ins = db.prepare(`INSERT INTO reprice_proposals
              (kind, item_id, title, from_price, to_price, currency, evidence, status, telegram_chat_id, expires_at)
            ...
            db.prepare('UPDATE reprice_proposals SET telegram_message_id=? WHERE id=?').run(r.result.message_id, id);
            return send(res, 201, { ok: true, id, message_id: r.result.message_id, title: live.title, from_price: fromCents / 100, to_price: toCents / 100 });
          }
