...
import {
  ...
  buildSetIndex, matchTitleToCard, sweepQuery, nearestReference, dropPct, isoDay,
} from './arb-core.mjs';
...
async function setCards(env, setId, memo) {
  ...
  const out = (!d || !d.cards) ? { cards: null, reason: 'set_not_cached' } : { cards: d.cards, at: d.at, ...pricedCount(d.cards) };
  if (out.cards) refreshIfPricesAreStale(d.at, () => getSetCards(env, setId, { refresh: true }));
  if (memo) memo.set(setId, out);
  return out;
}

// How much of a set TCGplayer prices at all. The newest set carries no prices for weeks after
// release, and every part of the page then went quiet with nothing to say why; this is the number
// the page names ("0 of 84 cards priced") instead of a bare "no market" on every row.
function pricedCount(cards) {
  let priced = 0;
  for (const c of cards) if (c && printingsWithSource(c).some((p) => p.which === 'market')) priced++;
  return { priced, total: cards.length };
}

const closer = (h, best) => (h.marginPct == null ? -Infinity : h.marginPct) > (best.margin_pct == null ? -Infinity : best.margin_pct);
...
async function loadScanCard(env, { card_id, printing_key }, memo) {
  ...
  if (!wanted) return { error: 'printing_not_found', card, facts, printings };
  if (!(wanted.marketUsd > 0)) return { error: 'no_market', card, facts, printings, wanted };
  return { card, facts, printings, wanted };
}

// The buyer's reference can price a printing pokemontcg.io cannot — the newest set carries no
// TCGplayer prices for weeks, and that is exactly where a stale buyer overpays. When the mode is on
// and a reference exists, a card with no live market is scanned anyway: one synthesised printing,
// no market, the reference paying, and the row chipped `no_live_market` because the too-good check
// cannot run without a market to compare against.
function standInForReference(loaded, want, ref) {
  if (!ref || !loaded.facts) return loaded;
  if (!(loaded.error === 'no_market' || loaded.error === 'printing_not_found')) return loaded;
  const printings = (loaded.printings || []).some((p) => p.key === want.printing_key)
    ? loaded.printings.map((p) => (p.key === want.printing_key ? { ...p, marketUsd: null } : p))
    : [{ key: want.printing_key, finish: '', edition: '', variant: '', marketUsd: null, which: null }];
  return { card: loaded.card, facts: loaded.facts, printings, wanted: { key: want.printing_key, marketUsd: null }, noLiveMarket: true };
}
...
async function resolveLines(env, db, base, { setId, lines }) {
  ...
  for (const raw of Array.isArray(lines) ? lines : []) {
    ...
    for (const card of found) {
      let { printings, chosen } = resolveCard(card, parsed.printing);
      const facts = cardFacts(card);
      // No TCGplayer prices (the newest set): the reference mode can still name a printing it knows.
      if (!printings.length && cfg.reference.enabled) {
        for (const key of [parsed.printing, 'holofoil', 'reverseHolofoil', 'normal'].filter(Boolean)) {
          if (referenceFor(db, card.id, key, cfg.reference.date, cfg.reference.max_gap_days)) { printings = [{ key, finish: '', variant: '', marketUsd: null, which: null }]; chosen = printings[0]; break; }
        }
      }
      const marketUsd = chosen && chosen.which === 'market' ? chosen.marketUsd : null;
      const ref = chosen && cfg.reference.enabled ? referenceFor(db, card.id, chosen.key, cfg.reference.date, cfg.reference.max_gap_days) : null;
      out.push({
        line, ...facts,
        reference: cfg.reference.enabled ? (ref ? { market_usd: ref.market_usd_cents / 100, day: ref.day, source: ref.source, gap_days: ref.gap_days, buyer_aud: Math.round(ref.market_usd_cents * cfg.reference.store_factor * cfg.buyer_pct) / 100, drop_pct: marketUsd != null ? dropPct(ref.market_usd_cents, toCents(marketUsd)) : null } : { missing: true }) : null,
  ...
  const setsTouched = [...memo].filter(([, v]) => v.cards).map(([id, v]) => ({ set_id: id, set_name: (findSet({ id }) || {}).name || (v.cards[0] && v.cards[0].set && v.cards[0].set.name) || id, priced: v.priced, total: v.total }));
  return { cards: out, unknown, sets: setsTouched, fx: fx ? { rate: fx.rate, date: fx.date } : null, set_id: current, buyer_pct: cfg.buyer_pct, reference: cfg.reference.enabled ? { date: cfg.reference.date, store_factor: cfg.reference.store_factor } : null };
}

// ---------------------------------------------------------------------------
// Market history — the finder's own, change-only, one row per card × printing per day it moved
// ---------------------------------------------------------------------------
// Fed from the pokemontcg.io set cache (no eBay calls; the cache already refreshes its prices daily),
// so a whole set costs one read. Written once per UTC day per set, at the start of a watch pass and
// on request. This is what makes the NEXT stale buyer snapshot answerable for every card in every
// set the finder touches, rather than for the one card the tracker happened to watch.

export async function takeMarketSnapshots(env, db, setIds, { memo = new Map(), force = false } = {}) {
  const day = utcDay();
  const out = { day, sets: 0, cards: 0, written: 0, skipped: [], errors: [] };
  const last = db.prepare('SELECT market_usd_cents FROM arb_market_snapshots WHERE card_id = ? AND printing_key = ? ORDER BY day DESC LIMIT 1');
  const ins = db.prepare('INSERT OR REPLACE INTO arb_market_snapshots (day, card_id, printing_key, market_usd_cents, market_at) VALUES (?, ?, ?, ?, ?)');
  const doneToday = db.prepare('SELECT 1 FROM arb_market_snapshots WHERE day = ? AND card_id LIKE ? LIMIT 1');
  for (const setId of [...new Set((setIds || []).filter(Boolean))]) {
    if (!force && doneToday.get(day, setId + '-%')) { out.skipped.push(setId); continue; }
    const { cards, reason } = await setCards(env, setId, memo);
    if (!cards) { out.errors.push({ set_id: setId, error: reason || 'set_not_cached' }); continue; }
    out.sets++;
    db.exec('BEGIN');
    try {
      for (const card of cards) {
        if (!card || !card.id) continue;
        const at = (card.tcgplayer && card.tcgplayer.updatedAt) || null;
        for (const p of printingsWithSource(card)) {
          if (p.which !== 'market' || !(p.marketUsd > 0)) continue;
          out.cards++;
          const cents = toCents(p.marketUsd);
          const prev = last.get(card.id, p.key);
          if (prev && prev.market_usd_cents === cents) continue;   // unchanged: the last row still stands
          ins.run(day, card.id, p.key, cents, at); out.written++;
        }
      }
      db.exec('COMMIT');
    } catch (e) { db.exec('ROLLBACK'); out.errors.push({ set_id: setId, error: String(e?.message || e) }); }
  }
  return out;
}

// Every set the finder touches: swept sets, the sets of watched cards, and the sets of stored hits.
function touchedSetIds(db) {
  const ids = new Set();
  for (const r of db.prepare('SELECT set_id FROM arb_sweep_sets').all()) ids.add(r.set_id);
  for (const r of db.prepare('SELECT DISTINCT set_id FROM arb_watch WHERE active = 1').all()) ids.add(r.set_id);
  for (const r of db.prepare('SELECT DISTINCT set_id FROM arb_hits WHERE status IN (\'new\', \'seen\')').all()) ids.add(r.set_id);
  return [...ids];
}

// The market as of the buyer's reference date for one card × printing, from the three sources
// (lib/db.mjs migrateArbitrage explains them). The tracker keys Pokémon rows by pokemontcg.io id and
// tells holo from normal only by its free-text variant, so its rows are matched the way its own
// mapper reads them (lib/normalize.mjs MAPPERS.pokemon): a 'holo' variant is a holo/reverse printing.
export function referenceFor(db, cardId, printingKey, refDate, maxGapDays) {
  const rows = [];
  for (const r of db.prepare('SELECT ref_date AS day, market_usd_cents FROM arb_reference_prices WHERE card_id = ? AND printing_key = ?').all(cardId, printingKey)) rows.push({ ...r, source: 'manual' });
  for (const r of db.prepare('SELECT day, market_usd_cents FROM arb_market_snapshots WHERE card_id = ? AND printing_key = ? AND day <= ? ORDER BY day DESC LIMIT 1').all(cardId, printingKey, isoDay(refDate))) rows.push({ ...r, source: 'snapshot' });
  for (const r of db.prepare('SELECT day, market_usd_cents FROM arb_market_snapshots WHERE card_id = ? AND printing_key = ? AND day > ? ORDER BY day ASC LIMIT 1').all(cardId, printingKey, isoDay(refDate))) rows.push({ ...r, source: 'snapshot' });
  const wantHolo = /holo|foil/i.test(printingKey);
  const watch = db.prepare('SELECT id, variant FROM watchlist WHERE game = \'pokemon\' AND identity_key = ?').all(cardId)
    .filter((w) => { const v = String(w.variant || '').toLowerCase(); const isHolo = v.includes('holo') && !v.includes('non') || v.includes('foil'); return isHolo === wantHolo; });
  for (const w of watch) {
    for (const r of db.prepare('SELECT ts AS day, market, currency FROM price_snapshots WHERE card_id = ? AND currency = \'USD\' AND ts BETWEEN ? AND ? ORDER BY ABS(julianday(ts) - julianday(?)) LIMIT 2')
      .all(w.id, new Date(Date.parse(isoDay(refDate)) - (maxGapDays + 1) * 86_400_000).toISOString(), new Date(Date.parse(isoDay(refDate)) + (maxGapDays + 2) * 86_400_000).toISOString(), isoDay(refDate))) {
      rows.push({ day: r.day, market_usd_cents: toCents(r.market), source: 'tracker' });
    }
  }
  return nearestReference(rows, refDate, maxGapDays);
}
...
// them on the row.
function hitRow({ facts, printing_key, row, f, h, fx, cfg, scanId, now, ref, noLiveMarket = false }) {
  const warnings = warningsFor(h, {
    ...
    marketAgeDays: noLiveMarket ? null : marketAgeDays(facts.market_at, Date.parse(now)), maxMarketAgeDays: cfg.max_market_age_days,
    printingAmbiguous: f.market.ambiguous, multi: MULTI_RE.test(row.title || ''), bestOffer: row.bestOffer,
    noReference: !!(cfg.reference && cfg.reference.enabled && !ref),
    refGapDays: ref ? ref.gap_days : null, refBefore: ref ? ref.before : null, noLiveMarket,
  });
  return {
    ...
    market_usd_cents: f.market.marketUsd > 0 ? toCents(f.market.marketUsd) : null, market_currency: 'USD', market_at: noLiveMarket ? null : facts.market_at,
    ...
    source: 'scan',
    buyer_basis: h.buyerBasis, ref_market_usd_cents: ref ? ref.market_usd_cents : null, ref_date: ref ? ref.day : null, ref_source: ref ? ref.source : null,
    store_factor: h.buyerBasis === 'reference' ? cfg.reference.store_factor : null,
  };
}
...
const HIT_COLS = ['game', 'set_id', 'card_id', 'number', 'name', 'printing_key', 'scored_printing_key', 'printing_ambiguous', 'item_id', 'legacy_item_id',
  ...
  'buyer_basis', 'ref_market_usd_cents', 'ref_date', 'ref_source', 'store_factor'];
...
// The buyer's reference for the printing a listing was scored against — null when the mode is off or
// nothing stands for the date. The lookup is per card × printing and a scan may hit the same one
// many times, so it is memoised for the life of the scan.
function referenceGetter(db, cfg) {
  const on = !!(cfg.reference && cfg.reference.enabled);
  const memo = new Map();
  return (cardId, printingKey) => {
    if (!on) return null;
    const k = cardId + '|' + printingKey;
    if (!memo.has(k)) memo.set(k, referenceFor(db, cardId, printingKey, cfg.reference.date, cfg.reference.max_gap_days));
    return memo.get(k);
  };
}
const mathsFor = (row, f, fx, cfg, ref) => hitMaths({
  priceCents: toCents(row.price), shipCents: toCents(row.ship), marketUsdCents: toCents(f.market.marketUsd), fx: fx.rate, buyerPct: cfg.buyer_pct, feeMode: cfg.buyer_fee_mode,
  refMarketUsdCents: ref ? ref.market_usd_cents : null, storeFactor: ref ? cfg.reference.store_factor : null,
});
...
async function runScan(env, db, base, { cards, mode, emit, shouldCancel, scanId }) {
  ...
  emit({ start: { scan_id: scanId, total: cards.length, fx, budget: budgetState(db, cfg), buyer_pct: cfg.buyer_pct, fee_mode: cfg.buyer_fee_mode, reference: cfg.reference.enabled ? { date: cfg.reference.date, store_factor: cfg.reference.store_factor } : null } });
  const excludeSeller = ownSeller(env, cfg);
  const memo = new Map();
  const refFor = referenceGetter(db, cfg);

  for (const want of cards) {
    if (shouldCancel && shouldCancel()) return finish(null, true);
    const now = nowIso();
    let loaded = await loadScanCard(env, want, memo);
    if (loaded.error) loaded = standInForReference(loaded, want, refFor(want.card_id, want.printing_key));
    if (loaded.error) {
      totals.errors++;
    ...
    for (const row of rows) {
      ...
      const ref = f.market ? refFor(facts.card_id, f.market.key) : null;
      if (!(f.market && (f.market.marketUsd > 0 || ref))) { dropped.no_market = (dropped.no_market || 0) + 1; continue; }
      kept++;
      const h = mathsFor(row, f, fx, cfg, ref);
      // The closest miss is worth a line: "kept 17, none qualified" says nothing about whether the
      // market is A$2 or A$40 away from a hit. Closest by MARGIN, not profit: by profit a A$2 card
      // A$2.20 short always "wins" over a A$300 card A$20 short, and the line said nothing.
      if (!best || closer(h, best)) best = { delivered_cents: h.deliveredCents, buyer_aud_cents: h.buyerAudCents, profit_cents: h.profitCents, margin_pct: h.marginPct, title: row.title, url: row.url, ambiguous: !!f.market.ambiguous, basis: h.buyerBasis };
      if (!qualifies(h, cfg)) { below++; continue; }
      current.push(hitRow({ facts, printing_key: wanted.key, row, f, h, fx, cfg, scanId, now, ref, noLiveMarket: !!loaded.noLiveMarket || !(f.market.marketUsd > 0) }));
    }
    ...
    const cardRef = refFor(facts.card_id, wanted.key);
    emit({ card: { ...facts, printing_key: wanted.key, market_usd: wanted.marketUsd, market_age_days: marketAgeDays(facts.market_at), reference: cardRef ? { market_usd: cardRef.market_usd_cents / 100, day: cardRef.day, source: cardRef.source } : (cfg.reference.enabled ? { missing: true } : null), query: q.q, rows: rows.length, kept, below_floor: below, hits: current.length, hits_new: rec.inserts.length, gone: rec.gone.length, dropped, best } });
...
async function runSweep(env, db, base, { sets, emit, shouldCancel, scanId }) {
  ...
  emit({ start: { scan_id: scanId, mode: 'sweep', total: sets.length, fx, budget: budgetState(db, cfg), buyer_pct: cfg.buyer_pct, fee_mode: cfg.buyer_fee_mode, queries: cfg.sweep.queries, reference: cfg.reference.enabled ? { date: cfg.reference.date, store_factor: cfg.reference.store_factor } : null } });
  const excludeSeller = ownSeller(env, cfg);
  const memo = new Map();
  const refFor = referenceGetter(db, cfg);
  ...
  const indexFor = async (sw) => {
    ...
    const { cards, priced, total } = await setCards(env, sw.set_id, memo);
    const built = cards ? { set: sw, meta: metaOf(sw), index: buildSetIndex(cards, metaOf(sw)), priced, total } : null;
  ...
  for (const sw of sets) {
    ...
    for (const mode of cfg.sweep.queries) {
      ...
      for (let page = 0; page < pagesCap && !reachedThisQuery; page++) {
        ...
        for (const it of items) {
          ...
          if (!printings) {
            printings = printingsWithSource(m.card).map((p) => ({ ...p, marketUsd: p.which === 'market' ? p.marketUsd : null }));
            // No TCGplayer prices at all (the newest set): the buyer's reference may still know it.
            if (!printings.length && cfg.reference.enabled) for (const key of ['holofoil', 'reverseHolofoil', 'normal']) if (refFor(m.card.id, key)) { printings = [{ key, finish: '', edition: '', variant: '', marketUsd: null, which: null }]; break; }
            printingsById.set(m.card.id, printings);
          }
          if (!printings.length) { dropped.no_market = (dropped.no_market || 0) + 1; continue; }
          ...
          if (!f.ok) { dropped[f.reason] = (dropped[f.reason] || 0) + 1; continue; }
          const ref = f.market ? refFor(m.card.id, f.market.key) : null;
          if (!(f.market && (f.market.marketUsd > 0 || ref))) { dropped.no_market = (dropped.no_market || 0) + 1; continue; }
          kept++;
          const h = mathsFor(row, f, fx, cfg, ref);
          if (!best || closer(h, best)) best = { name: facts.name, number: facts.number, printing_key: f.market.key, delivered_cents: h.deliveredCents, buyer_aud_cents: h.buyerAudCents, profit_cents: h.profitCents, margin_pct: h.marginPct, title: row.title, url: row.url, ambiguous: !!f.market.ambiguous, basis: h.buyerBasis };
          if (!qualifies(h, cfg)) { dropped.below_floor = (dropped.below_floor || 0) + 1; continue; }
          current.push({ ...hitRow({ facts, printing_key: f.market.key, row, f, h, fx, cfg, scanId, now, ref, noLiveMarket: !(f.market.marketUsd > 0) }), source: 'sweep' });
        }
    ...
    if (okPages) {
      totals.sets++;
      db.prepare('UPDATE arb_sweep_sets SET watermark = ?, last_run_at = ?, last_scan_id = ?, last_total = ?, last_calls = ?, last_rows = ?, last_new_rows = ?, last_matched = ?, last_hits = ?, last_hits_new = ?, last_dropped = ?, last_priced_cards = ?, last_total_cards = ? WHERE set_id = ?')
        .run(newest, now, scanId, total, calls, rows, newRows, matched, current.length, hitsNew, JSON.stringify(dropped), own.priced, own.total, sw.set_id);
    } else totals.errors++;
    emit({ set: { set_id: sw.set_id, set_name: sw.set_name, total, calls, pages_ok: okPages, rows, new_rows: newRows, matched, kept, hits: current.length, hits_new: hitsNew, dropped, priced_cards: own.priced, total_cards: own.total, watermark: newest, reached_watermark: reached, first_run: !sw.watermark, shared_number_query_with: sharedWith, best, error: okPages ? null : (aborted || 'ebay_unreachable') } });
...
export function getArbWatchState() {
  ...
  let reference = null; try { if (_db && cfg) reference = { ...cfg.reference, history: historyStats(_db) }; } catch { /* db closed */ }
  return {
    ...
    budget, sweep, reference, scan: getArbJobState(),
  };
}

function historyStats(db) {
  const s = db.prepare('SELECT COUNT(*) AS rows, MIN(day) AS first_day, MAX(day) AS last_day, COUNT(DISTINCT card_id || printing_key) AS series FROM arb_market_snapshots').get() || {};
  const manual = db.prepare('SELECT COUNT(*) AS n FROM arb_reference_prices').get().n;
  return { rows: s.rows || 0, series: s.series || 0, first_day: s.first_day || null, last_day: s.last_day || null, manual };
}

// The cards whose market has fallen since the reference date by at least `minDrop` percent, from
// every reference source, with what the buyer would pay off the stale figure against what the card
// costs at today's market. A positive gap is the theoretical trade before eBay's asking prices are
// consulted; the scan is what turns it into a hit or not.
function fallenSince(db, cfg, { minDrop } = {}) {
  const rf = cfg.reference;
  const drop = minDrop != null ? +minDrop : rf.min_drop_pct;
  const keys = new Map();
  const add = (card_id, printing_key) => keys.set(card_id + '|' + printing_key, { card_id, printing_key });
  for (const r of db.prepare('SELECT DISTINCT card_id, printing_key FROM arb_reference_prices').all()) add(r.card_id, r.printing_key);
  for (const r of db.prepare('SELECT DISTINCT card_id, printing_key FROM arb_market_snapshots WHERE day <= ?').all(isoDay(rf.date))) add(r.card_id, r.printing_key);
  for (const w of db.prepare('SELECT DISTINCT identity_key, variant FROM watchlist WHERE game = \'pokemon\'').all()) {
    const v = String(w.variant || '').toLowerCase(); const holo = (v.includes('holo') && !v.includes('non')) || v.includes('foil');
    add(w.identity_key, holo ? 'holofoil' : 'normal');
  }
  return { drop, keys: [...keys.values()] };
}

async function fallenList(env, db, cfg, fx, opts = {}) {
  const { drop, keys } = fallenSince(db, cfg, opts);
  const memo = new Map();
  const out = [];
  for (const k of keys) {
    const ref = referenceFor(db, k.card_id, k.printing_key, cfg.reference.date, cfg.reference.max_gap_days);
    if (!ref) continue;
    const loaded = await loadScanCard(env, k, memo);
    const facts = loaded.facts || null;
    // A holo reference from the tracker may sit on a card whose only holo printing is the reverse.
    let printing = loaded.wanted || (loaded.printings || []).find((p) => p.key === k.printing_key) || null;
    if (!printing && loaded.printings && /holo/.test(k.printing_key)) printing = loaded.printings.find((p) => /holo/i.test(p.key) && p.marketUsd > 0) || null;
    const nowCents = printing && printing.marketUsd > 0 ? toCents(printing.marketUsd) : null;
    const d = dropPct(ref.market_usd_cents, nowCents);
    const buyerCents = Math.round(ref.market_usd_cents * cfg.reference.store_factor * cfg.buyer_pct);
    const nowAudCents = nowCents != null && fx ? Math.round(nowCents * fx.rate) : null;
    out.push({
      card_id: k.card_id, printing_key: printing ? printing.key : k.printing_key, name: facts ? facts.name : k.card_id, number: facts ? facts.number : null, set_id: facts ? facts.set_id : setIdOfCard(k.card_id), set_name: facts ? facts.set_name : null, image: facts ? facts.image : null,
      ref_market_usd: ref.market_usd_cents / 100, ref_day: ref.day, ref_source: ref.source, ref_gap_days: ref.gap_days,
      market_usd: nowCents != null ? nowCents / 100 : null, market_at: facts ? facts.market_at : null, drop_pct: d,
      buyer_aud: buyerCents / 100, market_aud: nowAudCents != null ? nowAudCents / 100 : null,
      gap_aud: nowAudCents != null ? (buyerCents - nowAudCents) / 100 : null,
      error: loaded.error && loaded.error !== 'no_market' ? loaded.error : null,
    });
  }
  const filtered = out.filter((r) => r.drop_pct == null || r.drop_pct >= drop);
  filtered.sort((a, b) => (b.gap_aud ?? -Infinity) - (a.gap_aud ?? -Infinity));
  return { date: cfg.reference.date, store_factor: cfg.reference.store_factor, buyer_pct: cfg.buyer_pct, min_drop_pct: drop, fx: fx ? { rate: fx.rate, date: fx.date } : null, candidates: filtered, considered: out.length };
}
...
export async function runWatchPass({ env = _env, db = _db, base = _base } = {}) {
  ...
  const result = { snapshots: null, cards: null, sweep: null, alerts: null };

  // History first, so today's market is on record before anything is scored against it.
  try { const snap = await takeMarketSnapshots(env, db, touchedSetIds(db)); result.snapshots = { day: snap.day, sets: snap.sets, written: snap.written, skipped: snap.skipped.length, errors: snap.errors.length }; }
  catch (e) { result.snapshots = { error: String(e?.message || e) }; }
        ...
        try {
          ...
          if (p === '/config' && method === 'GET') {
            ...
            return send(res, 200, {
              ...
              fx: fx ? { rate: fx.rate, date: fx.date } : null,   // the strip shows the rate before anything is resolved
          ...
          // The buyer's stale reference: history, typed-in figures, and the fallen-since list.
          if (p === '/snapshot' && method === 'POST') {
            const b = await readJsonBody(req, 1e5);
            const sets = Array.isArray(b.sets) && b.sets.length ? b.sets.map((x) => String(x || '').trim()).filter(isSetId) : touchedSetIds(db);
            if (!sets.length) return send(res, 400, { error: 'no sets to snapshot — sweep or watch something first, or pass sets:[]', code: 'no_sets' });
            const out = await takeMarketSnapshots(env, db, sets, { force: b.force === true });
            return send(res, 200, { ok: true, ...out, history: historyStats(db) });
          }
          if (p === '/reference' && method === 'GET') {
            const rows = db.prepare('SELECT * FROM arb_reference_prices ORDER BY ref_date DESC, added_at DESC').all();
            return send(res, 200, { references: rows.map((r) => ({ ...r, market_usd: r.market_usd_cents / 100 })), config: loadArbConfig().reference, history: historyStats(db) });
          }
          if (p === '/reference' && method === 'POST') {
            const b = await readJsonBody(req, 1e5);
            const cfg = loadArbConfig();
            const cardId = String(b.card_id || '').trim(), printingKey = String(b.printing_key || '').trim();
            const day = isoDay(b.date || cfg.reference.date);
            const usd = +b.market_usd;
            if (!cardId || !printingKey) return send(res, 400, { error: 'card_id and printing_key required' });
            if (!/^\d{4}-\d{2}-\d{2}$/.test(day) || !isFinite(Date.parse(day))) return send(res, 400, { error: 'date must be YYYY-MM-DD' });
            if (!(usd > 0)) return send(res, 400, { error: 'market_usd must be a positive number (US dollars, as TCGplayer shows it)' });
            db.prepare(`INSERT INTO arb_reference_prices (card_id, printing_key, ref_date, market_usd_cents, note) VALUES (?, ?, ?, ?, ?)
                        ON CONFLICT(card_id, printing_key, ref_date) DO UPDATE SET market_usd_cents = excluded.market_usd_cents, note = excluded.note, added_at = datetime('now')`)
              .run(cardId, printingKey, day, toCents(usd), b.note ? String(b.note).slice(0, 200) : null);
            const row = db.prepare('SELECT * FROM arb_reference_prices WHERE card_id = ? AND printing_key = ? AND ref_date = ?').get(cardId, printingKey, day);
            return send(res, 200, { ok: true, reference: { ...row, market_usd: row.market_usd_cents / 100 } });
          }
          if ((m = p.match(/^\/reference\/(\d+)$/)) && method === 'DELETE') {
            const r = db.prepare('DELETE FROM arb_reference_prices WHERE id = ?').run(+m[1]);
            return r.changes ? send(res, 200, { ok: true }) : send(res, 404, { error: 'no such reference' });
          }
          if (p === '/fallen' && method === 'GET') {
            const cfg = loadArbConfig();
            const fx = await loadFx(base);
            const minDrop = url.searchParams.get('min_drop');
            return send(res, 200, await fallenList(env, db, cfg, fx, { minDrop: minDrop != null && minDrop !== '' ? +minDrop : null }));
          }
          ...
          if (p === '/sweep' && method === 'POST') {
            ...
            const added = [], failed = [], memo = new Map();
            const ins = db.prepare(`INSERT INTO arb_sweep_sets (set_id, set_name, printed_total, total, ptcgo_code, enabled, last_priced_cards, last_total_cards) VALUES (?, ?, ?, ?, ?, 1, ?, ?)
                                    ON CONFLICT(set_id) DO UPDATE SET enabled = 1, set_name = excluded.set_name, printed_total = excluded.printed_total, total = excluded.total, ptcgo_code = excluded.ptcgo_code,
                                                                     last_priced_cards = excluded.last_priced_cards, last_total_cards = excluded.last_total_cards`);
            for (const id of ids) {
              ...
              const { cards, reason, priced, total } = await setCards(env, id, memo);
              ...
              ins.run(id, name, meta.printedTotal || c0.printedTotal || null, meta.total || c0.total || null, meta.ptcgoCode || c0.ptcgoCode || null, priced, total);
          ...
          return send(res, 404, { error: 'unknown endpoint', endpoints: ['/config', '/resolve', '/scan', '/scan/state', '/scan/:id/stream', '/scan/:id/cancel', '/scan/:id', '/watch', '/watch/:id', '/watch/run', '/sweep', '/sweep/:set_id', '/snapshot', '/reference', '/reference/:id', '/fallen', '/hits', '/hits/:id', '/hits/:id/check-qty'] });
        } catch (e) {
