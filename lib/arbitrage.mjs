...
async function resolveLines(env, db, base, { setId, lines }) {
  ...
  return { cards: out, unknown, fx: fx ? { rate: fx.rate, date: fx.date } : null, set_id: current, buyer_pct: cfg.buyer_pct, reference: cfg.reference.enabled ? { date: cfg.reference.date, store_factor: cfg.reference.store_factor } : null };
}
...
async function runScan(env, db, base, { cards, mode, emit, shouldCancel, scanId }) {
  ...
  for (const want of cards) {
    ...
    for (const row of rows) {
      ...
      if (!best || h.profitCents > best.profit_cents) best = { delivered_cents: h.deliveredCents, buyer_aud_cents: h.buyerAudCents, profit_cents: h.profitCents, margin_pct: h.marginPct, title: row.title, url: row.url, ambiguous: !!f.market.ambiguous, basis: h.buyerBasis };
    ...
    for (const mode of cfg.sweep.queries) {
      ...
      for (let page = 0; page < pagesCap && !reachedThisQuery; page++) {
        ...
        for (const it of items) {
          ...
          kept++;
          const h = mathsFor(row, f, fx, cfg, ref);
          if (!best || h.profitCents > best.profit_cents) best = { name: facts.name, number: facts.number, printing_key: f.market.key, delivered_cents: h.deliveredCents, buyer_aud_cents: h.buyerAudCents, profit_cents: h.profitCents, margin_pct: h.marginPct, title: row.title, url: row.url, ambiguous: !!f.market.ambiguous, basis: h.buyerBasis };
