...
import { QUERY_MODES, SWEEP_QUERY_MODES } from './arb-core.mjs';
...
export const ARB_DEFAULTS = Object.freeze({
  ...
  sweep: { enabled: false, queries: ['name', 'number'], max_pages_per_set: 5, first_run_pages: 1 },
  // The buyer's stale reference (see migrateArbitrage in lib/db.mjs). Off = the buyer pays buyer_pct
  // of TODAY's market at the live rate. On = buyer_pct of the market as of `date`, converted at
  // `store_factor` (the flat 1.5 measured off the shelf), wherever a reference figure exists.
  reference: { enabled: false, date: '2026-07-24', store_factor: 1.5, max_gap_days: 7, min_drop_pct: 15 },
});
...
// A config seeded before a key existed is missing that key for good: the loader fills it from
// ARB_DEFAULTS, but the settings form reads the FILE, so the form showed blanks and a save was then
// refused for the very block it could not show. Healed on boot: any top-level key the defaults know
// and the file lacks is written in (a nested block is added whole, never merged field by field, so
// an owner's edits inside a block they do have are untouched). Same shape as the postage-label heal.
export function healArbConfig() {
  const path = arbConfigPath();
  if (!fs.existsSync(path)) return false;
  let raw;
  try { raw = JSON.parse(fs.readFileSync(path, 'utf8')); } catch { return false; }
  if (!raw || typeof raw !== 'object') return false;
  const missing = Object.keys(ARB_DEFAULTS).filter((k) => !(k in raw));
  if (!missing.length) return false;
  for (const k of missing) raw[k] = structuredClone(ARB_DEFAULTS[k]);
  const tmp = path + '.tmp';
  fs.writeFileSync(tmp, JSON.stringify(raw, null, 2) + '\n');
  fs.renameSync(tmp, path);
  console.log('[arb] healed data/arbitrage.config.json — added ' + missing.join(', '));
  return true;
}
...
export function loadArbConfig() {
  try {
    ...
    return { ...ARB_DEFAULTS, ...c, watch: { ...ARB_DEFAULTS.watch, ...(c.watch || {}) }, sweep: { ...ARB_DEFAULTS.sweep, ...(c.sweep || {}) }, reference: { ...ARB_DEFAULTS.reference, ...(c.reference || {}) } };
  } catch (e) {
...
// Returns an error string or null — the shape lib/status.mjs SETTINGS.validate expects. The nested
// blocks are validated as the LOADER will see them (defaults underneath), so a file that predates a
// block still saves; the top-level money and budget keys have no such grace, because a default
// silently standing in for a floor the owner thinks they set is the wrong kind of forgiving.
export function validateArbConfig(input) {
  if (!input || typeof input !== 'object') return 'not an object';
  const c = { ...input, watch: { ...ARB_DEFAULTS.watch, ...(input.watch || {}) }, sweep: { ...ARB_DEFAULTS.sweep, ...(input.sweep || {}) }, reference: { ...ARB_DEFAULTS.reference, ...(input.reference || {}) } };
  ...
  if (c.dashboard_url != null && c.dashboard_url !== '') {
    if (typeof c.dashboard_url !== 'string') return 'dashboard_url must be a string';
    let u = null;
    try { u = new URL(c.dashboard_url); } catch { return `dashboard_url "${c.dashboard_url}" is not a URL`; }
    if (u.protocol !== 'http:' && u.protocol !== 'https:') return 'dashboard_url must be http or https';
  }
  const w = c.watch;
  if (input.watch != null && typeof input.watch !== 'object') return 'watch must be an object';
  ...
  const sw = c.sweep;
  if (input.sweep != null && typeof input.sweep !== 'object') return 'sweep must be an object';
  if (typeof sw.enabled !== 'boolean') return 'sweep.enabled must be boolean';
  if (!Array.isArray(sw.queries) || !sw.queries.length || sw.queries.some((q) => !SWEEP_QUERY_MODES.includes(q))) return `sweep.queries must be a non-empty list drawn from ${SWEEP_QUERY_MODES.join(', ')}`;
  if (!intIn(sw.max_pages_per_set, 1, 50)) return 'sweep.max_pages_per_set must be a whole number, 1–50 (200 listings a page)';
  if (!intIn(sw.first_run_pages, 1, 50)) return 'sweep.first_run_pages must be a whole number, 1–50';
  const rf = c.reference;
  if (input.reference != null && typeof input.reference !== 'object') return 'reference must be an object';
  if (typeof rf.enabled !== 'boolean') return 'reference.enabled must be boolean';
  if (!/^\d{4}-\d{2}-\d{2}$/.test(String(rf.date || '')) || !isFinite(Date.parse(rf.date))) return 'reference.date must be a YYYY-MM-DD date';
  if (!(num(rf.store_factor) && rf.store_factor > 0 && rf.store_factor <= 5)) return 'reference.store_factor must be a number in (0, 5] — 1.5 means the buyer converts USD at 1.5';
  if (!intIn(rf.max_gap_days, 0, 365)) return 'reference.max_gap_days must be a whole number of days, 0–365';
  if (!(num(rf.min_drop_pct) && rf.min_drop_pct >= 0 && rf.min_drop_pct <= 100)) return 'reference.min_drop_pct must be a percentage 0–100';
  return null;
}
