...
/**
 * Closes the process singleton and forgets it, so a later openPostsaleDb() opens a fresh handle.
 *
 ...
 *
 * Clearing _pdb is the part that matters. Closing the handle without it would leave every later
 * openPostsaleDb() returning a CLOSED database, which fails in a way nobody would connect to a teardown.
 */
export function closePostsaleDb() {
  if (!_pdb) return false;
  try { _pdb.close(); } catch { /* already closed */ }
  _pdb = null;
  return true;
}
