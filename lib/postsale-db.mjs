...
CREATE TABLE IF NOT EXISTS notify_events (
  ...
  handled_at TEXT,
  action TEXT,                                        -- order_by_id|message_poll|record_only|postsale_disabled
  error TEXT,
...
function migratePostsale(db) {
  ...
  //
  // NOT stamped for the OTHER event that arrives through the same flag: a bare "mark as dispatched",
  // where the seller bulk-ticks parcels that have already gone. That one carries no tracking number,
  // and stamping it parked those orders in the queue forever, because only a human tap clears this
  // column. refreshOrder tells the two apart; settleLabelBought re-applies the rule to older rows.
