...
async function dressBand(sharp, variant, side, frame, bandH, text, cfg, hasFont, art, markMode) {
  ...
  if (side === 'top') {
    ...
    // THE SET BLOCK, laid out like the eBay square unrolled: the set WORDMARK at the left end, the
    // set SYMBOL (or the boxed code for the games that print one instead — SWU's SOR, Riftbound's
    // OGN) at the right, and the set name over the printed number centred between them. Those two
    // are different kinds of thing and are NOT interchangeable: a wordmark is the set's name as
    // type, a symbol is the mark printed on the card. Either may be absent — early Pokémon sets
    // printed no symbol, Scryfall publishes no Magic wordmark — and the block simply stays centred
    // on the frame, so a missing one never shifts the text off-axis.
    const setLogo = (art && art.logo) || null;
    const setMark = (art && (art.symbol || art.abbrev)) || null;
    const logoW = setLogo ? setLogo.width + pad : 0;
    const markW = setMark ? setMark.width + pad : 0;
    // Reserve the WIDER of the two on both sides, so the centred block cannot slide under either.
    const reserve = Math.max(logoW, markW);
    const budget = frame.width - 2 * pad - 2 * reserve;
    ...
    if (setLogo) {
      layers.push({ input: setLogo.buffer, left: pad, top: Math.round((bandH - setLogo.height) / 2) });
      drawn.setLogo = 'wordmark';
    }
    if (setMark) {
      layers.push({ input: setMark.buffer, left: frame.width - pad - setMark.width, top: Math.round((bandH - setMark.height) / 2) });
...
export async function composeOgImage(input, meta = {}, options = {}) {
  ...
  const contentHash = composeHash({
    ...
    // resolveOgGeometry reads shopify.og.* and the encoder reads shopify.quality; neither is in
    // `layout`, so without this the same card at two different og geometries shares one key.
    shopify: shopifyPixelKey(cfg, 'og'),
  });
...
export async function composeBandImage(input, meta = {}, options = {}) {
  ...
  // TWO DIFFERENT PIECES OF SET ART, exactly as the eBay square uses them:
  //   · the WORDMARK (the set's name as type — "Scarlet & Violet Stellar Crown"), foot of the left
  //     rail there, left end of the bottom band here;
  //   · the SYMBOL (the little mark printed on the card — the boxed SCR), foot of the right rail
  //     there, right end of the bottom band here.
  // They are not interchangeable, and putting the symbol at both ends threw the wordmark away.
  // The wordmark is WIDE and the symbol is square-ish, so each gets its own box and keeps its own
  // aspect — squeezing one into the other's shape is worse than letting each be itself.
  const markSize = Math.round(bandH * 0.56);
  const logoBox = { width: Math.round(frame.width * 0.24), height: Math.round(bandH * 0.72) };
  const setArt = await resolveSetArt(sharp, meta, layout, true, cfg, { symbol: markSize, logo: logoBox });
  const abbrev = (!setArt.symbol && meta.setAbbrev && avail.text)
    ? await renderBadgeAbbrev(sharp, meta.setAbbrev, layout, cfg, markSize)
    : null;
  const art = { symbol: setArt.symbol, abbrev, logo: setArt.setLogo };
  const markKey = `${setArt.symbol ? setArt.symbol.digest : ''}|${setArt.setLogo ? setArt.setLogo.digest : ''}|${abbrev ? abbrev.code : ''}`;
  const sourceBytes = Buffer.isBuffer(input) ? input
    : input instanceof Uint8Array ? Buffer.from(input)
      : fs.readFileSync(input);
  ...
  const contentHash = composeHash({
    ...
    // bandFraction sets bandH and the card box a few lines below, and shopify.quality picks the
    // encoder setting; neither reaches `layout`, so both have to be keyed here or two different
    // framings of one card collide on a single hash.
    shopify: shopifyPixelKey(cfg, 'band'),
  });
  const version = composeVersion(variant, assetDigest, target.id);
