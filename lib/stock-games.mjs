...
const pokemon = {
  ...
  setCardsUrl: (setId, refresh, lang = 'en', setMeta) => {
    ...
    if (m.tcgdexId || m.code) { qs.set('src', 'indexed'); qs.set('tcgdexId', String(m.tcgdexId || m.code)); }
  ...
  // bare number.
  intlCard: ({ lang, set, localId, name, rarity, image, source, full } = {}) => ({
  ...
  displayName: (c, dex) => {
    ...
    if (c.source === 'pricecharting') return String(c.name || '').trim();
  ...
  normalizeIntlCard({ card, setMeta, lang, dex } = {}) {
    ...
    const isPc = c.source === 'pricecharting';
    ...
    return {
      ...
      facts: {
        ...
        native_name: full.name || (!isPc ? c.name : '') || ni.native || '',
        romaji: ni.romaji || '',
