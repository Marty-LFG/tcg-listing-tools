...
import {
  formatCardNumber, langCode, variantToken, pkmSetFacts, pkmIdForPrinted, pkmSharedCount, pkmRarityFor,
...
// Moved verbatim from lib/listings.mjs (which re-exports it) so the browser-safe arbitrage engine can
// ask the same question the server's priceItem asks — one rule, not a mirror (GR9).
export function finishHint(s) {
...
// pokemontcg.io gives a subset the same ptcgoCode as its parent set: Celebrations and its Classic
// Collection are both CEL, 30th Celebration and its Classic Collection both 30C, and every Trainer
// Gallery, Shiny Vault and Galarian Gallery shares its main set's code. The Runner's catch line
// switches set on a typed code, so a shared code always landed on whichever set sorted first and
// the subset could never be reached by typing. The set whose name is shortest (the parent) keeps
// the printed code; each subset answers to its own set id instead (ME55C, SWSH9TG), which is a real
// identifier and cannot be mistaken for a card number. Rows keep the printed code — normalizeCard
// reads it off the card's own set, not off this list.
export function uniqueSetCodes(list) {
  const by = new Map();
  for (const s of list) { const k = s.code; if (!by.has(k)) by.set(k, []); by.get(k).push(s); }
  for (const group of by.values()) {
    if (group.length < 2) continue;
    const keep = group.slice().sort((a, b) => String(a.label).length - String(b.label).length
      || String(a.value).length - String(b.value).length)[0];
    for (const s of group) if (s !== keep && s.value) s.code = String(s.value).toUpperCase();
  }
  return list;
}
...
const pokemon = {
  ...
  setsFrom: (json) => uniqueSetCodes(((json && json.data) || []).map((s) => ({
  ...
  cardNumber: (c) => (c && c.__intl
    ...
    : formatCardNumber(c.number, c.set || {}, { source: 'ptcg', rarity: c.rarity, id: c.id })),
  ...
  //
  // A holo-only set (pkmSetFacts: 30th Celebration and its Classic Collection, Celebrations, the JP
  // and KO M6a) answers with ONE Holofoil printing when the source has no printing data yet — a
  // brand-new set carries no TCGplayer prices on pokemontcg.io for a week or more, and without this
  // every common in it fell to the rarity guess and was titled and priced non-holo. It is a fact
  // about the set, checked against TCGplayer, not a guess, so it is not flagged fromRarity. The
  // moment real price keys arrive they win, because they are consulted first.
  printingsFor: (c) => {
    const got = c && c.__intl ? intlPrintingsFor(c.full) : printingsFor(c);
    if (got.length || !pokemon.holoOnly(c)) return got;
    return [{ key: 'holofoil', finish: 'Holofoil', edition: '', variant: variantToken('', 'Holofoil'), marketUsd: null }];
  },
  // Is every card in this card's set holofoil? For the uploader, which sets its Finish dropdown
  // itself, and for printingsFor above.
  holoOnly: (c) => {
    if (!c) return false;
    const key = c.__intl
      ? dataLangOf(c.__lang) + ':' + String((c.__set && c.__set.code) || '').toUpperCase()
      : String((c.set && c.set.id) || '');
    const f = pkmSetFacts(key);
    return !!(f && f.holo);
  },
  // The card a typed PRINTED number names, where the set reuses numbers (Classic Collection #106 is
  // three cards; 106/106 is Palkia LV.X). '' when the number alone is enough or means nothing here.
  printedIdFor: (setId, typed) => pkmIdForPrinted(setId, typed),
  sharedNumberCount: (setId, typed) => pkmSharedCount(setId, typed),
  ...
  normalizeCard(c, setMeta, dex) {
    ...
    return {
      ...
      rawNumber: c.number,
      // Corrected where upstream is wrong: pokemontcg.io calls the three RGB Mews "Common".
      rarity: pkmRarityFor(c),
