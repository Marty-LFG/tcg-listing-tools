...
function migrateKeepers(db) {
  ...
  // What we last WROTE to the ORDER, same job as referral_code_written and for the same reason: the
  // per-order award is a projection of the ledger onto Shopify, so something has to remember what
  // Shopify was last told or every pass rewrites every order it has ever seen.
  addColumnIfMissing(db, 'keepers_orders', 'xp_written', 'INTEGER');
  addColumnIfMissing(db, 'keepers_orders', 'points_written', 'INTEGER');
}
...
export function orderLedger(db, orderGid) {
  ...
  const accrual = db.prepare(`
    SELECT COALESCE(SUM(xp_delta),0) AS xp, COALESCE(SUM(points_delta),0) AS points,
           ...
           -- The basis the accrual was actually computed on. Load-bearing: a refund has to be scored
           -- against what was EARNED, never against what the order is worth now. Shopify's
           -- currentSubtotalPriceSet is already net of refunds, so scoring a reversal against it made
           -- a FULLY refunded order reverse nothing at all — basis 0, share undefined, zero taken back.
           MAX(basis_cents) AS basis_cents,
           MIN(id) AS event_id
    FROM keepers_events
  ...
  return {
    ...
    accruedXp,
    accruedPoints,
    // The ORIGINAL basis, so a reversal can be scored proportionally against the accrual it reverses.
    basisCents: accrual?.basis_cents ?? null,
...
/**
 * ordersNeedingAwardProjection — orders whose ledger net differs from what Shopify was last told.
 *
 * WHY THIS IS A DIFF AND NOT A DIRTY FLAG. A flag has to be set by everything that could change the
 * answer — the accrual, every reversal, a cancellation, a manual correction — and the bug is always
 * the one path that forgot. Comparing the ledger to the projection cache cannot forget: anything that
 * moves the net shows up here on the next pass, including a row written by a future code path nobody
 * has thought of yet. It is the same self-healing shape as the projection sweep that repairs drift.
 *
 * The net is the accrual plus its reversals, which are stored NEGATIVE, so the sum is the arithmetic
 * rather than a subtraction. clampReversal makes it impossible for that to go below zero.
 *
 * `IS NOT` rather than `!=` on purpose: a NULL xp_written means "never written", and `!=` against
 * NULL is NULL, which is not true, so a never-written order would never be selected — the exact shape
 * of bug that leaves a feature silently doing nothing.
 */
export function ordersNeedingAwardProjection(db, { limit = 50 } = {}) {
  return db.prepare(`
    SELECT * FROM (
      SELECT ko.order_gid       AS orderGid,
             ko.customer_gid    AS customerGid,
             ko.xp_written      AS xpWritten,
             ko.points_written  AS pointsWritten,
             COALESCE(acc.xp, 0)     + COALESCE(rev.xp, 0)     AS netXp,
             COALESCE(acc.points, 0) + COALESCE(rev.points, 0) AS netPoints
      FROM keepers_orders ko
      JOIN (
        SELECT source_ref, MIN(id) AS id,
               SUM(xp_delta) AS xp, SUM(points_delta) AS points
        FROM keepers_events
        WHERE kind = 'order_accrual' AND status = 'applied'
        GROUP BY source_ref
      ) acc ON acc.source_ref = ko.order_gid
      LEFT JOIN (
        SELECT reverses_event_id,
               SUM(xp_delta) AS xp, SUM(points_delta) AS points
        FROM keepers_events
        WHERE kind = 'order_reversal' AND status = 'applied'
        GROUP BY reverses_event_id
      ) rev ON rev.reverses_event_id = acc.id
    )
    WHERE xpWritten IS NOT netXp OR pointsWritten IS NOT netPoints
    LIMIT ?
  `).all(limit);
}

/** Records what the ORDER metafields were last set to, so the next pass can skip an unchanged order. */
export function markOrderAwardWritten(db, orderGid, { xp, points }) {
  db.prepare('UPDATE keepers_orders SET xp_written = ?, points_written = ? WHERE order_gid = ?')
    .run(Math.trunc(Number(xp) || 0), Math.trunc(Number(points) || 0), String(orderGid));
}
