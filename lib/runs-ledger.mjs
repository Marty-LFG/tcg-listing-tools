...
export async function verifyChain(publicId, entries = []) {
  const errors = [];
  let prev = ZERO_HASH;
  let sales = 0;
...
export function availabilityFrom(unitCount, entries = []) {
  ...
  for (const e of entries) {
    if (accountsForBundle(e.kind)) {
      ...
      sold.set(no, e);
      if (e.ref) byToken.set(e.ref, e);
    } else if (e.kind === 'cancel') {
      const token = e.ref;
      const orig = byToken.get(token);
      if (orig && !cancelled.has(token)) {
        cancelled.add(token);
        sold.delete(Number(orig.bundleNo ?? orig.bundle_no));
      }
  ...
  return { available, sold, byToken, cancelled, paused, present };
}
