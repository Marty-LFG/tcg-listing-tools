...
import { startDataRefresh, stopDataRefresh, loadRefreshConfig, getRefreshState, runRefreshNow } from './refresh.mjs';
import { installLogCapture, getLogs } from './logbuffer.mjs';
...
export async function runProbe(source, base, { force = false } = {}) {
  ...
  try {
    ...
    try {
      ...
      if (j && j.error) detail = String(j.error).slice(0, 200);
      if (j && j.matched === false && j.reason) detail = String(j.reason).slice(0, 200);
    } catch { /* non-JSON body is fine */ }
    out = { state: classify(r.status), http: r.status, ms: Date.now() - t0, detail };
  } catch (e) {
    out = { state: 'down', http: null, ms: Date.now() - t0, detail: e?.name === 'TimeoutError' ? 'timeout' : String(e?.message || e).slice(0, 200) };
  }
...
function passiveSources(db) {
  ...
  try {
    ...
    for (const r of db.prepare(
      ...
      out[src] = { state, evidence: `watchlist last_error=${r.last_error} (${r.n} cards)`, last_evidence_at: r.last };
    }
...
export const SETTINGS = {
  ...
  refresh: {
    ...
    validate(c) {
      ...
      for (const b of c.bakes) if (!['riftbound', 'pokemon-intl', 'funko'].includes(b)) return `unknown bake '${b}'`;
      return null;
    },
    ...
    try {
      ...
      if (p === '/' && method === 'GET') {
        ...
        return send(res, 200, {
          ...
          sources,
          data: catalogStatus(),
          jobs: { refresh: getRefreshState(), collector: getCollectorState() },
          dbs: dbStats(db, rdb),
          subsystems: {
