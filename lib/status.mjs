...
export async function runProbe(source, base, { force = false } = {}) {
  ...
  try {
    ...
    try {
      ...
      if (j && j.error) detail = scrubSecrets(String(j.error)).slice(0, 200);
      if (j && j.matched === false && j.reason) detail = scrubSecrets(String(j.reason)).slice(0, 200);
    ...
    try {
      ...
      if (p === '/' && method === 'GET') {
        ...
        return send(res, 200, {
          ...
          sources,
          data: catalogStatus(),
          jobs: { refresh: getRefreshState(), collector: getCollectorState(), heartbeat: getHeartbeat() },
          dbs: dbStats(db, rdb),
          subsystems: {
