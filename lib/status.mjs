...
import { getPostsaleState, startPostsaleJobs, stopPostsaleJobs, NEEDS_PACKING_SQL, HOLD_SQL, CANCELLED_SQL } from './postsale.mjs';
  ...
  return {
    ...
    postsale: {
      ...
      ...(oneP(`SELECT
         ...
         FROM orders`) ?? { to_pack: null, holds: null, cancelled: null, unacked_holds: null }),
    },
  };
}
    ...
    try {
      ...
      if (p === '/' && method === 'GET') {
        ...
        return send(res, 200, {
          ...
          sources,
          data: catalogStatus(),
          jobs: { refresh: getRefreshState(), collector: getCollectorState(), heartbeat: getHeartbeat(), backup: getBackupState(), postsale: getPostsaleState(), sealed_value: getSealedRefreshState(), listing_reconcile: getReconcileState(), listing_batch: getBatchJobState(), repricer_scan: getRepricerScanState() },
          dbs: dbStats(db, rdb),
          subsystems: {
