...
import { getShopifyHooksState, startShopifyHooks, stopShopifyHooks, validateShopifyHooksConfig } from './shopify-hooks.mjs';
import { getKeepersEngineState, shopMismatchProblem, keepersEnv, validateKeepersConfig, startKeepersJobs, stopKeepersJobs } from './keepers.mjs';
import { openKeepersDb } from './keepers-db.mjs';
...
// Read straight from the config module and the reservation ledger rather than from lib/runs.mjs —
// that file imports diagTokenCheck from HERE to gate its manifest route, so importing it back would
// close a cycle.
import { loadRunsConfig, validateRunsConfig } from './runs-config.mjs';
import { devRunHoldings, brokenReservations } from './runs-reserve.mjs';
import { staleAnchors } from './runs-anchor.mjs';
import { getRunsJobsState, startRunsJobs } from './runs-jobs.mjs';
// The arbitrage finder. Its config module is separate from its plugin for the same no-cycle reason
// as runs-config: this file validates the config AND reads the job state, and lib/arbitrage.mjs
// imports nothing from here.
import { validateArbConfig } from './arb-config.mjs';
import { getArbWatchState, startArbWatchJob } from './arbitrage.mjs';
...
export const PROBES = {
  ...
  // The second loopback probe, same contract: a round trip against our own Shopify receiver with a
  // self-signed HMAC. A 200 proves the whole chain — bound, routed, HMAC verified, shop matched,
  // consumer stored — and touches no upstream. It answers 503 (never 404) when the receiver is down,
  // for the reason spelled out above.
  keepers: '/api/keepers/self-test',
};
...
export const SETTINGS = {
  ...
  // The Keepers receiver — the same safety boundary as 'ebay-notify' above, for the second listener.
  // The rules are NOT restated here: lib/shopify-hooks.mjs owns validateShopifyHooksConfig and its
  // docblock names this registry as its consumer. One copy of that reasoning is what stops the two
  // listeners' rules drifting apart.
  'shopify-hooks': {
    file: 'shopify-hooks.config.json', editable: true,
    validate: (c) => {
      const own = validateShopifyHooksConfig(c);
      if (own) return own;
      // AND THEN THE PAIR. This file decides which store's deliveries are accepted; keepers.config
      // .json decides which store the engine reads back from. Each is valid alone while disagreeing
      // with the other, and the disagreement is silent: point the receiver at live with the engine
      // still on dev and every live order is accepted, stored, drained — then re-read against a store
      // where that order id does not exist. Nothing is written wrongly and nothing works, which is
      // the hardest shape to notice. Refused here so the API cannot create it; also reported on
      // /api/status as shop_mismatch, because the file can still be edited by hand on the box, and
      // hand-editing both is exactly what going live requires.
      //
      // keepersEnv(), not process.env: loadEnv() never writes to process.env, so a process.env here
      // has no SHOPIFY_* keys, resolveShop throws, and the domain half of the check silently does
      // not run. That is how it shipped.
      return shopMismatchProblem(keepersEnv(), null, c);
    },
    apply() {   // the receiver reads the file when it binds → restart it (env is remembered)
      stopShopifyHooks();
      startShopifyHooks();
      const s = getShopifyHooksState();
      return s.listening
        ? `keepers receiver on ${s.host}:${s.port} · consumers ${s.consumers.map((c) => c.name).join(',') || '(none)'}`
        : (s.bind_error ? 'receiver did NOT start — ' + s.bind_error : 'keepers receiver stopped');
    },
  },
  // The Keepers ENGINE switches: mode, the projection pair, the per-run limits, the show windows.
  //
  // ⚠ WHY THIS validate() REFUSES MORE THAN validateKeepersConfig DOES. PUT /api/settings/:name has
  // no authentication of any kind — it checks `editable` and `validate()` and nothing else — and the
  // dev server binds 0.0.0.0:5273 so the LAN can reach it. Without the second half below, any device
  // on the network could PUT { store:'live', project:{ enabled:true, allowLive:true } } and arm writes
  // to real customer metafields on binderskeepers.cards, while POST /api/keepers/grant — which moves
  // 10 XP inside a local SQLite file — still demands DIAG_TOKEN. That asymmetry is indefensible.
  //
  // So the two switches that arm LIVE stay file-only: editable over the API for everything else,
  // refused for the pair that reaches the production store. Editing the file by hand needs shell
  // access to the box and a restart, which is the weight a decision like that should carry — and it
  // is what D-005 meant by "project.allowLive is a deliberate second switch".
  keepers: {
    file: 'keepers.config.json', editable: true,
    validate(c) {
      const base = validateKeepersConfig(c);
      if (base) return base;
      if (c.store === 'live') {
        return "store 'live' cannot be set through the API — edit data/keepers.config.json on the box. "
          + 'This endpoint has no authentication and the server listens on the LAN.';
      }
      // `!== false` and not `=== true`. Every consumer of this value reads it LOOSELY —
      // keepers-project.mjs's guard is `if (store === 'live' && !allowLive)` — so `1`, `"true"`, `[]`
      // and `{}` are all as good as true to them while sailing past a strict-equality refusal here.
      // The saved file would then read `"allowLive": 1` permanently, the dashboard would already say
      // "live writes allowed", and the operator's later hand-edit of `store` — the one switch this
      // endpoint does refuse, and therefore the one deliberate decision they think they are making —
      // would arm both at once.
      //
      // SETTINGS.shopify below applies the same rule to publish.allowLive. It did NOT when this comment
      // was first written — the sentence here claimed it did, and nobody checked until 2026-09-11, when
      // setting the live pins would have left a LAN PUT as the only thing between a stranger and a
      // live productSet. Both entries refuse now; neither claims the other does.
      if (c.project?.allowLive != null && c.project.allowLive !== false) {
        return 'project.allowLive cannot be set through the API — edit data/keepers.config.json on the '
          + 'box. It is the switch that lets the ledger write to real customers.';
      }
      return null;
    },
    apply(cfg, { env }) {   // startKeepersJobs reads mode when it ARMS → restart it
      stopKeepersJobs();
      const r = startKeepersJobs(env || process.env);
      return [
        `mode ${cfg.mode}`,
        `store ${cfg.store}`,
        cfg.project?.enabled ? 'projection ON' : 'projection off',
        r && r.started ? 'jobs armed' : `jobs not armed (${(r && r.reason) || 'unknown'})`,
      ].join(' · ');
    },
  },
  postsale: {
    ...
    validate(c) {
      ...
      // eBay inbox alerts. Same reasoning as deals above: every switch here is validated when
      // present, because a string in `enabled` reads as "not false" and would arm a job silently.
      const ia = c.inbox_alerts;
      if (ia != null) {
        if (typeof ia !== 'object' || Array.isArray(ia)) return 'inbox_alerts must be an object';
        for (const k of ['enabled', 'mark_read_button']) if (ia[k] != null && typeof ia[k] !== 'boolean') return 'inbox_alerts.' + k + ' must be boolean';
        // Only the two values eBay documents for GetMemberMessages. A typo would not error - it would
        // return nothing, which looks exactly like a quiet inbox.
        if (ia.message_types != null && !['All', 'AskSellerQuestion'].includes(ia.message_types)) {
          return "inbox_alerts.message_types must be 'All' or 'AskSellerQuestion'";
        }
        const range = (k, lo, hi) => Number.isInteger(ia[k]) && ia[k] >= lo && ia[k] <= hi;
        if (ia.max_alerts_per_run != null && !range('max_alerts_per_run', 1, 50)) return 'inbox_alerts.max_alerts_per_run must be an integer 1-50';
        if (ia.preview_chars != null && !range('preview_chars', 40, 1000)) return 'inbox_alerts.preview_chars must be an integer 40-1000';
        if (ia.nag_after_hours != null && !range('nag_after_hours', 1, 168)) return 'inbox_alerts.nag_after_hours must be an integer 1-168';
        if (ia.nag_max != null && !range('nag_max', 0, 5)) return 'inbox_alerts.nag_max must be an integer 0-5 (0 = never nag)';
        if (ia.nag_window_hours != null && !range('nag_window_hours', 1, 720)) return 'inbox_alerts.nag_window_hours must be an integer 1-720';
        // Blank means "use lib/ebay-links.mjs messagesUrl()". A non-blank value has to be a real eBay
        // link, or the card's only way back to the message is a dead button.
        if (ia.inbox_url) {
          if (typeof ia.inbox_url !== 'string' || !/^https:\/\//i.test(ia.inbox_url)) return 'inbox_alerts.inbox_url must be an https URL (or blank for the default)';
          if (!/(^|\.)ebay\.[a-z.]+\//i.test(ia.inbox_url)) return 'inbox_alerts.inbox_url must point at an ebay.* domain';
        }
      }
  ...
  'grading-economics': {
    ...
    // card-grader.html, inventory.html and index.html (the masthead centering panel) fetch
    // /data/grading.config.json per page load, and lib/inventory.mjs reads it per request —
    // there is no loop holding a parsed copy to restart.
    apply() { return 'live-read — applies on the next pre-grade report'; },
  },
  // The Shopify channel switches. Editable here because the file's own header always said the settings
  // dashboard owns it — the wiring just never happened, so until now arming a publish meant hand-editing
  // JSON on the box that trades, with no explanation of what each switch does and no check on the value.
  //
  // THE VALIDATOR IS THE POINT. Every refusal below is something that currently fails at PUBLISH time —
  // mid-batch, after files have been staged to a store that has no bulk delete — and can fail instead at
  // save time, in front of the person who typed it. `status` is the sharpest: it is compared with
  // === 'DRAFT' in lib/shopify.mjs, so "draft" lowercase silently means ACTIVE, which is the difference
  // between a hidden product and one on sale.
  shopify: {
    file: 'shopify.config.json', editable: true,
    validate(c) {
      if (!c || typeof c !== 'object') return 'not an object';
      if (!['dev', 'live'].includes(String(c.defaultStore || ''))) return "defaultStore must be 'dev' or 'live'";

      const p = c.publish;
      if (!p || typeof p !== 'object') return 'publish must be an object';
      if (typeof p.enabled !== 'boolean') return 'publish.enabled must be boolean';
      // Case matters and silently: publishStatus() upper-cases before comparing, but the mirror-state
      // rule and the theme both key off the exact value, so a stray 'Draft' is a real difference.
      if (!['ACTIVE', 'DRAFT'].includes(String(p.status || ''))) return "publish.status must be exactly 'ACTIVE' or 'DRAFT'";
      // Absent is allowed and means false — guardLiveStore requires === true. A string 'true' would be
      // truthy to a careless reader and is refused here so nobody concludes it armed something.
      if (p.allowLive != null && typeof p.allowLive !== 'boolean') return 'publish.allowLive must be boolean (absent means false)';
      // FILE-ONLY, for the reason the keepers entry above spells out: this endpoint has no
      // authentication and the server listens on the LAN, so with the live pins set, any device on the
      // network could PUT { publish: { allowLive: true } } and arm real product writes to
      // binderskeepers.cards. The keepers entry's comment says this entry "already gets this right" —
      // it did not, found 2026-09-11 while setting the live pins for the first publish. Until now the
      // only thing standing between a LAN PUT and a live productSet was the pins being blank, which is
      // the safety-by-absence this repo has already paid for once (the 2026-08-23 incident).
      //
      // `!== false` rather than `!== true`, same as keepers: guardLiveStore reads `=== true`, but a
      // saved `1` or "true" would show as "live writes allowed" on the dashboard and mislead the
      // operator's own later edit. Only an explicit false (or absence) passes.
      if (p.allowLive != null && p.allowLive !== false) {
        return "publish.allowLive cannot be set through the API — edit data/shopify.config.json on the box. "
          + 'This endpoint has no authentication and the server listens on the LAN.';
      }

      if (c.sync != null && typeof c.sync?.enabled !== 'boolean') return 'sync.enabled must be boolean';

      // A pin that is not a GID of the right type is the failure this whole entry exists to catch early.
      // Empty is legitimate — it is how a store stays deliberately unreachable (live ships unpinned).
      for (const [store, s] of Object.entries(c.stores || {})) {
        if (!s || typeof s !== 'object') return `stores.${store} must be an object`;
        for (const [k, prefix] of [['locationGid', 'gid://shopify/Location/'], ['publicationGid', 'gid://shopify/Publication/']]) {
          const v = String(s[k] ?? '').trim();
          if (v && !v.startsWith(prefix)) return `stores.${store}.${k} must be empty or start with ${prefix}`;
        }
      }

      // THE ONE CROSS-FIELD RULE, and the only one that can cost real money. Everything else here is a
      // typo check; this is the combination that points a real publish at the real shop. It does not
      // forbid it — the owner is allowed to go live — it refuses to let it happen by ACCIDENT, i.e.
      // without the live store having been deliberately pinned first.
      const live = (c.stores || {}).live || {};
      const livePinned = String(live.locationGid || '').trim() && String(live.publicationGid || '').trim();
      if (c.defaultStore === 'live' && p.allowLive === true && p.enabled === true && !livePinned) {
        return 'refusing: defaultStore=live with publishing armed and allowLive on, but the live store has no pins — measure them first';
      }
      return null;
    },
    // loadConfig() re-reads the file on every request (lib/shopify.mjs), so there is no cached copy to
    // invalidate and no loop to restart. Say so, because "saved" alone leaves the operator wondering
    // whether a restart is owed — it is not.
    apply(cfg) {
      const bits = [
        `store ${cfg.defaultStore}`,
        cfg.publish?.enabled ? `publishing ARMED as ${cfg.publish.status}` : 'publishing off',
        cfg.publish?.allowLive === true ? '⚠ LIVE WRITES ALLOWED' : 'live writes blocked',
      ];
      return 'live-read, no restart needed — ' + bits.join(' · ');
    },
  },

  runs: {
    file: 'runs.config.json', editable: true,
    // The whole schema lives in lib/runs-config.mjs beside the loader, so the validator and the
    // defaults cannot drift — and so a refusal reads the same whether it came from a settings PUT or
    // from the module refusing to publish.
    validate: validateRunsConfig,
    // The ROUTER is live-read: it calls loadRunsConfig() per request precisely so an arming change
    // takes effect without a restart. The anchor SWEEP is not — startRunsJobs reads
    // anchor.upgrade_interval_min once, when it arms the interval. So a settings save has to re-arm it,
    // or the saved value silently keeps the old cadence and the console reports the new one.
    apply() { startRunsJobs(); return 'runs jobs restarted · arming is live-read'; },
  },
  arbitrage: {
    file: 'arbitrage.config.json', editable: true,
    validate: validateArbConfig,
    // Floors, budget and query mode are live-read per scan. The WATCH TIMER is not — it reads
    // watch.enabled and interval_hours once when it arms — so a save re-arms it, the way runs does.
    apply() { startArbWatchJob(); return 'watch job re-armed · floors and budget are live-read'; },
  },
...
// ---- keepers: the engine, and the receiver it consumes from ----
//
// GUARDED, unlike its neighbours in the jobs block. Every other getter there reads a module-level
// object; getKeepersEngineState opens data/keepers.db and queries three tables, so on a box that has
// never run the plugin an unguarded call would 500 the entire status snapshot instead of showing one
// red panel (AGENTS.md GR7). The shape copied is ebay_oauth / pregrade_images below, not the bare
// calls above.
//
// hmac_reject_rate is DERIVED here because nothing in the repo computes one, and its denominator is
// received + sig_failures rather than received: lib/shopify-hooks.mjs increments sig_failures on a bad
// HMAC and on a right-signature-wrong-shop and returns 401 immediately, while received++ only happens
// after a delivery has been verified, shop-matched and stored. The two counters are disjoint, so
// `received` alone would understate the rate. null, never 0, while nothing has arrived: an idle
// listener must read "no data", not a reassuring 0%.
//
// A rejection spike says deliveries are being REFUSED — it does not say why. Three causes land on
// the one counter: a rotated app secret, an unsigned POST from the internet (the commonest, on a
// public endpoint), and a valid signature from the wrong shop, which means the receiver and the
// engine disagree about which store they serve. receiver.last_refusal carries which one it was.
function keepersJobState() {
  try {
    const receiver = getShopifyHooksState();
    const engine = getKeepersEngineState(openKeepersDb());
    const rejected = Number(receiver.sig_failures || 0);
    const seen = Number(receiver.received || 0) + rejected;
    return {
      ...engine,
      receiver,
      hmac_rejected: rejected,
      hmac_seen: seen,
      hmac_reject_rate: seen ? Math.round((rejected / seen) * 1000) / 10 : null,
    };
    ...
    try {
      ...
      if (p === '/' && method === 'GET') {
        ...
        return send(res, 200, {
          ...
          sources,
          data: catalogStatus(),
          jobs: { refresh: getRefreshState(), collector: getCollectorState(), heartbeat: getHeartbeat(), backup: getBackupState(), postsale: getPostsaleState(), sealed_value: getSealedRefreshState(), listing_reconcile: getReconcileState(), listing_batch: getBatchJobState(), repricer_scan: getRepricerScanState(), ebay_notify: getNotifyState(), keepers: keepersJobState(), arb_watch: getArbWatchState(), ...getRunsJobsState() },
          dbs: dbStats(db, rdb),
          subsystems: {
            ...
            // Keeper's Runs. `dev_hold_units` is the number that earns its place: a DEV rehearsal run
            // holds REAL stock and its reservations block real listings exactly as a live run's do, so
            // silently locked inventory is this design's failure mode. It gets a surface rather than a
            // comment.
            runs: (() => {
              try {
                const c = loadRunsConfig();
                const holds = devRunHoldings(db);
                // A broken reservation is stock that sold out from under a run. It blocks that run from
                // locking, and it is never cleared automatically, so it belongs on the dashboard rather
                // than only in a table someone would have to think to look at.
                const broken = brokenReservations(db);
                return {
                  publish_enabled: c.publish.enabled, publish_store: c.publish.store,
                  anchor_mode: c.anchor.mode,
                  // An anchor stuck on `submitted` is a run that cannot open for sale, and the gate's
                  // own message ('still pending') reads like patience rather than a problem. Surfaced
                  // here so it is visible without knowing to look at run_anchors.
                  stale_anchors: staleAnchors(db).length,
                  broken_reservations: broken.length,
                  broken: broken.slice(0, 20),
                  by_status: Object.fromEntries(db.prepare('SELECT status, COUNT(*) n FROM runs GROUP BY status').all().map((r) => [r.status, r.n])),
                  dev_holds: holds,
                  dev_hold_units: holds.reduce((n, r) => n + (r.units || 0), 0),
                };
              } catch (e) { return { error: String(e?.message || e) }; }
            })(),
          },
...
function settingsRouter({ env, db, base }) {
  return async (req, res) => {
    try {
      const url = new URL(req.url, 'http://localhost');
      const p = url.pathname.replace(/\/+$/, '') || '/';
      ...
      if (m) {
        ...
        if (method === 'PUT') {
          ...
          let applied = null;
          try { applied = s.apply ? s.apply(body, { db, base, env }) : null; }
...
export function statusPlugin(env) {
  return {
    ...
    configureServer(server) {
      ...
      server.middlewares.use('/api/settings', settingsRouter({ env, db, base }));
