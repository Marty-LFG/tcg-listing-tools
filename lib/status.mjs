...
import { LAYOUT_OVERRIDE_KEYS, TEXT_OVERRIDE_KEYS, VARIANTS, PROFILES, resolveLayout } from './listing-image-config.mjs';
...
export const SETTINGS = {
  ...
  'listing-image': {
    ...
    validate(c) {
      ...
      for (const [k, keys] of [['layoutOverrides', LAYOUT_OVERRIDE_KEYS], ['textOverrides', TEXT_OVERRIDE_KEYS]]) {
        const o = c[k];
        if (o == null) continue;
    ...
    try {
      ...
      if (p === '/' && method === 'GET') {
        ...
        return send(res, 200, {
          ...
          sources,
          data: catalogStatus(),
          jobs: { refresh: getRefreshState(), collector: getCollectorState(), heartbeat: getHeartbeat(), backup: getBackupState(), postsale: getPostsaleState(), sealed_value: getSealedRefreshState(), listing_reconcile: getReconcileState(), listing_batch: getBatchJobState() },
          dbs: dbStats(db, rdb),
          subsystems: {
