...
export const SETTINGS = {
  ...
  refresh: {
    ...
    validate(c) {
      ...
      for (const b of c.bakes) if (!['riftbound', 'pokemon-intl', 'pokemon-en-early', 'funko'].includes(b)) return `unknown bake '${b}'`;
      return null;
    },
    ...
    try {
      ...
      if (p === '/' && method === 'GET') {
        ...
        return send(res, 200, {
          ...
          sources,
          data: catalogStatus(),
          jobs: { refresh: getRefreshState(), collector: getCollectorState(), heartbeat: getHeartbeat(), backup: getBackupState() },
          dbs: dbStats(db, rdb),
          subsystems: {
