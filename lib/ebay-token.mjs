...
//
// DO NOT "fix" this in code — it has been chased twice. Verified 2026-08-01: the scope string below is
// exactly right, the grant flow is the documented one (client credentials, NOT the user consent token
// in lib/ebay-oauth.mjs), and eBay still answers the TOKEN endpoint with invalid_scope. A
// client-credentials grant has no consent step, so the keyset itself is simply not entitled. eBay's
// own marketplace-support table says the API is "restricted and not open to new users at this time",
// and as of that date the entire Marketplace Insights doc set has been REMOVED from developer.ebay.com.
//
// Australia IS a supported marketplace for it (verified in that table), and the lookback would be 90
// days — so it is worth having, it is just gated. Access needs two approvals, both eBay-side:
//   1. eBay Partner Network Buy API application — https://partnernetwork.ebay.com/page/developer-questionnaire
//      (documented 10-business-day response, then a Developer Support ticket and signed contracts)
//   2. Application Growth Check — https://developer.ebay.com/my/support/tickets?tab=app-check
// Small sellers are commonly declined. There is no substitute API: Finding `findCompletedItems` was
// decommissioned 2025-02-04, and Browse is asking-prices-only by design.
