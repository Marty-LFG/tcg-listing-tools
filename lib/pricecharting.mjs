...
// Each console row carries a 3-tier price (verified live 2026-07): td.used_price = Ungraded,
// td.cib_price = Grade 9, td.new_price = PSA 10, each `<span class="js-price">$X</span>`.
const RE_PC_USED = /<td class="[^"]*used_price[^"]*">\s*<span class="js-price">\s*([^<]*?)\s*<\/span>/
const RE_PC_CIB  = /<td class="[^"]*cib_price[^"]*">\s*<span class="js-price">\s*([^<]*?)\s*<\/span>/
const RE_PC_NEW  = /<td class="[^"]*new_price[^"]*">\s*<span class="js-price">\s*([^<]*?)\s*<\/span>/

// Parse a /console/<slug> page's games_table into [{ number, name, image, url, prices }]. Image is
// the base URL (append /320.jpg display or /1600.jpg). prices are INTEGER CENTS (GR3), null if the
// cell is blank/"-". Sealed rows (Booster Box/Pack — no "#<n>") are skipped.
export function parseConsole(html) {
  ...
  const out = []
  const pc = (chunk, re) => { const m = chunk.match(re); return m ? parseMoneyCents(m[1]) : null }
  for (const chunk of rows) {
    const nameM = chunk.match(/<td class="title"[^>]*>\s*<a[^>]*>\s*([^<]+?)\s*<\/a>/)
    if (!nameM) continue
    ...
    out.push({
      ...
      prices: { ungraded: pc(chunk, RE_PC_USED), grade9: pc(chunk, RE_PC_CIB), psa10: pc(chunk, RE_PC_NEW) },
    })
  }
  return out
}

// PriceCharting console tables server-render a fixed page of product rows (150 as of 2026-07) and
// hide the rest behind a ?cursor=<row-offset> param. A single fetch silently drops every card past
// that first page — a big set (M2A Mega Dream ex is 487 rows) then shows only ~#1-56 yet gets cached
// as if complete. So walk the cursor and concatenate every page.
const CONSOLE_PAGE_CAP = 20 // hard stop: ~3000 rows, far beyond any real set — kills a runaway loop
const consolePageUrl = (slug, cursor) =>
  ORIGIN + '/console/' + encodeURIComponent(slug) + '?sort=model-number' + (cursor ? '&cursor=' + cursor : '')

// Walk the ?cursor pagination. Pure over the injected fetchHtml(cursor)->html + parseRows(html)->[]
// (mirrors catalog.mjs paginateJson), so it is unit-testable offline. The cursor is a ROW offset, so
// it advances by the raw product-row count — the same whether the caller keeps singles or sealed.
// Stops on a page shorter than the first full page (the last one), an empty page, a non-advancing
// cursor (a repeated first-row id — defends a server change), or the hard page cap. Exported for the
// unit harness.
export async function walkConsolePages(fetchHtml, parseRows, cap = CONSOLE_PAGE_CAP) {
  const out = []
  let cursor = 0, firstPageRows = 0, prevFirstId = null
  for (let page = 0; page < cap; page++) {
    const html = await fetchHtml(cursor)
    const rowCount = (html.match(/<tr id="product-/g) || []).length
    const firstIdM = html.match(/<tr id="product-(\d+)/)
    const firstId = firstIdM ? firstIdM[1] : null
    if (rowCount && firstId && firstId === prevFirstId) break     // cursor not advancing — stop (no dupes)
    prevFirstId = firstId
    out.push(...parseRows(html))
    if (!rowCount) break                                          // no (more) rows on this page
    cursor += rowCount                                            // cursor is a row offset — advance by rows served
    if (page === 0) firstPageRows = rowCount
    else if (rowCount < firstPageRows) break                      // a page shorter than the first is the last page
  }
  return out
}
...
export async function enumerateConsole(slug) {
  ...
  try {
    const cards = await walkConsolePages((cur) => httpGet(consolePageUrl(slug, cur)), parseConsole)  // all pages, not just the first 150
...
// Directory of all Pokémon console slugs from /category/pokemon-cards. Cached 24h.
// Slugs encode the set: pokemon-japanese-<english-name>, pokemon-chinese-<code>,
// pokemon-korean-<english-name>, and ENGLISH sets carry NO language token (pokemon-<english-name>,
// e.g. pokemon-pitch-black, pokemon-scarlet-violet-151). The english bucket powers EN early-set
// discovery (data/pokemon-en-early.json) — PriceCharting lists a set's console weeks before
// pokemontcg.io catalogs it. Non-set English consoles (accessories/boxed/graded aggregates) are
// filtered by EN_CONSOLE_SKIP so they never become fake "sets".
const EN_CONSOLE_SKIP = /^pokemon-(cards|promo|sealed|boxed|graded|accessories|supplies|other|bulk|lots?)$|(-sealed|-booster-box|-elite-trainer-box|-etb)$|-$/
export function parseConsoleDirectory(html) {
  const out = { japanese: [], chinese: [], korean: [], english: [] }
  if (!html) return out
  const re = /\/console\/(pokemon-[a-z0-9.\-]+)/gi
  const seen = new Set()
  let m
  while ((m = re.exec(html))) {
    ...
    seen.add(slug)
    const langM = slug.match(/^pokemon-(japanese|chinese|korean)-/)
    if (langM) out[langM[1]].push(slug)
    else if (!EN_CONSOLE_SKIP.test(slug)) out.english.push(slug)   // no lang token = English set
  }
  return out
}

// A blocked or half-rendered directory used to come back as EMPTY BUCKETS at HTTP 200, and every
// caller read that as "PriceCharting lists no Japanese sets" rather than "we could not ask". The
// bakes then wrote a correct-looking index with no PriceCharting data in it and reported success —
// so GR7's "a failed refresh keeps the good copy" did not actually hold on this path. An empty
// directory is never a fact: serve the cached copy when there is one, and THROW when there is not.
const dirUsable = (d) => !!(d && ((d.japanese && d.japanese.length) || (d.chinese && d.chinese.length)))
const DIR_COLLAPSE_FLOOR = 0.6   // below 60% of the last good count, call it a bad render not churn
...
export async function listPokemonConsoles() {
  ...
  try {
    ...
    if (dirUsable(data)) {
      // The category page is one un-paginated index, so a partial render is indistinguishable from a
      // shrinking catalogue. It DOES legitimately churn — measured 2026-08-23, six JP consoles left
      // and four arrived in a day — so rejecting any shrink at all would pin the cache forever and
      // hide the new arrivals, which is the very staleness this cache exists to avoid. Only a
      // COLLAPSE is a parse failure: a block or a truncated render loses most of the page, not 2 of
      // 117. Below the floor, keep the good copy; above it, accept the churn.
      const was = (dirCache.data && dirCache.data.japanese.length) || 0
      if (was && data.japanese.length < was * DIR_COLLAPSE_FLOOR) {
        console.warn(`[pc] console directory collapsed ${was} -> ${data.japanese.length} Japanese consoles — treating as a bad render, keeping the cached copy`)
      } else {
        dirCache = { at: Date.now(), data }; diskWrite('directory.json', dirCache)
      }
      return dirCache.data
    }
    err = new Error('console directory returned no Japanese or Chinese consoles — blocked, or the markup changed')
  } catch (e) { err = e }
  if (dirCache.data) return dirCache.data          // stale but real — GR7
  throw err || new Error('console directory unavailable')
}
...
export function pickBestMatch(results, { name, number, setName }) {
  ...
  // Only a UNIQUE set-match is a confident hit. setMatch is deliberately loose (substring +
  // 60%-token overlap), so a card that exists at the same number in two same-named sets — e.g.
  // Charizard #4 in both "Base Set" and "Base Set 2" — yields >1 set-matching candidate. Returning
  // withSet[0] there surfaces whichever reprint PriceCharting happened to list first, at 'high'
  // confidence, and those prices differ hugely (GR4: don't present a wrong price as authoritative).
  // So accept withSet only when it uniquely resolves; otherwise fall through and refuse to guess.
  if (withSet.length === 1) return { match: withSet[0], confidence: 'high' }
  if (cands.length === 1) return { match: cands[0], confidence: 'medium' }
  return null // ambiguous (multiple name+number matches, none UNIQUELY resolving to the set) → don't guess
}
...
export function parseSealedProduct(html) {
  ...
  return {
    productName, consoleName,
    prices: {
      // Most sealed product pages label the box's market price "Ungraded" (the same cell a raw single
      // uses) rather than an explicit "New"/"Sealed" rung (verified live 2026-07: Surging Sparks ETB
      // lists only "Ungraded $126.25"). Prefer an explicit sealed rung, else fall back to the Ungraded
      // box price so a value actually resolves; opened/CIB fall through Ungraded too.
      sealed: first('New', 'Sealed', 'Brand New', 'Box only', 'Ungraded'),
      loose: first('Loose', 'Used', 'Ungraded'),
...
export async function enumerateSealedConsole(slug) {
  ...
  try {
    const products = await walkConsolePages((cur) => httpGet(consolePageUrl(slug, cur)), parseSealedConsole)  // all pages
...
// Invalidate a console's cached card + sealed lists (memory Maps + disk files) so the NEXT enumerate
// re-fetches live. Needed after a parser/pagination fix ships: the on-disk copy is otherwise served
// (fresh, but truncated) for up to 12h. Returns the removed filenames. Never throws (GR7).
export function clearConsoleCache(slug) {
  slug = String(slug || '').trim()
  const removed = []
  if (!slug) return { removed }
  consoleCache.delete(slug)
  sealedConsoleCache.delete(slug)
  for (const prefix of ['console-', 'sealed-console-']) {
    const file = prefix + safeName(slug) + '.json'
    try { rmSync(join(CACHE_DIR, file)); removed.push(file) } catch { /* absent → nothing to remove */ }
  }
  return { removed }
}
