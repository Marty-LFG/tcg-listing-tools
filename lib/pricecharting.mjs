...
export async function enumerateConsole(slug) {
  ...
  if (cached && Date.now() - cached.at < CONSOLE_TTL_MS) return cached.cards
  try {
    const html = await httpGet(ORIGIN + '/console/' + encodeURIComponent(slug) + '?sort=model-number')
    const cards = parseConsole(html)
    if (cards.length) { const entry = { at: Date.now(), cards }; consoleCache.set(slug, entry); diskWrite(file, entry); return cards }
    return cached ? cached.cards : []                                                    // parse empty → stale
  } catch { return cached ? cached.cards : [] }                                          // blocked → stale
}
...
export async function lookup(opts) {
  ...
  try {
    ...
    // Result cache keyed by cardId OR the direct card url (console path). Memory + disk.
    ...
    if (lang === 'jp' && !/japanese/i.test(card.consoleName || resolved.consoleName || '')) return { matched: false }
