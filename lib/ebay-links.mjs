...
// eBay Messages. THERE IS NO PUBLIC PER-MESSAGE DEEP LINK - do not go looking for one again.
// The old mesg.ebay.com/mesgweb/ViewMessages/0 host is dead (it broke sitewide in January 2025 and
// now redirects), and what replaced it, /cnt/ViewMessage, is a single-page app with no documented
// route to an individual message. `?group_type=CORE` narrows it to member-to-member messages, which
// is the closest a URL gets to "the buyer's message". So an inbox alert links to the INBOX and,
// separately, to the ITEM the message is about - itemUrl() is stable and is usually the thing you
// actually wanted to look at anyway. The config key inbox_alerts.inbox_url overrides this, so the
// next time eBay moves the page it is a settings edit rather than a release.
const MESSAGE_HOSTS = { EBAY_AU: BASE };
export function messagesUrl({ marketplace = 'EBAY_AU', memberOnly = true } = {}) {
  const host = MESSAGE_HOSTS[marketplace] || 'https://www.ebay.com';
  return host + '/cnt/ViewMessage' + (memberOnly ? '?group_type=CORE' : '');
}
