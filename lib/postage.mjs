...
export const KNOWN_SERVICES = Object.freeze({
  ...
  // The two sealed bands, added 2026-08-19 when the Sealed - Small and Sealed - Medium policies
  // were made. Read live off those policies, not guessed. TRAP WORTH THE COMMENT: the owner picked
  // Australia Post's Flat Rate Box/Satchel (MEDIUM) at A$16.00 and eBay assigned the code
  // AU_eBayAusPost500gFlatRateSatchel, so the code says 500g and the product is a medium satchel.
  // Label it by what the buyer receives, never by the code, which is the whole reason this map
  // exists. No weight tier appears in any of these: Australia Post retired that naming.
  AU_PrePaidParcelPostSatchels500g: { label: 'Tracked satchel, small', tier: 'tracked', tracked: true },
  AU_PrePaidExpressPostSatchel500g: { label: 'Express Post satchel, small', tier: 'express', tracked: true },
  AU_eBayAusPost500gFlatRateSatchel: { label: 'Tracked satchel, medium', tier: 'tracked', tracked: true },
  AU_ExpressPostMediumBoxSatchel: { label: 'Express Post satchel, medium', tier: 'express', tracked: true },
});
