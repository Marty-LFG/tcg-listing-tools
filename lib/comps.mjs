...
export function rowFromAsk(it) {
  ...
  return { price, ship: shipOf(it.shippingOptions), loc: (it.itemLocation && it.itemLocation.country) || '?', title: it.title || '', cond: it.condition || '', condId: String(it.conditionId || ''), seller: (it.seller && it.seller.username) || null, auction: isAuction(it), sold: false };
}
