...
//
// FIELD ORDER IS A BEST-EFFORT GUESS, UNLIKE SendInvoice's (which is WSDL-confirmed). The one real
// worked example found (eBay community, non-AU) showed CreatingUserRole, PaymentMethods, Total,
// TransactionArray in that order with no ShippingDetails present to place. ShippingDetails is put
// here between Total and TransactionArray as the most eBay-typical position (scalars before the
// array container) — NOT confirmed. If AddOrder returns Ack=Failure with a schema/sequence-flavoured
// error (not a business-rule error naming a specific field's value), reordering this is the first
// thing to try — same failure mode SendInvoice already hit once over doc-order vs WSDL-order.
//
