...
function invoiceAmount(cents, what) {
  if (cents == null || !Number.isFinite(Number(cents))) {
    ...
    throw new Error(`SendInvoice: ${what} must be a number of cents, got ${JSON.stringify(cents)}`);
  }
  if (!Number.isInteger(Number(cents))) throw new Error(`SendInvoice: ${what} must be whole cents (GR3), got ${cents}`);
