...
export async function getListingState(env, itemId) {
  ...
  const money = (s) => { const n = Number(String(s == null ? '' : s).replace(/[^0-9.]/g, '')); return Number.isFinite(n) ? Math.round(n * 100) : null; };
  ...
  return {
    ...
    best_offer_enabled, pricing_treatment, has_variations,
  };
}
...
export function parseItemImage(xml) {
  ...
  return xmlField(pd, 'GalleryURL') || xmlField(pd, 'PictureURL') || null;
}
export async function getItem(env, itemId) {
  const inner = `<ItemID>${xmlEscape(String(itemId))}</ItemID><OutputSelector>PictureDetails</OutputSelector>`;
  const res = await tradingCall(env, 'GetItem', inner);
  return { ...res, imageUrl: parseItemImage(res.xml) };
}
