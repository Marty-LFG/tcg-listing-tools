...
// 195003 is eBay's "you have not set the app-level config yet". Recognised by number rather than by
// message text so a wording change does not turn a known prerequisite back into a mystery.
function isConfigMissing(res) {
  return /195003/.test(JSON.stringify(res?.json || {})) || /195003/.test(String(res?.error || ''));
}
...
export function bestSchemaVersion(topic) {
  const versions = (topic?.supportedPayloads || [])
    .filter((p) => (p.deliveryProtocol || 'HTTPS') === 'HTTPS' && (p.format || 'JSON') === 'JSON')
...
export async function planReconcile(env, cfg, { call = notifyCall } = {}) {
  ...
  } else if (!cfgRes.ok && isConfigMissing(cfgRes)) {
    plan.setConfig = { action: 'create', alertEmail: wantEmail };
  } else if (cfgRes.ok && haveEmail !== wantEmail) {
  ...
  if (!dest) {
    ...
    if (String(dest.deliveryConfig?.endpoint || '').replace(/\/+$/, '') !== wanted) {
