...
async function paged(env, path, key, call = notifyCall, opts = undefined) {
  const out = [];
  let p = path + (path.includes('?') ? '&' : '?') + 'limit=100';
  let usedToken = null;
  let total = null;
  for (let guard = 0; p && guard < 20; guard++) {
    const r = await call(env, 'GET', p, null, opts);
    usedToken = r.usedToken;
    if (!r.ok) return { ok: false, error: firstErrorText(r.json) || r.error || ('http ' + r.httpStatus), items: out, usedToken, total };
    // A 2xx WITH NOTHING TO PARSE IS NOT AN EMPTY LIST. eBay's list endpoints always answer with an
    // envelope ({total, <key>, href, next}), so a bodyless or unparseable 200 means the read did not
    // happen — and `r.json || {}` used to turn exactly that into a confident "there are none".
    //
    // This distinction is load-bearing rather than tidy-minded, and the USER note below says why: an
    // empty subscription list is byte-for-byte what a mis-tokened read looks like. Anything that
    // manufactures an empty list out of a failure is manufacturing the most misleading answer this
    // module can give, because every caller downstream reasonably treats it as fact.
    if (r.json == null) {
      return { ok: false, error: `http ${r.httpStatus} with no JSON body — the list was not read`, items: out, usedToken, total };
    }
    const j = r.json;
  ...
  // eBay counts the whole set for us, so we can check our own read. If it says there are rows and we
  // finished with none, the read is wrong in a way each individual 200 concealed — a truncated page
  // walk, a renamed key, a filtered view. Reporting that as an empty list is the failure this whole
  // function exists to avoid.
  if (total != null && total > 0 && out.length === 0) {
    return { ok: false, error: `eBay reported total=${total} but returned no ${key} — the list read is wrong, not empty`, items: out, usedToken, total };
  }
  return { ok: true, items: out, usedToken, total };
}
...
/**
 * SUBSCRIPTIONS ARE USER-TOKEN ONLY, and the reason is worth spelling out because the failure is
 * silent rather than loud.
 *
 * Creating an ORDER_CONFIRMATION subscription with the application token returns
 * `403 [195011] Not authorized for this topic` — the topic is gated on sell.fulfillment, which is a
 * USER scope an application token cannot carry. That much announces itself.
 *
 * The dangerous half is the read: `GET /subscription` with the application token answers
 * `200 {"total":0}` rather than 403. It is not an error, so nothing escalates to the user token, and
 * the reconciler concludes there are no subscriptions — for an account that may have several. It
 * would then plan to create duplicates of things that already exist, forever.
 *
 * So every subscription call is pinned to the user token. Reading and writing must agree about whose
 * subscriptions they are, or the plan is computed against a view that does not exist.
 */
const USER = { userOnly: true };
export const getSubscriptions = (env, call) => paged(env, '/subscription', 'subscriptions', call, USER);
...
export const enableSubscription = (env, id, call = notifyCall) => call(env, 'POST', `/subscription/${encodeURIComponent(id)}/enable`, null, USER);
export const disableSubscription = (env, id, call = notifyCall) => call(env, 'POST', `/subscription/${encodeURIComponent(id)}/disable`, null, USER);
export const testSubscription = (env, id, call = notifyCall) => call(env, 'POST', `/subscription/${encodeURIComponent(id)}/test`, null, USER);
...
export async function applyPlan(env, cfg, plan, { call = notifyCall } = {}) {
  ...
  for (const c of plan.create) {
    ...
    // "[195012] Subscription already exists" is the outcome we wanted, not a failure. It happens
    // because apply recomputes the plan just before acting and eBay's subscription list is evidently
    // eventually consistent — a list that read empty a moment ago can already hold the row. A
    // reconciler has to be idempotent about that or it breaks whenever it is run twice in quick
    // succession, which is exactly when someone is trying to fix something.
    const already = !r.ok && /195012/.test(JSON.stringify(r.json || {}));
    if (!r.ok && !already) { fail('createSubscription ' + c.topicId, r); continue; }

    // Same as the destination: 201 Created, Location header, empty body. Enabling an undefined id
    // gets "[195013] The subscription id does not exist", which reads like eBay lost it rather than
    // like we never had it. Recover by identity — destination plus topic is unique, eBay documents
    // one subscription per topic.
    let id = r.ok ? r.json?.subscriptionId : null;
    if (!id) {
      const back = await getSubscriptions(env, call);
      id = back.ok ? (back.items.find((s) => s.destinationId === destId && s.topicId === c.topicId)?.subscriptionId || null) : null;
    }
    if (!id) {
      fail('createSubscription ' + c.topicId, { error: already
        ? 'eBay says it already exists, but re-reading did not find it'
        : 'created, but eBay returned no subscriptionId and re-reading did not find it' });
      continue;
    }
