...
export async function planReconcile(env, cfg, { call = notifyCall } = {}) {
  ...
  if (!subsRes.ok) { plan.ok = false; plan.errors.push('getSubscriptions: ' + subsRes.error); return plan; }
  ...
  plan.tokens = { topics: topicsRes.usedToken, destinations: destRes.usedToken, subscriptions: subsRes.usedToken };
  plan.changes = plan.create.length + plan.enable.length + plan.disable.length + (plan.updateDestination ? 1 : 0)
    + (plan.destination?.action === 'create' ? 1 : 0);
  return plan;
}
...
export async function applyPlan(env, cfg, plan, { call = notifyCall } = {}) {
  const done = { created_destination: null, created: [], enabled: [], disabled: [], errors: [] };
