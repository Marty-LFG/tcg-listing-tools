...
export function parseVariance(variance) {
  ...
  let finish;
  // Negations first — "Non-holo" contains "holo", "Nonfoil" contains "foil" (same ordering as
  // variantToken, which this feeds directly on the next line).
  if (/reverse/i.test(s)) finish = 'Reverse Holofoil';
  else if (/non[-\s]?(holo|foil)/i.test(s)) finish = 'Normal';
...
// THE LANGUAGE, which the export carries in the NAME or not at all.
//
// Collectr's schema has no language column. What it has is a tag on the Product Name — 'Marill (JP)',
// 'Mewtwo & Mew GX (JP)' — and this importer used to hardcode language:'EN' and leave the tag sitting
// in the name. Both halves of that were wrong, and expensively:
//
//   - The COMPS query is the only lever the pricing engine has on which market it reads. Without the
//     language, a Japanese card searched the English market, the downstream filter kept the English
//     results, and the row came back with a confident price for a card the seller does not own. That
//     is the GR4 failure arriving through a missing search term (see compsQueryFor in stock-games).
//   - The TITLE said 'Marill (JP)', which no buyer searches for, instead of naming the language the
//     way the builders do.
//   - And language is part of stockKey, so the duplicate check asked the shelf about the ENGLISH
//     printing of a Japanese card and came back with a clean bill for stock that may well be there.
//
// Deliberately conservative. Only the five lanes the stock tools actually have a catalogue for, only
// as a TRAILING tag, and the whole thing is anchored — a card genuinely named '... (Team Plasma)' or
// '(Full Art)' must survive untouched (GR5). 'JA' is accepted as an alias for JP because it is the
// ISO code and cheap to allow; an unrecognised tag is left exactly where it is.
const NAME_LANG_TAG = /\s*\((JP|JA|CN|TW|KO)\)\s*$/i;
export function langFromProductName(name) {
  const m = NAME_LANG_TAG.exec(String(name == null ? '' : name).trim());
  if (!m) return { language: 'EN', name: String(name == null ? '' : name).trim(), tagged: false };
  const code = m[1].toUpperCase() === 'JA' ? 'JP' : m[1].toUpperCase();
  return { language: code, name: String(name).trim().replace(NAME_LANG_TAG, '').trim(), tagged: true };
}
...
export function toImportRow(cr, opts = {}) {
  ...
  // Language tag FIRST: 'Misty (18) (JP)' has to lose the tag before cleanProductName can see the
  // trailing "(18)" it is looking for.
  const lang = langFromProductName(cr.product_name);
  const name = cleanProductName(lang.name, lookupNum);
  ...
  const row = {
    ...
    printing_key: null,
    // From the name tag, not a literal. See langFromProductName — an English default here is what
    // priced a Japanese card off the English market.
