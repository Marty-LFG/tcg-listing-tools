...
export async function composeListingImage(input, meta = {}, options = {}) {
  ...
  const contentHash = composeHash({ sourceBytes, layout, variant, textLines: avail.text ? lines : [], assetDigest, badge: badgeKey, flags: noTrim ? 'notrim' : '' });
...
export async function hashFor(sourceBytes, meta = {}, options = {}) {
  ...
  return {
    contentHash: composeHash({ sourceBytes, layout, variant, textLines: lines, assetDigest, badge, flags: options.trim === false ? 'notrim' : '' }),
