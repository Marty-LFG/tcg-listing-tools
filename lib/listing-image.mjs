...
async function loadSetArt(sharp, url, box) {
  if (!url) return null;
  try {
    const got = await fetchCached(url);
    if (!got.buffer) return null;
    const digest = crypto.createHash('sha256').update(got.buffer).digest('hex').slice(0, 12);
...
export async function composeListingImage(input, meta = {}, options = {}) {
  ...
  const symbol = wantBadge ? await loadSetArt(sharp, meta.setSymbolUrl, { width: symSize, height: symSize }) : null;
  const badgeNumber = wantBadge && avail.text ? String(meta.cardNumber || '') : '';

  const setLogo = layout.logo.rail !== 'none'
...
export async function hashFor(sourceBytes, meta = {}, options = {}) {
  ...
  const wantBadge = layout.badge.rail !== 'none' && (meta.cardNumber || meta.setSymbolUrl);
  const symSize = Math.round(layout.railWidth * layout.badge.symbolFraction);
  const symbol = wantBadge && sharp ? await loadSetArt(sharp, meta.setSymbolUrl, { width: symSize, height: symSize }) : null;
  const setLogo = sharp && layout.logo.rail !== 'none'
    ? await loadSetArt(sharp, meta.setLogoUrl, { width: Math.round(layout.railWidth * layout.logo.widthFraction), height: Math.round(layout.railWidth * layout.logo.heightFraction) })
    : null;
