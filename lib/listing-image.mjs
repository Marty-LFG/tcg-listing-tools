...
// game-logo fallback. Anchored + character-limited so a traversal ('logos/../.env') or an absolute
// path can never resolve, and the resolved path is re-checked against the logos/ dir besides.
// Everything else that is not http(s) stays rejected — the 'file-not-a-url' test pins that.
// PNG/JPEG only, deliberately: a local SVG would ride the recolourGlyph branch below and a
// full-colour game wordmark flattened to the badge colour is a defaced logo, not a fallback.
...
async function resolveSetArt(sharp, meta, layout, wantBadge, cfg = DEFAULT_CONFIG) {
  ...
  const symbol = sharp && wantBadge
    ...
    : null;
  // The logo slot's fallback chain: the set's own wordmark URL, else the GAME's logo — a repo
  // asset (meta.setLogoAsset), rail-only by design. The asset never reaches the description HTML,
  // which keeps its set-name text fallback; a local path in a buyer's browser is a broken image.
  const logoSrc = meta.setLogoUrl || meta.setLogoAsset || '';
  ...
  const invertLogo = logoSrc === PROMO_STAR_URL && (cfg && cfg.promoStar) !== 'normal';
  const setLogo = sharp && layout.logo.rail !== 'none'
    ? await loadSetArt(sharp, logoSrc, {
...
async function renderBadgeAbbrev(sharp, code, layout, cfg) {
  const text = String(code || '').trim().toUpperCase().slice(0, 6);
  if (!text) return null;
  const probe = await fontProbe(cfg);
  if (!probe.ok) return null;
  ...
  const glyph = await render(dpi).png().toBuffer();
  ...
  const buffer = base
    ? await sharp(base).composite([{ input: glyph, left: Math.round((W - g.width) / 2), top: Math.round((H - g.height) / 2) }]).png().toBuffer()
    : glyph;
  const m = await sharp(buffer).metadata();
  return { buffer, width: m.width, height: m.height, code: text };
}
...
export async function composeListingImage(input, meta = {}, options = {}) {
  ...
  // The code badge draws only when there is no symbol, and like the number it needs the font.
  const abbrevCode = wantBadge && !symbol && avail.text ? String(meta.setAbbrev || '').trim().toUpperCase().slice(0, 6) : '';

  // The abbrev segment is APPENDED only when present, so every composite that predates the code
  // badge keeps its exact key — an unconditional fourth field would re-render the whole store.
  const badgeKey = `${wantBadge ? badgeNumber : ''}|${symbol ? symbol.digest : ''}|${setLogo ? setLogo.digest : ''}` + (abbrevCode ? `|ab:${abbrevCode}` : '');
...
export async function hashFor(sourceBytes, meta = {}, options = {}) {
  ...
  const wantBadge = layout.badge.rail !== 'none' && (meta.cardNumber || meta.setSymbolUrl || meta.setAbbrev);
  const { symbol, setLogo } = await resolveSetArt(sharp, meta, layout, wantBadge, cfg);
  const abbrevCode = wantBadge && !symbol && font.ok ? String(meta.setAbbrev || '').trim().toUpperCase().slice(0, 6) : '';
  const badge = `${wantBadge && font.ok ? String(meta.cardNumber || '') : ''}|${symbol ? symbol.digest : ''}|${setLogo ? setLogo.digest : ''}` + (abbrevCode ? `|ab:${abbrevCode}` : '');
  return {
