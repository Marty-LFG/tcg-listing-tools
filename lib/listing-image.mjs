...
async function loadSetArt(sharp, url, box, fill) {
  if (!url) return null;
  try {
...
// pins that.
async function resolveSetArt(sharp, meta, layout, wantBadge) {
...
export async function composeListingImage(input, meta = {}, options = {}) {
  ...
  const { symbol, setLogo } = await resolveSetArt(sharp, meta, layout, wantBadge);
