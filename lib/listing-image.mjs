...
import { prepareSource, regionFor, orientedSize } from './listing-image-source.mjs';
import { DEFAULT_TARGET, TARGETS, TARGET_IDS, resolveTarget, targetFingerprint } from './listing-image-targets.mjs';

export { prepareSource, regionFor, orientedSize };
export { DEFAULT_TARGET, TARGETS, TARGET_IDS, resolveTarget, targetFingerprint };
...
// pins that.
// `box` is an OPTIONAL override for callers whose furniture is not a vertical rail — the horizontal
// bands and the social card size their set art off the band thickness, not off railWidth. It
// defaults to exactly what the eBay square computed before it existed, so that path is untouched.
// Everything still funnels through this ONE resolver: test/invariants/listing-image-setart.test.mjs
// counts the loadSetArt calls and refuses to let them scatter back out to the entry points.
export async function resolveSetArt(sharp, meta, layout, wantBadge, cfg = DEFAULT_CONFIG, box = null) {
  const symSize = box && box.symbol ? box.symbol : Math.round(layout.railWidth * layout.badge.symbolFraction);
...
// `boxWidth` is the same optional override as resolveSetArt's — the bands size the boxed code off
// the band thickness. Omitted, it computes exactly what it always did.
export async function renderBadgeAbbrev(sharp, code, layout, cfg, boxWidth = null) {
  const text = normAbbrev(code);
  if (!text) return null;
  const probe = await fontProbe(cfg);
  if (!probe.ok) return null;
  const b = layout.badge;
  const boxW = boxWidth || Math.round(layout.railWidth * b.symbolFraction);
...
export async function composeListingImage(input, meta = {}, options = {}) {
  ...
  // This function renders the VERTICAL-RAIL square and nothing else. The horizontal-band frames
  // have their own renderer; resolving the target here (rather than ignoring the option) means a
  // caller that asks for one gets told, instead of silently receiving an eBay square.
  const target = resolveTarget(options.target);
  if (target.id !== DEFAULT_TARGET) {
    throw new Error(`composeListingImage renders '${DEFAULT_TARGET}' only — '${target.id}' belongs to the band renderer`);
  }
  ...
  const contentHash = composeHash({ sourceBytes, layout, variant, textLines: avail.text ? lines : [], assetDigest, badge: badgeKey, flags: flagStr, target: targetFingerprint(target) });
  const version = composeVersion(variant, assetDigest, target.id);

  // The extension follows the target's format. For the eBay square that is 'jpg' unconditionally,
  // so every <hash>.jpg already on disk still hits — pinned in test/unit/listing-image.test.mjs.
  ...
  // Shared with every other output frame via lib/listing-image-source.mjs, so no two channels can
  // disagree about where the card is. `legacy: true` keeps the eBay square's frozen whole-frame
  // check — the comment in that module explains why it must not be "fixed" here.
  ...
  const region = regionFor(prep, { legacy: true });
  ...
  return {
    ...
    // `trimmed` reports the HONEST answer, not the frozen one: a sideways photo whose trim found
    // nothing still takes a whole-frame extract (see listing-image-source.mjs) but nothing was
    // cropped off it, and saying otherwise would send the Shopify aspect check chasing a trim that
    // never happened.
    card: { width: cardMeta.width, height: cardMeta.height, trimmed: !!region && !prep.frameFull },
  };
}
...
export async function hashFor(sourceBytes, meta = {}, options = {}) {
  ...
  return {
    contentHash: composeHash({ sourceBytes, layout, variant, textLines: lines, assetDigest, badge, flags: composeFlags(options.trim === false, meta), target: targetFingerprint(target) }),
    composeVersion: composeVersion(variant, assetDigest, target.id),
    variant,
    target: target.id,
  };
}
