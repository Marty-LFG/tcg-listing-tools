...
// The symbol comes off a CDN, so it goes through the shared disk cache: the second listing from a
// set is a local read. Returns null on anything going wrong — a missing symbol costs the badge its
// icon, never the listing its image.
async function loadSymbol(sharp, url, size) {
  if (!url) return null;
  try {
    const got = await fetchCached(url);
    if (!got.buffer) return null;
    const buffer = await sharp(got.buffer, { failOn: 'none' })
      .resize(size, size, { fit: 'inside', withoutEnlargement: false })
      .png().toBuffer();
    const m = await sharp(buffer).metadata();
    return { buffer, width: m.width, height: m.height, digest: crypto.createHash('sha256').update(got.buffer).digest('hex').slice(0, 12) };
  } catch { return null; }
}
...
export async function composeListingImage(input, meta = {}, options = {}) {
  ...
  const symbol = wantBadge ? await loadSymbol(sharp, meta.setSymbolUrl, Math.round(layout.railWidth * layout.badge.symbolFraction)) : null;
  const badgeNumber = wantBadge && avail.text ? String(meta.cardNumber || '') : '';
  const badgeKey = wantBadge ? `${badgeNumber}|${symbol ? symbol.digest : ''}` : '';
  ...
  return {
    ...
    badge: { number: number ? badgeNumber : '', symbol: !!symbol },
...
export async function hashFor(sourceBytes, meta = {}, options = {}) {
  ...
  const sharp = await getSharp();
  const wantBadge = layout.badge.rail !== 'none' && (meta.cardNumber || meta.setSymbolUrl);
  const symbol = wantBadge && sharp ? await loadSymbol(sharp, meta.setSymbolUrl, Math.round(layout.railWidth * layout.badge.symbolFraction)) : null;
  const badge = wantBadge ? `${font.ok ? String(meta.cardNumber || '') : ''}|${symbol ? symbol.digest : ''}` : '';
  return {
