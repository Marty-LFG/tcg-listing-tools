...
import { importSellerListings, loadConfig as loadListingConfig } from './listings.mjs';
import { decideReprice, eligibleForReprice } from './repricer-decide.mjs';
import { liveReservation } from './runs-reserve.mjs';
...
const GAMES = [
  ...
  // Leading boundary is `(?:^|[^a-z])`, not `\b`, because a real title writes the number and the game
  // with no space between them — "…(SOA) #107MTG NM/M" has no word boundary before MTG.
  [/(?:^|[^a-z])(magic:?\s*the gathering|mtg)\b/i, 'Magic The Gathering'],
  [/\byu-?gi-?oh\b/i, 'Yu-Gi-Oh'],
  // Set and sub-brand names, for the titles that never say "Pokemon" at all — the Black Star Promos
  // and the 25th Anniversary packs are listed by set, and each of these phrases belongs to exactly one
  // game. Last in the list so an explicit game word always wins.
  [/\b(scarlet\s*&?\s*violet|black star promo|team rocket|sword\s*&?\s*shield|pok[eé]ka)\b/i, 'Pokemon'],
];
...
// raise wants a second opinion from something that actually transacted. These gather it; the judgement
// is in repricer-decide.mjs corroborate().

// What WE last sold this exact listing for. The strongest signal available: real AUD, a real buyer, no
// identity resolution needed because order_line_items carries the eBay item id. Narrow by design — it
// only covers listings that have sold at least once, which for a multi-quantity listing is common.
export function ownSoldFor(pdb, itemId) {
  if (!pdb || !itemId) return null;
  try {
    const r = pdb.prepare(`SELECT li.unit_price_cents AS cents, o.paid_time AS at
                           FROM order_line_items li JOIN orders o ON o.order_id = li.order_id
                           WHERE li.ebay_item_id = ? AND li.unit_price_cents > 0
                           ORDER BY COALESCE(o.paid_time, o.created_time) DESC LIMIT 1`).get(String(itemId));
    return r && r.cents > 0 ? { name: 'own_sale', cents: r.cents, at: r.at || null } : null;
  } catch { return null; }        // GR7 — a missing postsale db must not end a scan
}

// PriceCharting's `ungraded` rung is eBay-sold-derived, but it is USD and the US market, so it needs
// FX and it is a corroborator rather than an anchor. Best-effort throughout: no rate, no match, or a
// dead scrape all return null and simply leave this source absent.
export async function priceChartingFor(identity, { fxUsdAud, token, enabled }) {
  if (!enabled || !(fxUsdAud > 0) || !identity || !identity.name || identity.number == null) return null;
  try {
    const pc = await pcLookup({ name: identity.name, number: identity.number, setName: identity.setName || undefined, token, enabled: true });
    const usdCents = pc && pc.matched && pc.prices ? pc.prices.ungraded : null;
    if (!(usdCents > 0)) return null;
    return { name: 'pricecharting_ungraded', cents: Math.round(usdCents * fxUsdAud), at: null };
  } catch { return null; }
}

// One USD->AUD rate per pass, from the same /api/fx the rest of the app uses.
export async function fxUsdToAud(base) {
  try {
    const r = await fetch(String(base || '').replace(/\/$/, '') + '/api/fx/latest?from=USD&to=AUD');
    if (!r.ok) return null;
    const j = await r.json();
    const v = (j && j.rates && (j.rates.AUD ?? j.rates.aud)) ?? null;
    return v > 0 ? v : null;
  } catch { return null; }
}
...
export async function scanListings({ env, rdb, tdb, cfg = {}, base, limit = 0, gapMs = 400, importFirst = true } = {}) {
  ...
  try {
    ...
    const rows = tdb.prepare(`SELECT listing_id, sku, title, price_cents, currency, available_qty, listing_type, state, created_via, item_id
    ...
    const ins = rdb.prepare(`INSERT INTO price_checks
      (item_id, scan_id, our_price_cents, our_postage_cents, target_delivered_cents, target_cents,
       ...
       reliable, confidence, mode, band_id, band_capped, query, uplift_cents, uplift_pct, verdict, code)
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`);
    const record = (itemId, priceCents, postageCents, d, query) => {
      ...
      ins.run(itemId, scanId, priceCents ?? null, postageCents ?? null,
        ...
        e.band ?? null, e.bandCapped == null ? null : (e.bandCapped ? 1 : 0),
    ...
    // The postage band table, read ONCE per pass. It lives in the eBay LISTING config, not the
    // repricer's own, because it is what the store actually charges — the repricer only needs it to
    // convert a delivered comp anchor into a list price without crossing a band. A read failure simply
    // leaves it null, and every listing falls back to the flat postage model (GR7).
    let shippingBands = null;
    try { shippingBands = (loadListingConfig().shipping || {}).bands || null; }
    catch (e) { console.warn('[repricer/scan] no postage band table — using flat postage: ' + (e?.message || e)); }

    // Corroboration inputs, resolved once per pass. A missing postsale db or a dead FX endpoint simply
    // removes a source rather than failing the scan (GR7) — with `corroboration: 'require'` that turns
    // into honest `no_corroboration` declines instead of silently-unsupported raises.
    let pdb = null;
    try { pdb = openPostsaleDb(); } catch { pdb = null; }
    const pcEnabled = cfg.corroborate_pricecharting === true && String(env.PRICECHARTING_ENABLED || 'true').toLowerCase() !== 'false';
    const pcToken = (env.PRICECHARTING_TOKEN || '').trim();
    const fxUsdAud = pcEnabled ? await fxUsdToAud(base) : null;
    if (pcEnabled && !fxUsdAud) console.warn('[repricer/scan] no USD->AUD rate — PriceCharting corroboration is off for this pass');
    stats.corroboration = { own_sales: !!pdb, pricecharting: !!(pcEnabled && fxUsdAud) };
    ...
    for (const row of rows) {
      ...
      try {
        ...
        // Reserved for a run? The reservation ledger is keyed on the stock row, so this needs the
        // mirror's item_id — filled in opportunistically by the import when a Custom label matched, and
        // null for a hand-made listing we never tied to stock. Resolved here rather than in
        // repricer-decide because that module is pure and takes no database handle.
        const reservedForRun = row.item_id ? !!liveReservation(tdb, 'inventory', row.item_id) : false;
        const cheap = { ...row, reservedForRun, listingId: row.listing_id, priceCents: row.price_cents, availableQty: row.available_qty, listingType: row.listing_type, postageCents: 0 };
        ...
        const listing = {
          listingId: row.listing_id, title: live.title || row.title, reservedForRun,
        ...
        const elig = eligibleForReprice(listing, identity, guardrails, shippingBands);
        ...
        // 4. what actually transacted, to second the asking-price anchor. Own sales are a free local
        //    lookup; PriceCharting costs a scrape and is off unless the owner turned it on.
        const corroborators = [
          ownSoldFor(pdb, row.listing_id),
          await priceChartingFor(identity, { fxUsdAud, token: pcToken, enabled: pcEnabled }),
        ].filter(Boolean);

        // 5. the judgement (pure)
        const d = decideReprice({ listing, identity, comps, guardrails, context: { nowIso: new Date().toISOString(), corroborators, shippingBands } });
...
export function guardrailsFrom(cfg = {}) {
  ...
  put('beatCheapestWithinCents', g.beat_cheapest_within_aud != null ? Math.round(g.beat_cheapest_within_aud * 100) : null);
  put('beatCheapestWithinPct', g.beat_cheapest_within_pct);
  put('corroboration', g.corroboration);
  put('corroborationTolerancePct', g.corroboration_tolerance_pct);
  return out;
}
