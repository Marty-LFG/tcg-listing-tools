...
export const DEFAULT_BANDS = [
  ...
  },
  {
    ...
    costCents: 826,
    copy: 'tracked',
    // Blank on purpose: eBay AU has no signature-on-delivery letter code among the seven a single card
    // plausibly uses, and serviceCode is only read if this tool ever has to CREATE the policy, which
    // pinned mode never does. verifyBandPolicies reads the real code off the live policy — paste it
    // back then rather than guessing one here (GR4).
    serviceCode: '',
...
// left unassigned.
const RE_SERVICE_CODE = /^[A-Z]{2}_[A-Za-z]+$/;
...
export function validateBands(bands) {
  ...
  for (let i = 0; i < bands.length; i++) {
    ...
    // A shape check, not an allow-list: hard-coding the seven codes the settings dropdown knows would
    // refuse a legitimate code the owner reads off their real policy. The check that actually matters
    // is verifyBandPolicies, live against eBay.
