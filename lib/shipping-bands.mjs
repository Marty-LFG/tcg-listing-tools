...
export const BAND_FIELD_DEFAULTS = {
  ...
  policyId: '',
  policyName: '',
  // Every DOMESTIC service the pinned policy offers, in eBay's own sortOrder: [{code,label,costCents}].
  // Captured from the live policy by bootstrap, never hand-guessed. The description renders it as a
  // table so a buyer sees the choice they actually get at checkout rather than one figure out of three.
  services: [],
};
...
export const DEFAULT_BANDS = [
  {
    ...
    policyName: '',
    // Read live off the pinned policy 2026-08-14. This band offers a CHOICE: the buyer can pay for
    // tracking, or a signed-for satchel, on a card under $49.98 if they want to.
    services: [
      { code: 'AU_AusPostStandardLetter', label: 'Regular letter', costCents: 170, sortOrder: 1 },
      { code: 'AU_AusPostPriorityLetterWithTracking', label: 'Priority letter, tracked', costCents: 826, sortOrder: 2 },
      { code: 'AUP_500G_SATCHEL_SIG', label: 'Tracked satchel, signature on delivery', costCents: 1520, sortOrder: 3 },
    ],
  },
  {
    id: 'tracked',
    ...
    policyName: '',
    services: [
      { code: 'AU_Regular', label: 'Tracked letter', costCents: 826, sortOrder: 1 },
      { code: 'AUP_500G_SATCHEL_SIG', label: 'Tracked satchel, signature on delivery', costCents: 1520, sortOrder: 2 },
    ],
  },
  {
    id: 'signature',
    ...
    policyName: '',
    // One service, so postageOptions returns no rows and this band renders the sentence alone.
    services: [
      { code: 'AUP_500G_SATCHEL_SIG', label: 'Tracked satchel, signature on delivery', costCents: 1520, sortOrder: 1 },
    ],
  },
];
...
export const POSTAGE_OPTIONS_NOTE = 'You pick the one you want at checkout.';

// Rows for the description's postage-options table: [[label, money], …].
//
// EMPTY when the band offers one service or none — a one-row table adds nothing the sentence has not
// already said, and empty is also what a config predating `services` renders, so a builder whose live
// config fetch failed shows no table rather than a wrong one (GR7). The rule lives here rather than in
// each frame so all five frames and eight builders inherit it from one place.
//
// Order is eBay's own sortOrder, carried through by fulfillmentTerms: the buyer-facing order should
// match what the buyer is about to see at checkout, not one we invented.
export function postageOptions(band) {
  const svcs = (band && Array.isArray(band.services)) ? band.services : [];
  if (svcs.length < 2) return [];
  return svcs.map((s) => [String(s.label || s.code || ''), money(s.costCents)]);
}
