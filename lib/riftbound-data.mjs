...
function cardToCanonical(c, setMeta) {
  ...
  return {
    ...
    // is GR4.
    illustrator: c.a || '',
    // The champion a Legend or a champion Unit belongs to, baked as `ch` (same script). Absent on an
    // older bake, and '' means riftboundDisplayName leaves the printed name alone.
    champion: c.ch || '',
  };
}

// The name a player uses, and the name the storefront prints (bk-shopify, Marty 2026-09-13).
//
// Riot names a Legend by its epithet alone — "Keeper of the Hammer", "Green Father", "Hand of
// Noxus" — with the champion only in the card's tag line, so a listing titled from the bake's
// `name` never said Poppy or Ivern anywhere a buyer could read it. A champion Unit is printed
// "<Champion>, <Epithet>". Both come out "<Champion> (<Epithet>)": the champion leads because it
// is what a buyer searches, the epithet follows in brackets because it is what tells two Poppy cards
// apart. Everything else — a non-champion Unit, a Spell, a Gear, a Battlefield, a Rune — keeps its
// printed name untouched. Pure; the treatment suffix is not part of `name` here (cardToCanonical
// strips it into `variant`), so it never lands inside the brackets.
export function riftboundDisplayName(card) {
  const name = String((card && card.name) || '').trim();
  const champion = String((card && card.champion) || '').trim();
  if (!name || !champion) return name;
  const type = String((card && card.type) || '').trim().toLowerCase();
  if (type === 'legend') return `${champion} (${name})`;
  if (type === 'unit' && name.startsWith(champion + ', ')) return `${champion} (${name.slice(champion.length + 2)})`;
  return name;
}
...
function runeToCanonical(pr, setMeta, data) {
  ...
  return {
    ...
    // A rune reprint IS the Origins rune, so it carries that card's artist. No champion: a rune is
    // nobody's card.
    illustrator: r.a || '',
    champion: '',
  };
