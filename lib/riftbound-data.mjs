...
export function mapRarity(r) { return (r === 'Alternate Art' || r === 'Overnumbered' || r === 'Signature') ? 'Showcase' : (r || ''); }
export function variantOf(r) { return (r === 'Alternate Art' || r === 'Overnumbered' || r === 'Signature') ? r : ''; }
export function finishOf(r) { return ['Epic', 'Showcase', 'Alternate Art', 'Overnumbered', 'Signature'].includes(r) ? 'Foil' : 'Non-foil'; }
...
function cardToCanonical(c, setMeta) {
  ...
  const cleanName = name.replace(/\s*\((Alternate Art|Overnumbered|Signature)\)\s*$/i, '');
...
function runeToCanonical(pr, setMeta, data) {
  ...
  const name = (r.name || (pr.dom + ' Rune')).replace(/\s*\((Alternate Art|Overnumbered|Signature)\)\s*$/i, '');
