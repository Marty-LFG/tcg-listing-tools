...
export function evidenceSummary(check, cfg = {}) {
  ...
  // The one thing the clamp costs is visibility, so it is spelled out on the card a human approves.
  // The market wanted more than this band's ceiling; moving the card higher means moving it onto the
  // next postage policy by hand, which this path deliberately cannot do.
  if (check.band_capped) bits.push('capped at the top of the postage band — worth more, needs the next band');
  return bits.join(' · ');
}
...
export async function promoteFromScan({
  ...
  for (const c of raises) {
    ...
    const evidence = JSON.stringify({
      ...
      band_id: c.band_id, band_capped: c.band_capped,
    });
