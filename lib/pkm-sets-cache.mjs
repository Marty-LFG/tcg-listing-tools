...
// An empty-but-HTTP-200 upstream is treated as a failure to serve, never as a cache write: a
// re-indexing upstream returns 200 with data:[] and would otherwise poison the cache with nothing
// (the same trap catalog.mjs decideCardsResponse guards).
export function decideSetsResponse(fresh, lastGood, nowIso) {
  const list = fresh && Array.isArray(fresh.data) ? fresh.data : null;
  if (list && list.length) return { store: true, status: 200, body: { ...fresh, stale: false, cached: false, cachedAt: nowIso } };
  ...
  if (list) return { store: false, status: 200, body: { ...fresh, stale: false, cached: false, cachedAt: nowIso } };   // upstream really is empty
  ...
  return {
    ...
    configureServer(server) {
      server.middlewares.use('/api/pkm/sets', async (req, res, next) => {
        ...
        const fresh = await fetchSets(env, search ? '?' + search : '');
