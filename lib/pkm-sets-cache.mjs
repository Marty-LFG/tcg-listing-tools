...
// ONLY the two pages the bake indexes. An unknown language resolves to null, not to English:
// defaulting meant a Korean card was handed the English set logo, which is the same wrong-product
// problem as the JP/EN shadowing, just arriving by a different route. Korean and Chinese sets have
// their own art we do not bake, so no logo is the honest answer until we do.
const INDEX_LANG = { ja: 'ja', jp: 'ja', japanese: 'ja', en: 'en', eng: 'en', english: 'en' };
const indexLang = (lang) => {
  const s = String(lang || '').trim().toLowerCase();
  if (!s) return 'en';                                  // unset means the store default
  return INDEX_LANG[s] || null;
};

function lookup(bucket, lang, candidates) {
  const key = indexLang(lang);
  if (!key) return null;
