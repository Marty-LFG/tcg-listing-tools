...
const _norm = (s) => String(s == null ? '' : s).trim().toLowerCase().replace(/[^\p{L}\p{N}]+/gu, '');
...
function loadSymbolIndex() {
  try {
    ...
    if (!_symbols || st.mtimeMs !== _symbolsAt) { _symbols = JSON.parse(fs.readFileSync(SYMBOLS_PATH, 'utf8')); _symbolsAt = st.mtimeMs; }
    return _symbols;
  } catch { return null; }
}

export function findSetSymbol(name) {
  const key = _norm(name);
  if (!key) return null;
  const idx = loadSymbolIndex();
  const hit = idx && idx.symbols && idx.symbols[key];
 ...
 * `SV3a_Raging_Surf_Logo.png`) and a stock row may carry any of them.
 */
export function findSetLogo(...candidates) {
  const idx = loadSymbolIndex();
  if (!idx || !idx.logos) return null;
  for (const c of candidates) {
    const key = _norm(c);
    if (!key) continue;
    const hit = idx.logos[key];
