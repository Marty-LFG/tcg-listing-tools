...
function lookup(bucket, candidates) {
  const idx = loadSymbolIndex();
  if (!idx || !idx[bucket]) return null;
  const table = idx[bucket];
...
export function findSetSymbol(...candidates) {
  return lookup('symbols', candidates);
}
...
export function findSetLogo(...candidates) {
  return lookup('logos', candidates);
}
