...
// `cn` and `tw` are the codes the stock tools actually store. Without them findIntlSet returned
// null for every Chinese row and composeMetaFor fell back to the ENGLISH set index — which is the
// precise failure the paragraph above describes, reached through a missing map key.
const LANG_TO_INTL = { jp: 'ja', ja: 'ja', japanese: 'ja', zh: 'zh-cn', cn: 'zh-cn', 'zh-cn': 'zh-cn', chinese: 'zh-cn', tw: 'zh-tw', 'zh-tw': 'zh-tw', ko: 'ko', korean: 'ko' };
...
export const SET_NAME_ALIASES = Object.freeze({
  ...
  // The baked TCGplayer roster (data/pokemon-mep.json) names the EN Mega promos "Mega Evolution
  // Promo", code `mep`; the wiki files the symbol as SetSymbolMEP_Black_Star_Promos.png. This
  // alias is the ONLY route to rail art for it — the set is absent from pokemontcg.io entirely,
  // which is why the roster is baked. Keyed by both identities a stock row may carry.
  megaevolutionpromo: 'MEP Black Star Promos',
  mep: 'MEP Black Star Promos',
  // Same drift for SVP: stock rows store pokemontcg.io's "Scarlet & Violet Black Star Promos",
  // the wiki files the boxed mark under the code. Without this the SVP badge falls back to the
  // generic black star, which now lives on the LEFT rail — the badge slot wants the SET's mark.
  scarletvioletblackstarpromos: 'SVP Black Star Promos',
...
// Which indexed page a card's language reads from — and it differs by BUCKET, because a set symbol
// and a set logo are not the same kind of thing.
//
...
// Japanese logo is the wrong product, the same failure as the JP/EN shadowing.
//
// Korean and Traditional Chinese sets ARE Japanese sets translated: 78/101 KO and 46/98 ZH-TW codes
// in the intl bake are Japanese set ids (SV6, SV9A, CS1.5…), so they resolve symbols from the JA
// page. Simplified Chinese is its own product line (CSV9C, CBB5C — only 8/65 Japanese-shaped) and
// resolves nothing.
const SYMBOL_LANG = {
  ja: 'ja', jp: 'ja', japanese: 'ja',
  ko: 'ja', korean: 'ja',                 // translated JP releases, same symbol
  'zh-tw': 'ja', zhtw: 'ja', 'zh-hant': 'ja',
  en: 'en', eng: 'en', english: 'en',
};
const LOGO_LANG = {
  ja: 'ja', jp: 'ja', japanese: 'ja',
  en: 'en', eng: 'en', english: 'en',
  // ko / zh deliberately absent: their wordmarks are in their own script and we do not index them.
};

const indexLang = (bucket, lang) => {
  const s = String(lang || '').trim().toLowerCase();
  if (!s) return 'en';                                  // unset means the store default
  return (bucket === 'symbols' ? SYMBOL_LANG : LOGO_LANG)[s] || null;
};

function lookup(bucket, lang, candidates) {
  const key = indexLang(bucket, lang);
  if (!key) return null;
  const idx = loadSymbolIndex();
  const table = idx && idx[bucket] && idx[bucket][key];
