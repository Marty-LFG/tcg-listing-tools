        ...
        try {
          ...
          if (p === '/cards' && method === 'GET') {
            ...
            try {
              ...
              if (cards.length) {
                setCardsPut(db, lang, setCode, source, cards);
                return send(res, 200, { cards, source, stale: false, cached: false, cachedAt: new Date().toISOString(), count: cards.length });
              }
              // Success but EMPTY (upstream returns 200 with no cards while a set re-indexes) doesn't
              // throw, so the catch-block fallback below never fires. Don't poison the cache with [],
              // and serve the last-good copy if we have one rather than showing the set as empty (GR7).
              const lastGood = setCardsLast(db, lang, setCode);
              if (lastGood) return send(res, 200, { cards: lastGood.cards, source: lastGood.source, stale: true, cached: true, cachedAt: lastGood.at, count: lastGood.cards.length });
              return send(res, 200, { cards, source, stale: false, cached: false, cachedAt: new Date().toISOString(), count: cards.length });
            } catch (e) {
