        ...
        try {
          ...
          if (p === '/cards' && method === 'GET') {
            ...
            try {
              ...
              if (lang === 'en') {
                cards = await fetchEnCards(setCode, env); source = 'pokemontcg';
                // Early/pre-release EN set: pokemontcg.io hasn't cataloged it yet (it lags to ~street
                // date), but PriceCharting has the English console weeks early. Fall back to it ONLY
                // when pokemontcg.io returns nothing AND the client supplied the set's pcSlug — so
                // authoritative data always wins and the set auto-GRADUATES to pokemontcg the moment
                // it lands there (fetchEnCards then returns cards and this branch never runs).
                if (!cards.length && pcSlug) { cards = await fetchPcCards(pcSlug); source = 'pricecharting-early'; }
              }
              ...
              const decision = decideCardsResponse(cards, source, cards.length ? null : setCardsLast(db, lang, setCode), new Date().toISOString());
              if (decision.store) setCardsPut(db, lang, setCode, source, cards);
