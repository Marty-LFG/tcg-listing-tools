...
/**
 * The one attribute a public tier below D must never carry.
 *
 * §4.4: the seal serial is opened only at tier D, because it is printed on the OUTSIDE of the parcel -
 * so publishing a chase bundle's serial lets anyone who photographed one in transit correlate it to a
 * chase, for no benefit to any buyer. Named rather than inlined so the rule is greppable from both
 * ends: the tier-C filter below, and the test that asserts it.
 */
export const SEAL_ATTRIBUTE = 'bundle.seal_serial';
...
export async function discloseTiers({ run, specs, claims, ladder = [], bundles, tiers = ['A', 'B', 'C'] }) {
  ...
  if (tiers.includes('C') || tiers.includes('D')) {
    // EVERY ATTRIBUTE OF A CHASE BUNDLE EXCEPT ITS SEAL SERIAL.
    //
    // §4.4 puts the serial at tier D and says why in the same breath: "publishing chase bundles'
    // serials would let anyone who photographed a parcel correlate it to a chase for no benefit."
    // This loop took b.attributes unfiltered, so a tier A+B+C close-out published the serial of every
    // labelled chase - the one bundle where the correlation is worth something to a stranger, and the
    // serial is printed on the OUTSIDE of the parcel where couriers and anyone in the postal chain can
    // photograph it.
    //
    // §5.5's tier table says "every attribute of the labelled-chase bundles" and §4.4 says the serial
    // is tier D only; the two contradict, and this is the reading that cannot hurt anybody. Tier D
    // below still opens it, because by then every bundle's contents are public anyway.
    const chase = ordered.filter((b) => b.isChase);
    for (const b of chase) {
      for (const a of b.attributes) {
        if (a.name === SEAL_ATTRIBUTE) continue;
        await push(b, a.name);
      }
    }
  }
