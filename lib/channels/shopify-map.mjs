...
// `slab` shares Gaming Cards with `single` (bk-shopify D-018's table: Gaming Cards ae-2-2-3-2, Card
// Games tg-2-7 for sealed, Card Game Accessories tg-2-6) — a graded card is still a gaming card. It is
// named explicitly rather than left to fall through to `single`, because toShopifyProduct used to
// hardcode TAXONOMY.single for every product type and nothing caught it: no test asserted a graded
// row's category, so a slab would have published under the raw-singles taxonomy by accident rather
// than by agreement.
export const TAXONOMY = Object.freeze({ single: 'ae-2-2-3-2', slab: 'ae-2-2-3-2', sealed: 'tg-2-7', accessory: 'tg-2-6' });

// --- dispatch weight -----------------------------------------------------------------------------
//
// THE WEIGHT OF THE ITEM AND ITS OWN PROTECTION, IN GRAMS — the card in its sleeve and toploader, the
// slab in its bag. NOT the mailer or the box: Shopify adds the parcel's packaging ONCE, from the store's
// default package (Settings → Shipping and delivery → Packages; 0 g on both stores as of 2026-09-12),
// and a per-item mailer weight is added once PER ITEM, which mis-sizes every multi-item order.
//
// That matters because the storefront's postage is WEIGHT-BANDED and its free-shipping rule is bound to
// the smallest band (bk-shopify D-033, option B): Standard is free over $300 only when the cart is an
// XS parcel, ≤ 650 g. With a rigid mailer counted on every card, a 22-single order weighed 660 g and a
// 5-slab order 750 g — both tipped out of the XS box they physically fit in, and out of free shipping.
// Revised 2026-09-12 from 30 g / 150 g; the earlier figures and their reasoning are in git history.
//
...
export const DISPATCH_WEIGHT_GRAMS = Object.freeze({
  // 1.8 g card + 0.4 g penny sleeve + 9 g 35pt toploader + 0.7 g team bag ≈ 12 g. The rigid mailer
  // CARD_PROTECTION promises is the parcel, counted once by the store package, not here. 54 singles
  // still sit inside the 650 g XS band, which is more than the box holds.
  [PRODUCT_TYPES.single]: 12,
  // The slab itself is the variable: PSA ≈ 57 g, CGC ≈ 70 g, BGS ≈ 86 g. Most of the stock is PSA, so
  // PSA plus its bag and a sleeve of bubble: 65 g. Ten PSA slabs = 650 g = the XS edge, which is about
  // what the XS flat-rate box holds stacked; a BGS-heavy lot tips a slab or two early, and pays $12.
  [PRODUCT_TYPES.slab]: 65,
...
// The games the storefront lane is open for. Widening this is a deliberate act with its own tests, not
// a config edit — which is why it stays an explicit frozen list rather than being derived from
// normalize.STOCK_GAMES. Deriving it would mean that adding a game to the card pipeline silently opened
// the storefront to it, on a mapping nobody had looked at.
//
// It is still not free to write anything here: test/unit/shopify-map.test.mjs asserts that every entry
// is a game inventory can actually HOLD (a subset of normalize.STOCK_GAMES — publishing what cannot be
// stocked is a listing with nothing behind it), and that each one maps to a real GAME_WORD rather than
// falling through to titleCase, which would put "Swu" in front of a buyer.
export const V1_GAMES = Object.freeze(['pokemon', 'mtg', 'lorcana', 'riftbound', 'swu', 'onepiece']);

/**
 * The SET a product belongs to, as a metaobject handle.
 *
 * One per (game, set code, language), because those are the three axes that make a set a set on this
 * storefront: White Flare in Japanese is a different shelf from White Flare in English, and a set code
 * is only unique within its game.
 *
 * It exists for the same reason bk_card_identity does, one grain up. Liquid cannot query products by
 * metafield, so any "everything from this set" relationship has to be MATERIALISED at write time. The
 * alternative — a per-set automated collection keyed on bkc.set_code — is decided (bk-shopify D-016)
 * and blocked: all 23 product metafield definitions report smartCollectionCondition eligible but NOT
 * enabled, so a metafield condition cannot be created by either path today.
 */
export function setIdentityHandleFor(item = {}) {
  const game = String(item.game || '').trim();
  const code = String(item.set_code || '').trim();
  const lang = String(item.language || 'EN').trim();
  if (!game || !code) return null;
  return slug(`${game}-${code}-${lang}`);
}
...
// --- rarity, as a shopper reads it ------------------------------------------------------------------

// The storefront prints bkc.rarity verbatim: in the Rarity filter, on the PDP pill, in the description
// and in the tags predictive search matches on. Two sources leak their own vocabulary into it (found
// on live 2026-09-13): Scrydex-era enums arrive as `MEGA_ATTACK_RARE`, and pokemontcg.io writes the
// noun first ("Rare Ultra", "Rare Secret", "Rare Holo") where every other source and every shopper
// says "Ultra Rare". So one product showed "Rare Ultra" beside sixteen "Ultra Rare"s, and a filter
// value read as a database constant. This is display normalisation only — the words are the source's
// own, reordered and spaced, never a different rarity (GR4). The eBay path keeps the raw value: its
// Rarity aspect has its own vocabulary and its own mapping.
const RARITY_REORDER = Object.freeze({
  'rare ultra': 'Ultra Rare', 'rare secret': 'Secret Rare', 'rare holo': 'Holo Rare',
  'rare rainbow': 'Rainbow Rare', 'rare shiny': 'Shiny Rare', 'rare shining': 'Shining Rare',
});
export function shopifyRarity(r) {
  let s = String(r || '').trim();
  if (!s || /^(none|null|undefined|n\/a)$/i.test(s)) return '';
  if (/^[A-Z0-9_]+$/.test(s) && s.includes('_')) {
    s = s.toLowerCase().split('_').filter(Boolean).map((w) => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
  }
  return RARITY_REORDER[s.toLowerCase()] || s;
}
...
//   foil  Inside Information 296 The Hobbit (Foil) [English] — Near Mint
//
// THE ALTERNATIVE PRINTING IS NAMED. A foil and its non-foil, a reverse holo and its plain print,
// are different products with different identity keys (GR5) — and until 2026-09-13 they published
// under one title, so two live products read "Inside Information 296 The Hobbit (HOB) [English] —
// Near Mint" and a search result could not tell them apart. The printing that is the ALTERNATIVE
// — Foil, Etched Foil, Surge Foil, Reverse Holo — is named in brackets after the set; the plain
// print and a holo rare (whose holo IS the card) carry nothing, so the common case stays clean.
//
// Shopify has no 80-character title cap, so unlike fitTitle() nothing is abbreviated or dropped here.
const NAMED_FINISH_RE = /^(foil|etched foil|surge foil|reverse holo(?:foil)?)$/i;
export function buildShopifyTitle(item, f) {
  const graded = !!(item.graded || item.grading_company);
  const finish = String(f.finish || '').trim();
  const finishPart = NAMED_FINISH_RE.test(finish) ? `(${finish.replace(/^reverse holofoil$/i, 'Reverse Holo')})` : '';
  const head = [f.name, f.num, f.set, finishPart].map((s) => String(s || '').trim()).filter(Boolean).join(' ');
...
export function buildShopifyDescription(item, f) {
  ...
  //
  // TWO SENTENCES, NOT THREE (bk-shopify launch review, 2026-09-13). The brand line ("Run by
  // collectors in Newcastle, not a warehouse.") and the parcel sentence ("Ships in a penny sleeve and
  // toploader inside a rigid mailer.") used to follow. Both are already on the PDP, in the theme's own
  // trust rows, a few centimetres above where this renders — so every product page said them twice,
  // and the tagline was on its third appearance by the time a buyer reached it. The description now
  // carries what only it can: the card, and how it is pictured. The parcel and the people stay the
  // theme's, schema-driven, said once.
  const brand = SHOPIFY_CATALOGUE_ART;
  ...
  return [`<p>${esc(identity)}</p>`, `<p>${esc(brand)}</p>`].join('\n');
}

// "from White Flare (WHF)" / "from White Flare" / "from WHF" — never a bare "()" and never the word
// "from" with nothing after it. And never the code twice: Riftbound's set names carry their own code
// ("Unleashed (UNL)", straight from the roster), and every Riftbound description on live read
// "Unleashed (UNL) (UNL)" until 2026-09-13. A name that already ends in the bracketed code is left as
// it is.
function setClause(f) {
  const name = String(f.set || '').trim();
  const code = String(f.setSymbol || '').trim();
  if (name && code && name.toLowerCase().endsWith(`(${code.toLowerCase()})`)) return name;
...
export function toShopifyProduct(item, opts = {}) {
  ...
  // Every Shopify surface reads f.rarity from here on — the description, the tags, bkc.rarity.
  f.rarity = shopifyRarity(f.rarity);
  const graded = !!(item.graded || item.grading_company);
  ...
  return {
    ...
    productType,
    // Keyed off the same `graded` decision as productType, so the two cannot disagree about what this
    // product is. It read TAXONOMY.single unconditionally until the slab lane opened.
    taxonomyCategory: graded ? TAXONOMY.slab : TAXONOMY.single,
    ...
    // The SET this card belongs to. Descriptive fields only: the `sealed` and `singles` lists are
    // recomputed whole by identity.rebuildSet, for the same lost-update reason `listings` is.
    setIdentity: setIdentityHandleFor({ ...item, set_code: item.set_code || f.setSymbol })
      ? {
        handle: setIdentityHandleFor({ ...item, set_code: item.set_code || f.setSymbol }),
        type: 'bk_set_identity',
        fields: {
          display_name: [f.set, f.lang ? `(${f.lang})` : ''].filter(Boolean).join(' '),
          game: item.game || '',
          set_code: String(item.set_code || f.setSymbol || ''),
          set_name: f.set || '',
          language: f.lang || '',
...
export function validateProduct(p, item = {}) {
  ...
  if (!V1_GAMES.includes(String(p.game || ''))) {
    errors.push(`"${p.game || '?'}" is not a game the storefront is open for — `
      + `${V1_GAMES.join(', ')} are. Widening that list is a deliberate act with its own tests.`);
  }
  if (p.kind !== 'inventory') {
    errors.push(`v1 publishes singles only — "${p.kind}" stock has its own tables and its own listing unit`);
  }

  // --- slabs: the facts that make one a slab, and the identity that keeps it apart ---
  //
  // The blanket "RAW singles only" refusal that used to sit here is gone: the slab mapping was already
  // written and tested behind it (grade-in-title, SLAB_PROTECTION copy, cert-as-handle, the grading
  // metafields, the Graded Slab type and its 150 g weight). What it was missing was any requirement
  // that a graded row actually CARRY the facts those depend on.
  //
  // Mirrors the eBay lane, which refuses a graded listing missing grader or grade (validateListing in
  // ebay-map.mjs) — there because eBay's conditionDescriptors demand both. Shopify demands nothing, so
  // without this a row with graded=1 and a null company publishes as a "Graded Slab" titled with no
  // grade, priced as a slab, and indistinguishable from a raw card on the storefront.
  if (graded) {
    if (!String(item.grading_company || '').trim()) errors.push('a graded row needs its grading company — PSA, BGS, CGC, SGC or TAG');
    if (item.grade == null || String(item.grade).trim() === '') errors.push('a graded row needs its grade');
    // THE CERT IS THE SKU (D-012), and it is what makes one slab distinguishable from another. Without
    // it productHandleFor falls back to slug(company-grade), which is NOT unique: two PSA 9s of one
    // card derive the same handle, and the second productSet would upsert straight over the first.
    // Refusing here is cheaper than discovering it as a silently overwritten product.
    if (!String(item.cert_number || '').trim()) {
      errors.push('a graded row needs its certification number — it is the SKU, and without it two slabs of one card collide on the same handle');
    }
  }
  ...
  if (!p.metafields.some((m) => m.key === 'set_code')) warnings.push('no set_code — the bk_card_identity metaobject REQUIRES it, so this card gets no identity and cannot appear in the PDP condition selector; the per-set automated collection also cannot pick it up');
