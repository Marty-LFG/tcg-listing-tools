 ...
 */
export function buildProductSetInput(p, { fileGids = [], identityGid = null, setIdentityGid = null, ogFileGid = null } = {}) {
  ...
  // The SET this product belongs to, on the same terms as the card identity above: spliced only when
  // the metaobject actually resolved. That conditional is load-bearing rather than defensive — the
  // bk_set_identity type and the bkc.set definition live in bk-shopify and are applied by hand, and a
  // metaobject_reference naming a definition the store does not have would fail the whole productSet
  // call, taking the product down with it. Absent until it resolves; present the moment it does.
  if (setIdentityGid) putMf({ namespace: 'bkc', key: 'set', value: setIdentityGid, type: 'metaobject_reference' });
  ...
  const input = {
    ...
    variants: [{
      ...
      inventoryItem: {
        ...
        // ProductVariantSetInput.inventoryItem is an InventoryItemInput, whose `measurement` is an
        // InventoryItemMeasurementInput { weight: WeightInput { unit: WeightUnit!, value: Float! } }
        // — read off shopify.dev at the pinned 2026-07, not remembered. WeightUnit is one of GRAMS /
        // KILOGRAMS / OUNCES / POUNDS, and the map layer works in whole grams.
        //
        // OMITTED rather than sent as 0 when the map has no weight: a scalar left out of productSet is
        // left alone, and clobbering a weight somebody corrected in admin back to zero is strictly
        // worse than leaving it. validateProduct already refuses such a row before this runs, so in
        // practice the branch is a guard rather than a path.
        ...(p.weight_grams > 0 ? { measurement: { weight: { unit: 'GRAMS', value: p.weight_grams } } } : {}),
      },
...
export async function publishProduct(env, {
  ...
  // Recorded on the step AND raised as a warning. On the step alone it was invisible: every driver
  // prints `warnings`, and none prints `steps`, so the one actionable sentence about a broken PDP never
  // reached the operator — while the downstream rebuild's misleading error did. The reason the upsert
  // refused is in `ident.res`, so it travels with the message instead of making someone go digging.
  if (!ident.ok) {
    const why = firstErrorText(ident.res) || 'the store refused the metaobject';
    steps[steps.length - 1].warning = 'published WITHOUT a card identity — ' + why;
    identityWarnings.push('NO card identity was created (' + why + ') — this card cannot appear in the PDP '
      + 'condition selector until the offending field is fixed on the stock row and it is re-published');
  }

  // 1b — the SET identity, on the same terms as the card one: a failure is a degraded page, not a
  // wrong one, so it warns and publishes rather than withholding a sellable card (GR7). It is also the
  // expected state of every store until bk-shopify's apply-metaobject-definitions has added the
  // bk_set_identity type, which is why the message names that script rather than blaming the row.
  let setIdentityGid = null;
  if (product.setIdentity) {
    const sident = await upsertIdentity(env, { identity: product.setIdentity, store, fetchImpl });
    steps.push(stepOf('set_identity', sident.res || { ok: sident.ok }, { handle: product.setIdentity.handle }));
    if (sident.ok) setIdentityGid = sident.gid;
    else {
      const why = firstErrorText(sident.res) || 'the store refused the metaobject';
      steps[steps.length - 1].warning = 'published WITHOUT a set identity — ' + why;
      identityWarnings.push('NO set identity was created (' + why + ') — this product cannot appear in '
        + "its set's related-items row. If bk_set_identity is not defined on this store yet, run "
        + 'bk-shopify/scripts/apply-metaobject-definitions.ps1 and re-publish.');
    }
  }

  // 2 — the product itself
  const input = buildProductSetInput(product, { fileGids, ogFileGid, identityGid: ident.gid, setIdentityGid });
  ...
  // `status` is returned because the caller cannot otherwise tell whether the publish step DID
  // anything. Shopify no-ops publishablePublish for a DRAFT product — ok, no userErrors, zero
  // publications — so a caller that reads only `ok` records a product as published to the Online Store
  // when it is on no channel at all.
  const status = set.product?.status || null;
  const warnings = [...v.warnings, ...identityWarnings];
  if (String(status).toUpperCase() === 'DRAFT') {
    warnings.push('published as a DRAFT, so it is on NO sales channel and no storefront page exists for it yet — '
      + 'set publish.status to ACTIVE and re-run with force once the draft has been checked');
  }
