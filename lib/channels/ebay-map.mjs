...
export function toEbayListing(item, batch, cats, opts) {
  ...
  // One Piece prefers the clean TCGplayer scan over the stored optcgapi art: Bandai's English
  // card images are SAMPLE-watermarked at every keyless mirror, and that watermark would ride
  // into the gallery, the rails and the description. Variant-strict (GR5) — no match, no swap.
  const cleanArt = game === 'onepiece' ? cleanOnepieceArt(item) : null;
  const primary = cleanArt || item.image_url || item.image || null;
  const imageUrls = [];
