...
import { buildTitle, buildDescription, rowToFields, variationTitle, variationAttrs, mtgColourName, mtgTreatmentOf, mtgPromoNote, lorcanaInks, lorcanaInkAspect, riftboundCharacter } from '../listing-copy.mjs';
import { resolveRiftboundCard, riftboundDisplayName } from '../riftbound-data.mjs';
import { resolveMtgCard } from '../mtg-cards-cache.mjs';
import { resolveLorcanaCard } from '../lorcana-cards-cache.mjs';
import { loadConfig as loadImageConfig } from '../listing-image-config.mjs';
import { catalogArtFor } from '../onepiece-clean-art.mjs';
import { GAMES, COND_ID, CARD_CONDITION, PROFESSIONAL_GRADER } from '../ebay-vocab.mjs';
...
export const LANG_NAME = {
  // TW (Traditional Chinese) is a distinct STORED code because it identifies a different product
  // from CN (GR5), but eBay's Language enum has a single 'Chinese' member, so the two face as one.
  // Without this entry ebayLanguageAspect('TW') returned null and the aspect was dropped silently.
  EN: 'English', JP: 'Japanese', JA: 'Japanese', ZH: 'Chinese', CN: 'Chinese', TW: 'Chinese', KO: 'Korean',
...
// Do the store departments named in config still exist on the storefront?
//
// resolveStoreCategoryNames deliberately does NOT repair an unknown path, because silently fixing a
// typo turns it into a plausible WRONG department. The cost of that correctness is that a path can
// rot silently when the owner reorganises the store, and eBay then files the listing under its
// default. That is not hypothetical: the Pokemon singles path sat as /Trading Card Games/Pokemon
// Singles after the store nested it under a Pokemon TCG parent, and every single published in
// between went to the wrong department.
//
// Pure, so it can be unit tested without a Trading API call. `live` is the flattened path list from
// getStoreCategories. Returns [] when everything configured is real.
export function checkStoreCategories(cfg, live) {
  const s = (cfg || {}).store || {};
  const known = new Set((live || []).map((c) => String((c && c.path) || c || '').trim()).filter(Boolean));
  if (!known.size) return [];                       // store unreadable: report nothing rather than everything
  const out = [];
  const check = (label, value) => {
    for (const path of storeCategoryList(value)) {
      if (!known.has(path)) out.push({ setting: label, path, error: 'no longer a department on this store' });
    }
  };
  check('store.defaultCategory', s.defaultCategory);
  Object.entries(s.categoryByGame || {}).forEach(([g, v]) => check('store.categoryByGame.' + g, v));
  check('store.sealedCategory', s.sealedCategory);
  Object.entries(s.sealedCategoryByType || {}).forEach(([t, v]) => check('store.sealedCategoryByType.' + t, v));
  return out;
}
...
export function ebayFinish(finish) {
  ...
  // Lorcana's foil-only chase rarities ARE foil printings — the specialness rides on the Rarity
  // aspect, not this one. They contain neither "holo" nor "foil", so without this they returned
  // null and the Finish facet went unset on the most valuable cards in the game.
  if (/enchanted|iconic|epic/i.test(f)) return 'Foil';
  if (/foil/i.test(f)) return 'Foil';             // incl. Cold Foil / Surge Foil / Etched Foil
...
// ---------------------------------------------------------------------------
// Disney Lorcana — Lorcast. Every enum below was read off the live Taxonomy on 2026-08-14
// (scripts/check-ebay-aspects.mjs --game "Disney Lorcana"); see docs/DATA_SOURCES.md for the field
// counts behind the choices.
// ---------------------------------------------------------------------------

// "Card Type" is SINGLE, and 174 Lorcana cards carry TWO types — always Action+Song. So this needs
// a priority the same way MTG_TYPE_PRIORITY does, and Song wins: it is the distinctive half of the
// pair and the one a buyer searches, where "Action" is the generic.
//
// NOTE none of Lorcana's five types is currently an enum member (the 74 members are Magic, Pokémon,
// Yu-Gi-Oh and Digimon words). The aspect is FREE_TEXT, so the value is carried on the listing as
// an item specific and simply earns no facet — worth sending, and it starts filtering for free if
// eBay ever adds them. Inventing a near-miss from the Magic list instead would be GR4.
const LORCANA_TYPE_PRIORITY = ['Song', 'Location', 'Item', 'Action', 'Character'];
export function lorcanaCardTypeAspect(type) {
  const list = (Array.isArray(type) ? type : String(type == null ? '' : type).split(/[,/]/))
    .map((t) => String(t || '').trim()).filter(Boolean);
  if (!list.length) return null;
  for (const want of LORCANA_TYPE_PRIORITY) {
    const hit = list.find((t) => t.toLowerCase() === want.toLowerCase());
    if (hit) return want;
  }
  return list[0];                                  // an unknown type goes verbatim (GR4)
}

// The ink goes into the category's generic colour axis — the same slot Pokémon puts its energy type
// in and Magic its colours. lorcanaInks / lorcanaInkAspect live in lib/listing-copy.mjs (the
// browser-safe shared vocabulary, because lib/stock-games.mjs needs them too) and are re-exported
// here beside the other aspect helpers.
//
// The six inks are NOT enum members (the 27 on offer are Magic colours and Pokémon energy words),
// so this earns no facet today. They are sent VERBATIM rather than translated to the nearest colour
// — "Amber" is not "Yellow", and a buyer filtering Yellow is not looking for Amber. Buying a facet
// by claiming something the card does not say is the trade GR4 refuses.
export { lorcanaInks, lorcanaInkAspect };

// "Features" (MULTI). 'Chase' and 'Promo' ARE live members; Enchanted, Epic and Iconic are not —
// they are Rarity words, and Rarity carries Enchanted but has no member for the other two. 'Chase'
// is the honest facet for all three: they are foil-only pull-rate cards and that is exactly what a
// buyer browsing the facet wants. Nothing here claims Full Art or Borderless, because no Lorcast
// field says so and guessing a treatment is GR4.
export function lorcanaFeatures(rarity) {
  const r = String(rarity == null ? '' : rarity);
  const out = [];
  if (/enchanted|iconic|epic/i.test(r)) out.push('Chase');
  if (/promo/i.test(r)) out.push('Promo');
  return out;
}

// Lorcast hands back "Super_rare"; the enum member is "Super Rare". Every other Lorcana rarity is
// already an exact member except Epic and Iconic, which have none — those go verbatim on a
// FREE_TEXT aspect and simply earn no facet.
export function lorcanaRarityAspect(rarity) {
  const r = String(rarity == null ? '' : rarity).replace(/_/g, ' ').trim();
  if (!r) return null;
  return r.replace(/\b\w/g, (ch) => ch.toUpperCase());
}

// ---------------------------------------------------------------------------
// Riftbound — Riot's baked card gallery. Every enum below was read off the live Taxonomy on
// 2026-08-25 (scripts/check-ebay-aspects.mjs --game "Riftbound"), category 183454, tree 15.
//
// What that probe found, because most of it is an ABSENCE and absences are what get quietly
// "fixed" later by someone reaching for a near-miss:
//   · Game            — 'Riftbound: League of Legends TCG' IS a member. It is the one required
//                       aspect and it is FREE_TEXT, which is how it sat wrong for months.
//   · Card Type       — only 'Spell' exists. Unit, Gear, Legend, Battlefield and Rune do not.
//   · Rarity          — Common / Uncommon / Rare exist. Epic and Showcase do NOT, and the enum's
//                       'Legendary' is a different game's word, not Riftbound's Epic.
//   · Attribute/Colour— the members are colours. None of the six domains is one.
//   · Features        — 'Showcase' and 'Alternative Art' exist. Overnumbered, Signature and
//                       Ultimate do not.
//   · Manufacturer    — 'Riot Games' is NOT a member (Ravensburger and Wizards of the Coast are).
//   · Card Size       — 'Standard'.   Material — 'Card Stock'.   Both members.
//   · Year Manufactured — SELECTION_ONLY, and Riot's set roster carries no release date at all
//                       ({id, name, collectorNumberMax}), so it stays unset. Inventing one is GR4.
// ---------------------------------------------------------------------------

// "Card Type" is SINGLE and Riftbound cards carry exactly one, so there is no priority to apply —
// only the question of what to send. VERBATIM is the answer: 'Spell' lands on a real member and
// earns a facet for 233 cards, and Unit / Gear / Legend / Battlefield / Rune earn none.
//
// In particular 'Gear' is NOT sent as 'Equipment'. Equipment is Magic's word for Magic's card, and
// buying a facet by claiming something the card does not say is the trade GR4 refuses — the same
// call the Lorcana branch above makes for its five types.
export function riftboundCardTypeAspect(type) {
  const t = String(type == null ? '' : type).trim();
  return t || null;
}

// Riftbound's six domains go into the category's generic colour axis — the same slot Lorcana puts
// its ink in and Magic its colours.
//
// VERBATIM, with ONE normalisation: Riot writes the US 'Colorless' and the AU enum member is
// 'Colourless'. That is a spelling of the same word, not another game's vocabulary, and
// MTG_COLOUR_ASPECT above already makes exactly that substitution. Fury / Calm / Mind / Body /
// Chaos / Order are NOT members and are NOT translated to Red / Blue / Purple / Green / Black /
// Yellow to buy the facet: Riot publishes no such mapping, a buyer filtering Red is not looking for
// Fury, and 175 cards are dual-domain and would have to pick one colour anyway.
//
// SINGLE cardinality, and domainDisp joins a dual-domain card as 'Fury / Mind'. The FIRST domain
// wins — the same rule ebayAttribute applies to a dual-type Pokémon.
export function riftboundDomainAspect(domain) {
  const first = String(domain == null ? '' : domain).split('/')[0].trim();
  if (!first) return null;
  return /^colorless$/i.test(first) ? 'Colourless' : first;
}

// "Features" (MULTI). 'Showcase' and 'Alternative Art' are live members, and they are what the
// premium treatments actually are. Overnumbered, Signature and Ultimate have NO member — they are
// deliberately forfeited here rather than folded into a near-miss, because they already do their
// work at priority 82 in the title (lib/listing-copy.mjs titleParts), which is where a buyer reads
// them.
//
// ⚠ 'Signature' is a PRINTING TREATMENT, not an autograph, and is never mapped to Signed /
// Autographed — see the note in ebayFeatures. Claiming a signature on a US$3,085 card that carries
// none is a misrepresentation.
//
// Nothing here claims Full Art, Borderless, Promo or Serial Numbered: no field in
// data/riftbound.json says so, and an aspect is a claim where the pitch prose is a description.
export function riftboundFeatures(rarity, variant) {
  const r = String(rarity == null ? '' : rarity), v = String(variant == null ? '' : variant);
  const out = [];
  if (/showcase/i.test(r)) out.push('Showcase');
  if (/alternate art/i.test(v)) out.push('Alternative Art');
  return out;
}
...
export function ebayManufacturer(game, language, releaseDate) {
  ...
  // Ravensburger publishes Lorcana under licence from Disney, and is a live enum member ('Disney'
  // is one too, but the publisher is the useful answer and matches how the sets are branded).
  if (game === 'lorcana') return 'Ravensburger';
  // Riot publishes Riftbound. 'Riot Games' is NOT an enum member (read live 2026-08-25), so this
  // earns no facet — it is sent for the same reason the Card Type and the domain are: it is a true
  // item specific a buyer reads, and it starts filtering for free if eBay adds the member. Seating
  // a different publisher to buy a facet would be a false statement about who made the card.
  if (game === 'riftbound') return 'Riot Games';
...
const DEFAULTS = {
  marketplace: 'EBAY_AU',
  categoryTreeId: '15',
  // The card vocabulary itself now lives in lib/ebay-vocab.mjs, because the COMPS SEARCH has to
  // spell these strings identically to the publish (lib/ebay-query.mjs). Two copies would let us
  // list a slab as PSA/8 and then go hunting for it under a different spelling, and neither side
  // would error — a wrong aspect value is silently ignored by eBay, not rejected. One table, so
  // there is nothing to keep in step (GR9). The JSON override below still wins over all of it.
  games: GAMES,
  requiredAspects: ['Game'],
  conditionIds: COND_ID,
  cardConditionAspectValues: CARD_CONDITION,
  professionalGrader: PROFESSIONAL_GRADER,
};
...
export function buildRowIn(item, cats) {
  ...
  const rowIn = {
    ...
    // Price rides along because postage is BANDED by it: rowToFields resolves which band the listing
    // is in, and the description quotes that band's amount. Same precedence toEbayListing uses for
    // price_cents (target over current), so the description and the offer cannot land on different bands.
    target_price_cents: item.target_price_cents, price_cents: item.price_cents,
    postage_band_id: item.postage_band_id,          // the owner's per-listing override, if set
    ...
    // Source CDN art; runPublish upgrades this to EPS. One Piece swaps in the clean TCGplayer
    // scan HERE, at the row level, so the description's art block, the CSV Description column and
    // the gallery all agree — swapping only the gallery left the SAMPLE watermark embedded in
    // every description no re-render ever touched (the CSV channel ships descriptionHtml verbatim).
    img: catalogArtFor(item, item.image_url || item.image || ''),
    // Non-English Pokémon provenance: the native card name + romaji, the native set beside the
    // English one, and the English-equivalent set as a cross-reference row. These are the rows that
    // tell a JP collector this is the printing they are looking for, and they are the first thing
    // that gets checked. Read off `item` here and put on rowIn, which is what rowToFields reads —
    // an aspect derived on the export path must come off rowIn or it works only for a row that
    // still has its facts attached and vanishes on the DB round trip (this is how Lorcana lost
    // Character). Gated on Pokémon: no other game has a second-language printing in this app.
    nativeName: pkm ? (item.native_name || '') : '',
    romaji: pkm ? (item.romaji || '') : '',
    nativeSet: pkm ? (item.native_set || '') : '',
    enSet: pkm ? (item.en_set || '') : '',
  };
  ...
  // An explicit value on the row always wins; card_facts stays the override channel, the same
  // three-part shape the Magic and Lorcana branches below use.
  if (game === 'riftbound') {
    ...
    if (card) {
      rowIn.rb_type = item.rb_type || card.type;
      rowIn.rb_domain = item.rb_domain || card.domain;
      rowIn.rb_tags = item.rb_tags || card.tags;
      rowIn.rb_e = has(item.rb_e) ? item.rb_e : card.e;
      rowIn.rb_p = has(item.rb_p) ? item.rb_p : card.p;
      rowIn.rb_m = has(item.rb_m) ? item.rb_m : card.m;
      // ⚠ FINISH. It is not an inventory_items column, and itemToListing back-fills a missing one
      // from the VARIANT — which for Riftbound is the printing treatment, so a foil Showcase used to
      // arrive here as finish:'Alternate Art' and ebayFinish returned null on the most valuable
      // cards in the game. The catalog knows the real answer, so take it when the row carried none.
      rowIn.finish = item.finish || card.finish || rowIn.finish;
      rowIn.variant = item.variant || card.variant || rowIn.variant;
      // The Character aspect. championTag cannot supply it — it splits on ' - ', a separator the
      // gallery bake never writes — so the name is read the way Riot actually prints it. See
      // riftboundCharacter in lib/listing-copy.mjs, which is gated on the card being a Unit.
      rowIn.character = item.character || riftboundCharacter(card.name, card.type);
      rowIn.illustrator = rowIn.illustrator || card.illustrator || '';
      // THE NAME A BUYER SEARCHES. The bake names a Legend by its epithet and a champion Unit
      // "<Champion>, <Epithet>"; every title, alt text and description built off this row says
      // "<Champion> (<Epithet>)" instead (riftboundDisplayName). Only while the row still carries the
      // bake's own name: an owner who renamed the card in a builder keeps the rename, and a row
      // that already holds the display form is left as it is.
      if (rowIn.name === card.name) rowIn.name = riftboundDisplayName(card);
    } else {
      // data/riftbound.json is GITIGNORED and re-baked daily, so a fresh clone — or a read landing
      // mid-rename — has no catalog at all. card_facts is what the stock tools persist for exactly
      // this moment, and an unresolved row has to degrade rather than publish a thinner listing than
      // the one already live: createOrReplaceInventoryItem is a full replace, not a patch (GR7).
      rowIn.rb_type = item.rb_type || '';
      rowIn.rb_domain = item.rb_domain || '';
      rowIn.rb_tags = item.rb_tags || '';
      rowIn.rb_e = has(item.rb_e) ? item.rb_e : '';
      rowIn.rb_p = has(item.rb_p) ? item.rb_p : '';
      rowIn.rb_m = has(item.rb_m) ? item.rb_m : '';
      rowIn.finish = item.finish || rowIn.finish;
      rowIn.variant = item.variant || rowIn.variant;
      rowIn.character = item.character || '';
    }
  }
  ...
  if (game === 'mtg') {
    ...
    if (card) {
      ...
      // mtgFeatures reads full_art/promo off the CARD, not off the row, so a cold Scryfall cache
      // (a restart on a box that never opened this set) would silently drop the Full Art and Promo
      // members of the Features aspect. The stock tools persist both flags in card_facts for exactly
      // this moment; a stand-in card carries them the rest of the way.
      if (item.full_art != null || item.promo != null) {
        rowIn.mtgCard = { full_art: !!item.full_art, promo: !!item.promo };
      }
    }
  }
  // Lorcana: same deal again. Ink, type, classifications and the four stats are not persisted on
  // inventory_items, so a DB-round-tripped row rendered a CARD DETAILS table with only the rows
  // that come off the identity — and the Character and Card Type aspects had nothing to read.
  // An explicit value on the row always wins; card_facts stays the override channel.
  if (game === 'lorcana') {
    const card = resolveLorcanaCard(item.identity_key || '');
    if (card) {
      // Lorcast keeps `name` and `version` apart and the stored item.name is the JOINED form, so
      // the bare character name is only available here. Counted: `name` never contains " - ".
      rowIn.character = item.character || card.name || '';
      rowIn.lorcana_types = Array.isArray(card.type) ? card.type : [];
      // '/'-joined, matching what the builder's doLookup puts in f_type (MIRROR, GR9).
      rowIn.type = item.card_type || (Array.isArray(card.type) ? card.type.join('/') : '') || '';
      rowIn.ink = item.ink || lorcanaInkAspect(card) || '';
      rowIn.cls = item.classifications || (Array.isArray(card.classifications) ? card.classifications.join(' · ') : '') || '';
      rowIn.cost = item.cost != null ? item.cost : (card.cost != null ? card.cost : '');
      rowIn.strength = item.strength != null ? item.strength : (card.strength != null ? card.strength : '');
      rowIn.willpower = item.willpower != null ? item.willpower : (card.willpower != null ? card.willpower : '');
      rowIn.lore = item.lore != null ? item.lore : (card.lore != null ? card.lore : '');
      rowIn.illustrator = rowIn.illustrator || (Array.isArray(card.illustrators) ? card.illustrators[0] : '') || '';
      rowIn.releaseYear = rowIn.releaseYear || String(card.released_at || '').slice(0, 4);
      rowIn.img = rowIn.img || ((card.image_uris && card.image_uris.digital && (card.image_uris.digital.large || card.image_uris.digital.normal)) || '');
    } else {
      // A cold Lorcast cache (a restart on a box that never opened this set). The stock tools
      // persist all of these in card_facts for exactly this moment.
      rowIn.character = item.character || '';
      rowIn.lorcana_types = item.lorcana_types || null;
      rowIn.type = item.card_type || '';
      rowIn.ink = item.ink || '';
      rowIn.cls = item.classifications || '';
      rowIn.cost = item.cost != null ? item.cost : '';
      rowIn.strength = item.strength != null ? item.strength : '';
      rowIn.willpower = item.willpower != null ? item.willpower : '';
      rowIn.lore = item.lore != null ? item.lore : '';
    }
  }
  return rowIn;
}
...
export function buildItemDescription(item, { imageUrl, artUrl, setLogoUrl, setSymbolUrl, cats, shipping } = {}) {
  if (item.desc_override && item.desc_override.trim()) return item.desc_override.trim();
  const graded = !!(item.graded || item.grading_company);
  // `shipping` MUST be threaded here as well as through toEbayListing: this re-render happens after
  // the EPS image upload, and a re-render that resolved a different band would quote a different
  // amount from the one the offer's policy actually charges.
  const f = rowToFields(buildRowIn(item, cats), { shipping });
  ...
  // The masthead's promo star follows the SAME settings toggle as the rails (default inverted), so
  // a promo listing's gallery image and description can never disagree about how the star reads.
  f.promoStarStyle = loadImageConfig().promoStar;
...
// opts.shipping is the loaded config's shipping block. Passed by the publish path; the bulk grid
// PREVIEW passes nothing and falls back to the code defaults, which are pinned equal to the tracked
// example config by test/invariants/shipping-band-copy.test.mjs.
export function toEbayListing(item, batch, cats, opts) {
  cats = cats || loadEbayCategories();
  const game = item.game;
  ...
  const f = rowToFields(rowIn, { shipping });
  ...
  const pkm = game === 'pokemon';
  const mtg = game === 'mtg';
  const lor = game === 'lorcana';
  const rb = game === 'riftbound';
  const subs = pkmSubtypes(item);
  // Magic release dates come off the resolved Scryfall record; Pokémon's from pokemontcg.io;
  // Lorcana's off the resolved Lorcast record, which buildRowIn has already reduced to a year.
  const relDate = item.set_release_date || (mtg && rowIn.mtgCard ? rowIn.mtgCard.released_at : null)
    || (lor ? rowIn.releaseYear : null) || null;
  ...
  // Riftbound stores "Origins (OGN)" for the same reason Magic stores "The Hobbit (HOB)" — the code
  // is what the title wants and the facet does not.
  put('Set', pkm ? ebaySetName(item.set_name) : (mtg || rb) ? stripSetCodeSuffix(item.set_name) : item.set_name);
  put('Card Number', item.number != null ? String(item.number) : null);
  put('Rarity', pkm ? ebayRarity(item.rarity) : lor ? lorcanaRarityAspect(item.rarity) : item.rarity);
  ...
  put('Card Type', pkm ? (ebayCardType(item.supertype || item.card_type, subs) || item.card_type)
    : mtg ? (item.card_type_aspect || mtgCardTypeAspect(rowIn.type))
      : lor ? lorcanaCardTypeAspect(item.lorcana_types || rowIn.type || item.card_type)
        // rowIn FIRST: a row that has been through the DB carries only its identity, and rb_type is
        // where buildRowIn's re-resolve put the answer. Reading item alone is exactly how Lorcana
        // lost its Character and ink aspects.
        : rb ? riftboundCardTypeAspect(rowIn.rb_type || item.rb_type || item.card_type)
          : item.card_type);
  // Lorcana's Character is FREE, and it is the best facet the game has: Lorcast keeps `name`
  // ("Ariel") and `version` ("Ethereal Voice") in separate fields and the builder joins them, so
  // the bare character name is already sitting there — no splitting, no guessing. Counted: `name`
  // never contains " - " on any of the 3,192 cards.
  //
  // ⚠ Reads rowIn, not just item. A row that has been through the DB carries only its identity, so
  // the name lives on rowIn where buildRowIn's resolveLorcanaCard put it — exactly the same reason
  // the Magic branches below read rowIn.colour rather than item.colour.
  // Riftbound's champion comes from the card NAME ("Darius, Trifarian"), gated on the card being a
  // Unit — see riftboundCharacter in lib/listing-copy.mjs for the counted reason for that gate.
  put('Character', item.character || (lor ? rowIn.character : null) || (rb ? rowIn.character : null)
    || (pkm ? ebayCharacters(item) : null));
  put('Attribute/MTG:Colour', pkm ? ebayAttribute(item.types) : mtg ? mtgColourAspect(rowIn.colour)
    : lor ? lorcanaInkAspect(item.ink || rowIn.ink)
      : rb ? riftboundDomainAspect(rowIn.rb_domain || item.rb_domain) : null);
  ...
  // Every other game's `variant` IS a printing token, so falling back to it is right. Riftbound's is
  // the TREATMENT, so ebayFinish('Signature') is null and the foil facet went unset on exactly the
  // cards worth the most — rowIn.finish is where buildRowIn's re-resolve put the real answer.
  put('Finish', ebayFinish(rb ? (rowIn.finish || item.finish) : (item.finish || item.variant)));
  ...
  put('Features', mtg ? mtgFeatures(rowIn.mtgCard, rowIn.treat, rowIn.promo)
    : lor ? lorcanaFeatures(item.rarity)
      : rb ? riftboundFeatures(item.rarity, rowIn.variant || item.variant) : ebayFeatures(item));
  ...
  put('Card Size', item.card_size || (pkm || mtg || lor || rb ? 'Standard' : null));
  ...
  put('Material', pkm || mtg || lor || rb ? 'Card Stock' : null); // FREE_TEXT enum member; free facet
  ...
  // f.img already carries the One Piece clean-art swap (buildRowIn), so gallery and description
  // read the same source: Bandai's English card images are SAMPLE-watermarked at every keyless
  // mirror, TCGplayer's product scan is the clean one. Variant-strict (GR5) — no match, no swap.
  const primary = f.img || item.image || null;
  const imageUrls = [];
  ...
  return {
    ...
    // Resolved ONCE, by rowToFields, from that same price. The offer's fulfilmentPolicyId and the
    // amount quoted in the description therefore come from one decision rather than two derivations
    // that can drift apart — which is the difference between charging the buyer what we said we would
    // and not.
    postageBand: f.postageBand || null,
