...
import { buildTitle, buildDescription, rowToFields, variationTitle, variationAttrs, mtgColourName, mtgTreatmentOf, mtgPromoNote, lorcanaInks, lorcanaInkAspect } from '../listing-copy.mjs';
...
const DEFAULTS = {
  ...
  categoryTreeId: '15',
  games: {
    // `Game` is the ONE required aspect on 183454, and it is FREE_TEXT — so a value that misses the
    // enum does not fail the publish, it just silently earns no facet on the only aspect eBay
    // insists on. That makes a near-miss here invisible and permanent. Every string below was
    // checked against the live 167-member enum on 2026-08-14; the two marked FIXED were near-misses
    // that had been shipping since their game landed.
    pokemon: { categoryId: '183454', gameAspect: 'Pokémon TCG' },
    lorcana: { categoryId: '183454', gameAspect: 'Disney Lorcana TCG' },        // FIXED: was 'Disney Lorcana'
    mtg: { categoryId: '183454', gameAspect: 'Magic: The Gathering' },
    // eBay has NO member for Star Wars: Unlimited — the Star Wars entries are all older games
    // (Destiny, CCG, Force Attax, TCG), and seating one of those would put the listing in the wrong
    // game outright. Verbatim is the honest answer until eBay adds one.
    swu: { categoryId: '183454', gameAspect: 'Star Wars: Unlimited' },
    riftbound: { categoryId: '183454', gameAspect: 'Riftbound: League of Legends TCG' },   // FIXED: was 'Riftbound'
  },
...
export function toEbayListing(item, batch, cats, opts) {
  ...
  put('Card Type', pkm ? (ebayCardType(item.supertype || item.card_type, subs) || item.card_type)
    : mtg ? (item.card_type_aspect || mtgCardTypeAspect(rowIn.type))
      : lor ? lorcanaCardTypeAspect(item.lorcana_types || rowIn.type || item.card_type) : item.card_type);
  ...
  put('Character', item.character || (lor ? rowIn.character : null) || (pkm ? ebayCharacters(item) : null));
  put('Attribute/MTG:Colour', pkm ? ebayAttribute(item.types) : mtg ? mtgColourAspect(rowIn.colour)
    : lor ? lorcanaInkAspect(item.ink || rowIn.ink) : null);
  ...
  put('Features', mtg ? mtgFeatures(rowIn.mtgCard, rowIn.treat, rowIn.promo)
    : lor ? lorcanaFeatures(item.rarity) : ebayFeatures(item));
  ...
  put('Card Size', item.card_size || (pkm || mtg || lor ? 'Standard' : null));
  ...
  put('Material', pkm || mtg || lor ? 'Card Stock' : null);       // FREE_TEXT enum member; free facet
