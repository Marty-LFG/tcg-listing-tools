...
// The ink, into the category's generic colour axis — the same slot Pokémon puts its energy type in
// and Magic its colours.
//
// ⚠ READ BOTH FIELDS. Lorcast populates `ink` (string) on some cards and `inks` (array) on others,
// and neither alone is complete: Jolly Roger has ink:null with inks:["Ruby"], Never Land has
// ink:"Amber" with inks:null. Reading one loses 160 of 3,192 cards.
//
// The six inks are NOT enum members (the 27 are Magic colours and Pokémon energy words), so this
// earns no facet today. They are sent VERBATIM anyway rather than translated to the nearest colour
// — "Amber" is not "Yellow", and a buyer filtering Yellow is not looking for Amber. Claiming
// otherwise on a listing would be a fabrication (GR4).
//
// The aspect is SINGLE and 187 cards are dual-ink, so those join on a hyphen, which is how the
// community writes them ("Amber-Steel").
