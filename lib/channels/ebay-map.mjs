...
export function mtgCardTypeAspect(typeLine) {
  ...
  if (!t) return null;
  const words = t.split(/\s+/).filter((w) => w && !MTG_SUPERTYPE.test(w));
...
// ⚠ Attribute/MTG:Colour ships UNSET until the live enum dump confirms its constraint mode. eBay
// silently drops an unmatched FREE_TEXT value but REJECTS a SELECTION_ONLY one, and nothing in this
// repo records which this aspect is — a wrong guess is a failed publish, so the safe default is to
// send nothing. Run scripts/check-ebay-aspects.mjs, then flip this to true.
export const MTG_COLOUR_ASPECT_VERIFIED = false;
const MTG_COLOUR_MEMBERS = new Set(['White', 'Blue', 'Black', 'Red', 'Green', 'Multicolour', 'Colourless']);
export function mtgColourAspect(colour) {
  if (!MTG_COLOUR_ASPECT_VERIFIED) return null;
  const c = String(colour == null ? '' : colour).trim();
  return MTG_COLOUR_MEMBERS.has(c) ? c : null;
}

// "Features" (MULTI) for Magic. Deliberately NOT driven by promo_types: `universesbeyond` is on all
// 321 HOB and all 158 HOC prints, so it marks the BRAND, not a promo. Scryfall's own `promo` flag is
// the conservative signal — in HOB exactly one print (#321, the bundle card) carries it, and the
// headliner #249 does not.
export function mtgFeatures(card) {
  const out = [];
...
export function toEbayListing(item, batch, cats) {
  ...
  put('Features', mtg ? mtgFeatures(rowIn.mtgCard) : ebayFeatures(item));   // MULTI — absorbs the old 1st-Edition line
