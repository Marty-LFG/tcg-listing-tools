...
export function ebayFinish(finish) {
  ...
  if (/^(normal|base|regular|non-?holo|standard)$/i.test(f)) return 'Regular';
...
export function buildRowIn(item, cats) {
  ...
  const rowIn = {
    ...
    poke: (ebayCharacters(item) || [])[0] || '',
    stage: item.stage || ebayStage(pkmSubtypes(item)) || '',
    type: item.energy_type || (game === 'pokemon' ? ebayAttribute(item.types) : '') || '',
    hp: item.hp || '', illustrator: item.illustrator || item.artist || '',
    regMark: item.regulation_mark || '', setSymbol: item.set_code || '',
...
export function toEbayListing(item, batch, cats) {
  ...
  const relDate = item.set_release_date || null;                  // pokemontcg.io set.releaseDate

  if (gcfg) put('Game', gcfg.gameAspect);                         // the ONE required aspect (verified live)
  put('Card Name', item.name);
  put('Set', pkm ? ebaySetName(item.set_name) : item.set_name);   // exact enum match earns the Set filter
  put('Card Number', item.number != null ? String(item.number) : null);
  put('Rarity', pkm ? ebayRarity(item.rarity) : item.rarity);
  put('Card Type', ebayCardType(item.supertype || item.card_type, subs) || item.card_type);
  put('Character', item.character || (pkm ? ebayCharacters(item) : null));
  put('Attribute/MTG:Colour', pkm ? ebayAttribute(item.types) : null);
  put('Stage', item.stage || ebayStage(subs));                    // needs Card Type='Pokémon' to be valid
  put('Speciality', item.speciality || ebaySpeciality(subs));
  put('HP', item.hp);
  put('Finish', ebayFinish(item.finish || item.variant));
  put('Features', ebayFeatures(item));                            // MULTI — absorbs the old 1st-Edition line
  put('Illustrator', item.illustrator || item.artist);
  put('Manufacturer', item.manufacturer || ebayManufacturer(game, item.language, relDate));
  put('Card Size', item.card_size || (pkm ? 'Standard' : null));
  ...
  put('Material', pkm ? 'Card Stock' : null);                     // FREE_TEXT enum member; free facet
