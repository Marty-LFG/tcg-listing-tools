...
// Stable aspect column order — the union of everything ebay-map emits, and it has to STAY the union:
// a column missing here is not a smaller CSV, it is an aspect silently dropped on the way to eBay.
// 'Illustrator' was already being emitted and already being lost this way before the Magic aspects
// arrived. Order is presentational only; File Exchange keys on the C: header names.
const ASPECT_COLUMNS = [
  'Game', 'Card Name', 'Set', 'Card Number', 'Rarity', 'Finish', 'Language',
  'Card Type', 'Character', 'Attribute/MTG:Colour', 'Stage', 'Speciality', 'HP',
  'Illustrator', 'Manufacturer', 'Card Size', 'Material', 'Year Manufactured', 'Autographed',
  'Card Condition', 'Graded', 'Professional Grader', 'Grade', 'Certification Number', 'Features',
];

// Grading moved out of `aspects` into structured condition descriptors (eBay-correct for the API).
// The File Exchange CSV still carries the legacy C:Graded/… item-specific columns, so source those
// five from the listing's grading fields; everything else comes from aspects.
function csvAspect(l, a) {
  switch (a) {
    case 'Graded': return l.graded ? 'Yes' : 'No';
    case 'Professional Grader': return l.graderName || '';
    case 'Grade': return l.grade != null ? String(l.grade) : '';
    case 'Certification Number': return l.cert || '';
    case 'Card Condition': return l.cardCondition || '';
    default: return (l.aspects && l.aspects[a]) || '';
  }
}
...
export function toPerCardCsv(listings, opts = {}) {
  ...
  for (const l of listings) {
    lines.push(csvLine([
      ...
      ...ASPECT_COLUMNS.map((a) => csvAspect(l, a)),
    ]));
  }
