...
// 1000 NEW is the sealed lane: a factory-sealed booster box is not a graded or ungraded CARD, and
// without this entry the fallback below published it as USED_VERY_GOOD. Unlike the two card
// conditions, NEW needs no conditionDescriptors, so the guard in publishListing lets it through.
const CONDITION_ENUM = { 1000: 'NEW', 2750: 'LIKE_NEW', 4000: 'USED_VERY_GOOD' };   // sealed / graded / ungraded
...
export async function publishListing(env, { listing, cfg, imageUrls, conditionDescriptors, existingOfferId, dryRun } = {}) {
  ...
  if (offerId) {
    ...
    if (!up.ok) return { ok: false, offerId, error: 'update offer: ' + up.error, httpStatus: up.httpStatus, requestId: up.requestId, rawResponse: up.raw, requestBody: itemBody, offerBody, steps };
    revised = true;
  } else {
    ...
    if (!co.ok) return { ok: false, error: 'create offer: ' + co.error, httpStatus: co.httpStatus, requestId: co.requestId, rawResponse: co.raw, requestBody: itemBody, offerBody, steps };
    offerId = co.offerId;
  }
  ...
  // Both bodies ride along. Until 2026-08-31 this return carried NEITHER, so every failed publish
  // audited as {"inventoryItem":null,"offer":null} — the one row where you most need to know what
  // was sent was the one row that never recorded it. Verified on items 366/367/368 that morning.
  if (!pub.ok) return { ok: false, offerId, revised, error: 'publish: ' + pub.error, httpStatus: pub.httpStatus, requestId: pub.requestId, rawResponse: pub.raw, requestBody: itemBody, offerBody, steps };
  return { ok: true, offerId, listingId: pub.listingId, url: listingUrl(pub.listingId, marketplaceId), revised, requestBody: itemBody, offerBody, steps };
}
