...
// `store` is part of the key, not a filter: a file_gid exists on ONE store, so a dev row is not a cache
// hit for a live publish — it is an id productSet will reject outright.
export function cachedFile(db, contentHash, store = 'dev') {
  if (!db || !contentHash) return null;
  const row = db.prepare('SELECT * FROM shopify_files WHERE content_hash = ? AND store = ?').get(contentHash, store);
 ...
 * by hand.
 */
export function pendingFile(db, contentHash, store = 'dev') {
  if (!db || !contentHash) return null;
  const row = db.prepare('SELECT * FROM shopify_files WHERE content_hash = ? AND store = ?').get(contentHash, store);
  return row && row.file_gid && row.status !== 'ready' ? row : null;
}

function rememberStaged(db, frame, resourceUrl, store) {
  if (!db) return;
  db.prepare(`INSERT INTO shopify_files (content_hash, store, resource_url, status, view, filename, width, height, bytes, compose_version)
              VALUES (?,?,?,'staged',?,?,?,?,?,?)
              ON CONFLICT(content_hash, store) DO UPDATE SET resource_url = excluded.resource_url, status = 'staged', error = NULL`)
    .run(frame.contentHash, store, resourceUrl, frame.view || null, frame.filename || null,
      frame.width || null, frame.height || null, frame.bytes || null, frame.composeVersion || null);
}
function rememberReady(db, contentHash, fileGid, store) {
  if (!db) return;
  db.prepare(`UPDATE shopify_files SET file_gid = ?, status = 'ready', ready_at = datetime('now'), error = NULL
              WHERE content_hash = ? AND store = ?`).run(fileGid, contentHash, store);
}
...
function rememberProcessing(db, contentHash, fileGid, error, store) {
  if (!db) return;
  db.prepare(`INSERT INTO shopify_files (content_hash, store, file_gid, status, error) VALUES (?,?,?,'processing',?)
              ON CONFLICT(content_hash, store) DO UPDATE SET file_gid = excluded.file_gid, status = 'processing', error = excluded.error`)
    .run(contentHash, store, fileGid, String(error || '').slice(0, 500));
}
...
function rememberFailed(db, contentHash, error, fileGid = null, store = 'dev') {
  if (!db) return;
  db.prepare(`INSERT INTO shopify_files (content_hash, store, file_gid, status, error) VALUES (?,?,?,'failed',?)
              ON CONFLICT(content_hash, store) DO UPDATE SET file_gid = COALESCE(excluded.file_gid, shopify_files.file_gid),
...
export async function ensureShopifyMedia(env, db, { imageSet, store = 'dev', fetchImpl, timeoutMs } = {}) {
  ...
  for (const frame of frames) {
    const hit = cachedFile(db, frame.contentHash, store);
  ...
  for (const frame of needed) {
    ...
    if (r.ok) { resolved.set(frame.contentHash, r.fileGid); out.uploaded++; if (r.adopted) out.adopted = (out.adopted || 0) + 1; }
      ...
      if (!r.fileGid) rememberFailed(db, frame.contentHash, r.error, null, store);
...
async function uploadFrame(env, db, frame, { store, fetchImpl, timeoutMs } = {}) {
  ...
  if (pending) {
    ...
    if (node?.failed) rememberFailed(db, frame.contentHash, 'Shopify failed to process the image: ' + node.failed, pending.file_gid, store);
  }
  ...
  rememberStaged(db, frame, target.resourceUrl, store);
  ...
  if (file.fileStatus !== 'READY') {
    ...
    if (timedOut.length) {
      ...
      rememberProcessing(db, frame.contentHash, file.id, error, store);
      return { ok: false, error, fileGid: file.id };
    }
    if (node?.failed) {
      const error = 'Shopify failed to process the image: ' + node.failed;
      rememberFailed(db, frame.contentHash, error, file.id, store);
      return { ok: false, error, fileGid: file.id };
    }
  }

  rememberReady(db, frame.contentHash, file.id, store);
