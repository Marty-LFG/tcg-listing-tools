...
CREATE TABLE IF NOT EXISTS price_checks (
  ...
  -- Which postage band the listing was on, and whether the raise was stopped at that band's ceiling.
  -- Without these a clamped proposal is indistinguishable from a market that simply did not move.
  band_id TEXT,
  band_capped INTEGER,
...
CREATE TABLE IF NOT EXISTS reprice_proposals (
  ...
  -- pending|approved|applying|applied|skipped|expired|failed|stranded.
  -- 'applying' is claimed the instant Approve is tapped, BEFORE the eBay round trip, so a double tap
  -- changes 0 rows instead of writing twice. 'stranded' is where reconcileApplying puts a row whose
  -- process died between the claim and the write, once eBay has confirmed the price never moved.
...
-- At most ONE live proposal per listing. Two open cards for the same item mean two Approve buttons
-- for one price: the second write fails its expectPriceCents precondition and reads as a bug. The
-- rule lives in the DATABASE rather than in a check-then-insert because two scans, or a scan and a
-- manual /propose, can interleave between the check and the insert. migrate() v2 collapses any
-- pre-existing duplicates so creating this index cannot fail a boot.
CREATE UNIQUE INDEX IF NOT EXISTS uq_prop_open_item ON reprice_proposals(item_id)
  WHERE item_id IS NOT NULL AND status IN ('pending', 'applying');
`;
...
export function migrate(db) {
  ...
  // v2: the open-proposal uniqueness rule. CREATE UNIQUE INDEX throws on existing duplicates, and it
  // runs on every boot from the DDL — so a database that already holds two open proposals for one
  // listing would refuse to open at all. Collapse them first, keeping the NEWEST (it carries the most
  // recent evidence) and expiring the rest rather than deleting anything.
  if (hasTable('reprice_proposals')) {
    const dupes = db.prepare(`SELECT item_id FROM reprice_proposals
                              WHERE item_id IS NOT NULL AND status IN ('pending','applying')
                              GROUP BY item_id HAVING COUNT(*) > 1`).all();
    for (const d of dupes) {
      const n = db.prepare(`UPDATE reprice_proposals SET status='expired', error='superseded — collapsed by the open-proposal uniqueness rule'
                            WHERE item_id=? AND status IN ('pending','applying')
                              AND id <> (SELECT MAX(id) FROM reprice_proposals WHERE item_id=? AND status IN ('pending','applying'))`)
        .run(d.item_id, d.item_id).changes;
      steps.push(`collapsed ${n} duplicate open proposal(s) for ${d.item_id}`);
    }
  }

  // v3: postage went price-banded, so a scan's band decision has to be auditable. Widen rather than
  // rebuild — unlike the v1 case above, price_checks now has a writer and real rows worth keeping.
  if (hasTable('price_checks')) {
    const have = cols('price_checks');
    if (!have.includes('band_id')) { db.exec('ALTER TABLE price_checks ADD COLUMN band_id TEXT'); steps.push('price_checks.band_id'); }
    if (!have.includes('band_capped')) { db.exec('ALTER TABLE price_checks ADD COLUMN band_capped INTEGER'); steps.push('price_checks.band_capped'); }
  }
...
/**
 * Closes the process singleton and forgets it, so a later openRepricerDb() opens a fresh handle.
 *
 ...
 *
 * Clearing _rdb is the part that matters. Closing the handle without it would leave every later
 * openRepricerDb() returning a CLOSED database, which fails in a way nobody would connect to a teardown.
 */
export function closeRepricerDb() {
  if (!_rdb) return false;
  try { _rdb.close(); } catch { /* already closed */ }
  _rdb = null;
  return true;
}
