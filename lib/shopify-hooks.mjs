 ...
 *
 * BUSINESS TOPICS ONLY. The three compliance topics are deliberately excluded: they are NOT members
 * of Shopify's WebhookSubscriptionTopic enum (verified by introspection against 2026-07), because
 * they are configured once at the APP level in the Dev Dashboard rather than subscribed per shop.
 * An earlier version of this returned them in the union, which would have made a reconciliation job
 * try to register three topics the API cannot accept, and fail on every run.
 *
 * They still reach us — on the compliance path, from the app-level configuration — and every consumer
 * still receives them regardless of subscription. They are just not ours to register.
 */
export function subscribedTopics() {
  const s = new Set();
  for (const c of _consumers.values()) for (const t of c.topics) s.add(t);
  return [...s].filter((t) => BUSINESS_TOPICS.includes(t)).sort();
}

/** The app-level topics, for the admin page to remind someone they are configured elsewhere. */
export function complianceTopics() { return [...COMPLIANCE_TOPICS]; }
...
async function handleWebhook(env, req, res, cfg, isCompliance) {
  ...
  //
  // This shares sig_failures with the bad-HMAC refusal above, deliberately: both mean "deliveries are
  // being refused", and that is the number worth watching. But the two have OPPOSITE causes and
  // opposite fixes — a bad HMAC means the app secret was rotated, a wrong shop means this receiver
  // and the engine disagree about which store they serve — so the refusal has to be RECORDED, not
  // only counted. It was not, and logRejection goes quiet after five, so a sustained wrong-shop
  // condition left the operator a climbing counter, a silent log, and a last_refusal still naming
  // whatever had failed before it. `detail` carries shopProblem because it names BOTH sides: the
  // shop that delivered and the one we expect. Nothing else in the state does.
  const shopProblem = shopDomainProblem(h.shopDomain, cfg.expect_shop_domain);
  if (shopProblem) {
    _state.sig_failures++;
    _state.last_refusal = { at: new Date().toISOString(), what: 'webhook', reason: 'wrong_shop', detail: shopProblem };
