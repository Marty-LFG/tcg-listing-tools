...
// The guard for a DESTRUCTIVE change — deleting a stock row, or taking it out of stock — as opposed to
// listing one. It differs from assertNotReserved in exactly one way, and the difference is the point:
//
//   THIS ONE REFUSES FOR SEALED TOO. Shrinking the sellable quantity is the right answer to "may I list
//   this pool?", because one sealed row genuinely supplies both a run and the shop. It is no answer at
//   all to "may I delete the row the run is drawing its boosters from" — there is no smaller version of
//   deleting the record. So a sealed row with any units held refuses outright.
//
// Returns a 409 body when the caller must stop, or null when the item is free. `force` PROCEEDS and
// records why: quietly dropping a hold the owner placed on purpose is the wrong failure, and so is
// having no override at all, because the physical world does occasionally win.
export function blockIfHeld(db, { kind, itemId, force = false, action = 'destructive change', actor = null } = {}) {
  const id = +itemId;
  const held = liveReservation(db, kind, id);
  const units = kind === 'sealed' ? reservedUnits(db, kind, id) : (held ? 1 : 0);
  if (!held && !units) return null;

  const where = held?.bundle_label ? `bundle ${held.bundle_label}`
    : held?.run_public_id ? `run ${held.run_public_id}` : 'a run';
  if (!force) {
    return {
      code: 'reserved_for_run', kind, item_id: id, reservation_id: held?.id ?? null,
      run: held?.run_public_id || null, bundle: held?.bundle_label || null,
      units_held: units,
      error: kind === 'sealed'
        ? `${units} unit(s) of sealed item ${id} are held for ${where}; release them, or pass ?force=1 to override`
        : `item ${id} is reserved for ${where}; release the reservation, or pass ?force=1 to override`,
    };
  }
  audit(db, {
    runId: held?.run_id ?? null, bundleId: held?.bundle_id ?? null,
    entity: kind === 'sealed' ? 'sealed_item' : 'inventory_item', entityId: id,
    action: 'reservation_override', actor,
    before: { reservation_id: held?.id ?? null, state: held?.state ?? null, units_held: units },
    note: `${action} forced while held for ${where}`,
  });
  return null;
}
...
//
// DELIBERATELY STORE-BLIND, and registered as such in test/invariants/shopify-store-scoping.test.mjs.
// This is the one query in the runs module that does not name a store, so here is the argument.
//
// The question is not "what does store X say", it is "could a buyer take this card off me right now".
// The two failure directions are not symmetric. Store-blind OVER-blocks: a row left behind by a dev
// rehearsal makes a real lock refuse until it is withdrawn, which costs one command. Store-scoped
// UNDER-blocks: any listing on a store this caller did not name goes unseen, the lock succeeds, and a
// card that is genuinely for sale is committed to a numbered bundle in a manifest that is published and
// anchored — the exact thing the whole system exists to make impossible, and unfixable after the fact
// because the commitment cannot be revised.
//
// It is also the same argument labelTaken's exemption already makes one table over: the subject is a
// physical object, and a conservative answer about a physical object is the safe one. lockRunPhase1
// carries no store and should not grow one to serve this — it operates on the manifest, not on a
// storefront.
...
// A RUN'S POOL INCLUDES THE UNARRANGED FLEET HOLDS, and that is the whole point of holding first.
//
// runs-intake.html holds stock the moment it arrives, with no run id, and says so in as many words:
// "It is not tied to a run yet: that is the point of holding first and arranging second." But this
// query filtered `r.run_id = ?`, and SQL NULL is equal to nothing, so every one of those holds was
// invisible to every run. The Pool panel was permanently empty, "Deal into bundles" had nothing to
// deal, and the stock was simultaneously UNPLACEABLE - /candidates reports available: 0 for a row
// carrying a live reservation, so the same hold that hid the item also blocked it. Intake's default is
// to tick the box, so the ordinary path made a run's own stock unreachable from both directions.
//
// `run_id IS NULL` is therefore included: unarranged stock belongs to whichever run claims it, and
// claiming is what assignToSlot and splitToSlots already do - both accept a null run_id and adopt the
// row. Two draft runs will each see the same unarranged hold, which is correct for a shared pool and
// self-resolving: the first to place it stamps run_id and it leaves the other's list.
export function poolHolds(db, runId) {
  return db.prepare(`SELECT r.id reservation_id, r.kind, r.item_id, r.qty, r.state, r.channel_hold,
                  ...
                  LEFT JOIN sealed_items    s ON r.kind = 'sealed'    AND s.id = r.item_id
                      WHERE (r.run_id = ? OR r.run_id IS NULL)
                        AND r.bundle_id IS NULL AND r.state IN ${HELD_SQL}
                      ORDER BY r.run_id IS NULL, r.id`).all(+runId);
}
...
// oversight: only sealed rows have product_type and upc, and an inventory row's finish lives in
// `variant`. Mapped explicitly rather than COALESCEd blindly, because a silently-null field reads as
// "this card has no set" instead of "ask the other table".
//
// set_code is now on BOTH tables and is COALESCEd, which it could not be before. It used to be sealed-
// only, so every graded line committed set_code = '' — and the chase ladder identifies a card by set
// code, number, language, grader and grade, so a ladder entry naming a set code could never match a
// slab however right the card was. lockRun's deriveChases documents the same gap from the other side.
export function boundLines(db, runId) {
  return db.prepare(`SELECT r.id reservation_id, r.bundle_id, r.slot, r.kind, r.item_id, r.qty,
                            ...
                            i.identity_key,
                            COALESCE(i.set_code, s.set_code) AS set_code,
...
// Split ONE pool hold across several bundles.
//
// assignToSlot promotes a hold by UPDATING it, which is exactly right for inventory: one row is one
// physical object, it goes in one bundle, and keeping it a single row is what makes
// uq_run_res_inventory_active meaningful through the whole lifecycle.
//
// SEALED CANNOT WORK THAT WAY. A pool hold of twelve boosters has to become twelve units spread across
// bundles, and one row cannot be in four places. So a sealed hold SPLITS: the pool row is consumed and
// replaced by one bound row per bundle, summing to exactly the same qty. That is safe precisely where
// the update rule is not — uq_run_res_inventory_active is `WHERE kind = 'inventory'`, so sealed rows
// were always allowed to be many.
//
// Refused for inventory, loudly. Splitting a graded slab across bundles is not a thing that can happen
// physically, and a silent success there would produce a manifest claiming one card is in two parcels.
export function splitToSlots(db, reservationId, targets = [], { actor = null } = {}) {
  const res = db.prepare('SELECT * FROM run_reservations WHERE id = ?').get(+reservationId);
  if (!res) throw new Error(`no such reservation: ${reservationId}`);
  if (res.kind !== 'sealed') {
    throw new Error(`reservation ${reservationId} holds ${res.kind} stock, which is one physical object and cannot be split across bundles`);
  }
  if (res.state !== 'active') throw new Error(`reservation ${reservationId} is ${res.state}; only an active hold can be assigned`);
  if (res.bundle_id != null) throw new Error(`reservation ${reservationId} is already bound to a bundle`);
  if (!targets.length) throw new Error('splitToSlots needs at least one target bundle');

  const want = targets.reduce((n, t) => n + Math.round(+t.qty || 0), 0);
  if (want !== res.qty) {
    throw new Error(`the split covers ${want} unit(s) but the hold is for ${res.qty}; a split must account for every unit or the ledger stops matching the shelf`);
  }

  const run = runRow(db, res.run_id);
  if (!run) throw new Error(`reservation ${reservationId} is not held for a specific run yet`);
  if (run.status !== 'draft') throw new Error(`run ${run.public_id} is ${run.status}; the manifest is no longer editable`);

  for (const t of targets) {
    const bundle = db.prepare('SELECT id, run_id FROM run_bundles WHERE id = ?').get(+t.bundleId);
    if (!bundle) throw new Error(`no such bundle: ${t.bundleId}`);
    if (bundle.run_id !== run.id) throw new Error(`bundle ${t.bundleId} belongs to another run`);
    const spec = db.prepare('SELECT kind FROM run_slot_specs WHERE run_id = ? AND slot = ?').get(run.id, t.slot);
    if (!spec) throw new Error(`run ${run.public_id} declares no slot named '${t.slot}'`);
    if (spec.kind !== 'sealed') throw new Error(`slot '${t.slot}' takes ${spec.kind} stock, not sealed`);
    if (!(Math.round(+t.qty) >= 1)) throw new Error(`the split gives bundle ${t.bundleId} ${t.qty} units; every share must be at least one`);
  }

  db.exec('BEGIN');
  try {
    const ins = db.prepare(`INSERT INTO run_reservations (run_id, bundle_id, kind, item_id, slot, qty, state, channel_hold)
                            VALUES (?,?,?,?,?,?, 'active', ?)`);
    const made = [];
    for (const t of targets) {
      ins.run(run.id, +t.bundleId, res.kind, res.item_id, t.slot, Math.round(+t.qty), res.channel_hold);
      made.push(db.prepare('SELECT last_insert_rowid() id').get().id);
    }
    // The pool row is RELEASED rather than deleted. Every reservation this run ever made stays
    // readable, which is what makes run_audit a history rather than a snapshot.
    db.prepare(`UPDATE run_reservations SET state = 'released', released_at = ? WHERE id = ?`).run(nowIso(), res.id);
    audit(db, { runId: run.id, entity: 'run_reservations', entityId: res.id, action: 'split', actor,
      before: { qty: res.qty, bundle_id: null },
      after: { into: made, targets: targets.map((t) => ({ bundle_id: +t.bundleId, slot: t.slot, qty: Math.round(+t.qty) })) } });
    db.exec('COMMIT');
    return { id: res.id, split_into: made, units: want };
  } catch (e) { db.exec('ROLLBACK'); throw e; }
}
...
// THE OTHER HALF OF "applyStockDecrements IS DELIBERATELY NOT GUARDED".
//
// If a reserved slab genuinely sells on eBay, the sale is real: blocking the decrement would leave the
// shop believing it owns a card someone has paid for. So the stock moves — and then the RUN has to be
// told, or the manifest goes on claiming a card that is in the post. Call this AFTER the decrement.
//
// INVENTORY is unambiguous. The row is one physical object; if it is gone, the reservation is broken.
//
// SEALED needs a rule, because one sealed row legitimately backs both a run and the shop and a sale off
// that row is usually fine. THE RULE IS OVERSELL: break only when the shelf can no longer cover what the
// ledger promised, and break the FEWEST reservations that restore it, NEWEST FIRST — the most recently
// placed hold is the one least likely to have been picked and packed already. The shortfall goes in the
// audit note so a human can see how far short it went rather than just that something broke.
//
// A broken reservation is NEVER deleted. It is the incident record, and lockRun refuses while one stands.
export function breakOversoldReservations(db, kind, itemId, note = null, actor = null) {
  const id = +itemId;
  const broken = [];
  if (kind === 'inventory') {
    const live = liveReservation(db, kind, id);
    if (live && onHandUnits(db, kind, id) <= 0) {
      const r = breakReservation(db, live.id, note, actor);
      if (r) broken.push({ ...r, run_id: live.run_id, run: live.run_public_id || null, bundle: live.bundle_label || null });
    }
    return broken;
  }
  let onHand = onHandUnits(db, kind, id);
  let held = reservedUnits(db, kind, id);
  if (held <= onHand) return broken;
  const rows = db.prepare(`SELECT r.id, r.qty, r.run_id, ru.public_id AS run_public_id, b.label AS bundle_label
                             FROM run_reservations r
                        LEFT JOIN runs ru ON ru.id = r.run_id
                        LEFT JOIN run_bundles b ON b.id = r.bundle_id
                            WHERE r.kind = ? AND r.item_id = ? AND r.state IN ${HELD_SQL}
                            ORDER BY r.id DESC`).all(kind, id);
  for (const row of rows) {
    if (held <= onHand) break;
    const short = held - onHand;
    const r = breakReservation(db, row.id, `${note || 'stock sold from under a run'} — ${short} unit(s) short`, actor);
    if (!r) continue;
    held -= row.qty;
    broken.push({ ...r, run_id: row.run_id, run: row.run_public_id || null, bundle: row.bundle_label || null });
  }
  return broken;
}
...
// Reservations that stock sold out from under. Never deleted, and surfaced rather than merely stored:
// a run cannot lock while one stands, so an operator who cannot SEE them is an operator who finds out at
// lock time, after the pool was assembled.
// Everything about this run's reservations that stops it locking, in the vocabulary this module owns.
//
// lockRun needs three different answers and none of them may spell a state literally outside this file:
// a reservation broken by a real sale elsewhere, an item still live on a sales channel, and stock held
// for the run but never placed in a bundle. That last one is NOT silently released - quietly dropping a
// hold the owner placed deliberately is the wrong failure, so it is reported and the operator decides.
export function lockBlockers(db, runId) {
  const out = [];
  for (const r of db.prepare('SELECT * FROM run_reservations WHERE run_id = ?').all(+runId)) {
    if (r.state === 'broken') {
      out.push({ code: 'broken_reservation', reservation_id: r.id, kind: r.kind, item_id: r.item_id,
        message: `reservation ${r.id} is broken: ${r.kind} item ${r.item_id} sold out from under the run` });
    }
    if (r.channel_hold && r.state !== 'released') {
      out.push({ code: 'channel_hold', reservation_id: r.id, kind: r.kind, item_id: r.item_id,
        channel_hold: r.channel_hold,
        message: `${r.kind} item ${r.item_id} is still live on ${r.channel_hold}; withdraw it first` });
    }
    if (r.state === 'active' && r.bundle_id == null) {
      out.push({ code: 'unassigned_hold', reservation_id: r.id, kind: r.kind, item_id: r.item_id,
        message: `${r.kind} item ${r.item_id} is held for this run but not placed in a bundle (reservation ${r.id})` });
    }
  }
  return out;
}

// §10.4 step 7: promote every live hold to committed, inside lockRun's transaction.
//
// Deliberately NOT wrapped in its own BEGIN. It is one statement in a larger atomic unit - if the lock
// rolls back, these must roll back with it, or the run would be draft with its stock frozen.
export function commitReservations(db, runId) {
  const r = db.prepare("UPDATE run_reservations SET state = 'committed' WHERE run_id = ? AND state = 'active'")
    .run(+runId);
  return Number(r.changes ?? 0);
}
export function brokenReservations(db) {
  return db.prepare(`SELECT r.id, r.kind, r.item_id, r.qty, r.created_at,
                            ru.public_id AS run, b.label AS bundle
                       FROM run_reservations r
                  LEFT JOIN runs ru ON ru.id = r.run_id
                  LEFT JOIN run_bundles b ON b.id = r.bundle_id
                      WHERE r.state = 'broken'
