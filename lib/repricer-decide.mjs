...
import { snapToEnding } from './comps-singles.mjs';
...
export function eligibleForReprice(listing = {}, identity = null, guardrails = {}) {
  ...
  // move it, so a successful raise silently widens the discount (A$10 list / A$8 accept → A$15 list
  // / A$8 accept turns a 20% floor into 47% off). Skip until Phase 5 moves the floors with the price.
  if (listing.bestOffer) return { ok: false, code: 'best_offer_active' };
  ...
  // Trap 3: no trustworthy card identity means the comps precision filter cannot be trusted either.
  if (!identity || !identity.confidence || identity.confidence === 'none') {
...
export function decideReprice({ listing = {}, identity = null, comps = null, guardrails = {}, context = {} } = {}) {
  ...
  // 3+4. The comps target is DELIVERED (competitor list + competitor postage). Convert it to a LIST
  // price by removing OUR postage, which is the number ReviseInventoryStatus actually sets (trap 1).
  const targetDelivered = cents(comps.recommended);
