...
// hence the 240s execFile timeout: 1200dpi is the default and 83s must never brush the kill.
  ...
  return async (req, res) => {
    ...
    try {
      ...
      if (p === '/' && method === 'POST') {
        ...
        try {
          const r = await runPs(['-Mode', 'scan', '-Dpi', String(dpi), '-Wmm', String(cfg.wmm), '-Hmm', String(cfg.hmm), '-Out', out], 240_000)
          if (!r.ok) {
          ...
          if (body.analyze !== false) {
            ...
            if (analyze) {
              let a = await analyze(png)   // contract: never throws; ok:false just means no analysis
              // Auto-orient: a card wider than tall is a sideways card, not a landscape card —
              // rotate the whole scan to portrait and re-analyze so L/R means left/right and the
              // AI's "top edge" is the top edge. 180° (upside down) is NOT detectable from
              // geometry; the client's per-shot rotate button covers that.
              ...
              // Auto-deskew: the analyzer measures the card's tilt from its own edge traces;
              // rotating by -skewDeg makes the sides truly vertical. sharp interpolates properly
              // (the client's canvas bake is the fallback for images that never pass through
              // here). Re-analyze after rotating — every coordinate moved.
              let straightenedBy = 0
              if (a && a.ok && a.analysis.skewDeg != null
                ...
                try {
                  ...
                  straightenedBy = a.analysis.skewDeg
                  a = await analyze(png)
              ...
              if (straightenedBy) straightened = straightenedBy
            }
          }
          // The no-crop fallback ALSO leaves as JPEG: a 1200dpi full-region PNG measured 96MB of
          // base64 — brutal to ship and impossible to save back through the 28MB body cap.
