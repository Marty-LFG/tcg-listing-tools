    ...
    try {
      ...
      if (p === '/' && method === 'POST') {
        ...
        try {
          ...
          let analysis = null, cropped = false, rotated = false
          ...
          console.log('[api/scan]', side, dpi + 'dpi', imgW + 'x' + imgH, Math.round(png.length / 1024) + 'KB', analysis ? '· analyzed' : '', cropped ? '· cropped' : '', rotated ? '· auto-rotated' : '')
          return send(200, {
            ok: true,
            shotId: 'scan-' + side,
