...
const CONFIG_PATH = configFile('tracker.config.json');
...
export function trackerPlugin(env) {
  return {
    ...
    configureServer(server) {
      ...
      startDataRefresh(env);   // daily bake of the baked catalogs — lib/refresh.mjs (env → pokemontcg key + Telegram early-set alerts)
      // NB: intentionally NO httpServer 'close' teardown here. Vite restarts in-process on
      // watched-file changes; stopping the loops on close raced the new server's start and
      // left them dead (collector stalled for days). The unref'd timers die with the process;
      // each restart replaces them (stop-then-start in start*). See lib/collector.mjs.
