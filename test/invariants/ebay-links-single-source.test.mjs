...
describe('the surfaces that show comps links still reach for it', () => {
  ...
  const IMPORTERS = {
    ...
    // The testbed's whole job is composing search URLs, which makes it the surface most likely to
    // grow its own builder — and the one where a private copy would be least obvious, because the
    // page is FOR experimenting with parameters.
    'ebay-testbed.html': /import \{[^}]*searchUrl[^}]*\} from '\/lib\/ebay-links\.mjs'/,
  };
