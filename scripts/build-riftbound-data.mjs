...
//
// Re-run with `node scripts/build-riftbound-data.mjs` whenever a new set drops.
//
...
const summary = Object.entries(ordered).map(([k, s]) => `${s.code}:${s.cards.length}`).join(' ')
console.log(`kept ${kept} of ${cards.length} cards (skipped ${skipped} non-numbered) across ${Object.keys(ordered).length} sets [${summary}] -> ${OUT}`)
