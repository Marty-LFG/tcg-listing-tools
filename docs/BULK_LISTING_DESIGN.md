...
- **Slab wording (⚠ owner-review):** graded listings swap the card condition/protection lines for slab-appropriate wording (`SLAB_CONDITION_SUFFIX`/`SLAB_PROTECTION` in `lib/listing-copy.mjs`) — new copy, pending owner verification per GR6. A slab also has a **band floor** (`shipping.minBandForSlab`): a graded card never travels untracked whatever it is worth, so a $20 PSA 8 is lifted off the $1.70 letter band.
- **Postage is price-banded, and the description quotes it:** three buyer-paid bands (`lib/shipping-bands.mjs`), resolved ONCE in `toEbayListing` and stamped on the listing, so the fulfilment policy the offer carries and the amount the description quotes are the same decision rather than two derivations that can drift. Nothing is free postage any more.
