...
## The buyer's stale reference — "fallen since July" (built 2026-09-13)

**What the shelf said.** The buyer is a shop. Three of its shelf labels, divided by their own TCGplayer charts, land on the same fortnight (Unfair Stamp US$12.00 → 12–16 July; Blastoise & Piplup GX US$460.67 → 23 July–10 Aug; Mega Darkrai ex US$280.67 → 24–28 July), and the label-to-label ratios (factor-free) match late July and nothing since. The factor is **1.5**; Frankfurter's fixings never went above 1.451 all summer, so it is a rule of thumb or the rate plus ~5%. Six other shelf cards sit at 1.39–1.51× today's market — stable cards priced at 1.5 in July after the rate drifted. Staff say they reprice weekly; the shelf says the reference they act on is a late-July Market snapshot. If their 80% is 80% of that, they overpay for anything that has fallen since (Darkrai: reference A$421, they pay A$337 credit, today's market converts to A$278).

**Nobody stored July.** The tracker watched one Pokémon card (added 23 Aug) and its 90-day export has no Pokémon rows; inventory valuations are eBay/PriceCharting figures, not TCGplayer. So the reference market comes from three sources, best first, all through `referenceFor` (lib/arbitrage.mjs) and `nearestReference` (lib/arb-core.mjs: latest on-or-before the date, else earliest after, within `max_gap_days`; typed beats recorded on the same day):
1. **Typed** — `arb_reference_prices`: a figure the owner read off TCGplayer's chart, entered on the page (card line + printing + US$ + date). This is how July is answered today.
2. **Recorded** — `arb_market_snapshots`: the finder's own change-only daily history for every set it touches (swept sets, watched cards' sets, sets with open hits), fed from the pokemontcg.io set cache at the start of each watch pass and on request (`POST /api/arb/snapshot`). No eBay calls. First pass on the dev box: 1,243 series across three sets. This is what makes the NEXT stale snapshot answerable for whole sets.
3. **Tracker** — `watchlist` + `price_snapshots`, Pokémon rows keyed by card id, holo vs normal by the variant text the way the tracker's own mapper reads it.

**The basis.** `reference.enabled` in settings. On: `buyer = buyer_pct × market-as-of-date × store_factor` wherever a figure exists (`buyer_basis = 'reference'` on the hit, with `ref_market_usd_cents`, `ref_date`, `ref_source`, `store_factor`); a card with no figure falls back to the live basis and is chipped `no ref`. The **too-good ratio stays against today's market at the live rate** — it is a wrong-card detector, and the wrong card is wrong whichever basis pays. A card pokemontcg.io cannot price at all (Pitch Black has no prices for weeks — exactly where the stale buyer overpays) is scanned anyway when a reference exists: `market_usd_cents` is NULL on the hit (the day-one NOT NULL was rebuilt away in `migrateArbitrage`), chipped `no live market`, ratio unknowable.

**The fallen-since list** (`GET /api/arb/fallen`, on the page): every card × printing with a reference figure, its market today, the drop, what the buyer pays off the reference, today's market converted, and the gap — the theoretical trade before eBay's asking prices are consulted. Listed when the drop ≥ `min_drop_pct` or there is no live market. "scan" runs the card through the normal scan on the reference basis.

**Measured on the day it was built:** Mega Darkrai ex 116/084, reference US$280.67 → the buyer pays A$336.80. eBay AU: 46 rows, 30 kept, cheapest landed A$317.64 — a real A$19 gap, 6% on outlay, under the 15% floor, so no hit. One of the 46 rows was our own listing; selling stock to the buyer off its stale figure is the same trade without the eBay leg.
