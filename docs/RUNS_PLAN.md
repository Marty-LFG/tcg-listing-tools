...
**Status: REVISION 5 — BUILT, and merged to `main` 2026-09-10.**

> The line above read *"BUILD CANDIDATE. Nothing here is implemented yet"* until this merge, which was
> true when revision 5 was written and had stopped being true 42 commits later. Every module §13's
> implementation map names now exists, with 26 unit-test files beside them, `migrateRuns(db)` in the
> schema chain and the three internal pages (`runs.html`, `runs-intake.html`, `runs-verify.html`). The
> public verification page shipped separately in the theme repo, as §13 says it would.
>
> **Rehearsed end to end on the dev store**, not only in tests: six `Keeper's Run` products from
> `DEV-V1` through `DEV-E2E4`, one storefront variant per bundle number, priced 129–169.
>
> **Not armed.** Publishing is behind this module's own flag in `data/runs.config.json`, deliberately
> separate from the singles switch. §12's three open questions are unchanged and none of them blocks a
> build — they are the owner's calls on grace period, multi-unit checkout and anchoring cadence.
